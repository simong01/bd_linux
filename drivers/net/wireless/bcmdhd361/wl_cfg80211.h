...
#if defined(CONFIG_ARCH_MESON) && defined(CONFIG_ANDROID)
...
#if !defined(BCM43456_CHIP)
#define WL_FILS
#endif /* CHIPs have no WL_FILS */
#endif /* WL_FILS */

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 18, 0)) && !defined(WL_FILS_ROAM_OFFLD)
#if !defined(BCM43456_CHIP)
#define WL_FILS_ROAM_OFFLD
#endif /* !defined(BCM43456_CHIP) */
#endif /* (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 18, 0)) && !defined(WL_FILS_ROAM_OFFLD) */
...
#define CHAN6G(_channel, _flags) { \
	...
	.hw_value		= CH_BASE_6G_CHANNEL + _channel, \
	.flags			= (_flags), \
	.max_antenna_gain	= 0, \
	.max_power		= 30, \
}
...
#define WLAN_AKM_SUITE_WAPI_PSK		0x000FAC13 /* WAPI */

#undef WLAN_AKM_SUITE_WAPI_CERT
#define WLAN_AKM_SUITE_WAPI_CERT	0x000FAC14 /* WAPI */
...
#if defined(STRICT_GCC_WARNINGS) && defined(__GNUC__) && (__GNUC__ > 4 || \
...
#define WLC_PMKLIST_V2_API_BACK_PORTED(ver)  \
	((ver.wlc_ver_major == 4) && (ver.wlc_ver_minor >= 3))

#define WLC_PMKLIST_V2_SUPPORT(ver) \
	((ver.wlc_ver_major == MIN_PMKID_LIST_V2_FW_MAJOR) || \
	 WLC_PMKLIST_V2_API_BACK_PORTED(ver))
...
#define CONST_DFS_MASTER_EVENT_POSTPONE_TIME_MS    50
#define CONST_DFS_MASTER_EVENT_POSTPONE_TIMEOUT    1000  // ms
#define CONST_DFS_MASTER_EVENT_TIMEOUT_QTY \
	(CONST_DFS_MASTER_EVENT_POSTPONE_TIMEOUT / CONST_DFS_MASTER_EVENT_POSTPONE_TIME_MS)
#define CONST_DFS_MASTER_EVENT_QTY                 5
#define CONST_DFS_MASTER_EVENT_BUFFER_QTY          (CONST_DFS_MASTER_EVENT_QTY + 1)
...
#define WL_ROAM_REVERT_STATUS		0x0002
	...
	/* for DFS master event delay call */
	timer_list_compat_t    dfs_master_timer;
	uint                   dfs_master_event_head;
	uint                   dfs_master_event_tail;
	event_param_t          dfs_master_event[CONST_DFS_MASTER_EVENT_BUFFER_QTY];
...
wl_delete_all_netinfo(struct bcm_cfg80211 *cfg)
{
	...
	BCM_LIST_FOR_EACH_ENTRY_SAFE(_net_info, next, &cfg->net_list, list) {
		...
		list_del(&_net_info->list);
		// no 'wdev' free action since not allocated in this layer
		_net_info->wdev = NULL;
		MFREE(cfg->osh, _net_info, sizeof(struct net_info));
	}
	cfg->iface_cnt = 0;
...
extern void wl_cfg80211_cancel_p2plo(struct bcm_cfg80211 *cfg);
#endif /* P2P_LISTEN_OFFLOADING */
...
extern bool wl_cfg80211_check_is2G20(struct net_device *ndev);
...
#define WL_BAND_INIT(band, band_chan_arr) \
do { \
	WL_CHANNEL_ARRAY_INIT(band_chan_arr); \
	band.n_channels = ARRAY_SIZE(band_chan_arr); \
} while (0)
...
extern chanspec_t wl_channel_to_chanspec(struct wiphy *wiphy,
	struct net_device *dev, chanspec_t chanspec, u32 bw_cap, u8 *_buf);
...
#ifdef DHD_DFS_MASTER
extern int wl_cfg80211_dfs_master_reset(struct bcm_cfg80211 *cfg, int init);
#endif /* DHD_DFS_MASTER */
...
extern int wl_cfg80211_set_btcoex_dhcp(struct net_device *dev, dhd_pub_t *dhd, char *command);
extern struct ieee80211_channel * wl_cfg80211_get_ieee80211_chan(struct net_device *dev,
	struct cfg80211_ap_settings *info);

extern int wl_cfg80211_reg_dump_all(const char *pTip);
extern int wl_cfg80211_set_softAP_csa(struct net_device *ndev);
