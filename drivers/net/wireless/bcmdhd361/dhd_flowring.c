                  ...
                  uint8 prio, char *sa, char *da, uint16 *flowid)
{
	...
	if ((FLOWID_INVALID != id) &&
	    (!((FLOWID_RESERVED <= id) &&
	       (dhdp->num_h2d_rings > id)))) {
		DHD_ERROR(("%s: * error, wrong find id=%d by ifidx=%d, prio=%d,"
		           " sa=%02X:%02X:%02X:%02X:%02X:%02X,"
		           " da=%02X:%02X:%02X:%02X:%02X:%02X"
		           " try to get new one!\n",
		           __FUNCTION__, id, ifindex, prio,
		           (unsigned char)sa[0], (unsigned char)sa[1],
		           (unsigned char)sa[2], (unsigned char)sa[3],
		           (unsigned char)sa[4], (unsigned char)sa[5],
		           (unsigned char)da[0], (unsigned char)da[1],
		           (unsigned char)da[2], (unsigned char)da[3],
		           (unsigned char)da[4], (unsigned char)da[5]));
		id = FLOWID_INVALID;
	}
	...
	if (id == FLOWID_INVALID) {
		...
		if (!((FLOWID_RESERVED <= id) &&
		      (dhdp->num_h2d_rings > id))) {
			DHD_ERROR(("%s: * error, wrong allocate id=%d by ifidx=%d, prio=%d,"
			           " sa=%02X:%02X:%02X:%02X:%02X:%02X,"
			           " da=%02X:%02X:%02X:%02X:%02X:%02X"
			           " try to get new one!\n",
			           __FUNCTION__, id, ifindex, prio,
