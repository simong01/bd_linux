...
/* STR is forced so need to insist to make it this time */
#define MAX_WKLK_IDLE_CHECK	20
#define MAX_WKLK_IDLE_TIME	100
#else
#define MAX_WKLK_IDLE_CHECK	3 /* times dhd_wake_lock checked before deciding not to suspend */
#define MAX_WKLK_IDLE_TIME	1
#endif /* LINUX */
...
#ifdef BL_SIM_OTP
static int dhdpcie_download_otp_file(dhd_bus_t *bus, char *path);
#endif /* BL_SIM_OTP */
#endif /* FW_SIGNATURE */
...
static bool dhdpcie_dongle_detach(dhd_bus_t *bus);
...
static void
dhdpcie_quirks_43711(dhd_bus_t *bus)
{
	/* 43711 might need to be informed it supports PME
	 * generation in case its OTP isn't mentioning it.
	 */
	uint16 chipid = (uint16)(-1);
	...
	switch (chipid) {
		...
		{
			/* Update device id */
			/* dhd -i wlan0 membytes  -h 0x180038c0 2 b044 */
			uint32 addr = PCIE_SR_DEVID;
			uint16 val = BCM43711_D11AX6E_ID;
			dhdpcie_bus_membytes(bus, TRUE, addr, (uint8*)&val, sizeof(val));
			if (dev) {
				dev->device = BCM43711_D11AX6E_ID;
			}

			/* Update PME capabilities */
			/* dhd -i wlan0 membytes  -h 0x180038c8 2 f53f */
			addr = PCIE_SR_PMC;
			val = PCIE_SR_PMC_DEFAULT_VAL;
			dhdpcie_bus_membytes(bus, TRUE, addr, (uint8*)&val, sizeof(val));
			if (dev) {
				dev->pme_support |=
					(1 << PCI_D0)| (1 << PCI_D1)| (1 << PCI_D2)|
					(1 << PCI_D3hot)|(1 << PCI_D3cold);
			}
...
#else
	if (bus) {
		// do the dongle release since we don't have 'bus' later
		dhdpcie_dongle_detach(bus);
	}
...
static void
dhdpcie_htclkratio_cal(dhd_bus_t *bus)
{
...
static void
dhdpcie_htclkratio_recal(dhd_bus_t *bus, char *nvram, uint nvram_sz)
{
	char *freq_c = NULL;
	uint len, p;
	int xtalfreq = 0;
...
#ifdef DHD_DEBUG
	DHD_ERROR(("si_attach OK, socitype:0x%x chip:0x%4x rev:0x%x pkg:0x%x\n",
...
static bool
dhdpcie_dongle_detach(dhd_bus_t *bus)
{
	DHD_TRACE(("%s: ENTER\n", __FUNCTION__));
...
dhdpcie_bus_remove_prep(dhd_bus_t *bus)
{
	...
	/* disable the irq for avoiding frequest
	 * irq calling exception case
	 */
	if ((bus->intr) && (bus->irq_registered)) {
		DHD_TRACE(("%s: disable IRQ for removing\n", __FUNCTION__));
		dhdpcie_disable_irq_nosync(bus);
	}
...
#ifdef BCMQT_HW
		DHD_ERROR(("%s: Firmware download progress: %d%%\n", __FUNCTION__,
			((read_len *100)/file_size)));
#endif /* BCMQT_HW */
...
#ifdef DHD_DEBUG_DOWNLOADTIME
	DHD_INFO(("Time for dhdpcie_download_state() to Keep arm in reset: %u ms\n",
...
#ifdef DHD_DEBUG_DOWNLOADTIME
	DHD_INFO(("Time for dhdpcie_download_code_file(): %u ms\n",
			jiffies_to_msecs(jiffies - initial_jiffies)));
#endif
...
#ifdef DHD_DEBUG_DOWNLOADTIME
	DHD_INFO(("Time for dhdpcie_download_nvram(): %u ms\n",
			jiffies_to_msecs(jiffies - initial_jiffies)));
#endif
...
static int
dhdpcie_read_bl_cons_log(dhd_bus_t *bus)
{
	dhd_timeout_t tmo;
	...
	while (!dhd_timeout_expired(&tmo)) {
		if (cons_addr == 0) {
			...
			ret =  dhdpcie_bus_membytes(bus, FALSE, bus->bl_cons_addr_reg,
			...
			if (bus->dhd->dhd_console_ms) {
				int ret = dhdpcie_bus_readconsole(bus);
				if (ret < 0) {
			...
			bootrom_ret =  dhdpcie_bus_membytes(bus, FALSE, bus->bl_cons_addr_reg,
				(uint8 *)&cons_addr, sizeof(cons_addr));
			if (bootrom_ret != BCME_OK) {
				...
				if (cons_addr == 0xffffffff) {
					...
					cons_addr = 0x0;
					dhdpcie_bus_membytes(bus, TRUE, bus->bl_cons_addr_reg,
						(uint8 *)&cons_addr, sizeof(cons_addr));
					return BCME_OK;
				}
			}
...
{
	return dhdpcie_bus_readconsole(bus);
}
#endif /* defined(DHD_DEBUG) || defined(DHD_FW_LOG_SUPPORT) */
...
dhd_bus_perform_flr(dhd_bus_t *bus, bool force_fail)
{
	...
	int retry = 0;
#ifdef BCMQT_HW
	uint16 time_remaining = 0;
	uint16 flr_time_delay = 0;
#endif /* BCMQT_HW */
...
#ifdef BCMQT_HW
	flr_time_delay = CAN_SLEEP() ? DHD_FUNCTION_LEVEL_RESET_DELAY :
		(DHD_FUNCTION_LEVEL_RESET_DELAY * USEC_PER_MSEC);

	while (flr_time_delay > 0) {
		time_remaining = (flr_time_delay >= FLR_SLEEP_INTERVAL) ?
			FLR_SLEEP_INTERVAL : flr_time_delay;

		if (CAN_SLEEP())
			OSL_SLEEP(time_remaining);
		else
			OSL_DELAY(time_remaining * USEC_PER_MSEC);
		flr_time_delay -= time_remaining;
		DHD_ERROR(("%s: Wait for %u ms out of %u ms\n", __func__, flr_time_delay,
				DHD_FUNCTION_LEVEL_RESET_DELAY));
	}
#else
...
#if defined(DHD_CONTROL_PCIE_CPUCORE_WIFI_TURNON)
			dhd_irq_set_affinity(bus->dhd, cpumask_of(CUSTOM_PCIE_IRQ_CPUCORE));
#endif /* DHD_CONTROL_PCIE_CPUCORE_WIFI_TURNON */
	...
	if (flag == TRUE) {
		...
		if (bus->dhd->dongle_reset) {
			...
			if (dhd_bus_download_firmware(bus, bus->dhd->osh,
				bus->fw_path, bus->nv_path, bus->sig_path, bus->boot_path) == 0) {
...
#define DHD_CAP_CUSTOMER "brcm syna "
#endif
	...
	if (enter) {
		...
		if (si_setcore(bus->sih, ARMCA7_CORE_ID, 0)) {
			...
			if (bus->fwsig_filename[0] != 0) {
				...
				if ((bcmerror =
					dhdpcie_bus_download_fw_signature(bus, &do_wr_flops))
					!= BCME_OK) {
					goto fail;
				}
			}
				...
				if (bcmerror == BCME_OK) {
					...
					if (bcmerror != BCME_OK) {
						DHD_ERROR(("WARNING: ROM protect bit set or "
							"PCIe error. Ignore and proceed\n"));
						bcmerror = BCME_OK;
					} else if (resetinstr_data != bus->resetinstr) {
						DHD_ERROR(("%s: Failed to write 0x%08x to addr 0\n",
							__FUNCTION__, bus->resetinstr));
						DHD_ERROR(("%s: Contents of addr 0 is 0x%08x, "
							"Check if Vector protect is set\n",
							__FUNCTION__, resetinstr_data));
						bcmerror = BCME_ERROR;
						goto fail;
					}
...
#ifdef BL_SIM_OTP
static int
dhdpcie_download_otp_file(dhd_bus_t *bus, char *path)
{
	int bcmerror = BCME_OK;
	void *filep = NULL;
	uint8 *srcbuf = NULL;
	...
	if (dest_size > 0) {
		...
		bcmerror = dhdpcie_bus_membytes(bus, TRUE, dest_addr, srcbuf, dest_size);
		if (bcmerror) {
			DHD_ERROR(("%s: error %d on writing %d membytes to 0x%08x\n",
				__FUNCTION__, bcmerror, dest_size, dest_addr));
			goto exit;
...
dhd_bus_gen_devmb_intr(struct dhd_bus *bus)
{
	...
	if (bus->db1_for_mb)  {
		...
		DHD_PCIE_INFO(("writing a mail box interrupt to the device, through doorbell 1\n"));
		if (DAR_PWRREQ(bus)) {
			dhd_bus_pcie_pwr_req(bus);
		}
#ifdef DHD_MMIO_TRACE
		...
		DHD_PCIE_INFO(("writing a mail box interrupt to the device,"
...
dhd_bus_inb_set_device_wake(struct dhd_bus *bus, bool val)
{
	...
	if (val) {
		/* Already up and running, can get out without touching lock */
		if ((dhdpcie_bus_get_pcie_inband_dw_state(bus) ==
		...
		if (unlikely(dhdpcie_bus_get_pcie_inband_dw_state(bus) ==
		...
		} else if (unlikely(dhdpcie_bus_get_pcie_inband_dw_state(bus) ==
			DW_DEVICE_DS_DISABLED_WAIT)) {
		...
		/* Already down and sleeping, can get out without touching lock */
		if ((dhdpcie_bus_get_pcie_inband_dw_state(bus) ==
			DW_DEVICE_DS_DEV_SLEEP)) {
				ret = BCME_OK;
				goto exit;
...
dhd_bus_ringbell(struct dhd_bus *bus, uint32 value)
{
	...
	if ((bus->sih->buscorerev == 2) || (bus->sih->buscorerev == 6) ||
		...
		DHD_PCIE_INFO(("writing a door bell to the device\n"));
		if (IDMA_ACTIVE(bus->dhd)) {
			if (DAR_PWRREQ(bus)) {
				dhd_bus_pcie_pwr_req(bus);
			}
...
dhdpcie_send_mb_data(dhd_bus_t *bus, uint32 h2d_mb_data)
{
	...
	if (cur_h2d_mb_data != 0) {
		uint32 i = 0;
		DHD_PCIE_INFO(("GRRRRRRR: MB transaction is already pending 0x%04x\n",
				cur_h2d_mb_data));
...
dhdpcie_wrt_rnd(struct dhd_bus *bus)
{
	...
	/* try to get hardare related random data first */
	(void)get_random_bytes_arch(rand_buf, count);
#elif (LINUX_VERSION_CODE <= KERNEL_VERSION(5, 18, 0))
	/* try to get hardare related random data first */
	ret = (int)get_random_bytes_arch(rand_buf, count);
#endif /* (LINUX_VERSION_CODE <= KERNEL_VERSION(5, 18, 0)) */
	if (ret != count) {
		/* try to get regular random data */
		RANDOM_BYTES(rand_buf, count);
	}
