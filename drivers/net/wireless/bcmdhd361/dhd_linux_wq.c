...
dhd_kfifo_init(u8 *buf, int size, spinlock_t *lock)
{
	...
	fifo = (struct kfifo *)KVZALLOC(sizeof(struct kfifo), flags);
	if (!fifo) {
		return NULL;
	}
...
dhd_deferred_work_init(void *dhd_info)
{
	...
	work = (struct dhd_deferred_wq *)KVZALLOC(sizeof(struct dhd_deferred_wq),
		flags);
	if (!work) {
	...
	buf = (u8*)KVZALLOC(fifo_size, flags);
	if (!buf) {
