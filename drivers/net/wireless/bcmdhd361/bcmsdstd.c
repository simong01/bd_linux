...
int sd_uhsiclk = SD3CLKMODE_DISABLED;
...
MODULE_PARM_DESC(sd_txglom, "Enable TX gloming for BRCM STD controller (def: 0)");
...
sdioh_attach(osl_t *osh, void *bar0, uint irq)
{
	...
	sd->mem_space = (volatile char *)sdstd_reg_map(osh, *((dmaaddr_t *)(&bar0)),
	                SDIOH_REG_WINSZ);
	sd_init_dma(sd);
	sd->irq = irq;
	...
	if (sdstd_driver_init(sd) != SUCCESS) {
		...
		if (sdstd_driver_init(sd) != SUCCESS) {
			...
			if (sd->mem_space) {
				sdstd_reg_unmap(osh, *((dmaaddr_t *)(&sd->mem_space)),
				                SDIOH_REG_WINSZ);
				sd->mem_space = NULL;
			}
			sdstd_osfree(sd);
	...
	if (sdstd_register_irq(sd, irq) != SUCCESS) {
		...
		if (sd->mem_space) {
			sdstd_reg_unmap(osh, *((dmaaddr_t *)(&sd->mem_space)), SDIOH_REG_WINSZ);
			sd->mem_space = NULL;
		}
               ...
               void *params, int plen, void *arg, uint len, bool set)
{
	...
	case IOV_GVAL(IOV_DS):
	{
		...
		if (status != BCME_OK) {
			sd_err(("sd_ds error for read "
			        "SDIOD_CCCR_DRIVER_STRENGTH : 0x%x\n", status));
			bcmerror = BCME_SDIO_ERROR;
		} else {
...
sdstd_host_init(sdioh_info_t *sd)
{
	 ...
	 */
	if (sd->mem_space) {
		sdstd_reg_unmap(sd->osh, *((dmaaddr_t *)(&sd->mem_space)), SDIOH_REG_WINSZ);
		sd->mem_space = NULL;
	}
...
/* XXX :Ugly define, is there a better way */
	...
	sd->mem_space = (volatile char *)sdstd_reg_map(sd->osh, (dmaaddr_t)&bar, SDIOH_REG_WINSZ);
#else
...
sdstd_3_set_highspeed_uhsi_mode(sdioh_info_t *sd, int sd3ClkMode)
{
	...
	*/
	...
	if ((sd_uhsiclk > SD3CLKMODE_DISABLED) && (sd_uhsiclk < sd3ClkMode)) {
		val1 = SFIELD(val1, HOSTCtrl2_UHSMODE_SEL, sd_uhsiclk);
	} else {
	...
	uint32 *drvstrn, uint16 *presetval)
{
	...
	if (sd3_requested_clkmode != SD3CLKMODE_AUTO) {
		...
		/* HOSTC: get the addr of preset register indexed by the clkmode */
			...
			if ((sd_uhsiclk > SD3CLKMODE_DISABLED) &&
					(sd_uhsiclk < sd3_requested_clkmode)) {
				presetreg = SD3_PresetValStart + (2*sd_uhsiclk + 6);
			} else {
		...
		if (GFIELD(cmd_arg, CMD53_BLK_MODE)) {
			...
			if (use_dma) {
				...
				case DMA_MODE_SDMA:
					...
					sdstd_wreg(sdioh_info, SD_SysAddr,
					      *((uint32 *)(&sdioh_info->dma_phys)));
					break;
				case DMA_MODE_ADMA1:
				case DMA_MODE_ADMA2:
					...
					if ((sdioh_info->glom_info.count != 0) &&
						...
						for (i = 0;
							...
							sd_create_adma_descriptor(sdioh_info,
								i,
								*((uint32 *)(&glom_info->dma_phys_arr[i])),
								glom_info->nbytes[i],
						...
						sd_create_adma_descriptor(sdioh_info,
							i,
							*((uint32 *)(&sdioh_info->glom_info.dma_phys_arr[i])),
					...
					sdstd_wreg(sdioh_info, SD_ADMA_SysAddr,
					           *((uint32 *)(&sdioh_info->adma2_dscr_phys)));
					break;
				default:
...
sd_map_dma(sdioh_info_t * sd)
{
	...
	if ((va = DMA_ALLOC_CONSISTENT(sd->osh, dma_buf_size, SD_PAGE_BITS, &alloced,
		...
		sd->dma_start_buf = 0;
		sd->dma_buf = (void *)0;
		memset(&(sd->dma_phys), 0, sizeof(sd->dma_phys));
		sd->alloced_dma_size = 0;
		sd_err(("%s: DMA_ALLOC failed. Disabling DMA support.\n", __FUNCTION__));
	} else {
		sd->dma_start_buf = va;
		sd->dma_buf = (void *)ROUNDUP((uintptr)va, SD_PAGE);
		/* change for compatibility for
		 * 'sd->dma_phys = ROUNDUP((sd->dma_start_phys), SD_PAGE);'
		 */
		{
#ifdef BCMDMA64OSL
			unsigned long  val = *((unsigned long *)(&sd->dma_start_phys));
#else // BCMDMA64OSL
			unsigned long  val = sd->dma_start_phys;
#endif // BCMDMA64OSL
			val = ROUNDUP(val, SD_PAGE);
			sd->dma_phys = *((dmaaddr_t *)(&val));
		}
	...
	if ((va = DMA_ALLOC_CONSISTENT(sd->osh, SD_PAGE, SD_PAGE_BITS, &alloced,
		...
		memset(&sd->adma2_dscr_phys, 0, sizeof(sd->adma2_dscr_phys));
		...
		{
#ifdef BCMDMA64OSL
			unsigned long  val = *((unsigned long *)(&sd->adma2_dscr_start_phys));
#else // BCMDMA64OSL
			unsigned long  val = sd->adma2_dscr_start_phys;
#endif // BCMDMA64OSL
			val = ROUNDUP(val, SD_PAGE);
			sd->adma2_dscr_phys = *((dmaaddr_t *)(&val));
		}
