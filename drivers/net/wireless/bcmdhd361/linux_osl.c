...
MODULE_PARM_DESC(g_assert_type, "For Synaptics internal use (def: 1)");
#else
uint32 g_assert_type = 0; /* By Default Kernel Panic */
MODULE_PARM_DESC(g_assert_type, "For Synaptics internal use (def: 0)");
...
osl_debug_mfree(osl_t *osh, void *addr, uint size, int line, const char* file)
{
	...
	if (p->size == 0) {
		printk("%s: double free on addr %p size %d at line %d file %s\n",
	...
	if (osh && ((osl_t*)p->osh)->cmn != osh->cmn) {
		printk("%s: alloc osh %p does not match dealloc osh %p\n",
			__FUNCTION__, ((osl_t*)p->osh)->cmn, osh->cmn);
		printk("Dealloc addr %p size %d at line %d file %s\n", addr, size, line, file);
		printk("Alloc size %d line %d file %s\n", p->size, p->line, p->file);
		prhex("bcm_mem_link_t", (void *)p, sizeof(*p));
...
osl_debug_memdump(osl_t *osh, struct bcmstrbuf *b)
{
	...
	if (osh == NULL) {
		printk("%s: ***Error, invalid osh\n", __FUNCTION__);
		WARN_ON(1);
		return -1;
	} else if (osh->magic != OS_HANDLE_MAGIC) {
		printk("%s: ***Error, overwritten, osh->magic=0x%X\n",
		       __FUNCTION__, osh->magic);
		WARN_ON(1);
	}
...
osl_assert(const char *exp, const char *file, int line)
{
	...
	if (!in_interrupt() && g_assert_type != 1 && g_assert_type != 3) {
		const int delay = 3;
		printk("%s", tempbuf);
		printk("panic in %d seconds, g_assert_type=%d\n", delay, g_assert_type);
