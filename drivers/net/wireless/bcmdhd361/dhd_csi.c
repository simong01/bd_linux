                            ...
                            void      *p)
{
	...
	void		     *skb;
	int		     size = ptr->total_size;

	/* For SYNA_CSI_DATA_MODE_UDP_SOCKET allocated buffer
	 * is the skb in ptr->pNode. (p = PKTDATA(dhdp->osh, skb))
	 * Otherwise p and p->pNode are the same (ptr->pNode = p)
	 */
	if (SYNA_CSI_DATA_MODE_UDP_SOCKET == ptr->send_manner) {
		...
		ptr->pHeader_csi = ptr->pNode = NULL;
		MFREE(dhdp->osh, ptr, size);
	}
...
int dhd_csi_config(dhd_pub_t *dhdp, char *pBuf, uint length, bool is_set)
{
	...
	if (is_set) {
		...
		if (CONST_SYNA_CSI_HEADER_VERSION_V2 < pRX_config->data_header_version_output) {
			...
			ret = BCME_BADOPTION;
			goto done;
		} else if (CONST_SYNA_CSI_HEADER_VERSION_V1 > pRX_config->data_header_version_output) {
