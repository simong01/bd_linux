...
{
	static char resp_buf[WLC_IOCTL_SMLEN];
...
{
	int ret;
	ftm_subcmd_info_t subcmd_info = {0, };
	...
	struct ether_addr *peer_addr)
{
	...
	if (peer_addr) {
		DHD_RTT(("session cnt update, upd = %d, cnt = %d, peer : "MACDBG", "
			" ret = %d\n", incr, geofence_cfg->geofence_sessions_cnt,
			MAC2STRDBG(peer_addr), ret));
	} else {
		DHD_RTT(("session cnt update, upd = %d, cnt = %d, ret = %d\n",
...
dhd_rtt_get_geofence_cur_target_idx(dhd_pub_t *dhd)
{
	...
	if (cur_idx >= target_cnt) {
		DHD_RTT(("dhd_rtt_get_geofence_cur_target_idx: "
...
dhd_rtt_add_geofence_target(dhd_pub_t *dhd, rtt_geofence_target_info_t *target)
{
	...
	DHD_RTT(("dhd_rtt_add_geofence_target: " MACDBG
...
dhd_rtt_remove_geofence_target(dhd_pub_t *dhd, struct ether_addr *peer_addr)
{
	...
	DHD_RTT(("dhd_rtt_remove_geofence_target: " MACDBG
		...
		for (idx = rtt_status->start_idx;
			...
			if ((!rtt_target->disable) &&
				...
				if (wl_cfgnan_ranging_is_in_prog_for_peer(cfg, &rtt_target->addr)) {
					...
					if (unlikely(0 > ret) || unlikely(status)) {
...
dhd_rtt_parse_result_event(wl_proxd_event_t *proxd_ev_data,
	int tlvs_len, rtt_result_t *rtt_result, rtt_status_info_t *rtt_status)
{
	int ret = BCME_OK;
	...
	/* fill out the results from the configuration param */
	rtt_result->report.type = rtt_target_info->type;
	DHD_RTT(("report->type: %d\n", rtt_result->report.type));
	DHD_RTT(("report->ftm_num : %d\n", rtt_result->report.ftm_num));
	rtt_result->report_len = RTT_REPORT_SIZE;
        ...
        wl_proxd_event_t *proxd_ev_data, int tlvs_len, rtt_result_t *rtt_result, bool is_nan)
{
	...
	if (tlvs_len > 0) {
		...
		ret = dhd_rtt_parse_result_event(proxd_ev_data, tlvs_len, rtt_result, rtt_status);
...
#ifdef WL_CFG80211
	ret = dhd_rtt_parse_result_event(proxd_ev_data, tlvs_len, &nan_rtt_res, rtt_status);
	if (ret != BCME_OK) {
