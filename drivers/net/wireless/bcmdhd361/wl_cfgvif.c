...
#ifdef DHD_APPLY_NL_BW
		band_width,
#else /* Apply max bw */
		bw,
#endif /* DHD_APPLY_NL_BW */
		CHSPEC_BAND(chspec));
#ifdef WL_6G_BAND
	if (cfg->acs_chspec &&
	...
	if (wf_chspec_valid(cur_chspec)) {
		...
		if (cur_chspec != INVCHANSPEC) {
			if ((err = wldev_iovar_setint(dev, "chanspec",
				...
				/* Disable Frameburst only for stand-alone 2.4G 20MHz SoftAP/P2P */
				if ((dhd->disable_2g20_fb) &&
					(wl_get_mode_by_netdev(cfg, dev) == WL_MODE_AP) &&
					(DHD_OPMODE_SUPPORTED(cfg->pub, DHD_FLAG_HOSTAP_MODE) ||
					 DHD_OPMODE_SUPPORTED(cfg->pub, DHD_FLAG_P2P_MODE)) &&
					(CHSPEC_IS2G(cur_chspec)) &&
					(CHSPEC_BW(cur_chspec) == WL_CHANSPEC_BW_20) &&
...
static u32 wl_get_suite_auth_key_mgmt_type(uint8 type, const wpa_suite_mcast_t *mcast)
{
	...
	if (bcmp(mcast->oui, WFA_OUI, WFA_OUI_LEN) == 0) {
		...
		case RSN_AKM_MFP_PSK:
			...
			break;
		case RSN_AKM_MFP_1X:
			ret = WPA2_AUTH_1X_SHA256;
			break;
		case RSN_AKM_FILS_SHA256:
			ret = WPA2_AUTH_FILS_SHA256;
			break;
		case RSN_AKM_FILS_SHA384:
			ret = WPA2_AUTH_FILS_SHA384;
			break;
#if defined(WL_SAE) || defined(WL_CLIENT_SAE)
	...
	const bcm_tlv_t *wpa2ie, s32 bssidx)
{
	...
	/* check the AKM */
	mgmt = (const wpa_suite_auth_key_mgmt_t *)&ucast->list[suite_count];
	suite_count = count = ltoh16_ua(&mgmt->count);
	while (count--) {
		wpa_auth2 |= wl_get_suite_auth_key_mgmt_type(mgmt->list[count].type, mcast);
	}
	WL_ERR(("wpa ie wpa_suite_auth_key_mgmt count=%d, key_mgmt = 0x%X\n", suite_count, wpa_auth2));
...
#endif /* DISABLE_MODE_CHANGE */
			if (is_mode_different(dhd, ifidx, nmode, vhtmode, hemode,
			                      chanspec, FALSE)) {
...
#endif /* LINUX_VER >= 5.19.2 || CFG80211_BKPORT_MLO */
		NL80211_CHAN_HT20, info->chandef.width) < 0)) {
		WL_ERR(("Set channel failed \n"));
		goto fail;
	}
#endif /* defined(WL_HOSTAPD_CFG) || defined(SYNA_80211_MODE) */
	...
	case WLC_E_TDLS_PEER_DISCONNECTED :
		...
		if (dhdp && dhdp->disable_2g20_fb) {
			if (wl_cfg80211_check_is2G20(ndev)) {
				...
				WL_INFORM(("Enabling frameburst for "
				           "2GHz-20MHz TDLS after teardown\n"));
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION (3, 8, 0))
	chandef->center_freq1 =  wl_channel_to_frequency(CHSPEC_CHANNEL(chanspec),
