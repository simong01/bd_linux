...
s32 wl_inform_single_bss(struct bcm_cfg80211 *cfg, wl_bss_info_t *bi, bool update_ssid)
{
	...
	if (DBG_RING_ACTIVE(dhdp, DHD_EVENT_RING_ID) &&
		...
		/* replace 'tlv_data = event_data->tlvs;' to avoid GCC warning/error
		 * "error: taking address of packed member of 'struct log_conn_event'
		 *  may result in an unaligned pointer value"
		 */
		temp_ptr = event_data->tlvs;
		tlv_data = temp_ptr;
...
wl_inform_bss(struct bcm_cfg80211 *cfg)
{
	...
	/* giv a tip to check if FW scan is fine or not */
	...
	for_each_bss(bss_list, bi, i) {
		/*
		 * With Lenovo tablet (43752), sometimes p2p_find can't get honor TV,
		 * because TV put a wrong vht OP ie:
		 * VHT Operation Info
		 *     Channel Width: 80 MHz, 160 MHz or 80+80 MHz BSS Bandwidth (1)
		 *     Channel Center Segment 0: 7
		 * FW doesn't handle it properly and return bss with
		 * chanspec=0xe007 to dhd, then wl_inform_single_bss reports error.
		 * Here fix the bad chanspec.
		 */
		chanspec_t chanspec_old = bi->chanspec;
		int pri_chan = wf_chspec_primary20_chan(chanspec_old);
		if (CHSPEC_IS80(chanspec_old) &&
			CHSPEC_IS5G(chanspec_old) &&
			pri_chan <= CH_MAX_2G_CHANNEL) {
			bi->chanspec = CH20MHZ_CHSPEC(pri_chan);
			WL_ERR(("fix broken chanspec %x to %x\n", chanspec_old, bi->chanspec));
		}
	...
	const wl_event_msg_t *e, void *data)
{
	...
	if (status == WLC_E_STATUS_PARTIAL) {
		...
		if (wl_get_drv_status_all(cfg, FINDING_COMMON_CHANNEL)) {
			...
			if (cfg->p2p_supported && p2p_scan(cfg) && cfg->scan_request &&
...
#ifdef WL_P2P_6G
bool
wl_chaninfo_is_vlp_psc(struct bcm_cfg80211 *cfg, chanspec_t chspec)
{
	u32 ch;
	s32 err;
	...
	u16 *channel_list, u32 *num_channels, bool use_chanspecs, bool skip_dfs)
{
	...
	/* early get the chanspecs to avoid duplicate retrive in loop */
	buf = (u8 *)MALLOCZ(cfg->osh, buf_len);
	if (!buf) {
		WL_ERR(("*Warning, buf memory alloc failed\n"));
	} else {
		int  err = BCME_OK;
		err = wldev_iovar_getbuf_bsscfg(bcmcfg_to_prmry_ndev(cfg),
				"chanspecs", NULL, 0,
				buf, buf_len, 0, &cfg->ioctl_buf_sync);
		if (err != BCME_OK) {
			WL_ERR(("*Warning, get chanspecs failed with %d\n", err));
		}
	}
	...
	for (i = 0; i < n_channels; i++) {
		...
		support_chanspec = wl_channel_to_chanspec(cfg->wdev->wiphy,
			bcmcfg_to_prmry_ndev(cfg), chanspec,
			WL_CHANSPEC_BW_80, buf);
		if (!support_chanspec)
			continue;
...
#ifdef WL_P2P_6G
			if (CHSPEC_IS6G(chanspec)) {
				if (((dev = wl_to_p2p_bss_ndev(cfg, P2PAPI_BSSCFG_CONNECTION1)) &&
						(wl_get_mode_by_netdev(cfg, dev) == WL_MODE_AP)) ||
						((dev = wl_to_p2p_bss_ndev(cfg, P2PAPI_BSSCFG_CONNECTION2)) &&
						(wl_get_mode_by_netdev(cfg, dev) == WL_MODE_AP))) {
					if (!wl_chaninfo_is_vlp_psc(cfg, chanspec)) {
						WL_ERR(("When P2P GO, p2p_find skip non vlp&psc channel\n"));
						continue;
					}
				}
			}
	...
	struct cfg80211_scan_request *request, uint16 action)
{
	...
	struct net_device *dev = NULL;
#if defined(USE_INITIAL_2G_5G_NODFS_SCAN) || defined(USE_INITIAL_SHORT_DWELL_TIME)
	bool is_first_init_2g_5g_scan = false;
#endif /* USE_INITIAL_2G_5G_NODFS_SCAN || USE_INITIAL_SHORT_DWELL_TIME */
	...
	defined(SUPPORT_RANDOM_MAC_SCAN)
		...
		if (ndev == bcmcfg_to_prmry_ndev(cfg) && g_first_broadcast_scan == true) {
#ifdef USE_INITIAL_2G_5G_NODFS_SCAN
			int i, j;
			int max_5g_freq = ieee80211_channel_to_frequency(CH_MAX_5G_CHANNEL, NL80211_BAND_5GHZ);
			j = 0;
			for (i = 0; i < request->n_channels; i++) {
				int tmp_chan = ieee80211_frequency_to_channel
					(request->channels[i]->center_freq);
				/* Avoid 6G channels */
				if (request->channels[i]->center_freq > max_5g_freq) {
					continue;
				}
				/* Avoid DFS channels */
				if ((tmp_chan > 48)  && (tmp_chan < 149)){
						continue;
				}
				if (i != j) {
					request->channels[j] = request->channels[i];
				}
			...
			is_first_init_2g_5g_scan = true;
#endif /* USE_INITIAL_2G_5G_NODFS_SCAN */
	...
	struct cfg80211_scan_request *request)
{
	...
	// skip escan if event handler is not ready
#ifdef OEM_ANDROID
	...
	// postpone escan to avoid interrupt roaming scan
	if (request) {
		dhd_pub_t *dhdp = (dhd_pub_t *)(cfg->pub);
		err = 1000; // 10ms *1000 = 10 seconds
		while ((dhdp->during_roaming) && // under roaming scan
		       (0 < err)                 // not timeout
		      ) {
			osl_sleep(10);
			err--;
...
wl_cfgscan_handle_scanbusy(struct bcm_cfg80211 *cfg, struct net_device *ndev, s32 err)
{
	...
	if ((cfg && cfg->p2p && !p2p_scan(cfg)) && wl_get_drv_status_all(cfg, REMAINING_ON_CHANNEL)) {
...
static void _wl_cfgscan_cancel_scan(struct bcm_cfg80211 *cfg)
{
	...
	struct net_device *ndev = NULL;
	/*
	 * To cancel the non p2p interface listen on channel
	 * to prevent the scan state lock in FW.
	 */
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 8, 0))
		if (aborted && cfg->p2p_supported && p2p_scan(cfg) &&
			(cfg->scan_request->flags & NL80211_SCAN_FLAG_FLUSH)) {
			WL_ERR(("scan list is changed"));
...
static void wl_cfg80211_scan_supp_timerfunc(void *data)
{
	...
	const wl_event_msg_t *e, void *data)
{
	...
	if (unlikely(0 > err) && unlikely(!cfg->scan_suppressed)) {
...
scan_done_out:
	...
	WL_CFG_DRV_LOCK(&cfg->cfgdrv_lock, flags);
	if (cfg->scan_request) {
		_wl_notify_scan_done(cfg, (WLC_E_STATUS_ABORT == status));
		cfg->scan_request = NULL;
	}
	WL_CFG_DRV_UNLOCK(&cfg->cfgdrv_lock, flags);
...
wl_cfg80211_random_mac_enable(struct net_device *dev)
{
	...
	DEV_ADDR_GET(bcmcfg_to_prmry_ndev(cfg), random_mac);
...
{
	static u16 chan_list[WL_NUMCHANNELS] = {0};
	u32 num_channels = 0;
	ushort pno_time = 0;
	int pno_repeat = 0;
	int pno_freq_expo_max = 0;
	static wlc_ssid_ext_t ssids_local[MAX_PFN_LIST_COUNT];
	struct bcm_cfg80211 *cfg = wiphy_priv(wiphy);
	dhd_pub_t *dhdp = (dhd_pub_t *)(cfg->pub);
	...
	bzero(chan_list, sizeof(chan_list));
...
static void wl_scan_timeout(void *data)
{
	wl_event_msg_t msg;
	struct bcm_cfg80211 *cfg = (struct bcm_cfg80211 *)data;
	struct wireless_dev *wdev = NULL;
