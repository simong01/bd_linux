...
static bool
dhd_use_external_device(void)
{
	if (mmc_sdcard_slot_present() &&
#ifdef VIM3_TRY_INTERNAL_CNT
		(dhd_load_retry_cnt_get() < VIM3_TRY_INTERNAL_CNT) &&
#endif /* VIM3_TRY_INTERNAL_CNT */
		1) { // use ext card
		printk("%s: Use external SD adapter\n", __func__);
		return TRUE;
	} else {
		printk("%s: Use internal SD adapter\n", __func__);
		return FALSE;
	}
}
#endif /* BCMSDIO */
...
dhd_wifi_init_gpio(void)
{
    ...
    cpumask_set_cpu(HOST_WAKE_IRQ_CPUCORE, &host_wake_cpu_mask); // Set CPU core 0
...
//	dhd_wlan_resources.flags &= IRQF_TRIGGER_MASK;
#else
#ifdef BCMSDIO
	if (dhd_use_external_device()) { // use ext card
	...
	if (irq_set_affinity(wlan_host_wake_irq, &host_wake_cpu_mask) == 0) {
...
#else
		if (wlan_reg_on == -1) {
			DHD_ERROR(("%s: Skip setting gpio direction as gpio is invalid",
			           __FUNCTION__));
			return 0;
		}
		if (gpio_direction_output(wlan_reg_on, 1)) {
