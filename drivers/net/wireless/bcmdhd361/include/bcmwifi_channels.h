...
#define CH_BASE_6G_CHANNEL              6000u    /* channel base in 6G band */
...
/* Note channel is not enough to judge band */
#define WL_CHANNEL_BAND(ch)             \
(                                       \
   ((uint)(ch) <= CH_MAX_2G_CHANNEL)    \
 ? (WL_CHANSPEC_BAND_2G)                \
 : ( ((uint)(ch) <= CH_MAX_5G_CHANNEL)  \
    ? (WL_CHANSPEC_BAND_5G)             \
    : (WL_CHANSPEC_BAND_6G)             \
   )                                    \
)
...
#define CH6G20MHZ_CHSPEC(channel)	(chanspec_t)((chanspec_t)(channel) |WL_CHANSPEC_BW_20 | \
					WL_CHANSPEC_BAND_6G)
...
#if (defined(WL_6G_BAND) && !defined(WL_BAND6G_DISABLED))
 ...
 */
extern bool wf_valid_20MHz_chan(uint channel, chanspec_band_t band);

/**
 * Verify that the center channel is a valid 40MHz center channel according to 802.11.
 */
extern bool wf_valid_40MHz_center_chan(uint center_channel, chanspec_band_t band);

/**
 * Verify that the center channel is a valid 80MHz center channel according to 802.11.
 */
extern bool wf_valid_80MHz_center_chan(uint center_channel, chanspec_band_t band);

/**
 * Verify that the center channel is a valid 160MHz center channel according to 802.11.
 */
extern bool wf_valid_160MHz_center_chan(uint center_channel, chanspec_band_t band);

/**
 * Verify that the center channel is a valid 240MHz center channel according to 802.11.
 */
extern bool wf_valid_240MHz_center_chan(uint center_channel, chanspec_band_t band);

/**
 * Verify that the center channel is a valid 320MHz center channel according to 802.11.
 */
extern bool wf_valid_320MHz_center_chan(uint center_channel, chanspec_band_t band);

/**
 * Create a 20MHz chanspec for the given band.
 */
extern chanspec_t wf_create_20MHz_chspec(uint channel, chanspec_band_t band);

/**
 * Returns the chanspec for a 40MHz channel given the primary 20MHz channel number,
 * the center channel number, and the band.
 */
extern chanspec_t wf_create_40MHz_chspec(uint primary_channel, uint center_channel,
 ...
 */
extern chanspec_t wf_create_40MHz_chspec_primary_sb(uint primary_channel,
 ...
 * the center channel number, and the band.
 */
extern chanspec_t wf_create_80MHz_chspec(uint primary_channel, uint center_channel,
                                  chanspec_band_t band);
 ...
 * the center channel number, and the band.
 */
extern chanspec_t wf_create_160MHz_chspec(uint primary_channel, uint center_channel,
                                   chanspec_band_t band);
 ...
 * the center channel number, and the band.
 */
extern chanspec_t wf_create_240MHz_chspec(uint primary_channel, uint center_channel,
                                   chanspec_band_t band);
 ...
 * the center channel number, and the band.
 */
extern chanspec_t wf_create_320MHz_chspec(uint primary_channel, uint center_channel,
                                   chanspec_band_t band);
 ...
 */
extern chanspec_t wf_create_8080MHz_chspec(uint primary_channel, uint chan0, uint chan1,
                                    chanspec_band_t band);
 ...
 * the center channel numbers for each frequency segment, and the band.
 */
extern chanspec_t wf_create_160160MHz_chspec(uint primary_channel, uint chan0, uint chan1,
                                      chanspec_band_t band);
/**
 ...
 */
extern chanspec_t wf_create_chspec(uint primary_channel, uint center_channel,
 ...
 */
extern chanspec_t wf_create_chspec_from_primary(uint primary_channel, chanspec_bw_t bw,
                                         chanspec_band_t band);
 ...
 * The channel width must be 20, 40, 80, or 160 MHz.
 */
extern chanspec_t wf_create_chspec_sb(uint sb, uint center_channel, chanspec_bw_t bw,
                               chanspec_band_t band);
 ...
 * the center channel numbers for each frequency segment, and the band.
 */
extern chanspec_t wf_create_160160MHz_chspec_sb(uint sb, uint chan0, uint chan1,
                                         chanspec_band_t band);
 ...
 */
extern chanspec_band_t wf_mhz2chanspec_band(uint freq);
 ...
 */
extern bool wf_chanspec_iter_init(wf_chanspec_iter_t *iter, chanspec_band_t band, chanspec_bw_t bw);
 ...
 */
extern bool wf_chanspec_iter_next(wf_chanspec_iter_t *iter, chanspec_t *chspec);
 ...
 */
extern chanspec_t wf_chanspec_iter_current(wf_chanspec_iter_t *iter);
...
extern uint8 wf_chspec_320_id2cch(chanspec_t chanspec);

extern uint8 wf_chspec_240_id2cch(chanspec_t chanspec);

extern int wl_channel_to_frequency(uint32 chan, chanspec_band_t band);
