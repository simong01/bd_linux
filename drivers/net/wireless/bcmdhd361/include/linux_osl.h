...
extern struct device *osl_get_device(osl_t *osh);
...
#define printf(fmt, args...)	printk(CUSTOM_PREFIX_NORTCTIME fmt , ## args)
#else
...
/* Navarro define this to be 'const void iomem *' as
 * other platform may defined to be 'volatile void iomem *'
 */
#ifdef CONFIG_NAVARRO_SOC
#define REG_UNMAP(va)		iounmap(((const void *)(va)))
#else /* CONFIG_NAVARRO_SOC */
#define REG_UNMAP(va)		iounmap(va)
#endif /* CONFIG_NAVARRO_SOC */
...
#define START_TIMER(timer, duration, extra_delay)                            \
do {                                                                         \
	mod_timer(timer, jiffies + msecs_to_jiffies(duration + extra_delay));    \
} while (0);
...
#else
#define KVMALLOC(size, flags)	kvmalloc(size, flags)
#define KVZALLOC(size, flags)	kvzalloc(size, flags)
#define KVFREE(addr)	kvfree(addr)
#endif
