...
bcm_compute_xor32(volatile uint32 *u32_val, int num_u32)
{
	...
	uint32 xor32 = 0;
	uint64 xor64 = 0;
	int num_u64 = num_u32/2;
	volatile uint64 *p64 = NULL;
	int rest = 0;
	if ((((uint64)u32_val)%8) != 0) {
		/* 4 bytes aligned, skip forware */
		xor32 = u32_val[0];
		p64 = (volatile uint64*)(&u32_val[1]);
		u32_val = &u32_val[1];
		num_u32--;
		num_u64 = num_u32/2;
		rest = num_u32%2;
	} else {
		p64 = (volatile uint64*)u32_val;
		num_u64 = num_u32/2;
		rest = num_u32%2;
	}

	for (idx = 0; idx < num_u64; idx++) {
		xor64 ^= *(p64 + idx);
	}
	xor32 ^= (xor64 & 0x00000000FFFFFFFF);
	xor32 ^= (xor64 >> 32);

	if (rest) {
		xor32 ^= u32_val[num_u32-1];
	}
...
#define bcm_print_bytes(name, cdata, len)    prhex(name, cdata, len)
