...
#define free_netdev(ndev)		kfree(ndev)
#endif /* HAVE_FREE_NETDEV */
...
#define DEV_ADDR_SET(ndev, mac_addr)    (void)memcpy_s(((ndev)->dev_addr), ETHER_ADDR_LEN, mac_addr, ETHER_ADDR_LEN)
#else /* LINUX_VERSION_CODE < KERNEL_VERSION(5, 15, 0) */
#define DEV_ADDR_SET(ndev, mac_addr)    dev_addr_set(ndev, (const u8*)mac_addr)
#endif /* LINUX_VERSION_CODE < KERNEL_VERSION(5, 0, 0) */
#define DEV_ADDR_GET(ndev, pBuf)        (void)memcpy_s(pBuf, ETHER_ADDR_LEN, ((ndev)->dev_addr), ETHER_ADDR_LEN)
...
{ \
	...
	(tsk_ctl)->up_cnt = 0; \
	(tsk_ctl)->p_task  = kthread_run(thread_func, tsk_ctl, "%s%s", (char*)(name), NAME_SUFFIX); \
...
#define SRANDOM32(entropy) \
do { \
	uint _entropy = (entropy); \
	add_device_randomness(&_entropy, sizeof(uint)); \
} while (0)
#elif (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 8, 0))
...
// New google android GKI now allow kernel_write/kernel_read, and use
...
#elif defined(CONFIG_ARCH_MESON) && defined(CONFIG_ANDROID) && \
	(LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 43))
#define	WDEV_CLIENT(wdev, field)	(wdev->u.client.field)
#else
