...
#ifndef DHD_TXMINMAX
#define DHD_TXMINMAX	10	/* Max tx frames if rx still pending */
#endif /* DHD_TXMINMAX */
...
#if defined(BCMQT) || defined(BL_RAM_BOOT)
...
// modify for retrieving log
#define CONSOLE_LINE_MAX	(500)
#define CONSOLE_BUFFER_MAX	(1024 * 100)
...
#ifdef BT_MCU_RTOS_CONSOLE_LOGS

#define BT_CONSOLE_LINE_MAX	(256)
#define BT_CONSOLE_BUFFER_MAX	(1024 * 8)

/* Created custom memory section in linker and placed console buffer in end of BT-SRAM*/
#define CONS_BUF_ADDR           0x19275ff8
#define WRITE_IDX_ADDR		0x19277ff8

typedef struct dhd_bt_console {
	uint			bt_count;		/* Poll interval msec counter */
	uint			bt_bufsize;		/* Size of log buffer */
	uint8			*bt_buf;		/* Log buffer (host copy) */
	uint 			bt_last;		/* Last buffer read index */
} dhd_bt_console_t;

#endif /* BT_MCU_RTOS_CONSOLE_LOGS */
...
#define BT2WLAN_PWRUP_ADDR_43012B0	0x640894        /* This address is specific to 43012B0 */
...
/* XXX RAM Bootloader loading address */
#define ARMCR4_RAM_BOOTLOADER                  0x250000

/* Values of these defines shall not be changed
 * HOST_LOGS_PROCESSED (0x00000000) serves dual purpose:
 *      1. To inform BL about completion of logging on host
 *      2. To initialize the reg, to start next cycle of logging
 * END_OF_BL_LOGS (0xFFFFFFFF):
 *      To be compatible with existing 4612 ROM bootloader
 *      (already taped out) and new console logging infra
 * BL_NO_UPDATE: (0x00000000):
 *      Before BL advertises console address, register
 *      value will be initially zero
 */
#define BL_NO_UPDATE		0x00000000
#define HOST_RXED_CONS_ADDR	0xAAAAAAAA
#define END_OF_BL_LOGS		0xFFFFFFFF
#define HOST_LOGS_PROCESSED	0x00000000
...
typedef struct dhd_bus {
	...
	char		*sig_path;		/* module_param: path to signature file */
	...
	uint32		sh_addr;		/* sdpcm_shared address */
...
#define PKTGEN_RCV_ONGOING  (1)
	...
	bool set_fw_addr_len;	      /* Store FW addr, len */
	...
	char clm_filename[DHD_FILENAME_MAX];	  /* Name of clm blob file */
#if defined(FW_SIGNATURE)
	...
	uint32		otp_addr;	/* module_param: dst_addr to otp file */
...
#ifdef BL_SIM_OTP
static int dhdsdio_download_otp_file(dhd_bus_t *bus, char *path);
#endif /* BL_SIM_OTP */
#endif /* FW_SIGNATURE */
...
#ifdef BCMDBG
extern uint dhd_dongle_rambase;
#endif /* BCMDBG */
...
uint dhd_gci_chipctrl_reg4 = 0;
module_param(dhd_gci_chipctrl_reg4, uint, 0644);

#ifdef linux
module_param(dhd_doflow, uint, 0644);
MODULE_PARM_DESC(dhd_doflow, "Flow control enabled (def: 1)");
module_param(dhd_dpcpoll, uint, 0644);
MODULE_PARM_DESC(dhd_dpcpoll, "For Synaptics internal use only (def: 0)");
#endif
...
#ifdef DEBUG_SDIO_TRANSFER_TIME
#define MAX_SLIST 15
typedef struct speed_list
{
	uint32 glom_time[MAX_SLIST];
	uint32 glom_cnt[MAX_SLIST];
	uint32 dpc_time[MAX_SLIST];

	uint32 avg[3];
	uint32 min[3];
	uint32 max[3];

	uint64 last_dpc_time;
	uint32 index;
	...
	uint64 last_time;
	uint32 last_total_cnt;
	uint32 last_total_bytes;
	uint32 max_avg;
} speed_list_t;

speed_list_t sdio_transfer_time_rx;
speed_list_t sdio_transfer_time_tx;
uint32 sdio_transfer_glom_list[41][2];
uint32 sdio_transfer_time_rxglom;

int sdio_transfer_calc_rx_bytes(uint32 bytes);
int sdio_transfer_calc_time_rx(uint32 cnt);
int sdio_transfer_calc_time_tx(uint32 cnt, uint32 bytes, uint64 time_a);
#endif /* DEBUG_SDIO_TRANSFER_TIME */
...
static void dhd_get_metadata_param(uint64 *var, int *prev_pos,
		int next_pos, uint8 *metadata_block);
static int dhd_is_within_ram_range(uint64 addr, dhd_bus_t *bus);
static int dhd_get_metadata_struct_params(uint8 *metadata_block, dhd_bus_t *bus);
static int dhd_get_metadata_ver(dhd_bus_t *bus, char *fwpath);
static int dhd_get_metadata_size(dhd_bus_t *bus, char *fwpath);
static int dhd_fetch_metadata_array(dhd_bus_t *bus, char *fwpath, uint8 *metadata_block);
static int dhdsdio_sectionwise_fw_download(struct dhd_bus *bus, char *pfw_path);
static int dhd_update_reset_instr(dhd_bus_t *bus, char *pfw_path);
static int dhd_update_tag_info(dhd_bus_t *bus, char *fwpath);
static int dhdsdio_download_clm_file(dhd_bus_t *bus, char *path);
...
static int dhdsdio_download_btcert_file(dhd_bus_t *bus, char *path);
#endif /* defined (BT_OVER_SDIO) */
...
dhdsdio_sr_init(dhd_bus_t *bus)
{
	...
	if (!(wkctl & val)) {
		DHD_ERROR(("%s: Set wake up ctrl failed. "
		           "SBSDIO_FUNC1_WAKEUPCTRL=%08x, expect %08x\n",
	...
	if (CHIPID(bus->sih->chip) == BCM43430_CHIP_ID ||
		...
		CHIPID(bus->sih->chip) == BCM43710_CHIP_ID ||
		CHIPID(bus->sih->chip) == BCM4612_CHIP_ID  ||
		CHIPID(bus->sih->chip) == BCM5312_CHIP_ID)
...
dhdsdio_clk_kso_enab(dhd_bus_t *bus, bool on)
{
	...
	if ((BCM4612_CHIP(bus->sih->chip) ||
		BCM5312_CHIP(bus->sih->chip)) && !on) {
...
dhd_bus_txdata(struct dhd_bus *bus, void *pkt)
{
	...
	if (dhd_deferred_tx || bus->fcstate || pktq_n_pkts_tot(&bus->txq) || bus->dpc_sched ||
		...
		qhead = (PKTMACLEN(pkt) == TCPACK_TAG)?TRUE:FALSE;
		deq_ret = dhd_prec_enq(bus->dhd, &bus->txq, pkt, prec, qhead);
...
#ifdef DEBUG_SDIO_TRANSFER_TIME
	sdio_transfer_calc_time_tx(num_pkt, total_len, tx_time_t);
#endif
	...
	for (i = 0; i < num_pkt; i++) {
		...
		if (!wlfc_enabled) {
			...
			if (free_pkt) {
				...
				if (wlc_freepkt_ring_unused(bus->dhd))
					wlc_freepkt_ring_enq(bus->dhd, pkt);
				else
#endif
...
#ifdef DEBUG_SDIO_TRANSFER_TIME
	sdio_transfer_glom_list[num_pkt][1] += OSL_SYSUPTIME_US() - freepkt_time;
#endif

#ifdef FREE_PKT_IN_RXF
	/* freepkt cnt over half will trigger rxf_thread */
	if (wlc_freepkt_ring_unused(bus->dhd) <= (FREEPKT_MAX_RING / 2)) {
		dhd_sched_rxf_fromq(bus->dhd);
	}
#endif
...
#ifdef DEBUG_SDIO_TRANSFER_TIME
		sdio_transfer_glom_list[num_pkt][0]++;
#endif
		for (i = 0; i < num_pkt; i++) {
...
#ifdef DHD_FW_COREDUMP
		/* collect socram dump */
		if (bus->dhd->memdump_enabled && !bus->dhd->dongle_trap_occured && !memdump) {
...
#if defined(FW_SIGNATURE)
	{"dwnldinfo",   IOV_SET_DOWNLOAD_INFO,  0, 0, IOVT_BUFFER, sizeof(fw_download_info_t) },
#endif
				...
				if ((bus->srmemsize > 0) && (i++ == 0)) {
					...
					if (bus->console_addr) {
						...
						if (dhdsdio_force_console_dump(bus))
							DHD_ERROR(("%s: Firmware console has"
							" been initialized\n", __FUNCTION__));
					}
					DHD_INFO(("%s: address (0x%08x) of sdpcm_shared invalid\n",
						__FUNCTION__, addr));
#endif /* BCMQT */
					return BCME_ERROR;
				}
...
#else /* !BL_CONSOLE_LEGACY */

/* Enhanced Bootloader Console Logging Mechanism:
 * 4-way message exchange algorithm for providing sync
 * between DHD and Bootloader on logging status
 *
 * Refer Jira: W5312-1171 to know more about the design
 *
 * This design also supports backwards compatibility for old console
 * logging mechanism in Bootloader existing in 4612 ROM (taped out)
 */
static int
dhdsdio_read_bl_cons_log(dhd_bus_t *bus)
{
	uint32 timeout = MAX_READ_TIMEOUT;
	uint32 cons_reg_msg = 0, stop_logging = 0, filled_cons_addr = 0;
	int ret = BCME_ERROR;
	uint64 start, curr;

	/* bus->fwsig_filename will be non-zero on 2 cases:
	 * 	1. If signature_path provided in insmod module param
	 * 	2. If sig file present combinedly within fwpkg (new update)
	 *
	 * If this var is zero, then it means bootloader either on
	 * ROM / RAM will not be executed, direct firmware exection
	 * starts. Hence skip reading bootloader console logs.
	 */
	if (bus->fwsig_filename[0] == 0)
		return BCME_OK;
	...
	start = DIV_U64_BY_U32(OSL_LOCALTIME_NS(), NSEC_PER_USEC);
	while (!stop_logging) {

		curr = DIV_U64_BY_U32(OSL_LOCALTIME_NS(), NSEC_PER_USEC);

		/* Stop Logging if timeout reached */
		if ((curr - start) > timeout) {
			stop_logging = 1;
			break;
		}

		/* Monitor the curent value in console register */
		ret = dhdsdio_membytes(bus, FALSE, bus->bl_cons_addr_reg,
			(uint8 *)&cons_reg_msg, sizeof(cons_reg_msg));
		if (ret != BCME_OK) {
			DHD_ERROR(("%s: error %d on reading %zu membytes at 0x%08x\n",
				__FUNCTION__, ret, sizeof(cons_reg_msg), bus->bl_cons_addr_reg));
			continue;
		}

		switch (cons_reg_msg) {
		case BL_NO_UPDATE:
			/* Continue polling until BL fills console address in register */
			continue;

		case HOST_RXED_CONS_ADDR:
			ret = dhdsdio_readconsole(bus);
			if (ret < 0)
				DHD_ERROR(("%s: Warning, Host cant read into"
						"console region\n", __FUNCTION__));
			break;

		case END_OF_BL_LOGS:

			/* Check and read for any unread console msgs */
			if (filled_cons_addr)
				ret = dhdsdio_readconsole(bus);
			if (ret != BCME_OK)
				DHD_ERROR(("%s: Warning, read console error\n", __FUNCTION__));
			...
			/* Send the ack to Bootloader */
			cons_reg_msg = HOST_LOGS_PROCESSED;
			ret = dhdsdio_membytes(bus, TRUE, bus->bl_cons_addr_reg,
					(uint8 *)&cons_reg_msg, sizeof(cons_reg_msg));
			if (ret != BCME_OK) {
				DHD_ERROR(("%s: error %d on writing %zu membytes at 0x%08x\n",
					__FUNCTION__, ret, sizeof(cons_reg_msg),
					bus->bl_cons_addr_reg));
			}

			DHD_ERROR(("BOOTLOADER CONSOLE DONE. total boot time: %llu us\n",
				DIV_U64_BY_U32((bus->rd_sboot_stats_time -
					bus->arm_oor_time), NSEC_PER_USEC)));

			stop_logging = 1;
			bus->console_addr = dngl_console_addr; /* Update FW console address */
			break;

		default:
		/* Only Possible value in register other than BL_NO_UPDATE,
		 * END_OF_BL_LOGS and HOST_RXED_CONS_ADDR is console address
		*/

			/* Initialize console_addr for first time */
			if (!filled_cons_addr) {
				bus->rd_shared_pass_time = OSL_LOCALTIME_NS();
				bus->console_addr = cons_reg_msg; /* Update BL Console address */
				filled_cons_addr = 1;
				DHD_ERROR(("Total time to fill BL console addr took %llu usec ##\n",
					DIV_U64_BY_U32((bus->rd_shared_pass_time -
						bus->arm_oor_time), NSEC_PER_USEC)));
				DHD_ERROR(("%s: BL_CONS_ADDR: 0x%x \n", __func__, cons_reg_msg));

				/* Send the ack to Bootloader */
				cons_reg_msg = HOST_RXED_CONS_ADDR;
				ret = dhdsdio_membytes(bus, TRUE, bus->bl_cons_addr_reg,
						(uint8 *)&cons_reg_msg, sizeof(cons_reg_msg));
				if (ret != BCME_OK) {
					DHD_ERROR(("%s: error %d on writing %zu "
						"membytes at 0x%08x\n", __FUNCTION__, ret,
						sizeof(cons_reg_msg), bus->bl_cons_addr_reg));
				}
			}
			break;
		}
...
#if defined(DHD_DEBUG) || defined(DHD_FW_LOG_SUPPORT)
int
dhd_bus_console_dump(struct dhd_bus *bus)
{
	return dhdsdio_readconsole(bus);
}
...
dhdsdio_readconsole(dhd_bus_t *bus)
{
	...
	if (c->buf == NULL) {
		...
		// add to avoid exception
		idx = ltoh32(c->log.idx);
		if (c->bufsize < idx) {
			DHD_ERROR(("%s: *overwrite the buf size=%d to idx=%d\n",
			           __FUNCTION__, c->bufsize, idx));
			c->bufsize = idx;
		}
		if (!c->bufsize) {
			DHD_ERROR(("%s: *invalid log len\n", __FUNCTION__));
			return BCME_BADARG;
		}
...
#ifdef BT_MCU_RTOS_CONSOLE_LOGS
static int
dhdsdio_readbtconsole(dhd_bus_t *bus)
{
	dhd_bt_console_t *c = &bus->bt_console;
	uint8 line[BT_CONSOLE_LINE_MAX], ch;
	uint32 n, idx;
	int rv;

	/* After downloading the FreeRTOS image to BT-RAM, configure the DHD assert type to 10.
	 * This is necessary for the console buffer of FreeRTOS to be polled from DHD and for
	 * log dumping to occur.
	 */

	if (g_assert_type == 10) {
		/* Read write index of console log buffer */
		if ((rv = dhdsdio_membytes(bus, FALSE, WRITE_IDX_ADDR, (uint8 *)&idx, 4)) < 0)
			return rv;
		...
		/* Allocate console buffer (one time only) */
		if (c->bt_buf == NULL) {
			c->bt_bufsize = BT_CONSOLE_BUFFER_MAX;

			if (c->bt_bufsize < idx) {
				DHD_ERROR(("%s: *overwrite the buf size=%d to idx=%d\n",
						__FUNCTION__, c->bt_bufsize, idx));
				c->bt_bufsize = idx;
			}
			if (!c->bt_bufsize) {
				DHD_ERROR(("%s: *invalid log len\n", __FUNCTION__));
			...
			if ((c->bt_buf = MALLOC(bus->dhd->osh, c->bt_bufsize)) == NULL)
				return BCME_NOMEM;
		}
		...
		if ((rv = dhdsdio_membytes(bus, FALSE, CONS_BUF_ADDR, c->bt_buf, c->bt_bufsize)) < 0)
			return rv;

		while (c->bt_last != idx) {
			for (n = 0; n < BT_CONSOLE_LINE_MAX - 2; n++) {
				if (c->bt_last == idx) {
					/* This would output a partial line.  Instead, back up
					 * the buffer pointer and output this line next time around.
					 */
					if (c->bt_last >= n) {
						...
						c->bt_last = c->bt_bufsize - n;
					goto break2;
				}
				ch = c->bt_buf[c->bt_last];
				c->bt_last = (c->bt_last + 1) % c->bt_bufsize;
				if (ch == '\n')
					break;
				line[n] = ch;
			}
...
#endif /* BT_MCU_RTOS_CONSOLE_LOGS */
#endif /* DHD_DEBUG || DHD_FW_LOG_SUPPORT */
                ...
                void *params, uint plen, void *arg, uint len, uint val_size)
{
	...
	case IOV_SVAL(IOV_SET_DOWNLOAD_INFO):
	{
		...
		DHD_ERROR(("dwnldinfo: sig=%s fw=%x,%u bl=%s,0x%x\n",
...
dhdsdio_write_vars(dhd_bus_t *bus)
{
	...
	{
#ifdef DHD_METADATA_DOWNLOAD
	if ((metadata.enabled == TRUE) && (metadata.nvram_loc.present == FALSE)) {
	...
	DHD_INFO(("Vars are at %d, Updated varsize is %d\n",
		varaddr, varsize));
	/*
...
dhdsdio_download_state(dhd_bus_t *bus, bool enter)
{
	...
	if (enter) {
		...
		if (!si_setcore(bus->sih, ARMCR4_CORE_ID, 0)) {
...
#if defined(FW_SIGNATURE)
			/* Condition for entering secure boot mode is modified, as read to
			 * MPU control register by host driver is protected when secure otp bit
			 * SDIO_RD_NONSECURE is set. Hence, presence of signature file is
			 * considered chip in SECURE mode
			 */

			if ((metadata.enabled == TRUE) && (metadata.sign_file.present == TRUE) &&
				(metadata.sign_file.sign_size)) {
				DHD_INFO(("%s: Signature File present in FW Pkg "
					"Entering Secure Boot Mode\n", __FUNCTION__));
				strlcpy(bus->fwsig_filename, bus->fw_path, sizeof(bus->fwsig_filename));
				if ((bcmerror =
					dhdsdio_bus_download_fw_sig(bus, bus->fwsig_filename, &do_wr_flops))
					!= BCME_OK) {
					goto fail;
				}
#ifdef BL_CONS_LOG
			...
			if (bus->fwsig_filename[0] != 0) {
				DHD_INFO(("%s: Signature File %s present "
						"Entering Secure Boot Mode \n",
						__FUNCTION__, bus->fwsig_filename));
				if ((bcmerror =
					dhdsdio_bus_download_fw_signature(bus, &do_wr_flops))
					!= BCME_OK) {
					goto fail;
				}
			}
...
#if defined(FW_SIGNATURE)
static int
dhdsdio_bus_download_fw_sig(dhd_bus_t *bus, char *path, bool *do_write)
{
	int bcmerror = -1;
#ifdef DHD_LINUX_STD_FW_API
	const struct firmware *fw = NULL;
	int buf_offset = 0, fw_size = 0;
#else
	int len;
	void *filep = NULL;
#endif /* DHD_LINUX_STD_FW_API */
	uint8 *srcbuf = NULL;
	uint32 cert_size = 0;
	uint32 type = DNGL_RTLV_TYPE_FW_SIGNATURE;

	if (path == NULL || path[0] == '\0') {
		DHD_ERROR(("%s: No FW Package file\n", __FUNCTION__));
		bcmerror = BCME_NOTFOUND;
		goto err;
	}

#ifdef DHD_LINUX_STD_FW_API
	fw = open_fimware_img(bus, &buf_offset, &fw_size);
	if (fw == NULL) {
		bcmerror = BCME_ERROR;
		DHD_ERROR(("%s: dhd_os_get_img(Request Firmware API) error : %d\n",
			__FUNCTION__, bcmerror));
		goto err;
	}
	DHD_INFO(("%s: dhd_os_get_img(Request Firmware API) success\n", __FUNCTION__));
#else
	/* Open file */
	filep = dhd_os_open_image1(bus->dhd, path);
	if (filep == NULL) {
		DHD_ERROR(("%s: Failed to open fwcert pkg file !\n", __FUNCTION__));
		goto err;
	}
#endif /* DHD_LINUX_STD_FW_API */

	cert_size = metadata.sign_file.sign_size;
	if (cert_size > MAX_CERT_LEN) {
		DHD_ERROR(("%s: Cert Size (%d bytes) larger than Max Cert Size (%d bytes)!\n",
			__FUNCTION__, cert_size, MAX_CERT_LEN));
		goto err;
	} else {
		DHD_INFO(("%s: FW Cert: Offset:0x%llx\t Size 0x%x(%u bytes)\n",
			__FUNCTION__, metadata.sign_file.sign_addr, cert_size, cert_size));
	}

#ifndef DHD_LINUX_STD_FW_API
	/* Update the file pointer to start of CERT File */
	dhd_os_seek_file(filep, metadata.sign_file.sign_addr);
#endif /* DHD_LINUX_STD_FW_API */
	/* Update the FW Cert download location */
	sig_file_download_addr(bus);
	DHD_INFO(("Download FW Cert file: %s,0x%x.\n", path, bus->ramtop_addr));

	/* Allocate src buffer, read in the FW Cert file */
	srcbuf = MALLOC(bus->dhd->osh, cert_size + DHD_SDALIGN);
	if (srcbuf == NULL) {
		DHD_ERROR(("%s: Failed to allocate memory %d bytes\n", __FUNCTION__, cert_size));
		goto err;
	}

#ifdef DHD_LINUX_STD_FW_API
	bcopy((const uint8 *)fw->data + metadata.sign_file.sign_addr,
		(char *)srcbuf, cert_size);
#else
	len = dhd_os_get_image_block((char*)srcbuf, cert_size, filep);
	if (len < 0) {
	...
	bcmerror = dhdsdio_download_rtlv(bus, type, cert_size, srcbuf);

	bus->fwsig_download_addr = bus->ramtop_addr;
	bus->fwsig_download_len = cert_size;
...
err:
	...
	if (srcbuf)
		MFREE(bus->dhd->osh, srcbuf, cert_size + DHD_SDALIGN);
...
dhdsdio_bus_download_fw_signature(dhd_bus_t *bus, bool *do_write)
{
	...
	DHD_INFO(("%s: Enter\n", __FUNCTION__));
	DHD_INFO(("%s: bl=%s,%x fw=%x,%u sig=%s,%x,%u"
		" stat=%x,%u ram=%x,%x\n", __FUNCTION__,
	...
	if (bus->fwsig_filename[0] == 0) {
		DHD_ERROR(("%s: missing signature file\n", __FUNCTION__));
		bcmerror = BCME_NOTFOUND;
		goto exit;
	}
...
dhdsdio_bus_download_ram_bootloader(dhd_bus_t *bus)
{
	...
	DHD_ERROR(("download_bloader: %s,0x%x. ramtop=0x%x\n",
...
open_fimware_img(dhd_bus_t *bus, uint32* buf_offset, uint32* fw_size)
{
	int ret = BCME_OK;
	 ...
	 */
	ret = dhd_os_get_img_fwreq(&fw, bus->fw_path);
	if (ret < 0) {
	...
	ret = fwpkg_init(&bus->fwpkg, fw->size, fw->data);
	if (ret == BCME_ERROR) {
		goto exit;
	}
	fwpkg = &bus->fwpkg;

	ret = fwpkg_open_firmware_img(fwpkg, buf_offset);
	if (ret == BCME_ERROR) {
		goto exit;
	}

	if (ret == BCME_UNSUPPORTED) {
		*fw_size = fwpkg->file_size;
		DHD_ERROR(("%s: Using SINGLE image (size %d)\n",
			__FUNCTION__, fwpkg->file_size));
		ret = BCME_OK;
	} else {
		*fw_size = fwpkg_get_firmware_img_size(fwpkg);
		strlcpy(bus->fwsig_filename, bus->fw_path, sizeof(bus->fwsig_filename));
...
static const struct firmware *
open_signature_img(dhd_bus_t *bus, uint32* buf_offset, uint32* sig_size)
{
	int bcmerror = BCME_OK;
	fwpkg_info_t *fwpkg = NULL;
	const struct firmware *fw = NULL;

	bcmerror = dhd_os_get_img_fwreq(&fw, bus->fwsig_filename);
	if (bcmerror < 0) {
		DHD_ERROR(("dhd_os_get_img(Request Firmware API) error : %d\n",
			bcmerror));
		goto exit;
	...
	*sig_size = fwpkg_get_signature_img_size(fwpkg);
	return fw;
...
#else
static int
open_signature_img(dhd_bus_t *bus, char *path, void **filep)
{
	int bcmerror = BCME_OK;
	fwpkg_info_t *fwpkg = NULL;
	int sig_act_size = 0;
	...
	/* Fetch File Size */
	sig_act_size = fwpkg_get_signature_img_size(fwpkg);
...
static int
dhdsdio_verify_sig_file_size(dhd_bus_t *bus, char *path)
{
	int bcmerror = BCME_OK;
	uint8 buf[SIG_HEADER_LEN];
	int sig_act_size = 0, sig_exp_size = 0, i = 0;
#ifdef DHD_LINUX_STD_FW_API
	const struct firmware *fw = NULL;
	int buf_offset = 0;
#else
...
#ifdef DHD_LINUX_STD_FW_API
	fw = open_signature_img(bus, &buf_offset, &sig_act_size);
	if (fw == NULL) {
		bcmerror = BCME_ERROR;
		DHD_ERROR(("%s: dhd_os_get_img(Request Firmware API) error : %d\n",
			__FUNCTION__, bcmerror));
	...
	/* read the 'X' bytes of the buffer */
	memset(buf, 0, SIG_HEADER_LEN);
	bcopy((const uint8 *)fw->data + buf_offset,
		(uint8 *)buf, SIG_HEADER_LEN);
#else
	/* Open file, get size and set File Pointer */
	if (!(sig_act_size = open_signature_img(bus, path, &filep)))
		goto exit;

	/* Read the first 'X' bytes of the file to a buffer */
	memset(buf, 0, SIG_HEADER_LEN);
	if (dhd_os_get_image_block(buf, SIG_HEADER_LEN, filep) < 0) {
		bcmerror = BCME_ERROR;
		goto exit;
	}
#endif /* DHD_LINUX_STD_FW_API */
	for (i = SIG_HEADER_LEN - 1; i >= SIG_HEADER_LEN - SIG_SIZE_LEN; i--) {
		sig_exp_size = (sig_exp_size << 8) | buf[i];
	}

	sig_exp_size += SIG_HEADER_LEN;

	if (sig_exp_size != sig_act_size)
	{
...
dhdsdio_download_sig_file(dhd_bus_t *bus, char *path, uint32 type)
{
	...
	fw = open_signature_img(bus, &buf_offset, &sig_size);
	if (fw == NULL) {
		bcmerror = BCME_ERROR;
		DHD_ERROR(("%s: dhd_os_get_img(Request Firmware API) error : %d\n",
			__FUNCTION__, bcmerror));
	...
	if (sig_size <= 0 || sig_size > MEMBLOCK) {
		DHD_ERROR(("%s: invalid fwsig size %u\n", __FUNCTION__, sig_size));
		bcmerror = BCME_BUFTOOSHORT;
		goto exit;
	}

	dest_size = ROUNDUP(sig_size, 8);
#ifdef DHD_METADATA_DOWNLOAD
	if ((metadata.enabled == TRUE) && (metadata.sign_file.present == TRUE) &&
		(metadata.sign_file.sign_size)) {
	...
	ret = memcpy_s(srcbuf, dest_size, (fw->data + buf_offset), sig_size);
	if (ret != 0) {
	...
	if (bcmerror) {
		DHD_ERROR(("%s: error %d on writing %d membytes at 0x%08x\n",
			__FUNCTION__, bcmerror, sig_size, bus->ramtop_addr));
		goto exit;
	}
...
dhdsdio_download_sig_file(dhd_bus_t *bus, char *path, uint32 type)
{
	...
	/* Open file, get size and set File Pointer */
	if (!(srcsize = open_signature_img(bus, path, &filep)))
		goto exit;
...
dhdsdio_bus_write_fwsig(dhd_bus_t *bus, char *fwsig_path, char *nvsig_path)
{
	...
	/* Validate size of FW Signature File */
	bcmerror = dhdsdio_verify_sig_file_size(bus, fwsig_path);
	if (bcmerror) {
		goto exit;
	}
...
#ifdef BL_SIM_OTP
static int
dhdsdio_download_otp_file(dhd_bus_t *bus, char *path)
{
	int bcmerror = BCME_OK;
	void *filep = NULL;
	uint8 *srcbuf = NULL;
	...
	if (srcsize <= 0 || srcsize > MEMBLOCK) {
		DHD_ERROR(("%s: invalid simulation otp size %u\n", __FUNCTION__, srcsize));
		bcmerror = BCME_BUFTOOSHORT;
		goto exit;
	}
	...
	/* 4612 simulation otp address */
	/* Write the value block */
	if (dest_size > 0) {
		dest_addr = bus->otp_addr;
		DHD_ERROR(("%s: writing %d membytes to 0x%08x\n",
			__FUNCTION__, dest_size, dest_addr));
		bcmerror = dhdsdio_membytes(bus, TRUE, dest_addr, srcbuf, dest_size);
		if (bcmerror) {
			DHD_ERROR(("%s: error %d on writing %d membytes to 0x%08x\n",
				__FUNCTION__, bcmerror, dest_size, dest_addr));
			goto exit;
...
dhd_bus_stop(struct dhd_bus *bus, bool enforce_mutex)
{
	...
	if ((bus->dhd->busstate == DHD_BUS_DOWN) || bus->dhd->hang_was_sent) {
		...
		if (KSO_ENAB(bus)) {
			...
			saveclk = bcmsdh_cfg_read(bus->sdh,
	 ...
	 * Gets updated to correct value after receiving the first
...
#ifdef CUSTOM_TXGLOM
	bcmsdh_set_mode(bus->sdh, CUSTOM_TXGLOM);
	DHD_ERROR(("%s: tx glom mode = %s\n", __func__,
		(CUSTOM_TXGLOM)?"SDPCM_TXGLOM_MDESC":"SDPCM_TXGLOM_CPY"));
#endif /* CUSTOM_TXGLOM */
}
...
dhd_bus_init(dhd_pub_t *dhdp, bool enforce_mutex)
{
...
#if defined(BCMQT)
	{
		...
		ret = dhdsdio_membytes(bus, FALSE, SEC_H2D_REG_ADDR,
				(uint8 *)&status, sizeof(status));
		if (ret != BCME_OK) {
			DHD_ERROR(("%s: error %d on reading %zu membytes at 0x%08x\n",
				__FUNCTION__, ret, sizeof(status), SEC_H2D_REG_ADDR));
	        }
		DHD_ERROR(("BL SEC: status %d\n", status));
		if (status != 0) {
			ret = -1;
			goto exit;
		}
	...
	dhd_timeout_start(&tmo, (qt_dngl_timeout ? qt_dngl_timeout : DHD_WAIT_READSHARED) * 1000);
	fw_init_start = OSL_LOCALTIME_NS();
	ready = 0;
	...
	fw_init_end = OSL_LOCALTIME_NS();
	elapsed = tmo.elapsed;
	DHD_ERROR(("### Total time for chip to be ready %llu usec ###\n",
		DIV_U64_BY_U32((fw_init_end - fw_init_start), NSEC_PER_USEC)));
	DHD_ERROR(("SDIO read shared took %u usec "
	...
	if (bus->glom) {
		...
		for (num = 0; pfirst; rxseq++, pfirst = pnext) {
			...
			if (PKTLEN(osh, pfirst) == 0) {
				...
				&reorder_info_len) != 0) {
				DHD_ERROR(("%s: drop since rx protocol error\n", __FUNCTION__));
				bus->dhd->rx_errors++;
				PKTFREE(osh, pfirst, FALSE);
				continue;
			}
		...
		for (idx = 0; idx < DHD_MAX_IFS; idx++) {
			if (list_head[idx]) {
				...
				temp = list_head[idx];
				while (temp) {
					void *next = PKTNEXT(osh, temp);
#ifdef DHD_LB_RXP
					PKTSETNEXT(osh, temp, NULL);
					dhd_lb_rx_pkt_enqueue(bus->dhd, temp, idx);
#endif /* DHD_LB_RXP */
#ifdef DEBUG_SDIO_TRANSFER_TIME
					sdio_transfer_calc_rx_bytes(PKTLEN(osh, temp));
#endif
					temp = next;
					cnt++;
				}
...
#ifdef DEBUG_SDIO_TRANSFER_TIME
			sdio_transfer_time_rxglom += OSL_SYSUPTIME_US() - time;
#endif
...
deliver:
...
#ifdef DHD_LB_RXP
		{
			...
			while (temp) {
				void *next = PKTNEXT(osh, temp);
				PKTSETNEXT(osh, temp, NULL);
				dhd_lb_rx_pkt_enqueue(bus->dhd, temp, ifidx);
				temp = next;
			}
		}
#else
...
#ifdef DHD_LB_RXP
	dhd_lb_rx_napi_dispatch(bus->dhd);
#endif /* DHD_LB_RXP */
...
#ifdef DEBUG_SDIO_TRANSFER_TIME
			sdio_transfer_calc_time_rx(framecnt);
#endif
...
#ifdef BT_MCU_RTOS_CONSOLE_LOGS
		bus->bt_console.bt_count += dhd_watchdog_ms;
                if (bus->bt_console.bt_count >= dhdp->dhd_console_ms) {
                        bus->bt_console.bt_count -= dhdp->dhd_console_ms;
                        if (dhdsdio_readbtconsole(bus) < 0)
                                dhdp->dhd_console_ms = 0;	/* On error, stop trying */
                }
#endif /* BT_MCU_RTOS_CONSOLE_LOGS */
	}
...
dhdsdio_chipmatch(uint16 chipid)
{
	...
	if (chipid == BCM43711_CHIP_ID)
		return TRUE;
	if (chipid == BCM43710_CHIP_ID)
		return TRUE;
	if (chipid == BCM4612_CHIP_ID)
		return TRUE;
	if (chipid == BCM4611_CHIP_ID)
		return TRUE;
	if (chipid == BCM5312_CHIP_ID)
		return TRUE;
	return FALSE;
}
...
#ifdef WL_STATIC_IF
		/* Populate address from file if existing,
		 * or randomly otherwise.
		 */
		dhd_read_macaddr_from_file(bus->dhd, &bus->dhd->mac);
#else
                     ...
                     uint16 devid)
{
	...
	if (dhd_gci_chipctrl_reg4 && (bcmsdh_reg_read(bus->sdh, si_enum_base(devid), 2) == 0x4612)) {
	...
	if (!DHD_NOPMU(bus)) {
		...
		if (!si_setcore(bus->sih, ARMCR4_CORE_ID, 0)) {
			...
			case BCM4611_CHIP_ID:
				...
				break;
			case BCM5312_CHIP_ID:
				bus->dongle_ram_base = CR4_5312_RAM_BASE;
				break;
...
#ifdef BCMDBG
		if (dhd_dongle_rambase)
			bus->dongle_ram_base = dhd_dongle_rambase;
#endif /* BCMDBG */
...
int
dhd_bus_download_firmware(struct dhd_bus *bus, osl_t *osh, char *pfw_path,
                          char *pnv_path, char *psig_path, char *pboot_path,
                          uint32 otp_addr, char *pclm_path)
{
	int ret;
...
#ifdef FW_SIGNATURE
	strlcpy(bus->fwsig_filename, psig_path, sizeof(bus->fwsig_filename));

	DHD_ERROR(("%s: firmware path=%s, nvram path=%s sig path:%s\n",
			__FUNCTION__, bus->fw_path, bus->nv_path, bus->fwsig_filename));
#endif
#ifdef BL_RAM_BOOT
	strlcpy(bus->bootloader_filename, pboot_path, sizeof(bus->bootloader_filename));
	DHD_ERROR(("%s:: boot path %s\n", __FUNCTION__, bus->bootloader_filename));
#endif /* BL_RAM_BOOT */
#ifdef BL_SIM_OTP
	bus->otp_addr = otp_addr;
	DHD_ERROR(("%s:: otp addr 0x%04x\n", __FUNCTION__, bus->otp_addr));
#endif /* BL_SIM_OTP */
	strlcpy(bus->clm_filename, pclm_path, sizeof(bus->clm_filename));
	bus->clm_addr = 0;
	DHD_ERROR(("%s: clm path:%s\n", __FUNCTION__, bus->clm_filename));
...
dhdsdio_release(dhd_bus_t *bus, osl_t *osh)
{
	...
	if (bus) {
		...
		// always call to free the si resources
...
dhd_is_metadata_present(dhd_bus_t *bus, char *fwpath)
{
	...
	char metadata_str[METADATA_LEN] = {0x3e, 0x7a, 0xda, 0x7a};
	int ret = BCME_OK;
#ifdef DHD_LINUX_STD_FW_API
	const struct firmware *fw = NULL;
	int buf_offset = 0, fw_size = 0;
	...
	/* read the 'X' bytes of the buffer */
	memset(buf, 0, METADATA_LEN);
	bcopy((const uint8 *)fw->data + buf_offset + fw_size - METADATA_LEN,
		(uint8 *)buf, METADATA_LEN);
#else
	if (!fwpath)
		return BCME_BADARG;
	 ...
	 * entry or in module param.
	 */
	ret = fwpkg_open_firmware_img(fwpkg, fwpath, &file);
	if (ret == BCME_ERROR) {
		goto exit;
	}
	...
	if (ret == BCME_UNSUPPORTED) {
		...
		size = fwpkg_get_firmware_img_size(fwpkg);
		strlcpy(bus->fwsig_filename, fwpath, sizeof(bus->fwsig_filename));
		DHD_ERROR(("%s Using COMBINED image (size %d)\n",
			__FUNCTION__, size));
	}

	/* seek to the last 'X' bytes in the file */
	if (dhd_os_seek_file(file, size - METADATA_LEN) != BCME_OK) {
		ret = BCME_ERROR;
		goto exit;
	}
	...
	memset(buf, 0, METADATA_LEN);
	if (dhd_os_get_image_block(buf, METADATA_LEN, file) < 0) {
		ret = BCME_ERROR;
		goto exit;
	}
#endif /* DHD_LINUX_STD_FW_API */
	...
	ret = memcmp(buf, metadata_str, sizeof(metadata_str));

	DHD_ERROR(("%s: Metadata %s\n", __FUNCTION__, (ret == BCME_OK) ? "Present" : "Absent"));
...
#else
	if (file) {
		dhd_os_close_image1(bus->dhd, file);
	}
#endif /* DHD_LINUX_STD_FW_API */
	return ret;
}
...
dhd_get_metadata_param(uint64 *var, int *prev_pos, int next_pos, uint8 *metadata_block)
{
	...
	DHD_INFO(("%s: prev_pos: %d next_pos: %d \n",
...
static int
dhd_get_metadata_ver(dhd_bus_t *bus, char *fwpath)
{
	int ret = BCME_OK;
	char version[METADATA_VER_LEN];
#ifdef DHD_LINUX_STD_FW_API
	const struct firmware *fw = NULL;
	int buf_offset = 0, fw_size = 0;
#else
	...
	size = size - METADATA_LEN;

	if (dhd_os_seek_file(file, size - METADATA_VER_LEN) != BCME_OK) {
		ret = BCME_ERROR;
		goto exit;
	}

	memset(version, 0, METADATA_VER_LEN);
	if (dhd_os_get_image_block(version, METADATA_VER_LEN, file) < 0) {
		ret = BCME_ERROR;
		goto exit;
	}
#endif /* DHD_LINUX_STD_FW_API */
	metadata.version = *version;
	DHD_ERROR(("%s: Metadata Version: 0x%x \n", __FUNCTION__, metadata.version));

	if (metadata.version != METADATA_STRUCT_VERSION)
	{
		DHD_ERROR(("%s: Invalid Meta Data Version (0x%x)\n"
				"Failed to allocate Meta Data structure \n",
				__FUNCTION__, metadata.version));
		ret = BCME_ERROR;
		goto exit;
	}
...
static int
dhd_get_metadata_size(dhd_bus_t *bus, char *fwpath)
{
	int i = 0;
	int ret = BCME_OK;
	uchar buf[METADATA_JUMP_VALUE_LEN];
#ifdef DHD_LINUX_STD_FW_API
	const struct firmware *fw = NULL;
	int buf_offset = 0, fw_size = 0;
#else
	...
	memset(buf, 0, METADATA_JUMP_VALUE_LEN);
	bcopy((const uint8 *)fw->data + buf_offset + fw_size -
		METADATA_LEN - METADATA_VER_LEN - METADATA_JUMP_VALUE_LEN,
		(uint8 *)buf, METADATA_JUMP_VALUE_LEN);
#else
	if (!fwpath)
		return BCME_BADARG;
	...
	size = size - METADATA_LEN - METADATA_VER_LEN;

	if (dhd_os_seek_file(file, size - METADATA_JUMP_VALUE_LEN) != BCME_OK) {
		ret = BCME_ERROR;
		goto exit;
	}

	memset(buf, 0, METADATA_JUMP_VALUE_LEN);
	if (dhd_os_get_image_block(buf, METADATA_JUMP_VALUE_LEN, file) < 0) {
		ret = BCME_ERROR;
		goto exit;
	}
#endif /* DHD_LINUX_STD_FW_API */
	for (i = 0; i < METADATA_JUMP_VALUE_LEN; i++) {
		metadata.jump_value = (metadata.jump_value << 8) | buf[i];
	}

	DHD_ERROR(("%s: Metadata Size: 0x%x (%d bytes)\n",
			__FUNCTION__, metadata.jump_value, metadata.jump_value));
exit:
#ifdef DHD_LINUX_STD_FW_API
	if (fw)
		dhd_os_close_img_fwreq(fw);
...
static int
dhd_fetch_metadata_array(dhd_bus_t *bus, char *fwpath, uint8 *metadata_block)
{
	int ret = BCME_OK;
	int i = 0;
#ifdef DHD_LINUX_STD_FW_API
	...
	/* read the 'X' bytes of the buffer */
	memset(metadata_block, 0, metadata.jump_value);
	/* Fetch Complete Metadata contents */
	bcopy((const uint8 *)fw->data + buf_offset + fw_size -
		METADATA_LEN - METADATA_VER_LEN - METADATA_JUMP_VALUE_LEN - metadata.jump_value,
		(uint8 *)metadata_block, metadata.jump_value);
#else
	if (!fwpath)
		return BCME_BADARG;
	...
	size = size - METADATA_LEN - METADATA_VER_LEN - METADATA_JUMP_VALUE_LEN;

	if (dhd_os_seek_file(file, size - metadata.jump_value) != BCME_OK) {
		ret = BCME_ERROR;
		goto exit;
	}
	...
	if (dhd_os_get_image_block((char*)metadata_block, metadata.jump_value, file) < 0) {
		ret = BCME_ERROR;
		goto exit;
	}
#endif /* DHD_LINUX_STD_FW_API */

	DHD_ERROR(("%s: Metadata Array: \n", __FUNCTION__));
	for (i = 0; i < metadata.jump_value; i++)
	{
		DHD_ERROR(("0x%02x ", metadata_block[i]));
		if ((i + 1) % 10 == 0) {
			DHD_ERROR(("\n")); // Start a new line after every 10 entries
		}
	}
	DHD_ERROR(("\n"));
exit:
...
dhd_metadata_end_reached(uint8 *metadata_block, int prev_pos, int next_pos)
{
	...
	dhd_get_metadata_param(&metadata_value,
			&prev_pos, next_pos, metadata_block);

	DHD_INFO(("%s: metadata value: 0x%llx \n", __FUNCTION__, metadata_value));

	if (metadata_value != METADATA_END) {
		DHD_INFO(("Metadata end not reached !!!\n"));
		return FALSE;
	}

	DHD_ERROR(("Metadata End Reached !\n"));
	return TRUE;
}
...
dhd_get_metadata_struct_params(uint8 *metadata_block, dhd_bus_t *bus)
{
	...
	do {
		...
		dhd_get_metadata_param(&tlv_type, &prev_pos,
				next_pos, metadata_block);

		/* Fetch Length */
		next_pos += METADATA_TLV_LEN_LEN;
		dhd_get_metadata_param(&tlv_len, &prev_pos,
				next_pos, metadata_block);

		/* Fetch Version */
		next_pos += METADATA_TLV_VER_LEN;
		dhd_get_metadata_param(&tlv_ver, &prev_pos,
				next_pos, metadata_block);
		...
		switch (tlv_type) {
			...
			case CHIP_DETAILS:
				...
				DHD_ERROR(("CHIP DETAILS \n"
					"\tType:\t\t0x%2x \n"
					"\tLength:\t\t0x%2x \n"
					"\tVersion:\t0x%2x \n"
					"\tChip ID:\t%s \n",
					metadata.chip_detail.type,
					metadata.chip_detail.len,
					metadata.chip_detail.ver,
			...
			case EXCL_MEM_LOCATION:
				...
				next_pos += content_len/MAX_EXCL_MEM_CONTENT;
				...
				DHD_ERROR(("FW EXCLUSIVE MEMORY REGION DETAILS \n"
					"\tType:\t\t0x%2x \n"
					"\tLength:\t\t0x%2x \n"
					"\tVersion:\t0x%2x \n"
					"\tMemory Start:\t0x%llx \n"
					...
					metadata.excl_mem.type,
					metadata.excl_mem.len,
					metadata.excl_mem.ver,
			...
			case STARTUP_LOCATION:
				...
				DHD_ERROR(("STARTUP LOCATION DETAILS \n"
					"\tType:\t\t0x%2x \n"
					"\tLength:\t\t0x%2x \n"
					"\tVersion:\t0x%2x \n"
					"\tOffset:\t\t0x%llx \n",
					metadata.startup_loc.type,
					metadata.startup_loc.len,
					metadata.startup_loc.ver,
			...
			case NVRAM_LOCATION:
				...
				next_pos += content_len/MAX_NVRAM_CONTENT;
				...
				DHD_ERROR(("NVRAM LOCATION DETAILS \n"
					"\tType:\t\t0x%2x \n"
					"\tLength:\t\t0x%2x \n"
					"\tVersion:\t0x%2x \n"
					"\tNVRAM Start:\t0x%llx \n"
					...
					metadata.nvram_loc.type,
					metadata.nvram_loc.len,
					metadata.nvram_loc.ver,
			...
			case CONS_ADDR:
				...
				dhd_get_metadata_param(&(metadata.cons_addr.cons_start),
						&prev_pos, next_pos, metadata_block);
				DHD_ERROR(("CONSOLE ADDRESS DETAILS \n"
					"\tType:\t\t0x%2x \n"
					"\tLength:\t\t0x%2x \n"
					"\tVersion:\t0x%2x \n"
					"\tCons Start:\t0x%llx \n",
					metadata.cons_addr.type,
					metadata.cons_addr.len,
					metadata.cons_addr.ver,
			...
			case FW_DL_LOCATION:
				...
				DHD_ERROR(("FW SECTION DOWNLOAD LOCATION DETAILS \n"
							"\tType:\t\t0x%2x \n"
							"\tLength:\t\t0x%2x \n"
							"\tVersion:\t0x%2x \n"
							"\tNo. of Sections:\t%lld \n",
							metadata.fw_dl_loc.type,
							metadata.fw_dl_loc.len,
							metadata.fw_dl_loc.ver,
			...
			case TAG_INFO_LOCATION:
				...
				dhd_get_metadata_param(&(metadata.tag_info.addr),
						&prev_pos, next_pos, metadata_block);
				DHD_ERROR(("TAG-INFO LOCATION DETAILS \n"
					"\tType:\t\t0x%2x \n"
					"\tLength:\t\t0x%2x \n"
					"\tVersion:\t0x%2x \n"
					"\tAddress:\t0x%llx \n",
					metadata.tag_info.type,
					metadata.tag_info.len,
					metadata.tag_info.ver,
			...
			case SIGNATURE_FILE:
				...
				content_len = metadata.sign_file.len - METADATA_TLV_VER_LEN;

				next_pos += content_len / MAX_CERT_CONTENT;
				dhd_get_metadata_param(&(metadata.sign_file.sign_addr),
						&prev_pos, next_pos, metadata_block);

				next_pos += content_len / MAX_CERT_CONTENT;
				dhd_get_metadata_param(&(metadata.sign_file.sign_size),
						&prev_pos, next_pos, metadata_block);

				DHD_ERROR(("SIGNATURE DETAILS \n"
					"\tType:\t\t0x%2x \n"
					"\tLength:\t\t0x%2x \n"
					"\tVersion:\t0x%2x \n"
					"\tSignature Addr:\t0x%llx \n"
					"\tSignature Size:\t0x%llx \n",
					metadata.sign_file.type,
					metadata.sign_file.len,
					metadata.sign_file.ver,
					metadata.sign_file.sign_addr,
					metadata.sign_file.sign_size));

				if ((metadata.enabled == TRUE) && (metadata.sign_file.present == TRUE) &&
					((!metadata.sign_file.sign_addr) || (metadata.sign_file.sign_addr < bus->dongle_ram_base))) {
						DHD_INFO(("FW SIGN NOT PART OF FW\n"));
				} else
			...
			case CLM_BLOB_LOCATION:
				...
				dhd_get_metadata_param(&(metadata.clm_blob_loc.addr),
						&prev_pos, next_pos, metadata_block);
				DHD_ERROR(("CLM BLOB LOCATION DETAILS \n"
					"\tType:\t\t0x%2x \n"
					"\tLength:\t\t0x%2x \n"
					"\tVersion:\t0x%2x \n"
					"\tAddress:\t0x%llx \n",
					metadata.clm_blob_loc.type,
					metadata.clm_blob_loc.len,
					metadata.clm_blob_loc.ver,
...
static int
dhd_update_tag_info(dhd_bus_t *bus, char *fwpath)
{
	int ret = BCME_OK;
	uint8* buffer = NULL;
	int i = 0, j = 0;
	...
	if ((metadata.enabled == TRUE) && (metadata.sign_file.present == TRUE) &&
		(metadata.sign_file.sign_size)) {
		if (dhd_os_seek_file(file, metadata.sign_file.sign_addr - MAX_TAG_INFO_LEN) != BCME_OK) {
			ret = BCME_ERROR;
			goto exit;
		}
	} else
	if (dhd_os_seek_file(file, size - metadata.jump_value - MAX_TAG_INFO_LEN) != BCME_OK) {
		ret = BCME_ERROR;
		goto exit;
	}
	...
	if (dhd_os_get_image_block((char *)buffer, MAX_TAG_INFO_LEN, file) < 0) {
		ret = BCME_ERROR;
		goto exit;
	}
#endif /* DHD_LINUX_STD_FW_API */

	DHD_INFO(("%s: BUFFER Array: \n", __FUNCTION__));

	for (i = 0; i < MAX_TAG_INFO_LEN; i++)
	{
		DHD_INFO(("0x%02x ", buffer[i]));
		if ((i + 1) % 10 == 0) {
			DHD_INFO(("\n")); // Start a new line after every 10 entries
		}
	}
	...
	for (i = 0; i < MAX_TAG_INFO_LEN; i++)
	{
		if (buffer[i] == 0x3c && buffer[i+1] == 0x55 && buffer[i+2] == 0x43)
		{
			start_index = i;

			for (j = 0; j < MAX_TAG_INFO_LEN; j++) {
				buffer[j] = buffer[i+j - 1];
			}
		}
	}

	tag_info_len = MAX_TAG_INFO_LEN - start_index;

	DHD_ERROR(("%s: Writing %d membytes at 0x%08llx\n",
...
#if defined(DHD_DEBUG_DOWNLOADTIME) || defined(BCMQT_HW)
	unsigned long initial_jiffies = 0;
	uint firmware_sz = 0;
#endif
	int offset_end = bus->ramsize;
	const struct firmware *fw = NULL;
	int buf_offset = 0, residual_len = 0, file_size = 0;

	DHD_INFO(("%s: download firmware %s\n", __FUNCTION__, pfw_path));

	fw = open_fimware_img(bus, &buf_offset, &file_size);
	if (fw == NULL) {
		bcmerror = BCME_ERROR;
		DHD_ERROR(("%s: dhd_os_get_img(Request Firmware API) error : %d\n",
			__FUNCTION__, bcmerror));
...
dhdsdio_download_code_file(struct dhd_bus *bus, char *pfw_path)
{
...
#ifdef DHD_METADATA_DOWNLOAD
	if ((metadata.enabled == TRUE) && (metadata.startup_loc.present == TRUE)) {
		startup_addr = metadata.startup_loc.addr; /* Overwrite default RAMBASE addr
							   * if TLV present.
							   */
	}
#endif
	...
	if (file_size > bus->ramsize) {
		DHD_ERROR(("%s: FW Size (%d bytes) larger than RAM Size (%d bytes)!\n",
				__FUNCTION__, file_size, bus->ramsize));
		goto err;
	}
...
#ifdef BL_RAM_BOOT
	if (bus->dongle_ram_base == ARMCR4_RAM_BOOTLOADER)
		startup_addr = bus->dongle_ram_base;
#endif /* BL_RAM_BOOT */

	/* Move FW Download Start address to Startup Address */
	offset += startup_addr;

#ifdef DHD_METADATA_DOWNLOAD
	if (metadata.enabled == TRUE &&
		metadata.startup_loc.present == FALSE) {
		DHD_ERROR(("%s: (0x%x) - TLV Absent."
					"Proceeding with Normal "
					"Startup Location Download...\n",
					__FUNCTION__, STARTUP_LOCATION));
	}

	if (metadata.enabled == TRUE &&
		metadata.excl_mem.present == FALSE) {
		DHD_ERROR(("%s: (0x%x) - TLV Absent.\n",
...
#if defined(DHD_DEBUG_DOWNLOADTIME) || defined(BCMQT_HW)
		firmware_sz += len;
		DHD_ERROR(("FW Download Progress: %d%% \n", ((firmware_sz*100)/file_size)));
#endif
	}
...
#ifdef DHD_METADATA_DOWNLOAD
static int
dhd_update_reset_instr(dhd_bus_t *bus, char *pfw_path)
{
	int bcmerror = -1;
	int offset = 0;
	int len;
	...
	while (file_size) {
		len = MIN(file_size, memblock_size);
		bcopy((const uint8 *)fw->data + buf_offset, (uint8 *)memptr, len);
...
dhdsdio_sectionwise_fw_download(struct dhd_bus *bus, char *pfw_path)
{
	...
	int total_fw_size = 0;
#ifdef DHD_LINUX_STD_FW_API
	int offset_end = bus->dongle_ram_base + bus->ramsize;
	const struct firmware *fw = NULL;
...
#else
	/* Open FW image provided in the insmod command */
	bcmerror = fwpkg_init(&bus->fwpkg, pfw_path);
	if (bcmerror == BCME_ERROR) {
		goto err;
	}
	...
	total_fw_size = fw_all_sec_size + metadata.jump_value + tag_info_len +
		METADATA_LEN + METADATA_VER_LEN + METADATA_JUMP_VALUE_LEN + 1;

	if ((metadata.enabled == TRUE) && (metadata.sign_file.present == TRUE) &&
		((!metadata.sign_file.sign_addr) || (metadata.sign_file.sign_addr < bus->dongle_ram_base))) {
		DHD_INFO(("%s: FW contains FW Cert\n", __FUNCTION__));
	} else
	if (file_size > total_fw_size) {
		DHD_ERROR(("%s: FW Size (%d bytes) Metadata FW Size (%d bytes) mismatch !\n",
				__FUNCTION__, file_size, total_fw_size));
		goto err;
	}
#if defined(DHD_DEBUG_DOWNLOADTIME) || defined(BCMQT_HW)
	initial_jiffies = jiffies;
	...
	for (idx = 0; idx < metadata.fw_dl_loc.fw_sec_num; idx++) {
		...
		/* Update download block size to section size */
		memptr = memblock = MALLOC(bus->dhd->osh, memblock_size + DHD_SDALIGN);
		...
		while (residual_len) {
			...
			if (offset >= offset_end) {
				DHD_ERROR(("%s: invalid address access to %x (offset end: %x) "
					"rem_size 0x%x dl_end 0x%x\n",
					__FUNCTION__, offset, offset_end, rem_size, dl_end));
				bcmerror = BCME_ERROR;
				goto err;
			}
			residual_len -= len;
		...
		while ((len = dhd_os_get_image_block((char*)memptr,
			(offset + memblock_size < dl_end) ? memblock_size : rem_size, image))
			&& offset < dl_end)
		{
			if (len < 0) {
				DHD_ERROR(("%s: dhd_os_get_image_block failed (%d)\n",
			...
			DHD_INFO(("%s: Writing %d membytes at 0x%08x\n",
...
#if defined(DHD_DEBUG_DOWNLOADTIME) || defined(BCMQT_HW)
	DHD_ERROR(("Firmware download time for %u bytes: %u ms\n",
			file_size, jiffies_to_msecs(jiffies - initial_jiffies)));
#endif

	/* Update Reset Instruction */
	dhd_update_reset_instr(bus, pfw_path);
...
dhdsdio_download_clm_file(dhd_bus_t *bus, char *path)
{
	 ...
	 * entry or in module param.
	 */
	bcmerror = dhd_os_get_img_fwreq(&fw, bus->clm_filename);
	if (bcmerror < 0) {
		DHD_ERROR(("dhd_os_get_img(Request Firmware API) error : %d\n",
			bcmerror));
		goto err;
	...
	if (fw == NULL || fw->size <= 0) {
		DHD_ERROR(("%s: invalid clm file size %ld\n", __FUNCTION__, fw->size));
		bcmerror = BCME_ERROR;
		goto err;
	}

	if (fw->size > bus->ramsize) {
		DHD_ERROR(("%s: FW Size (%ld bytes) larger than RAM Size (%d bytes)!\n",
				__FUNCTION__, fw->size, bus->ramsize));
		goto err;
	}
	...
	DHD_INFO(("download clmblob file: %s,0x%x.\n",
		path, bus->clm_addr));
...
dhdsdio_download_clm_file(dhd_bus_t *bus, char *path)
{
	...
	/* Open file, get size */
	image = dhd_os_open_image1(bus->dhd, path);
	if (image == NULL) {
		DHD_ERROR(("%s: Failed to open fw file !\n", __FUNCTION__));
		goto err;
	}
	...
	while ((len = dhd_os_get_image_block((char*)memptr, memblock_size, image))) {
		...
		offset += memblock_size;
		if ((offset - bus->clm_addr) >= file_size)
			break;
	}
...
#ifdef DHD_DEBUG_DOWNLOADTIME
	DHD_INFO(("nvram download time for %u bytes: %u ms\n",
		firmware_sz, (OSL_SYSUPTIME() - initial_jiffies)));
#endif
err:
	if (memblock)
...
_dhdsdio_download_firmware(struct dhd_bus *bus)
{
	...
	if (BCM4612_CHIP(bus->sih->chip) || BCM5312_CHIP(bus->sih->chip)) {
		chipclkcsr = bcmsdh_cfg_read(bus->sdh, SDIO_FUNC_1, SBSDIO_FUNC1_CHIPCLKCSR, &err);
		if (!(chipclkcsr & SBSDIO_FORCE_HT))
			bcmsdh_cfg_write(bus->sdh, SDIO_FUNC_1, SBSDIO_FUNC1_CHIPCLKCSR,
					(chipclkcsr | SBSDIO_FORCE_HT), &err);
	}

#ifdef DHD_DEBUG_DOWNLOADTIME
	DHD_INFO(("Time for dhdsdio_download_state() to Keep arm in reset: %u ms\n",
			jiffies_to_msecs(jiffies - initial_jiffies)));
#endif
	/* External image takes precedence if specified */
	if ((bus->fw_path != NULL) && (bus->fw_path[0] != '\0')) {
#if defined(FW_SIGNATURE)
		bus->set_fw_addr_len = true;
#endif /* FW_SIGNATURE */
	...
	if ((metadata.enabled == TRUE) && (metadata.fw_dl_loc.enabled == TRUE)) {
		if (dhdsdio_sectionwise_fw_download(bus, bus->fw_path)) {
		...
		if ((metadata.enabled == TRUE) &&
			(metadata.fw_dl_loc.enabled == FALSE)) {
			DHD_ERROR(("%s: (0x%x) - TLV Absent."
						"Proceeding with Normal "
						"Firmware Download...\n",
						__FUNCTION__, FW_DL_LOCATION));
		}
#endif /* DHD_METADATA_DOWNLOAD */
...
#if defined(FW_SIGNATURE)
		bus->set_fw_addr_len = false;
#endif /* FW_SIGNATURE */
	}

#ifdef DHD_DEBUG_DOWNLOADTIME
	DHD_INFO(("Time for dhdsdio_download_code_file(): %u ms\n",
			jiffies_to_msecs(jiffies - initial_jiffies)));
#endif
...
#ifdef DHD_DEBUG_DOWNLOADTIME
	DHD_ERROR(("Time for si_pmu_43711a0_pll_war(): %u ms\n",
			jiffies_to_msecs(jiffies - initial_jiffies)));
#endif
...
#ifdef DHD_DEBUG_DOWNLOADTIME
	DHD_INFO(("Time for dhdsdio_download_nvram(): %u ms\n",
			jiffies_to_msecs(jiffies - initial_jiffies)));
#endif
	...
	if ((metadata.enabled == TRUE) && (metadata.clm_blob_loc.present == TRUE)) {
		...
		if ((bcmerror = dhdsdio_download_clm_file(bus, bus->clm_filename))) {
			DHD_ERROR(("%s: could not download CLM blob file, err %d\n",
				__FUNCTION__, bcmerror));
		}

#ifdef DHD_DEBUG_DOWNLOADTIME
		DHD_INFO(("Time for dhdsdio_download_clm_file(): %u ms\n",
			jiffies_to_msecs(jiffies - initial_jiffies)));
#endif
	}
#endif /* DHD_METADATA_DOWNLOAD */
...
#ifdef DHD_DEBUG_DOWNLOADTIME
	DHD_INFO(("Time to Take arm out of reset: %u ms\n",
			jiffies_to_msecs(jiffies - initial_jiffies)));
#endif
...
dhd_bus_devreset(dhd_pub_t *dhdp, uint8 flag)
{
	...
	int fw_download_start = 0, fw_download_end = 0;
...
void
dhd_bus_update_clm_path(struct dhd_bus *bus, char *pclm_path)
{
	strlcpy(bus->clm_filename, pclm_path, sizeof(bus->clm_filename));
...
#ifdef FW_SIGNATURE
void
dhd_bus_update_fw_sig_path(struct dhd_bus *bus, char *psig_path)
{
	strlcpy(bus->fwsig_filename, psig_path, sizeof(bus->fwsig_filename));

	DHD_ERROR(("%s: sig path:%s\n",
			__FUNCTION__, bus->fwsig_filename));
}
#endif /* FW_SIGNATURE */
...
#ifdef BL_RAM_BOOT
void
dhd_bus_update_boot_path(struct dhd_bus *bus, char *pboot_path)
{
	strlcpy(bus->bootloader_filename, pboot_path, sizeof(bus->fwsig_filename));
...
#ifdef BL_SIM_OTP
void
dhd_bus_update_otp_addr(struct dhd_bus *bus, uint32 otp_addr)
{
	bus->otp_addr = otp_addr;

	DHD_ERROR(("%s: otp_addr:0x%04x\n",
		__FUNCTION__, bus->otp_addr));
}
...
{
	if (bus->sih->chip == BCM43012_CHIP_ID) {
		return BT2WLAN_PWRUP_ADDR_43012B0;
	} else if ((bus->sih->chip == BCM43711_CHIP_ID) ||
			(bus->sih->chip == BCM43710_CHIP_ID)) {
		return BT2WLAN_PWRUP_ADDR_43711;
	} else if ((bus->sih->chip == BCM4612_CHIP_ID) ||
			(bus->sih->chip == BCM4611_CHIP_ID)) {
		return BT2WLAN_PWRUP_ADDR_4612;
	} else if (BCM4345_CHIP(bus->sih->chip)) {
		return BT2WLAN_PWRUP_ADDR_4345C5;
	} else {
		DHD_ERROR(("%s:DHD support for BT doesn't available for chipid %x\n",
		           __FUNCTION__, bus->sih->chip));
	}
...
_dhdsdio_download_btfw(struct dhd_bus *bus)
{
	...
	bt2wlan_pwrup_addr = dhd_btfw_start_addr(bus);
	if (!bt2wlan_pwrup_addr) {
		DHD_ERROR(("%s: Failed to get bt2wlanpwrup reg addr\n", __FUNCTION__));
		goto err;
	}
	...
	char *pbtfw_path)
{
	...
	uint8 boot_trigger_reload[2] = {0xEE, 0xFF};
...
static int
dhdsdio_download_btcert_file(dhd_bus_t *bus, char *path)
{
	int bcmerror = BCME_OK;
	void *filep = NULL;
	uint8 *srcbuf = NULL;
	int srcsize = 0;
	int len;
	uint32 dest_addr = 0;	/* cert file dest address */
	uint32 dest_size = 0;   /* cert file size */

	DHD_ERROR(("Inside this %s start \n", __FUNCTION__));
	...
	srcsize = dhd_os_get_image_size(filep);
	if (srcsize <= 0 || srcsize > MEMBLOCK) {
		DHD_ERROR(("%s: invalid certificate file size %u\n", __FUNCTION__, srcsize));
		bcmerror = BCME_BUFTOOSHORT;
		goto exit;
	}
	dest_size = ROUNDUP(srcsize, 4);
	...
	/* Write the value block */
	if (dest_size > 0) {
		dest_addr = BOOTLOADER_CERT_ADDR;
		DHD_ERROR(("%s: writing %d membytes to 0x%08x\n",
			__FUNCTION__, dest_size, dest_addr));
		bcmerror = dhdsdio_membytes(bus, TRUE, dest_addr, srcbuf, dest_size);
		if (bcmerror) {
...
#ifdef FREE_PKT_IN_RXF
int
wlc_freepkt_init_ring(dhd_pub_t *dhdp)
{
	memset(&dhdp->freepkt_ring, 0, sizeof(freepkt_msg_ring_t));
	dhdp->freepkt_ring.len = FREEPKT_MAX_RING;

	return 0;
}

int
wlc_freepkt_ring_unused(dhd_pub_t *dhdp)
{
	uint32 unuse = 0;
	freepkt_msg_ring_t *ring;

	ring = &dhdp->freepkt_ring;

	if (ring->w == ring->r) {
		unuse = ring->len;
	} else if (ring->w < ring->r) {
		unuse = ring->r - ring->w - 1;
	} else {
		unuse = ring->len + ring->r - ring->w - 1;
	}
...
void *
wlc_freepkt_ring_enq(dhd_pub_t *dhdp, void *sdu)
{
	freepkt_msg_ring_t *ring;

	ring = &dhdp->freepkt_ring;
	ring->ring[ring->w++] = sdu;
...
void *
wlc_freepkt_ring_deq(dhd_pub_t *dhdp)
{
	void *p = NULL;
	freepkt_msg_ring_t *ring;
...
int sdio_transfer_calc_rx_bytes(uint32 bytes)
{
	sdio_transfer_time_rx.total_bytes += (bytes > 54) ? (bytes - 54) : 0;
	sdio_transfer_time_rx.total_cnt++;
...
int sdio_transfer_calc_time_rx(uint32 cnt)
{
	uint32 idx = sdio_transfer_time_rx.index;
	uint64 time = OSL_SYSUPTIME_US();
	speed_list_t *time_rx = &sdio_transfer_time_rx;

	if (cnt <= 4)
		return -1;

	if (cnt == (dhd_rxbound)) {
		time_rx->glom_time[idx] = (uint32)(sdio_transfer_time_rxglom);
		time_rx->glom_cnt[idx] = cnt;
		time_rx->dpc_time[idx] = (uint32)(time - time_rx->last_dpc_time);
		time_rx->last_dpc_time = time;
		sdio_transfer_time_rxglom = 0;

		if ((time_rx->dpc_time[idx] < (30000)) && (time_rx->dpc_time[idx] > (9000))) {
			if (time_rx->max[1] == 0) {
				time_rx->max[0] = time_rx->glom_time[idx];
				time_rx->max[1] = time_rx->dpc_time[idx];
				time_rx->max[2] = time_rx->glom_cnt[idx];
			}

			if (time_rx->avg[1] == 0) {
				time_rx->avg[0] = time_rx->glom_time[idx];
				time_rx->avg[1] = time_rx->dpc_time[idx];
				time_rx->avg[2] = time_rx->glom_cnt[idx];
			}

			if (time_rx->min[1] == 0) {
				time_rx->min[0] = time_rx->glom_time[idx];
				time_rx->min[1] = time_rx->dpc_time[idx];
				time_rx->min[2] = time_rx->glom_cnt[idx];
			}

			if (time_rx->dpc_time[idx] > time_rx->max[1]) {
				time_rx->max[0] = time_rx->glom_time[idx];
				time_rx->max[1] = time_rx->dpc_time[idx];
				time_rx->max[2] = time_rx->glom_cnt[idx];
			}

			time_rx->avg[0] = (time_rx->glom_time[idx] + time_rx->avg[0]) / 2;
			time_rx->avg[1] = (time_rx->dpc_time[idx] + time_rx->avg[1]) / 2;
			time_rx->avg[2] = time_rx->glom_cnt[idx];

			if (time_rx->dpc_time[idx] < time_rx->min[1]) {
				time_rx->min[0] = time_rx->glom_time[idx];
				time_rx->min[1] = time_rx->dpc_time[idx];
				time_rx->min[2] = time_rx->glom_cnt[idx];
			}
		...
		time_rx->index++;
		time_rx->index = time_rx->index % MAX_SLIST;
	}
	time_rx->last_dpc_time = time;
	sdio_transfer_time_rxglom = 0;

	if ((time - time_rx->last_time) > 1000000) {
		uint32 avg = time_rx->total_bytes / time_rx->total_cnt;

		if (time_rx->total_cnt > 20000) {
			time_rx->last_total_cnt = time_rx->total_cnt;
			time_rx->last_total_bytes = time_rx->total_bytes;
			time_rx->max_avg = (avg + time_rx->max_avg) / 2;
		}
		time_rx->last_time = time;
		time_rx->total_cnt = 0;
		time_rx->total_bytes = 0;
	}
...
int sdio_transfer_calc_time_tx(uint32 cnt, uint32 bytes, uint64 time_a)
{
	uint32 idx = sdio_transfer_time_tx.index;
	uint64 time = OSL_SYSUPTIME_US();
	speed_list_t *time_tx = &sdio_transfer_time_tx;

	if (cnt <= 4)
		return -1;

	if (time_tx->last_dpc_time && (cnt > 5)) {
		time_tx->glom_time[idx] = (uint32)(time - time_a);
		time_tx->glom_cnt[idx] = cnt;
		time_tx->dpc_time[idx] = (uint32)(time - time_tx->last_dpc_time);
		time_tx->last_dpc_time = time;

		if (idx == (MAX_SLIST - 1)) {
			uint32 gtime = 0, dtime = 0, cnt = 0, i = 0;

			for (i = 0; i <= idx; i++) {
				gtime += time_tx->glom_time[i];
				dtime += time_tx->dpc_time[i];
				cnt += time_tx->glom_cnt[i];
			}
			if ((time_tx->max[1] == 0) && ((dtime / cnt)  < 50)) {
				time_tx->max[0] = gtime;
				time_tx->max[1] = dtime;
				time_tx->max[2] = cnt;
			}
			...
			if (time_tx->avg[1] == 0) {
				time_tx->avg[0] += gtime / cnt;
				time_tx->avg[1] += dtime / cnt;
				time_tx->avg[2] = 1;
			} else {
				time_tx->avg[0] = gtime / cnt + time_tx->avg[0];
				time_tx->avg[1] = dtime / cnt + time_tx->avg[1];
				time_tx->avg[2]++;
			}
			...
			if (((dtime / cnt) < 50) &&
				(dtime / cnt) > (time_tx->max[1] / time_tx->max[2])) {
				time_tx->max[0] = gtime;
				time_tx->max[1] = dtime;
				time_tx->max[2] = cnt;
			}

			if ((dtime / cnt) < (time_tx->min[1] / time_tx->min[2])) {
				time_tx->min[0] = gtime;
				time_tx->min[1] = dtime;
				time_tx->min[2] = cnt;
			}
		...
		time_tx->index++;
		time_tx->index = time_tx->index % MAX_SLIST;
	}

	time_tx->last_dpc_time = time;
	time_tx->total_cnt += cnt;
	time_tx->total_bytes += (bytes > (cnt * 90)) ? (bytes - cnt * 90) : 0;

	if ((time - time_tx->last_time) > 1000000) {
		uint32 avg = time_tx->total_bytes / time_tx->total_cnt;

		if ((time_tx->total_cnt > 20000)) {
			time_tx->last_total_cnt = time_tx->total_cnt;
			time_tx->last_total_bytes = time_tx->total_bytes;
			time_tx->max_avg = (avg + time_tx->max_avg) / 2;
		}

		time_tx->last_time = time;
		time_tx->total_cnt = 0;
		time_tx->total_bytes = 0;
	}

	return 0;
}

int sdio_transfer_time_dump(int val)
{
	uint32 tput, i;
	uint32 total_cnt = 0;
	uint32 total_glom = 0;
	...
	if (val & 0x1) {
		time_tx->max[0] = time_tx->max[0] * 520 / time_tx->max[2];
		time_tx->max[1] = time_tx->max[1] * 520 / time_tx->max[2];
		time_tx->avg[0] = time_tx->avg[0] * 520 / time_tx->avg[2];
		time_tx->avg[1] = time_tx->avg[1] * 520 / time_tx->avg[2];
		time_tx->min[0] = time_tx->min[0] * 520 / time_tx->min[2];
		time_tx->max[2] = 520;
		time_tx->avg[2] = 520;
		time_tx->min[2] = 520;

		if (time_tx->max[0] < time_tx->avg[0]) {
			tput = time_tx->max[0];
			time_tx->max[0] = time_tx->avg[0];
			time_tx->avg[0] = tput;
		}
		if (time_tx->max[0] < time_tx->min[0]) {
			tput = time_tx->max[0];
			time_tx->max[0] = time_tx->min[0];
			time_tx->min[0] = tput;
		}
		if (time_tx->avg[0] < time_tx->min[0]) {
			tput = time_tx->avg[0];
			time_tx->avg[0] = time_tx->min[0];
			time_tx->min[0] = tput;
		}

		tput = (time_tx->last_total_cnt * time_tx->max_avg * 8) / 1000000;

		printk("tx tput %d Mbp/s, %d, %d Mbp/s, %d, %d\n",
			tput, time_tx->last_total_cnt,
			time_tx->last_total_bytes * 8 / 1000000, time_tx->last_total_bytes,
			time_tx->last_total_bytes / time_tx->last_total_cnt);
		printk("min %4d,%4d Mbp/s, %d, %d, %d\n",
			time_tx->max[2] * time_tx->max_avg * 8 / time_tx->max[1],
			time_tx->max[2] * time_tx->max_avg * 8 / time_tx->max[0],
			520, time_tx->max[1], time_tx->max[0]);
		printk("avg %4d,%4d Mbp/s, %d, %d, %d\n",
			time_tx->avg[2] * time_tx->max_avg * 8 / time_tx->avg[1],
			time_tx->avg[2] * time_tx->max_avg * 8 / time_tx->avg[0],
			520, time_tx->avg[1], time_tx->avg[0]);
		printk("max %4d,%4d Mbp/s, %d, %d, %d\n",
			time_tx->min[2] * time_tx->max_avg * 8 / time_tx->min[1],
			time_tx->min[2] * time_tx->max_avg * 8 / time_tx->min[0],
			520, time_tx->min[1], time_tx->min[0]);
	}

	printk("\n");

	if (val & 0x2) {
		if (time_rx->max[0] < time_rx->avg[0]) {
			tput = time_rx->max[0];
			time_rx->max[0] = time_rx->avg[0];
			time_rx->avg[0] = tput;
		}
		if (time_rx->max[0] < time_rx->min[0]) {
			tput = time_rx->max[0];
			time_rx->max[0] = time_rx->min[0];
			time_rx->min[0] = tput;
		}
		if (time_rx->avg[0] < time_rx->min[0]) {
			tput = time_rx->avg[0];
			time_rx->avg[0] = time_rx->min[0];
			time_rx->min[0] = tput;
		}

		tput = (time_rx->last_total_cnt * time_rx->max_avg * 8) / 1000000;

		printk("rx tput %d Mbp/s, %d, %d Mbp/s, %d, %d\n",
			tput, time_rx->last_total_cnt,
			time_rx->last_total_bytes * 8 / 1000000, time_rx->last_total_bytes,
			time_rx->last_total_bytes / time_rx->last_total_cnt);
		printk("min %4d,%4d Mbp/s, %d, %d, %d\n",
			time_rx->max[2] * time_rx->max_avg * 8 / time_rx->max[1],
			time_rx->max[2] * time_rx->max_avg * 8 / time_rx->max[0],
			time_rx->max[2], time_rx->max[1], time_rx->max[0]);
		printk("avg %4d,%4d Mbp/s, %d, %d, %d\n",
			time_rx->avg[2] * time_rx->max_avg * 8 / time_rx->avg[1],
			time_rx->avg[2] * time_rx->max_avg * 8 / time_rx->avg[0],
			time_rx->avg[2], time_rx->avg[1], time_rx->avg[0]);
		printk("max %4d,%4d Mbp/s, %d, %d, %d\n",
			time_rx->min[2] * time_rx->max_avg * 8 / time_rx->min[1],
			time_rx->min[2] * time_rx->max_avg * 8 / time_rx->min[0],
			time_rx->min[2], time_rx->min[1], time_rx->min[0]);
	}

	if (val & 0x4) {
		for (i = 0; i < 41; i++) {
			printk("glom %02d: %d, free time %d, free one pkt time %d\n",
				i, sdio_transfer_glom_list[i][0], sdio_transfer_glom_list[i][1],
				sdio_transfer_glom_list[i][1] ? (sdio_transfer_glom_list[i][1]
					/ sdio_transfer_glom_list[i][0] / i) : 0);

			total_cnt += sdio_transfer_glom_list[i][0] * i;
			total_glom += sdio_transfer_glom_list[i][0];
		}

		printk("glom %d, %d %d\n", total_cnt, total_glom, total_cnt / total_glom);
	}

	return 0;
}

int sdio_transfer_time_set(int val)
{
	if (val == 0) {
		memset(&sdio_transfer_time_rx, 0, sizeof(speed_list_t));
		memset(&sdio_transfer_time_tx, 0, sizeof(speed_list_t));
		memset(&sdio_transfer_glom_list, 0, sizeof(sdio_transfer_glom_list));
		sdio_transfer_time_rxglom = 0;
	} else {
		sdio_transfer_time_dump(val);
	}
