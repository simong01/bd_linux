...
_dhd_wlfc_flow_control_check(athost_wl_status_info_t* ctx, struct pktq* pq, uint8 if_id)
{
	...
	if ((pq->n_pkts_tot <= WLFC_FLOWCONTROL_STOPORPHOFF) &&
		(dhdp->wlfc_stop_orphan == ON))
		dhdp->wlfc_stop_orphan = OFF;
	else if ((pq->n_pkts_tot >= WLFC_FLOWCONTROL_STOPORPHON) &&
		(dhdp->wlfc_stop_orphan == OFF))
		dhdp->wlfc_stop_orphan = ON;
...
_dhd_wlfc_enque_suppressed(athost_wl_status_info_t* ctx, int prec, void* p)
{
	...
	order of delivery.
	*/
	qhead = (PKTMACLEN(p) == TCPACK_TAG)?TRUE:FALSE;
	if (qhead)
		PKTSETMACLEN(p, 0);

	if (_dhd_wlfc_prec_enq_with_drop(ctx->dhdp, &entry->psq, p, ((prec << 1) + 1), qhead,
...
_dhd_wlfc_enque_delayq(athost_wl_status_info_t* ctx, void* pktbuf, int prec)
{
	...
	if (pktbuf != NULL) {
		...
		order of delivery.
		*/
		qhead = (PKTMACLEN(pktbuf) == TCPACK_TAG)?TRUE:FALSE;
		if (qhead)
			PKTSETMACLEN(pktbuf, 0);

		if (_dhd_wlfc_prec_enq_with_drop(ctx->dhdp, &entry->psq, pktbuf, (prec << 1),
			qhead, WLFC_SEQCOUNT(entry, prec))
			== FALSE) {
			WLFC_DBGMESG(("D"));
			ctx->stats.delayq_full_error++;
			return BCME_ERROR;
...
bool dhd_wlfc_is_header_only_pkt(dhd_pub_t * dhd, void *pktbuf)
{
	...
	if (!PKTLEN(dhd->osh, pktbuf)) {
		rc = TRUE;

		if ((dhd->wlfc_state) && ((WLFC_FCMODE_IMPLIED_CREDIT == dhd->proptxstatus_mode) ||
