...
static int cpuhp_state = -1;
/* define the cpuhp_state used for dhd
 * When use CPUHP_AP_ONLINE_DYN, kernel will
 * select a reserved cpuhp_state, else kernel
 * will use the fixed value defined here
 */
#define DEF_DHD_CPUHP_STATE CPUHP_AP_ONLINE_DYN
...
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 10, 0))
	/* When using CPUHP_AP_ONLINE_DYN, return value is the real
	 * cpuhp_state number, when using fixed value, return 0 for
	 * success.
	 * return negative error code when failed.
	 */
	cpuhp_state = cpuhp_setup_state(DEF_DHD_CPUHP_STATE, "dhd",
		dhd_cpu_startup_callback, dhd_cpu_teardown_callback);

	if (cpuhp_state < 0) {
		DHD_ERROR(("%s(): cpuhp_setup_state failed %d RX LB won't happen \r\n",
			__FUNCTION__, cpuhp_state));
	} else if (cpuhp_state == 0) {
		/* When using a fixed cpuhp_state, return value = 0
		 * use cpuhp_sate to save the fixed value
		 */
		cpuhp_state = DEF_DHD_CPUHP_STATE;
	}
#else
	/*
...
static void
dhd_lb_stats_dump_napi_latency(dhd_pub_t *dhdp,
...
static void
dhd_lb_stats_dump_histo(dhd_pub_t *dhdp,
...
static void
dhd_lb_stats_update_napi_latency(uint64 *bin, uint32 latency)
{
	uint64 *p;
	uint32 bin_power;
...
static void
dhd_lb_stats_update_histo(uint32 **bin, uint32 count, uint32 cpu)
{
	uint32 bin_power;
	uint32 *p;
