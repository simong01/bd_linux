...
static wme_ac_param acparam_init_sta[AC_COUNT];

int dhd_wme_ac_init_prameter(wme_ac_param *acparam_init, int ac_queue,
	char *list_str)
{
	char	temp_str[16];
	char	*next = NULL;
	...
	while (((len = strcspn(next, " ,")) > 0) && (idx < 4)) {
		...
		temp_str[len] = 0;
		tmpval[idx] = (uint32)simple_strtoul(temp_str, NULL, 0);

		next += len;
		next += strspn(next, " ,");
		idx++;
	}
	acparam_init[ac_queue].ecwmax = tmpval[0];
	acparam_init[ac_queue].ecwmin = tmpval[1];
	acparam_init[ac_queue].txop = tmpval[2];
	acparam_init[ac_queue].aifsn = tmpval[3];
	acparam_init[ac_queue].enabled = 1;
...
{
	edcf_acparam_t acparam_cur[AC_COUNT], *acp;
	int ret = 0;
	int aci = 0;
	int isset = 0;
	int aifsn, txop, ecwmax, ecwmin;

	ret = dhd_iovar(dhd, ifidx, "wme_ac_sta", NULL, 0, (char*)&acparam_cur,
			sizeof(acparam_cur), FALSE);
	if (ret < 0) {
		DHD_ERROR(("%s: can't get wme_ac_sta info, error=%d\n", __FUNCTION__, ret));
		return ret;
	}
	...
	for (aci = 0; aci < AC_COUNT; aci++) {
		...
		if (!acparam_init_sta[aci].enabled) {
			continue;
		}
		/* add aifsn */
		aifsn = acparam_init_sta[aci].aifsn;
		if ((aifsn >= EDCF_AIFSN_MIN) && (aifsn <= EDCF_AIFSN_MAX)) {
			acp->ACI = (acp->ACI & ~EDCF_AIFSN_MAX) |
				(aifsn & EDCF_AIFSN_MAX);
			isset = 1;
		} else {
			DHD_INFO(("%s: can't get invalid wme_ac[%d].aifsn = %d\n",
				__FUNCTION__, aci, aifsn));
		}
		/* add ecwmax */
		ecwmax = acparam_init_sta[aci].ecwmax;
		if ((ecwmax >= EDCF_ECW_MIN) && (ecwmax <= EDCF_ECW_MAX)) {
			acp->ECW = (acp->ECW & EDCF_ECWMIN_MASK) |
				((ecwmax << EDCF_ECWMAX_SHIFT) & EDCF_ECWMAX_MASK);
			isset = 1;
		} else {
			DHD_INFO(("%s: can't get invalid wme_ac[%d].ecwmax = %d\n",
				__FUNCTION__, aci, ecwmax));
		}
		/* add ecwmin */
		ecwmin = acparam_init_sta[aci].ecwmin;
		if ((ecwmin >= EDCF_ECW_MIN) && (ecwmin <= EDCF_ECW_MAX)) {
			acp->ECW = (acp->ECW & EDCF_ECWMAX_MASK) |
				(ecwmin & EDCF_ECWMIN_MASK);
			isset = 1;
		} else {
			DHD_INFO(("%s: can't get invalid wme_ac[%d].ecwmin = %d\n",
				__FUNCTION__, aci, ecwmin));
		}
		/* add txop */
		txop = acparam_init_sta[aci].txop;
		if ((txop >= EDCF_TXOP_MIN) && (txop <= EDCF_TXOP_MAX)) {
			acp->TXOP = htod16(txop);
			isset = 1;
		} else {
			DHD_INFO(("%s: can't get invalid wme_ac[%d].txop = %d\n",
				__FUNCTION__, aci, txop));
		}
		/* set it */
		if (isset) {
			ret = dhd_iovar(dhd, ifidx, "wme_ac_sta", (char*)acp,
					sizeof(edcf_acparam_t), NULL, 0, TRUE);
			if (ret) {
				DHD_ERROR(("%s: can't set wme_ac_sta info, error=%d\n",
					__FUNCTION__, ret));
			}
		}
	}
...
dhd_preinit_config_proc(dhd_pub_t *dhd, int ifidx, char *name, char *value)
{
	...
	/* retry times */
		...
		if (ret < 0) {
			DHD_ERROR(("rateset %s Set lrl failed %d\n", __FUNCTION__, ret));
		}
		return ret;
	}
	...
	/* packet filter */
		...
		ret = dhd_iovar(dhd, 0, "pkt_filter_add", (char *)&pkt_filter, sizeof(pkt_filter),
				NULL, 0, TRUE);
		if (ret < 0) {
...
#ifdef SYNA_CUSTOMER_WME_AC
	else if (!strcasecmp(name, "wme_ac_sta_be")) {
		DHD_TRACE(("%s: wme_ac_sta_be value %s\n", __FUNCTION__, value));
		dhd_wme_ac_init_prameter(acparam_init_sta, AC_BE, value);
	} else if (!strcasecmp(name, "wme_ac_sta_bk")) {
		DHD_TRACE(("%s: wme_ac_sta_bk value %s\n", __FUNCTION__, value));
		dhd_wme_ac_init_prameter(acparam_init_sta, AC_BK, value);
	} else if (!strcasecmp(name, "wme_ac_sta_vi")) {
		DHD_TRACE(("%s: wme_ac_sta_vi value %s\n", __FUNCTION__, value));
		dhd_wme_ac_init_prameter(acparam_init_sta, AC_VI, value);
	} else if (!strcasecmp(name, "wme_ac_sta_vo")) {
		DHD_TRACE(("%s: wme_ac_sta_vo value %s\n", __FUNCTION__, value));
		dhd_wme_ac_init_prameter(acparam_init_sta, AC_VO, value);
	}
#endif /* SYNA_CUSTOMER_WME_AC */
...
dhd_preinit_config(dhd_pub_t *dhd, int ifidx, char * config_file_path)
{
	...
	if ((!config_file_path) || ('\0' == config_file_path[0])) {
...
#ifdef SYNA_CUSTOMER_WME_AC
	memset(acparam_init_sta, 0, sizeof(acparam_init_sta));
#endif /* SYNA_CUSTOMER_WME_AC */
