...
#ifndef WL_SCAN_IE_LEN_MAX
#define WL_SCAN_IE_LEN_MAX    2048
#endif // WL_SCAN_IE_LEN_MAX
...
_dhd_pno_cfg(dhd_pub_t *dhd, uint16 *channel_list, int nchan)
{
	...
	if ((dhd->wlc_ver_major >= DHD_PNO_CHSPEC_SUPPORT_VER) || use_chanspec) {
		/* Set flag to indicate chanspec list for FW */
		pfncfg_param.flags |= WL_PFN_CFG_FLAGS_CHANSPEC;
	}
	...
	int pno_freq_expo_max, uint16 *channel_list, int nchan)
{
	...
	if (_pno_state->pno_mode & (DHD_PNO_BATCH_MODE | DHD_PNO_HOTLIST_MODE)) {
		...
		if (_pno_state->pno_mode & DHD_PNO_BATCH_MODE) {
			...
			if ((WL_NUMCHANNELS >= _params2->params_batch.nchan) &&
				(_params2->params_batch.nchan > 0 && tot_nchan > 0)) {
...
dhd_pno_set_for_batch(dhd_pub_t *dhd, struct dhd_pno_batch_params *batch_params)
{
	...
	if (WL_NUMCHANNELS < batch_params->nchan) {
		DHD_ERROR(("%s:error, exceed max channel qty_MAX=%d!\n",
		           __FUNCTION__, WL_NUMCHANNELS));
		err = BCME_BADARG;
		goto exit;
	}
	...
	struct dhd_pno_hotlist_params *hotlist_params)
{
	...
	if (WL_NUMCHANNELS < hotlist_params->nchan) {
		DHD_ERROR(("%s: error, exceed max channel qty_MAX=%d!\n",
		           __FUNCTION__, WL_NUMCHANNELS));
		err = BCME_BADARG;
		goto exit;
...
static void *
dhd_pno_update_pfn_v3_results(dhd_pub_t *dhd, const wl_pfn_scanresults_v3_t *pfn_result,
	uint32 *mem_needed, struct dhd_pno_gscan_params *gscan_params, uint32 event)
{
	...
	*mem_needed = sizeof(dhd_epno_results_t) * pfn_result->count;
	...
	for (i = 0; i < pfn_result->count; i++) {
		const wl_pfn_net_info_v3_t *net_info = &pfn_result->netinfo[i];
...
dhd_pno_process_epno_result(dhd_pub_t *dhd, const void *data, uint32 event, int *size)
{
	...
	*size = 0;
	...
	if (event == WLC_E_PFN_NET_FOUND || event == WLC_E_PFN_NET_LOST) {
		const wl_pfn_scanresults_v1_t *pfn_result = NULL;
		const wl_pfn_scanresults_v2_t *pfn_result_v2 = NULL;
		const wl_pfn_scanresults_v3_t *pfn_result_v3 = NULL;

		pfn_result = (const wl_pfn_scanresults_v1_t *)data;
		pfn_result_v2 = (const wl_pfn_scanresults_v2_t *)data;
		pfn_result_v3 = (const wl_pfn_scanresults_v3_t *)data;
		if (pfn_result->version == PFN_SCANRESULT_VERSION_V1) {
			...
			for (i = 0; i < count; i++) {
				const wl_pfn_net_info_v1_t *net = &pfn_result->netinfo[i];
			...
			for (i = 0; i < count; i++) {
				const wl_pfn_net_info_v2_t *net_v2 = &pfn_result_v2->netinfo[i];
...
static void *
dhd_pno_update_hotlist_v3_results(dhd_pub_t *dhd, const wl_pfn_scanresults_v3_t *pfn_result,
	int *send_evt_bytes, hotlist_type_t type,  u32 *buf_len)
{
	...
	dhd_pno_status_info_t *_pno_state = PNO_GET_PNOSTATE(dhd);
	const wl_pfn_net_info_v3_t *pnetinfo = (const wl_pfn_net_info_v3_t*)&pfn_result->netinfo[0];
	...
	*buf_len = malloc_size;
	...
	for (i = 0; i < pfn_result->count; i++, pnetinfo++) {
		...
		ssid_len = pnetinfo->pfnsubnet.SSID_len;
		...
		(void)memcpy_s(hotlist_found_array->ssid, DOT11_MAX_SSID_LEN,
			pnetinfo->pfnsubnet.u.SSID, ssid_len);
		hotlist_found_array->ssid[ssid_len] = '\0';
        ...
        int *send_evt_bytes, hotlist_type_t type, u32 *buf_len)
{
	...
	dhd_pno_status_info_t *_pno_state = PNO_GET_PNOSTATE(dhd);
	struct dhd_pno_gscan_params *gscan_params;
	const wl_pfn_scanresults_v1_t *results_v1 = (const wl_pfn_scanresults_v1_t *)event_data;
	const wl_pfn_scanresults_v2_t *results_v2 = (const wl_pfn_scanresults_v2_t *)event_data;
	const wl_pfn_scanresults_v3_t *results_v3 = (const wl_pfn_scanresults_v3_t *)event_data;
	wifi_gscan_result_t *hotlist_found_array;
	gscan_results_cache_t *gscan_hotlist_cache;
	...
	*buf_len = 0;
	if (results_v1->version == PFN_SCANRESULTS_VERSION_V1) {
		const wl_pfn_net_info_v1_t *pnetinfo = &results_v1->netinfo[0];
		...
		*buf_len = malloc_size;
		...
		for (i = 0; i < fwcount; i++, pnetinfo++) {
			...
			(void)memcpy_s(hotlist_found_array->ssid, DOT11_MAX_SSID_LEN,
				pnetinfo->pfnsubnet.SSID, ssid_len);
			hotlist_found_array->ssid[ssid_len] = '\0';
		...
		const wl_pfn_net_info_v2_t *pnetinfo_v2 = NULL;

		pnetinfo_v2 = (const wl_pfn_net_info_v2_t*)&results_v2->netinfo[0];
		...
		*buf_len = malloc_size;
		...
		for (i = 0; i < fwcount; i++, pnetinfo_v2++) {
			...
			ssid_len = pnetinfo_v2->pfnsubnet.SSID_len;
			if (ssid_len > DOT11_MAX_SSID_LEN) {
				DHD_ERROR(("Invalid SSID length %d: trimming it to max\n",
					ssid_len));
				ssid_len = DOT11_MAX_SSID_LEN;
			...
			(void)memcpy_s(hotlist_found_array->ssid, DOT11_MAX_SSID_LEN,
				pnetinfo_v2->pfnsubnet.u.SSID, ssid_len);
			hotlist_found_array->ssid[ssid_len] = '\0';
