...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 13, 0))
	.ops = wl_genl_ops,
	.n_ops = ARRAY_SIZE(wl_genl_ops),
	.mcgrps = wl_genl_mcast,
	.n_mcgrps = ARRAY_SIZE(wl_genl_mcast),
#endif /* (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 13, 0) */
};
#endif /* WL_GENL */
...
#ifdef CUSTOMER_HW4
	{CMD_BTCOEXMODE, TRUE},
	{CMD_BTCOEXSCAN_START, TRUE},
	{CMD_COUNTRY, TRUE},
	{CMD_RXFILTER_ADD, TRUE},
	{CMD_RXFILTER_START, TRUE},
	{CMD_RXFILTER_STOP, TRUE},
	{CMD_SETSUSPENDMODE, TRUE},
	{CMD_SET_AP_WPS_P2P_IE, TRUE},
#endif /* CUSTOMER_HW4 */
	{"\0", FALSE}
};
...
wl_android_set_scan_dfs_channel_mode(struct net_device *dev, char *command)
{
	...
	if (mode == 1) {
		...
		WL_ERR(("Failed to set Scan DFS channel mode %d\n", mode));
		return BCME_ERROR;
	}
...
wl_android_reassoc(struct net_device *dev, char *command, int total_len)
{
	...
	sscanf(command, "%"SIZE_CONST_STRING(WL_PRIV_CMD_LEN)"s *", pcmd);
...
wl_android_legacy_check_command(struct net_device *dev, char *command)
{
	...
	while (strlen(legacy_cmdlist[cnt]) > 0) {
		if (strnicmp(command, legacy_cmdlist[cnt], strlen(legacy_cmdlist[cnt])) == 0) {
			...
			sscanf(command, "%"SIZE_CONST_STRING(WL_PRIV_CMD_LEN)"s ", cmd);
...
int wl_android_wifi_on(struct net_device *dev)
{
	...
	if (!g_wifi_on) {
		do {
			...
			DHD_ERROR(("*power up wifi chip fail, ret=%d, retry=%d\n",
			           ret, retry));
#ifdef BCMPCIE
			...
			if (wbrc_wl2bt_reset()) {
				DHD_ERROR(("Failed to reset BT, nothing to be done!!!!\n"));
			}
#endif /* WBRC */
		} while (retry-- > 0);
		if (0 > ret) {
...
int wl_android_wifi_off(struct net_device *dev, bool on_failure)
{
	...
	if (!dev) {
		...
		DHD_ERROR(("wl_android_wifi_off: dev is null\n"));
		return -EINVAL;
	}
...
wl_android_twt_setup(struct net_device *ndev, char *command, int total_len)
{
	...
	token = strsep((char**)&pos, " ");
	if (!token) {
		WL_ERR(("Mandaory param setup_cmd not present\n"));
		bw = -EINVAL;
		goto exit;
	}
	...
	if (val.desc.setup_cmd == WL_TWT_CMD_INVAL) {
		WL_ERR(("Unrecognized TWT Setup command '%s'\n", token));
	}
	WL_INFORM(("TWT_SETUP val.desc.setup_cmd %s\n", token));
	...
	WL_INFORM(("TWT_SETUP val.desc.negotiation_type %d\n", val.desc.negotiation_type));

	if (pos != NULL) {
		WL_INFORM(("TWT_SETUP string %s\n", pos));
		while ((token = bcmstrtok(&pos, " ", 0)) != NULL) {
			WL_INFORM(("TWT_SETUP token is %s\n", token));
			...
			if (!strnicmp(token, "u", 1)) {
				...
				WL_INFORM(("TWT_SETUP broadcast persistence %d\n",
			...
			/* Wake Duration */
				...
				if ((token = bcmstrtok(&pos, " ", 0)) != NULL) {
					...
					WL_INFORM(("TWT_SETUP val.desc.wake_dur %d\n",
						val.desc.wake_dur));
				}
			}
			/* Wake Interval */
				...
				if ((token = bcmstrtok(&pos, " ", 0)) != NULL) {
					...
					WL_INFORM(("TWT_SETUP val.desc.wake_int %d\n",
						val.desc.wake_int));
				}
			}
			/* flow id */
				...
				if ((token = bcmstrtok(&pos, " ", 0)) != NULL) {
					...
					WL_INFORM(("TWT_SETUP val.desc.flow_id %d\n",
						val.desc.flow_id));
				}
			}
				...
				if ((token = bcmstrtok(&pos, " ", 0)) != NULL) {
					...
					WL_INFORM(("TWT_SETUP val.desc.bid %d\n", val.desc.bid));
				}
			}
				...
				if ((token = bcmstrtok(&pos, " ", 0)) != NULL) {
					...
					val.desc.bid = tmp;
					WL_INFORM(("TWT_SETUP frame recommendation %d\n",
				...
				if ((token = bcmstrtok(&pos, " ", 0)) != NULL) {
					...
					WL_INFORM(("TWT_SETUP duty_cycle_min %d\n",
						val.desc.duty_cycle_min));
				}
			}
				...
				if ((token = bcmstrtok(&pos, " ", 0)) != NULL) {
					...
					WL_INFORM(("TWT_SETUP wake_int_max %d\n",
	...
	for (i = 0; i < WL_TWT_MAX_ITWT; i ++) {
		if ((status->itwt_status[i].state == WL_TWT_ACTIVE) ||
			...
			WL_DBG(("Unannounced %d\tTriggered %d\tProtection %d\t"
				...
				!!(desc->flow_flags & WL_TWT_FLOW_FLAG_PROTECT),
				!!(desc->flow_flags & WL_TWT_FLOW_FLAG_INFO_FRM_DISABLED)));
			WL_DBG(("target wake time: 0x%08x%08x\t",
				desc->wake_time_h, desc->wake_time_l));
...
wl_android_twt_info(struct net_device *ndev, char *command, int total_len)
{
	...
	token = strsep((char**)&pos, " ");
	if (!token) {
		WL_ERR(("Mandatory all TWT type not present\n"));
		bw = -EINVAL;
		goto exit;
	}
	...
	token = strsep((char**)&pos, " ");
	if (!token) {
		WL_ERR(("flow ID  not provided, using default\n"));
	} else {
	...
	token = strsep((char**)&pos, " ");
	if (!token) {
		WL_ERR(("Peer Addr not provided, using default\n"));
	} else {
		/* get peer mac */
		if (!bcm_ether_atoe(token, &val.peer)) {
			DHD_ERROR(("%s : Malformed peer addr\n", __FUNCTION__));
			bw = BCME_ERROR;
			goto exit;
		}
	}
		...
		char *command, int total_len)
{
	...
	argc = sscanf(data,
	    "%"SIZE_CONST_STRING(BUFSZ)"s %"SIZE_CONST_STRING(BUFSZ)"s %"SIZE_CONST_STRING(BUFSZ)"s",
	    rssi, band, weight);
