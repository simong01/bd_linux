...
wl_dongle_down(struct net_device *ndev)
{
	...
	if (unlikely(0 > err)) {
		WL_ERR(("WLC_DOWN error (%d) (ignore when remove)\n", err));
	}
#ifdef WLAN_ACCEL_BOOT
	err = wldev_iovar_setint(ndev, "bus:host_access", bus_host_access);
	if (unlikely(0 > err)) {
		WL_ERR(("bus:host_access(%d) error (%d)(Ignore when remove)\n",
		        bus_host_access, err));
	}
#endif /* WLAN_ACCEL_BOOT */
...
#define DHD_SDALIGN	32
	...
	if (unlikely(0 > err)) {
		/* avoid interface up failure */
		if (BCME_NOTSTA != err) {
			WL_ERR(("wl_dongle_roam failed\n"));
			goto default_conf_out;
		} else {
			WL_ERR(("ignore previous roam setting result!\n"));
			err = BCME_OK;
		}
	}
