...
dhd_prot_inc_hostactive_devwake_assert(dhd_bus_t *bus)
{
	...
	if (likely(INBAND_DW_ENAB(bus))) {
		DHD_BUS_INB_DW_LOCK(bus->inb_lock, flags);
		bus->host_active_cnt++;
		DHD_BUS_INB_DW_UNLOCK(bus->inb_lock, flags);
		if (unlikely(dhd_bus_set_device_wake(bus, TRUE) != BCME_OK)) {
			DHD_BUS_INB_DW_LOCK(bus->inb_lock, flags);
			bus->host_active_cnt--;
...
dhd_prot_process_edl_complete(dhd_pub_t *dhd, void *evt_decode_data)
{
	...
	while (n > 0) {
		 ...
		 */
		ring->curr_rd = (ring->curr_rd + 1) % ring->max_items;
	...
	ring->rd = (ring->rd + max_items_to_process) % ring->max_items;
	DHD_RING_UNLOCK(ring->ring_lock, flags);
...
BCMFASTPATH(dhd_prot_txdata)(dhd_pub_t *dhd, void *PKTBUF, uint8 ifidx)
{
	...
	if (unlikely(dhd->flow_ring_table == NULL)) {
	...
	if (unlikely(dhd->dhd_induce_error == DHD_INDUCE_TX_BIG_PKT)) {
	...
	if (unlikely((dhd->dhd_induce_error == DHD_INDUCE_TX_BIG_PKT) && big_pktbuf)) {
...
#if defined(DHD_PCIE_PKTID)
	if (unlikely(pktid == DHD_PKTID_INVALID)) {
		DHD_ERROR_RLMT(("%s: Pktid pool depleted.\n", __FUNCTION__));
		/*
...
#if defined(DHD_HTPUT_TUNABLES)
	if (DHD_IS_FLOWID_HTPUT(dhd, flowid) &&
		!dhd->rsdb_mode && dhd->vsdb_mode) {
		uint16 ring_used_cnt = (uint16)(USED_SPACE(ring->rd,
			ring->wr, ring->max_items));
		if (ring_used_cnt > HP2P_MCC_MAX_ITEM) {
			if (dhd->dma_d2h_ring_upd_support) {
				ring->rd = dhd_prot_dma_indx_get(dhd,
					H2D_DMA_INDX_RD_UPD, ring->idx);
			} else {
				dhd_bus_cmn_readshared(dhd->bus, &(ring->rd),
					RING_RD_UPD, ring->idx);
			}
			ring_used_cnt = (uint16)(USED_SPACE(ring->rd,
				ring->wr, ring->max_items));
			if (ring_used_cnt > HP2P_MCC_MAX_ITEM)
				goto err_free_pktid;
		}
	...
	*/
	...
	if (unlikely(prot->tx_metadata_offset && (headroom < prot->tx_metadata_offset))) {
		DHD_ERROR(("No headroom for Metadata tx %d %d\n",
		prot->tx_metadata_offset, headroom));
	}

	if (likely(prot->tx_metadata_offset && (headroom >= prot->tx_metadata_offset))) {
		...
		if (unlikely(PHYSADDRISZERO(meta_pa))) {
...
dhd_msgbuf_iovar_timeout_dump(dhd_pub_t *dhd)
{
	...
	if (prot->curr_ioctl_cmd == WLC_SET_VAR ||
		...
		prhex("ioctl_buf", ioctl_buf, dump_size);
	...
	bool exactly_nitems)
{
	...
	if (nitems > ring->max_items) {
		DHD_ERROR(("%s: ***invalid nitems=%d, ring->max_items=%d!\n",
		           __FUNCTION__, nitems, ring->max_items));
		WARN_ON(1);
		return NULL;
	}
	...
	uint16 nitems)
{
	...
	if (IDMA_ACTIVE(dhd) ||
		...
		if (prot->mb_2_ring_fn) {
			prot->mb_2_ring_fn(dhd->bus, db_index, TRUE);
		}
	} else if (prot->mb_ring_fn) {
		prot->mb_ring_fn(dhd->bus, DHD_WRPTR_UPDATE_H2D_DB_MAGIC(ring));
	}
}
...
dhd_hp2p_write(struct hrtimer *timer)
{
	hp2p_info_t *hp2p_info;
	unsigned long flags;
	dhd_pub_t *dhdp;
