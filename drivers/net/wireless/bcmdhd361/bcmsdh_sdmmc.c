...
// SDM660 kernel may have merged such functions
...
int sdio_sw_reset(sdioh_info_t *sd, struct mmc_card *card)
{
	int err = 0;

#if defined(DHD_IGNORE_SD_RESET) || defined(BCMQT)
	// nothing to do
	...
	(void)sd;
	sdio_reset_comm(card);
#endif // DHD_IGNORE_SD_RESET
...
#if defined(BCMDHD_MODULAR) && defined(RMMOD_CARD_DETECT)
static struct sdio_func *g_func = NULL;

int sdioh_reset_sdio(void)
{
	int ret = 0;

	if (g_func == NULL) {
		sd_err(("%s Failed, no device!\n", __FUNCTION__));
		return BCME_NODEVICE;
	}

	sdio_claim_host(g_func);
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 19, 0)
	ret = mmc_sw_reset(g_func->card);
#else
	ret = mmc_sw_reset(g_func->card->host);
#endif
...
#endif /* defined(BCMDHD_MODULAR) && defined(RMMOD_CARD_DETECT) */
...
sdioh_attach(osl_t *osh, struct sdio_func *func)
{
	...
	if ((func->device == BCM43430_CHIP_ID) ||
		...
		if (sdmmc_get_clock_rate(sd) > 50000000) {
			/* Current clk is larger than 50Mhz, reduce it to 50Mhz. */
			sdmmc_set_clock_rate(sd, 50000000);
		}
...
#if defined(CONFIG_ARCH_ASTRA) && defined(CONFIG_ARCH_ASTRA_1620)
	if (func->device == BCM4362_CHIP_ID) {
		sdioh_set_driver_strength(sd->func[0], 0x30);
		sd->func[0]->device = func->device;
	}
#endif /* CONFIG_ARCH_ASTRA && CONFIG_ARCH_ASTRA_1620 */
#if defined(CONFIG_ARCH_MESON) || defined(CONFIG_ARCH_ASTRA) || defined(CONFIG_X86_64)
	/* Override SDR Mode and Clk rate */
	if ((dhd_sd_mmc_timing_uhs >= MMC_TIMING_UHS_SDR12) &&
		(dhd_sd_mmc_timing_uhs <= MMC_TIMING_UHS_DDR50)) {
		sdmmc_set_timing(sd, dhd_sd_mmc_timing_uhs);
	}
	if (dhd_sd_mmc_clk_rate) {
		sdmmc_set_clock_rate(sd, dhd_sd_mmc_clk_rate);
	}
#endif /* CONFIG_ARCH_MESON || CONFIG_ARCH_ASTRA || CONFIG_X86_64 */
	...
	if (sd->func[3]) {
		...
		sd->client_block_size[3] = sd_f3_blocksize;
		err_ret = sdio_set_block_size(sd->func[3], sd_f3_blocksize);
		sdio_release_host(sd->func[3]);
		if (err_ret) {
			sd_err(("bcmsdh_sdmmc: Failed to set F3 blocksize to %d(%d)\n",
				sd_f3_blocksize, err_ret));
			goto fail;
		}
	}
#endif /* BT_OVER_SDIO */
...
#endif
	{"sd_speed",	IOV_SPEED,  	0, 0,	IOVT_BUFFER,	0},
                           ...
                           void *params, int plen, void *arg, uint len, bool set)
{
	...
	case IOV_SVAL(IOV_DS):
	{
		...
		sdmmc_set_drv_type(si, (type & 0xF0) >> 4);
		if (err) {
	...
	case IOV_SVAL(IOV_SPEED):
		bcmerror = sdio_transfer_time_set(int_val);
		break;
#endif
                     ...
                     uint addr, void *pkt)
{
	...
	if (sd->txglom_mode == SDPCM_TXGLOM_MDESC) {
		while (pnext != NULL) {
			...
			/* Set up scatter-gather DMA descriptors. this loop is to find out the
			 * max data we can transfer with one command 53. blocks per command is
			 * limited by host max_req_size and 9-bit max block number. when the total
			 * length of this packet chain is bigger than max_req_size, use multiple
			...
			while (pnext != NULL && ttl_len < max_req_size) {
				...
				ASSERT(pdata != NULL);
				pkt_length = PKTLEN(sd->osh, pnext);
				sd_trace(("%s[%d] data=%p, len=%d\n",
					__FUNCTION__, write, pdata, pkt_length));
				/* sg_count is unlikely larger than the array size, and this is NOT
				 * something we can handle here, but in case it happens, PLEASE put
				...
				sg_data_size = pkt_length - pkt_offset;
		...
		if (stage == 0) {
			...
			*/
			/* sdio_reset_comm() - has been fixed in latest kernel/msm.git for Linux
			   ...
			   patch for it
			*/
			if ((ret = sdio_sw_reset(sd, sd->func[0]->card))) {
				sd_err(("%s Failed, error = %d\n", __FUNCTION__, ret));
				return ret;
			} else {
				sd->num_funcs = 2;
				 ...
				 */
				if ((sd->func[0]->device == BCM4612_CHIP_ID) ||
					(sd->func[0]->device == BCM4611_CHIP_ID)) {
					if (*dhd_sd_mmc_ds == DRVSTRN_IGNORE_CHAR) {
						/* Default drive strength 'A'
						 * for 4612 chip on VIM3 platform
						 */
						sdioh_set_driver_strength(sd->func[0], 0x10);
						sdmmc_set_drv_type(sd, 0x1);
					}
				}
				...
				if ((sd->func[0]->device == BCM43430_CHIP_ID) ||
					...
					if (sdmmc_get_clock_rate(sd) > 50000000) {
						/* Current clk is larger than 50Mhz,
						 * reduce it to 50Mhz.
						 */
						sdmmc_set_clock_rate(sd, 50000000);
					}
				...
				if (sd->func[3]) {
					...
					sd->client_block_size[3] = sd_f3_blocksize;
					ret = sdio_set_block_size(sd->func[3], sd_f3_blocksize);
					if (ret) {
						sd_err(("bcmsdh_sdmmc: Failed to set F3 blocksize"
							" to %d(%d)\n", sd_f3_blocksize, ret));
					}

					/* Release host controller F3 */
					sdio_release_host(sd->func[3]);
				}
#endif /* BT_OVER_SDIO */
...
uint8
sdmmc_get_timing(sdioh_info_t *sd)
{
...
void
sdmmc_set_timing(sdioh_info_t *sd, uint8 timing)
{
	struct sdio_func *sdio_func = sd->func[0];
	struct mmc_host *host = sdio_func->card->host;
