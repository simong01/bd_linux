...
int wifi_platform_get_irq_level(wifi_adapter_info_t *adapter)
{
	...
	if (!adapter) {
		return BCME_BADARG;
	} else if (!(plat_data = adapter->wifi_plat_data)) {
		return BCME_BADARG;
	}
#ifdef DHD_USE_HOST_WAKE
	else if (!plat_data->get_oob_gpio_level) {
		/* use legacy way */
...
MODULE_PARM_DESC(dhd_vid, "For Synaptics internal use only (def: 0xa5c)");
module_param(dhd_pid, int, 0);
MODULE_PARM_DESC(dhd_pid, "For Synaptics internal use only (def: 0x48f)");
