...
MODULE_PARM_DESC(sd_msglevel, "SD debug message level (def: " XSTR(SDH_ERROR_VAL) ")");

extern uint sd_power;	/* 0 = SD Power OFF, 1 = SD Power ON. */
module_param(sd_power, uint, 0);
MODULE_PARM_DESC(sd_power, "For Synaptics internal use only (def: 1)");

extern uint sd_clock;	/* SD Clock Control, 0 = SD Clock OFF, 1 = SD Clock ON */
module_param(sd_clock, uint, 0);
MODULE_PARM_DESC(sd_clock, "For Synaptics internal use only (def: 1)");

extern uint sd_divisor;	/* Divisor (-1 means external clock) */
module_param(sd_divisor, uint, 0);
MODULE_PARM_DESC(sd_divisor, "SDIO clock divisor (-1 means external clock) (def: 2)");

extern uint sd_sdmode;	/* Default is SD4, 0=SPI, 1=SD1, 2=SD4 */
module_param(sd_sdmode, uint, 0);
MODULE_PARM_DESC(sd_sdmode, "SDIO mode: 0=SPI, 1=SD1, 2=SD4 (def: 2)");

extern uint sd_hiok;	/* Ok to use hi-speed mode */
module_param(sd_hiok, uint, 0);
MODULE_PARM_DESC(sd_hiok, "For Synaptics internal use only (def: 0)");

extern uint sd_f2_blocksize;
module_param(sd_f2_blocksize, int, 0);
MODULE_PARM_DESC(sd_f2_blocksize, "F2 access block size (def: " XSTR(CUSTOM_SDIO_F2_BLKSIZE) ")");

extern uint sd_f1_blocksize;
module_param(sd_f1_blocksize, int, 0);
MODULE_PARM_DESC(sd_f1_blocksize, "F1 access block size (def: " XSTR(CUSTOM_SDIO_F1_BLKSIZE) ")");

#ifdef BCMSDIOH_STD
extern int sd_uhsimode;
module_param(sd_uhsimode, int, 0);
MODULE_PARM_DESC(sd_uhsimode, "For Synaptics internal use only (def: " XSTR(SD3CLKMODE_DISABLED) ")");
extern uint sd_tuning_period;
module_param(sd_tuning_period, uint, 0);
MODULE_PARM_DESC(sd_tuning_period, "For Synaptics internal use only (def: " XSTR(CAP3_RETUNING_TC_OTHER) ")");
extern int sd_delay_value;
module_param(sd_delay_value, uint, 0);
MODULE_PARM_DESC(sd_delay_value, "For Synaptics internal use only (def: 50000)");

extern int sd_uhsiclk;
module_param(sd_uhsiclk, uint, 0);
MODULE_PARM_DESC(sd_uhsiclk, "This param can be additionally used for \
	control of SDIO clock irrespective of sd_uhsimode \
	Eg: If sd_uhsimode is 3, its SDR 104. Hence Max freq supported = \
	104 * 2 = 208 MHz. \
	But if Host controller wants to limit to 100 MHz, sd_uhsiclk \
	can be set to 2 (SDR 50) 0-SDR12 [25MHz]; 1-SDR25 [50MHz]; \
	2-SDR50 [100MHz]; 3-SDR104 [208MHz]");
...
MODULE_PARM_DESC(dhd_sdiod_uhsi_ds_override, "For Synaptics internal use only (def: ' ')");
#endif /* BCMSDIOH_STD */
