...
static const struct proc_ops proc_fops_ts_fw_clk_dump = {
	.proc_read = dhd_read_proc_ts_fw_clk_dump,
	.proc_open = dhd_open_proc_ts_fw_clk_dump,
	.proc_release = seq_release,
};
static const struct proc_ops proc_fops_ts_tx_dump = {
	.proc_read = dhd_read_proc_ts_tx_dump,
	.proc_open = dhd_open_proc_ts_tx_dump,
	.proc_release = seq_release,
};
static const struct proc_ops proc_fops_ts_rx_dump = {
	.proc_read = dhd_read_proc_ts_rx_dump,
	.proc_open = dhd_open_proc_ts_rx_dump,
	.proc_release = seq_release,
};
#else
...
dhd_timesync_detach(dhd_pub_t *dhdp)
{
	...
	MFREE(dhdp->osh, dhdp->ts, sizeof(dhd_ts_t));
...
dhd_timesync_attach(dhd_pub_t *dhdp)
{
	...
	ts = MALLOC(dhdp->osh, sizeof(dhd_ts_t));
	if (!ts) {
		DHD_ERROR(("%s: OOM - alloc dhd_ts_t\n", __FUNCTION__));
		goto fail;
	}
            ...
            void *params, uint plen, void *arg, uint len, int val_size)
{
	...
	case IOV_SVAL(IOV_TS_NO_RETRY):
		int_val = dhd_prot_pkt_noretry(ts->dhdp, int_val, TRUE);
		break;
	...
	case IOV_SVAL(IOV_TS_NO_AGGR):
		int_val = dhd_prot_pkt_noaggr(ts->dhdp, int_val, TRUE);
		break;
	...
	case IOV_SVAL(IOV_TS_FIXED_RATE):
		int_val = dhd_prot_pkt_fixed_rate(ts->dhdp, int_val, TRUE);
		break;
	...
	case IOV_SVAL(IOV_TS_TXS_LOG):
		int_val = dhd_prot_data_path_tx_timestamp_logging(ts->dhdp, int_val,  TRUE);
		break;
	...
	case IOV_SVAL(IOV_TS_RXS_LOG):
		int_val = dhd_prot_data_path_rx_timestamp_logging(ts->dhdp, int_val,  TRUE);
		break;
