...
MODULE_PARM_DESC(fw_ap_select, "Skip supplicant hints and let fw decides which AP to join (def: false)");
...
MODULE_PARM_DESC(wl_reassoc_support, "Support reassociation (def: false)");
...
/* XV without 6G  need 5 rules like, better to set 1 more item
* (2402 - 2472 @ 20), (6, 20), (N/A)
* (2457 - 2482 @ 20), (6, 20), (N/A), PASSIVE-SCAN
* (5170 - 5250 @ 80), (6, 20), (N/A), PASSIVE-SCAN
* (5250 - 5730 @ 80), (6, 20), (0 ms), DFS, PASSIVE-SCAN
* (5735 - 5835 @ 80), (N/A, 0), (N/A), PASSIVE-SCAN
*/
...
#endif /* WL_CHAN_UTIL */
static s32 wl_cfg80211_wsec_info_pmk(struct bcm_cfg80211 *cfg, struct net_device *ndev,
...
static s32 wl_cfg80211_parse_vndr_ies(const u8 *parse, u32 len,
	struct parsed_vndr_ies *vndr_ies, const bcm_tlv_t **ppExt_cap_ies);
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 10, 0)) && !defined(CONNECT_WITH_BSSID)
static int wl_cfg80211_update_connect_params(struct wiphy *wiphy, struct net_device *dev,
	struct cfg80211_connect_params *sme, u32 changed);
#endif /* LINUX_VERSION_CODE >= KERNEL_VERSION(4, 10, 0) && !CONNECT_WITH_BSSID */
...
static int wl_cfg80211_get_tx_power(struct wiphy *wiphy,
	...
	int *dbm);
#else
static int wl_cfg80211_get_tx_power(struct wiphy *wiphy, int *dbm);
#endif /* WL_CFG80211_P2P_DEV_IF */
static s32 wl_cfg80211_config_default_key(struct wiphy *wiphy,
	struct net_device *dev,
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 1, 0) || defined(CFG80211_BKPORT_MLO)
	int link_id,
#endif /* LINUX_VER >= 6.1 || defined(CFG80211_BKPORT_MLO) */
...
static int wl_eq_empty(struct bcm_cfg80211 *cfg);
...
wl_cfg80211_event_roaming_priv(struct bcm_cfg80211 *cfg, bcm_struct_cfgdev *cfgdev,
	u32 event_type, const wl_event_msg_t *e, void *data);
...
static s32 wl_phy_cal_event(struct bcm_cfg80211 *cfg,
		bcm_struct_cfgdev *cfgdev, const wl_event_msg_t *e, void *data);
...
static struct ieee80211_sband_iftype_data __wl_he_cap_2ghz = {
	.types_mask = BIT(NL80211_IFTYPE_STATION) | BIT(NL80211_IFTYPE_AP)
};
static struct ieee80211_sband_iftype_data __wl_he_cap_5ghz = {
	.types_mask = BIT(NL80211_IFTYPE_STATION) | BIT(NL80211_IFTYPE_AP)
};
#ifdef CFG80211_6G_SUPPORT
static struct ieee80211_sband_iftype_data __wl_he_cap_6ghz = {
	.types_mask = BIT(NL80211_IFTYPE_STATION) | BIT(NL80211_IFTYPE_AP)
};
#endif /* CFG80211_6G_SUPPORT */
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 0, 0))
	{WLAN_CIPHER_SUITE_CCMP_256, AES_ENABLED, CRYPTO_ALGO_AES_CCM256},
#endif /* LINUX_VERSION_CODE >= KERNEL_VERSION(4, 0, 0) */
...
chanspec_t wl_cfg80211_get_shared_freq(struct wiphy *wiphy)
{
	...
	u16 channel = 0;
	char *buf;
	chanvec_t  customer_list = { {CONST_CUSTOMER_CHANLIST_INVALID} };

	/* customer_chanlist support case */
	err = wldev_iovar_getbuf_bsscfg(dev, "customer_chanlist",
			NULL, 0,
			&customer_list, sizeof(customer_list),
			0, &cfg->ioctl_buf_sync);
	if ((err == BCME_OK) &&
	    (SYNA_CUSTOMER_CHANLIST_VALID(customer_list.vec))) {
		int idx;
		/* show bytes only if needs */
		/* prhex("customer_chanlist", (uchar *)&customer_list, sizeof(customer_list)); */
		for (idx = 0; idx < MAXCHANNEL; idx++) {
			if (isset(customer_list.vec, idx)) {
				channel = idx;
				WL_TRACE(("%s-%d: found usable channel=%d\n",
	...
	if ((err = wldev_ioctl_get(dev, WLC_GET_BSSID, &bssid, sizeof(bssid)))) {
		...
		if (!channel) {
			cur_band = 0;
			err = wldev_ioctl_get(dev, WLC_GET_BAND,
					&cur_band, sizeof(int));
			if (unlikely(err)) {
				WL_ERR(("Get band failed, use 'auto'!\n"));
				cur_band = WLC_BAND_AUTO;
			}
			if (cur_band == WLC_BAND_5G || cur_band == WLC_BAND_6G) {
				channel = WL_P2P_TEMP_CHAN_5G;
			} else {
				channel = WL_P2P_TEMP_CHAN;
			}
		}

		WL_TRACE(("%s-%d: wll return channel=%d -> 0x%X\n",
		          __func__, __LINE__,
		          channel, wl_ch_host_to_driver(channel)));
	...
	if (enable) {
		if (!cfg->wlfc_on && !disable_proptx) {
			...
			if (!wlfc_enabled && dhd->op_mode != DHD_FLAG_HOSTAP_MODE &&
				...
				dhd_wlfc_init(dhd);
				// use right dhd->up flag
				err = wldev_ioctl_set(primary_ndev, WLC_UP, &dhd->up, sizeof(s32));
				if (err < 0)
...
wl_cfg80211_add_monitor_if(struct wiphy *wiphy, const char *name)
{
	...
	wdev = KVZALLOC(sizeof(*wdev), GFP_KERNEL);
	if (!wdev) {
	...
	struct net_device *ndev)
{
	...
	chipnum = revinfo.chipnum;
	WL_DBG(("wl_legacy_chip_check: GET_REVINFO device 0x%x, vendor 0x%x,"
		" chipnum 0x%x\n",
		dtoh32(revinfo.deviceid), dtoh32(revinfo.vendorid), dtoh32(chipnum)));
	if (
#ifdef BCM4350_CHIP_ID
		...
		((chipnum == BCM4355_CHIP_ID) && (revinfo.chiprev < BCM4355_REV_C1)) ||
#endif /* BCM4355_CHIP_ID */
#if defined(BCM4345_CHIP_ID) || defined(BCM43456_CHIP)
		(chipnum == BCM4345_CHIP_ID) ||
#endif /* BCM4345_CHIP_ID */
#ifdef BCM43598_CHIP_ID
		(chipnum == BCM43598_CHIP_ID) ||
#endif /* BCM43598_CHIP_ID */
		false) {
		/* WAR required */
		WL_DBG(("%s: Customer6 legacy chip identified\n", __FUNCTION__));
		return true;
	...
	u8 *mac_addr, const char *name)
{
	...
	if (likely(!mac_addr)) {
		 ...
		 *  Secondary STA I/F
		 */
		DEV_ADDR_GET(primary_ndev, addr);
		addr[0] |= 0x02;
	} else {
...
wl_is_fils_supported(struct net_device *ndev)
{
	...
	if ((err == BCME_UNSUPPORTED) || (0 > err)) {
		WL_ERR(("FILS NOT supported, err %d(ignore)\n", err));
		return false;
	}
	...
	struct cfg80211_connect_params *sme, wlcfg_assoc_info_t *assoc_info)
{
	...
	WL_ERR(("multi_akm 0x%x wpa_ver 0x%x ciphers_pairwise 0x%x cipher_group 0x%x\n",
		multi_akm, sme->crypto.wpa_versions,
	...
	wlcfg_assoc_info_t *assoc_info)
{
	...
	if (sme->crypto.n_akm_suites) {
		...
		/* Avoid WPA version 1 for multi-AKM */
...
#if defined(BCM43456_CHIP) || !defined(CONFIG_SOC_GOOGLE)
			key.flags = WL_PRIMARY_KEY;
#endif /* BCM43456_CHIP || !CONFIG_SOC_GOOGLE */
	...
	struct cfg80211_connect_params *sme, wlcfg_assoc_info_t *assoc_info)
{
	 ...
	 * NetworkManager appends WPA-PSK WPA-PSK-SHA256 FT-PSK SAE FT-SAE for "key-mgmt=wpa-psk"
	 * for existing connections can benefit from the improved security when the AP gets upgraded
	 * That will lead the number of AKM suites to be more than one, therefore,
	 * when connecting WPA-PSK(ver 1) TKIP/AES by NetworkManager, we need cipher setting
	 * even the number of AKM suites is more than one.
	 */
	if (!(sme->crypto.n_akm_suites > 1) ||
		((sme->crypto.n_akm_suites > 1) &&
		(sme->crypto.akm_suites[0] == WLAN_AKM_SUITE_PSK) &&
		(sme->crypto.wpa_versions == NL80211_WPA_VERSION_1))) {
	...
	UNUSED_PARAMETER(wl_set_passphrase);
#ifdef WL_SAE
	if (cfg && dev) {
		struct wl_security  *sec = wl_read_prof(cfg, dev, WL_PROF_SEC);
		if (sec && ((sec->fw_wpa_auth & WPA3_AUTH_SAE_PSK) ||
			(assoc_info->seamless_psk == TRUE))) {
	...
	struct cfg80211_connect_params *sme)
{
	...
	/* postpone connect to aovid state conflicting */
	if (sme) {
		err = 200; /* 50ms * 200 = 10 seconds */
		while (0 < err) {
			/* use LOCK to confirm single event processing was done */
			DHD_EVENT_WAKE_LOCK(cfg->pub);
			/* waiting for 'no roaming scan' and no pending events */
			if ((!dhdp->during_roaming) && (wl_eq_empty(cfg))) {
		...
		/* check if still BUSY or not for new connecting */
		if ((dhdp->during_roaming) || (!wl_eq_empty(cfg))) {
			WL_ERR(("fail, busy since roaming=%d, eq_empty=%d\n",
			       dhdp->during_roaming, wl_eq_empty(cfg)));
			err = BCME_BUSY;
			goto fail;
		} else {
			err = BCME_OK;
	...
	/* set/recovery 802.11 mode for STA role */
	dhd_80211_mode_apply((dhd_pub_t *)(cfg->pub),
	                     dhd_net2idx(dhdp->info, dev), FALSE, TRUE);
	...
	u16 reason_code)
{
	...
	/* show more information about actively disconnect */
	WL_ERR(("%s: disconnect in connect state [%d:%d:%d]. reason:%d\n",
	        __FUNCTION__, connected, conn_in_progress, act, reason_code));
		...
		*/
		...
		if (conn_in_progress || connected ||
			WDEV_CLIENT(wdev, ssid_len)) {
				...
				if (unlikely(err)) {
					wl_clr_drv_status(cfg, DISCONNECTING, dev);
					WL_ERR(("error (%d) (ignore when remove)\n", err));
					goto exit;
				}
		}
#ifdef WL_WPS_SYNC
	...
	s32 *dbm)
{
	...
	if (!dbm) {
		...
		WL_ERR(("invalid wiphy!\n"));
		err = -2;
	} else if (!(cfg = wiphy_priv(wiphy))) {
		WL_ERR(("wiphy=0x%p, invalid cfg!\n", wiphy));
		err = -3;
	} else if (!cfg->wdev) {
		WL_ERR(("wiphy=0x%p, cfg=0x%p, invalid wdev!\n", wiphy, cfg));
		err = -4;
	} else if (!(ndev = bcmcfg_to_prmry_ndev(cfg))) {
		WL_ERR(("wiphy=0x%p, cfg=0x%p, invalid ndev!\n", wiphy, cfg));
		err = -5;
	} else {
		RETURN_EIO_IF_NOT_UP(cfg);
...
#endif /* WL_GCMP */
/* CCKM roam scb authorize turn on too early will cause packet drop */
/* After the key installed, to set authorize here */
#if defined(WLAN_CIPHER_SUITE_PMK) && defined(BCMCCX)
	sec = wl_read_prof(cfg, dev, WL_PROF_SEC);
	if (sec->wpa_auth == WLAN_AKM_SUITE_CCKM) {
		if (key_idx == 1) {
			s32 val;
			u8 *curbssid = NULL;
			curbssid = wl_read_prof(cfg, dev, WL_PROF_BSSID);
			 val = wldev_ioctl_set(dev, WLC_SCB_AUTHORIZE, curbssid, ETH_ALEN);
			if (unlikely(val)) {
				WL_ERR(("WLC_SCB_AUTHORIZE error (%d)\n", val));
			} else {
				WL_ERR(("[%s] WLC_SCB_AUTHORIZE " MACDBG "\n",
					dev->name, MAC2STRDBG(curbssid)));
			}
			wl_del_roam_timeout(cfg);
		}
	}
#endif /* WLAN_CIPHER_SUITE_PMK && BCMCCX */
	return err;
}
...
get_station_err:
		...
		case WL_IF_TYPE_AP:
			...
			/* RSSI report */
			for (i = 0, cnt = 0; i < WL_STA_ANT_MAX; i++) {
				if (sta->rssi[i] != WLC_RSSI_INVALID) {
					rssi += sta->rssi[i];
					cnt++;
				}
			}
	...
	if (WLC_PMKLIST_V3_SUPPORT(cfg->wlc_ver)) {
		WL_INFORM_MEM(("%s: PMKID_LIST_V3\n", __func__));
			...
			err = wldev_iovar_setbuf(dev, "pmkid_info", (char *)pmk_list,
				...
				WLC_IOCTL_MAXLEN, &cfg->ioctl_buf_sync);
	} else if (WLC_PMKLIST_V2_SUPPORT(cfg->wlc_ver)) {
	...
	wsec_pmk_t *pmk, bool validate_sec)
{
	...
	if (err) {
		...
		WL_INFORM_MEM(("set okc_info_pmk complete\n"));
	}
	return;
}
	...
	if (i < WL_NUM_PMKIDS_MAX) {
		...
		if (pmksa->pmk_len) {
			if (memcpy_s(&cfg->pmk_list->pmkids.pmkid[i].pmk, PMK_LEN_MAX, pmksa->pmk,
				pmksa->pmk_len)) {
				WL_ERR(("invalid pmk len = %lu", (long)pmksa->pmk_len));
			} else {
	...
	struct mgmt_frame_regs * flags)
{
	 ...
	 *         for these subtypes
	 */
	WL_DBG(("dev_type=0x%X(mcast=0x%X), itf_type=0x%X(mcast=0x%X)\n",
	        flags->global_stypes, flags->interface_stypes,
	        flags->global_mcast_stypes, flags->interface_mcast_stypes));
	...
	struct cfg80211_chan_def *chandef)
{
	...
	if (!cfg->pub) {
		WL_ERR(("cfg->pub parameter invalid!\n"));
		return -EINVAL;
	} else if (!wdev->netdev) {
		WL_ERR(("wdev->netdev parameter invalid!\n"));
		return -EINVAL;
	} else {
		dhd_pub_t *  dhd = (dhd_pub_t *)(cfg->pub);
		bool         fw_assoc_state;
		...
		fw_assoc_state = dhd_fw_assoc_state(dhd, wdev->netdev);
		if (!fw_assoc_state) {
			WL_ERR(("skip since not associated, fw_assoc_state=%d\n",
			        fw_assoc_state));
			return -EINVAL;
		}
	}
	...
	if (!chandef || (wl_chspec_to_chandef(chanspec, chandef, wiphy) != BCME_OK)) {
		return BCME_ERROR;
	}
	return 0;
}
...
static void wl_cfg80211_regd_duplicate(struct ieee80211_regdomain *regd_copy,
		const struct ieee80211_regdomain *regd_orig)
{
	...
	for (i = 0; i < regd_orig->n_reg_rules; i++) {
		memcpy_s(&regd_copy->reg_rules[i],
		         sizeof(struct ieee80211_reg_rule),
		         &regd_orig->reg_rules[i],
		         sizeof(struct ieee80211_reg_rule));
	}
...
static int wl_cfg80211_regd_update(struct bcm_cfg80211 *cfg, char *ccode)
{
	...
	if (!IS_REGDOM_SELF_MANAGED(wiphy)) {
		...
		if (err) {
			WL_ERR(("%s: update country change to "
			        "upper layers failed\n", __FUNCTION__));
		}
	}
		...
		regd_copy = KVZALLOC(regd_len, GFP_ATOMIC);
	...
	bool notify, bool user_enforced, int revinfo)
{
	...
	struct wiphy *wiphy = wdev->wiphy;
	struct bcm_cfg80211 *cfg = wiphy_priv(wiphy);
	u8 country_bak[WLC_CNTRY_BUF_SZ];
	...
	// backup
	strlcpy(country_bak, cfg->country, WLC_CNTRY_BUF_SZ);
	...
	if (ret < 0) {
		...
		strlcpy(cfg->country, country_bak, WLC_CNTRY_BUF_SZ);
		goto exit;
	}
#if defined(SYNA_80211_MODE)
	else {
		...
		/* set 802.11 mode
		 * It's safe to call WLC_UP once 'wl country'
		 * can execute successful
		 */
		dhd_80211_mode_apply(dhd, ifidx, TRUE, TRUE);
	}
#endif /* defined(SYNA_80211_MODE) */
...
#else /* defined(CUSTOMER_HW6) || defined(WIPHY_DYNAMIC_UPDATE) */
	if (!IS_REGDOM_SELF_MANAGED(wiphy)) {
...
/**
 * Adjust start and end frequencies based on respective start and end
 * channels flags
 */
static int wl_cfg80211_regd_adj_freq(int *freq_start, int *freq_end,
	uint32 ieee_flags_start, uint32 ieee_flags_end)
{
	if (!freq_start || !freq_end) {
		return BCME_BADARG;
	}
	if (ieee_flags_start & IEEE80211_CHAN_NO_20MHZ) {
		if (!(ieee_flags_start & CONST_SYNA_CNAN_NO_80MHZ_HT40)) {
			*freq_start -= 20;
		} else if (!(ieee_flags_start & IEEE80211_CHAN_NO_160MHZ)) {
			*freq_start -= 80;
		} else if (!(ieee_flags_start & IEEE80211_CHAN_NO_80MHZ)) {
			*freq_start -= 40;
		} else if (!(ieee_flags_start & IEEE80211_CHAN_NO_HT40)) {
			*freq_start -= 20;
		} else if (!(ieee_flags_start & IEEE80211_CHAN_NO_20MHZ)) {
			*freq_start -= 10;
		} else {
			*freq_start -= 5;
		}
	...
	if (ieee_flags_end & IEEE80211_CHAN_NO_20MHZ) {
		if (!(ieee_flags_end & CONST_SYNA_CNAN_NO_80MHZ_HT40)) {
			*freq_end   += 20;
		} else if (!(ieee_flags_end & IEEE80211_CHAN_NO_160MHZ)) {
			*freq_end   += 80;
		} else if (!(ieee_flags_end & IEEE80211_CHAN_NO_80MHZ)) {
			*freq_end   += 40;
		} else if (!(ieee_flags_end & IEEE80211_CHAN_NO_HT40)) {
			*freq_end   += 20;
		} else if (!(ieee_flags_end & IEEE80211_CHAN_NO_20MHZ)) {
			*freq_end   += 10;
		} else {
			*freq_end   += 5;
		}
		...
		int gain, int eirp)
{
	...
	if (!regd) {
		...
		return -2;
	} else if (0 >= (band_width = wl_cfg80211_regd_ieee_flag_to_bandwidth(ieee_flags))) {
		return -3;
	} else if (band_width > (freq_end - freq_start)) {
		WL_ERR(("*Error %dMHz-%dMHz too narrow accomodate %dMHz bw\n",
			freq_start, freq_end, band_width));
		return BCME_NORESOURCE;
	} else {
		int    index = -1;
...
#if defined(DHD_DEBUG)
		WL_DBG(("rule[%d]: start_freq_khz=%d, end_freq_khz=%d, "
	...
	if ((!channels) || (0 >= qty)) {
		...
		if ((i + 1) >= qty) {
			...
			WL_DBG(("=> confirm last(i=%d), CH[%d]=%d(freq=%d) ~ CH[%d]=%d(freq=%d),"
			        ...
			        channels[range_end].flags, flags));
			freq_start = channels[range_start].center_freq;
			freq_end = channels[range_end].center_freq;
			wl_cfg80211_regd_adj_freq(&freq_start, &freq_end,
				channels[range_start].flags, channels[range_end].flags);

			wl_cfg80211_regd_update_rule(regd, freq_start, freq_end,
			...
			if (flags != (mask_compare & channels[i+1].flags)) {
				...
				if (BCME_NORESOURCE ==
					wl_cfg80211_regd_update_rule(regd,
						freq_start, freq_end,
						channels[range_start].flags,
						CONST_SYNA_REG_ANTENNA_GAIN_DEFAULT,
						CONST_SYNA_REG_EIRP_DEFAULT)) {
					/* Retry as 20 MHz channel */
					wl_cfg80211_regd_update_rule(regd,
						freq_start, freq_end,
						(channels[range_start].flags |
							 (IEEE80211_CHAN_NO_160MHZ |
...
static s32 wl_setup_wiphy(struct wireless_dev *wdev, struct device *sdiofunc_dev, void *context)
{
	...
	*/
	brcm_wowlan_config = KVMALLOC(sizeof(struct cfg80211_wowlan), GFP_KERNEL);
...
static void wl_free_wdev(struct bcm_cfg80211 *cfg)
{
	...
	if ((!cfg) || (! cfg->wdev)) {
		WL_ERR(("cfg or wdev is invalid\n"));
		return;
	} else {
		wdev =  cfg->wdev;
		// early mark wdev remove
		cfg->wdev = NULL;
	}
...
#endif /* LINUX_VERSION_CODE >= KERNEL_VERSION(3, 11, 0) */
#endif /* CONFIG_PM */
		// here the 'wiphy_unregister' will also free the 'regd' we registered
		wiphy_unregister(wdev->wiphy);
		wdev->wiphy->dev.parent = NULL;
		wdev->wiphy = NULL;

		// free the wiphy bands
...
#if defined(READ_CONFIG_FROM_FILE) && defined(SYNA_CUSTOMER_WME_AC)
	dhd_wme_ac_set_sta_param(dhdp, dhd_net2idx(dhdp->info, ndev));
#endif /* READ_CONFIG_FROM_FILE */
#if defined(CONFIG_TIZEN)
	...
	wl_sae_key_info_t *sae_key)
{
	...
	skb = CFG80211_VENDOR_EVENT_ALLOC(wiphy, ndev_to_wdev(ndev),
		BRCM_SAE_VENDOR_EVENT_BUF_LEN, BRCM_VENDOR_EVENT_SAE_KEY, kflags);
	if (!skb) {
...
static void
wl_cache_assoc_req_ies(struct bcm_cfg80211 *cfg, struct net_device *ndev,
	const wl_event_msg_t *e, void *data)
{
...
#define SOFTAP_DEFAULT_2G_CH	6
int wl_cfg80211_set_softAP_csa(struct net_device *ndev)
{
	...
	csa_arg.mode = DOT11_CSA_MODE_NO_TX;
	csa_arg.count = 20;
	csa_arg.reg = 0;
	csa_arg.chspec = wf_create_20MHz_chspec(SOFTAP_DEFAULT_2G_CH, WL_CHANSPEC_BAND_2G);
	err = wldev_iovar_setbuf(ndev, "csa", &csa_arg, sizeof(csa_arg),
		smbuf, sizeof(smbuf), NULL);
	if (err) {
		WL_ERR(("set csa error=%d\n", err));
	}
	return err;
}
...
wl_handle_link_down(struct bcm_cfg80211 *cfg, wl_assoc_status_t *as)
{
	...
	chanspec = (chanspec_t *)wl_read_prof(cfg, ndev, WL_PROF_CHAN);
	if ((chanspec) && (ndev == bcmcfg_to_prmry_ndev(cfg))) {
		channel = wf_chspec_ctlchan(*chanspec);
		if (CHSPEC_IS5G(*chanspec) && (channel > 48) && (channel < 149)) {
			struct net_info *iter, *next;
			/* check softAP status */
			GCC_DIAGNOSTIC_PUSH_SUPPRESS_CAST();
			for_each_ndev(cfg, iter, next) {
	...
	if (memcmp(as->curbssid, as->addr, ETHER_ADDR_LEN) != 0) {
		if (// some disconnect early before sending event case will
			// clear BSSID as 00:00:00:00:00:00 before sending event
			(ETHER_ISNULLADDR(as->addr))
#ifdef WLFBT
			// link down when roaming to nearly successful state(15)
			|| (!memcmp(&cfg->start_roam_addr, as->addr, ETHER_ADDR_LEN))
#endif // WLFBT
		   ) {
			WL_ERR(("Overwrite "MACDBG" with old MAC "
				MACDBG" for pending FT roaming\n",
				MAC2STRDBG(as->addr), MAC2STRDBG(as->curbssid)));
			memcpy_s(as->addr, ETHER_ADDR_LEN, as->curbssid, ETHER_ADDR_LEN);
		} else {
	...
	void *data, wl_assoc_state_t assoc_state)
{
	...
	switch (as.event_type) {
		...
		case WLC_E_ASSOC_REQ_IE:
			wl_cache_assoc_req_ies(cfg, as.ndev, e, data);
			break;
	...
	struct wireless_dev *wdev, const wl_event_msg_t *e, void *data)
{
	...
	if (wl_get_drv_status(cfg, CONNECTING, ndev)) {
		...
		if (event_type == WLC_E_JOIN ||
			event_type == WLC_E_BSSID ||
			event_type == WLC_E_ROAM) {
			bzero((u8*)&cfg->start_roam_addr, ETHER_ADDR_LEN);
		}
#endif
...
#ifdef WLFBT
		/*   When roaming,After WLC_E_AUTH SUCCESS,fw will clear
		 * PENDING_AUTH flag,starting ASSOC REQ. But During this time,
		 * if device received special packets,such as PS_POLL,
		 * BLOCKACK,BLOCKACK_REQ coming from origin router(due to two
		 * routers in the same band),fw will send DEAUTH packets
		 * to origin router and sendup WLC_E_DEAUTH to driver,because
		 * the WLC_E_DEAUTH event came earlier than
		 * WLC_E_JOIN/WLC_E_BSSID/WLC_E_ROAM,it will cause uplayer
		 * thinking the roaming was fail.
		 *   In fw logical,when roaming complete,it must wait for the
		 * first beacon frame of the roamed router, then will sendup
		 * WLC_E_JOIN/WLC_E_BSSID/WLE_E_ROAM event to driver.
		 * So do the patch below :
		 * 1, After driver received WLC_E_AUTH SUCCESS event,save the
		 *    roamed addr.
		 * 2, Between WLC_E_AUTH SUCCESS and
		 *    WLC_E_JOIN/WLC_E_BSSID/WLC_E_ROAM,if received
		 *    WLC_E_DEAUTH,
		 *    check the addr whether the same with the saved
		 *    roamed addr,if not,just ingore this event.
		 * 3, After driver received WLE_E_JOIN/WLC_E_BSSID/WLC_E_ROAM,
		 *    clear the saved roamed addr.
		 */
		...
		if (event_type == WLC_E_AUTH) {
			...
			WL_INFORM_MEM(("AUTH: cfg->fbt_auth_done=%d\n", cfg->fbt_auth_done));
		} else if (event_type == WLC_E_ASSOC) {
			if (cfg->fbt_auth_done) {
				/* clean the fbt_auth done since assoc instead of reassoc */
				cfg->fbt_auth_done = 0;
				WL_INFORM_MEM(("ASSOC: cfg->fbt_auth_done=%d\n", cfg->fbt_auth_done));
			}
		} else if (event_type == WLC_E_JOIN || event_type == WLC_E_BSSID || event_type == WLC_E_ROAM)
			bzero((u8*)&cfg->start_roam_addr, ETHER_ADDR_LEN);
		else if (event_type == WLC_E_DEAUTH && !ETHER_ISNULLADDR(&cfg->start_roam_addr.octet) &&
				memcmp(&e->addr, (u8*)&cfg->start_roam_addr, ETHER_ADDR_LEN)) {
			WL_INFORM_MEM(("Skip unknown bssid %pM WLC_E_DEAUTH, current bssid %pM\n",
					&e->addr, (u8*)&cfg->start_roam_addr));
			goto exit;
		}
#endif /* WLFBT */
	} else {
...
#ifdef WL_MON_OWN_PKT
	wl_cfg80211_event_roaming_priv(cfg, cfgdev, WLC_E_ROAM, e, data);
#endif /* WL_MON_OWN_PKT */

	if ((cfg->disable_roam_event) && (event == WLC_E_ROAM)) {
		// roaming is done no mater trigger the roaming or not
		wl_del_roam_timeout(cfg);
		return err;
	}
	...
	const wl_event_msg_t *e, void *data)
{
	...
	if (status == WLC_E_STATUS_SUCCESS && reason != WLC_E_REASON_INITIAL_ASSOC) {
		...
		// allow roaming event processing
...
#ifdef WL_WPS_SYNC
	if (wl_get_wps_inst_match(cfg, ndev) == BCME_ERROR)
	/* No active WPS session */
#endif /* WL_WPS_SYNC */
       {
		/* Admit the connecting status */
		if (!wl_get_drv_status(cfg, CONNECTED, ndev)) {
			wl_set_drv_status(cfg, CONNECTING, ndev);
		}
	}

#ifdef WL_MON_OWN_PKT
	wl_cfg80211_event_roaming_priv(cfg, cfgdev, WLC_E_ROAM_PREP, e, data);
#endif /* WL_MON_OWN_PKT */
...
#ifdef WL_MON_OWN_PKT
	wl_cfg80211_event_roaming_priv(cfg, cfgdev, WLC_E_ROAM_START, e, data);
#endif /* WL_MON_OWN_PKT */

	dhdp->during_roaming = 1;
	mod_timer(&cfg->roam_timeout, jiffies + msecs_to_jiffies(WL_ROAM_TIMEOUT_MS * 3));
	...
	const wl_event_msg_t *e, void *data)
{
	...
	if (   (0 != conn_info->req_ie_len)
	    && (0 != conn_info->resp_ie_len)
	   ) {
		// nothing to do here
	} else if ((err = wl_get_assoc_ies(cfg, ndev)) != BCME_OK) {
#ifdef BCMDONGLEHOST
		DHD_STATLOG_CTRL(dhdp, ST(DISASSOC_INT_START),
			dhd_net2idx(dhdp->info, ndev), WLAN_REASON_DEAUTH_LEAVING);
#endif /* BCMDONGLEHOST */
...
#endif /* LINUX_VERSION_CODE >= KERNEL_VERSION(6, 0, 0) || defined(CFG80211_BKPORT_MLO) */
...
#if defined(WL_FILS_ROAM_OFFLD)
	if (   cfg->fils_supported
	    && (   (NL80211_AUTHTYPE_FILS_SK == sec->auth_type)
	        || (NL80211_AUTHTYPE_FILS_SK_PFS == sec->auth_type)
	       )
	   ) {
...
exit:
	if (iov_buf_in) {
		bzero(iov_buf_in, WLC_IOCTL_SMLEN);
...
wl_stop_wait_next_action_frame(struct bcm_cfg80211 *cfg, struct net_device *ndev, u8 bsscfgidx)
{
	...
	if (wl_get_drv_status_all(cfg, FINDING_COMMON_CHANNEL)) {
		if ((cfg->p2p) && timer_pending(&cfg->p2p->listen_timer)) {
	...
	const wl_event_msg_t *e, void *data)
{
	...
	u8 *mgmt_frame = NULL;
	u8 bsscfgidx;
	u8 *rxframe_data = NULL;
	uint16 rx_frame_version;
	...
	if (event != WLC_E_PROBREQ_MSG) {
		if (!data) {
			WL_ERR(("rxframe: NULL\n"));
			return -EINVAL;
		}

		rx_frame_version = ntoh16(((wl_event_rx_frame_data_t *)data)->version);
		if (rx_frame_version == BCM_RX_FRAME_DATA_VERSION_1) {
			...
			rxframe_data = (u8*)(rxframe + 1);
			chspec = ntoh16(rxframe->channel);
		} else if (rx_frame_version == BCM_RX_FRAME_DATA_VERSION_2) {
			...
			WL_ERR(("wrong Rxframe Version %d \n", rx_frame_version));
			return -EINVAL;
		}
	...
	if (event == WLC_E_ACTION_FRAME_RX) {
		...
		if (rxframe_data) {
			err = wl_frame_get_mgmt(cfg, FC_ACTION, &da, &e->addr, &bssid,
					&mgmt_frame, &mgmt_frame_len, rxframe_data);
		}

		if (err < 0 || mgmt_frame == NULL) {
	...
	const wl_event_msg_t *event, void *data)
{
	...
	if (strncmp(cfg->country, country_str_lookup, WL_CCODE_LEN) == 0) {
		...
		strlcpy(cfg->country, country_str_lookup, WLC_CNTRY_BUF_SZ);
	}
	...
	wl_cfg80211_regd_update(cfg, country_str_lookup);
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 17, 0))
static int wl_cfg80211_dfs_master_event(struct bcm_cfg80211 *cfg,
		event_param_t  *event_param, int need_enqueue)
{
	struct wireless_dev       *wdev  = NULL;
	struct wiphy              *wiphy = NULL;
	chanspec_t                 chanspec = 0x0;
	struct cfg80211_chan_def   chandef;
	uint                       event_subtype = 0x0;
	uint                       status = 0;
	...
	} else if (!(wdev = event_param->ndev->ieee80211_ptr)) {
		if (!(wdev = cfg->wdev)) {
			WL_ERR(("*invalid wdev!\n"));
			return BCME_BADARG;
		}
	...
	if (!(wiphy = wdev->wiphy)) {
		...
		event_subtype = event_param->event_subtype;
		chanspec      = event_param->chanspec;
		status        = event_param->status;

		/* when generate CAC realted event like 'NL80211_RADAR_CAC_STARTED',
		   kernel will check the previous state and valid wdev->chandef.chan,
		   but since the FW event send up too quickly make the calling
		   kernel CFG80211->wlc_cfg80211_start_ap()->FW_start_AP hasn't
		   finished to accept/change the wdev->chandef.chan in the kernel
		   CFG80211 nl80211_start_ap(), so here directly send up the event
		   by 'cfg80211_cac_event()' will encounter the checking failure.
		   So here try to use extra timer to postpone the event till the
		   wdev->chandef.chan is ready */
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 19, 0) || defined(CFG80211_BKPORT_MLO)
		if (!wdev_chandef(wdev, 0)->chan)
#else
		if (!wdev->chandef.chan)
#endif /* KERNEL_VERSION(5, 19, 0) || defined(CFG80211_BKPORT_MLO) */
		{
			if (need_enqueue) {
				int  cur_idle  = cfg->dfs_master_event_tail;
				int  next_idle = 0;
				next_idle = (cur_idle+1) % CONST_DFS_MASTER_EVENT_BUFFER_QTY;
				if (next_idle == cfg->dfs_master_event_head) {
					WL_ERR(("*error, drop sync too many(%d) blocking,"
					        " head=%d, tail=%d!\n", next_idle,
					        cfg->dfs_master_event_head,
					        cfg->dfs_master_event_tail));
					return BCME_BADARG;
				} else {
					cfg->dfs_master_event[cur_idle] = *event_param;
					/* use as timeout counter */
					cfg->dfs_master_event[cur_idle].flags = 0;
					cfg->dfs_master_event[cur_idle].data = NULL;
					cfg->dfs_master_event_tail = next_idle;

					if (!timer_pending(&cfg->dfs_master_timer)) {
						START_TIMER(&cfg->dfs_master_timer,
							CONST_DFS_MASTER_EVENT_POSTPONE_TIME_MS, 0);
					}
				}
			}
			return BCME_NOTREADY;
		...
		if (wf_chspec_malformed(chanspec)) {
			if (0 > wldev_iovar_getint(event_param->ndev, "chanspec",
			                           (s32 *)&chanspec)) {
				WL_ERR(("*can't get valid chanspec!\n"));
				return BCME_BADCHAN;
			}
		}

		if (0 > wl_chspec_to_chandef(chanspec, &chandef, wiphy)) {
			WL_ERR(("*fail to convert chandef, CH=0x%X(%d)!\n",
			        chanspec, 0xff&chanspec));
			return BCME_BADCHAN;
		} else if (!cfg80211_chandef_valid(&chandef)) {
			WL_ERR(("*error, cfg80211 chandef is invalid!\n"));
			return BCME_BADCHAN;
		}
	}

	if (WLC_E_EXTEVENT_AP_DFS_RADAR == event_subtype) {
		// status => uint8 radar_type
		// data   => wl_event_radar_detect_data_t  radar_data
		WL_ERR(("* RADAR detected, radar_type=%d, CH=0x%X(%d)!\n",
		        status, chanspec, wf_chspec_ctlchan(chanspec)));
		cfg80211_radar_event(wiphy, &chandef,
		                     CAN_SLEEP()?GFP_KERNEL:GFP_ATOMIC);
	} else if (WLC_E_EXTEVENT_AP_DFS_STATE_CHANGE == event_subtype) {
		uint old_state = 0x0, new_state = 0x0;

		/* status => DFS state */
		old_state = 0xffff & (status >> 16);
		new_state = 0xffff & (status >>  0);

		switch (new_state) {
			case WL_DFS_CACSTATE_IDLE:
				if ((WL_DFS_CACSTATE_PREISM_CAC  == old_state) ||
				    (WL_DFS_CACSTATE_POSTISM_CAC == old_state)) {
					WL_ERR(("state:%d->%d, NL80211_RADAR_CAC_ABORTED\n",
					        old_state, new_state));
					/* WPA-Supplicant SM has issue that send
					 * NL80211_RADAR_CAC_ABORTED will make its state switch
					 * back to 'DFS', and later CSA can't change the state.
					 * So fake a NL80211_RADAR_CAC_FINISHED instead.
					 */
					cfg80211_cac_event(event_param->ndev, &chandef,
					                   NL80211_RADAR_CAC_FINISHED,
					                   CAN_SLEEP()?GFP_KERNEL:GFP_ATOMIC);
				}
				break;
			case WL_DFS_CACSTATE_PREISM_CAC:
			case WL_DFS_CACSTATE_POSTISM_CAC:
				WL_ERR(("state:%d->%d, NL80211_RADAR_CAC_STARTED\n",
				        old_state, new_state));
				cfg80211_cac_event(event_param->ndev, &chandef,
			...
			case WL_DFS_CACSTATE_ISM:
				/* generate extra event to confirm logical correct since
				 * call to cfg80211_cac_event() will check 'cac_started'
				 */
				if ((WL_DFS_CACSTATE_PREISM_CAC  != old_state) &&
				    (WL_DFS_CACSTATE_POSTISM_CAC != old_state)) {
					WL_ERR(("state:%d->%d, pesudo NL80211_RADAR_CAC_STARTED\n",
					        old_state, new_state));
					cfg80211_cac_event(event_param->ndev, &chandef,
					                   NL80211_RADAR_CAC_STARTED,
					                   CAN_SLEEP()?GFP_KERNEL:GFP_ATOMIC);
				...
				// all work done here
				WL_ERR(("state:%d->%d, NL80211_RADAR_CAC_FINISHED\n",
				        old_state, new_state));
				cfg80211_cac_event(event_param->ndev, &chandef,
				                   NL80211_RADAR_CAC_FINISHED,
				                   CAN_SLEEP()?GFP_KERNEL:GFP_ATOMIC);
				break;
			case WL_DFS_CACSTATE_CSA:
				/* There is another real WLC_E_CSA_COMPLETE_IND event later
				 * call to wl_csa_complete_ind() for trigger wpa-supplicant
				 * switch channel with keep state at 'ENABLED'
				 */
				break;
			case WL_DFS_CACSTATE_PREISM_OOC:
			case WL_DFS_CACSTATE_POSTISM_OOC:
				if ((WL_DFS_CACSTATE_PREISM_CAC  == old_state) ||
				    (WL_DFS_CACSTATE_POSTISM_CAC == old_state)) {
					WL_ERR(("state:%d->%d, NL80211_RADAR_CAC_ABORTED\n",
					        old_state, new_state));
...
#define wl_cfg80211_dfs_master_event(cfg, event, enqueue)    (0)
#endif /* (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 17, 0)) */

static void wl_dfs_master_timeout(void *data)
{
	struct bcm_cfg80211  *cfg = (struct bcm_cfg80211 *)data;
	event_param_t        *event_ptr = cfg->dfs_master_event;

	UNUSED_PARAMETER(event_ptr);
	DHD_EVENT_WAKE_LOCK(cfg->pub);
	while (cfg->dfs_master_event_head != cfg->dfs_master_event_tail) {
		event_param_t  *event_param = &event_ptr[cfg->dfs_master_event_head];
		int             ret = 0;

		/* check if event life-time arrived before process */
		event_param->flags++;
		if (CONST_DFS_MASTER_EVENT_TIMEOUT_QTY <= event_param->flags) {
			ret = BCME_NORESOURCE;  /* drop since timeout */
		} else {
			ret = wl_cfg80211_dfs_master_event(cfg, event_param, FALSE);
		}
		/* check event invalid or consumed */
		if (BCME_NOTREADY == ret) {
			break;
		} else {
			cfg->dfs_master_event_head =   (cfg->dfs_master_event_head + 1)
			                             % CONST_DFS_MASTER_EVENT_BUFFER_QTY;
		}
	}
	DHD_EVENT_WAKE_UNLOCK(cfg->pub);

	// schedule next time
	if (cfg->dfs_master_event_head != cfg->dfs_master_event_tail) {
		START_TIMER(&cfg->dfs_master_timer,
			CONST_DFS_MASTER_EVENT_POSTPONE_TIME_MS, 0);
	}
}

int wl_cfg80211_dfs_master_reset(struct bcm_cfg80211 *cfg, int init)
{
	int  ret = 0;
	...
	if (!cfg) {
		...
		cfg->dfs_master_event_head = cfg->dfs_master_event_tail = 0;
		if (init) {
			init_timer_compat(&cfg->dfs_master_timer, wl_dfs_master_timeout, cfg);
		}
		del_timer(&cfg->dfs_master_timer);
	}
...
static void
wl_cfg80211_extra_event_sta_role(struct bcm_cfg80211 *cfg,
	struct net_device *ndev, const wl_event_msg_t *e, void* data)
{
	...
	uint32 event_subtype = ntoh32(e->reason);
	uint32 auth_type = ntoh32(e->auth_type);
	uint32 ifidx = ntoh32(e->ifidx);

	UNUSED_PARAMETER(cfg);
	UNUSED_PARAMETER(ndev);
	UNUSED_PARAMETER(event_type);
	UNUSED_PARAMETER(flags);
	UNUSED_PARAMETER(datalen);
	UNUSED_PARAMETER(status);
	UNUSED_PARAMETER(reason);
	UNUSED_PARAMETER(auth_type);
	UNUSED_PARAMETER(ifidx);
	UNUSED_PARAMETER(data);

	WL_INFORM(("%s: event_type=%d(%s),flags=%d,datalen=%d,status=0x%X(%d),"
	           " reason=%d, auth_type=%d(0x%X), "
	           "scb=%02X:%02X:%02X:%02X:%02X:%02X, ifidx=%d, ifname='%s'\n",
	           __FUNCTION__, event_type, bcmevent_get_name(event_type),
	           flags, datalen, status, status,
	           reason, auth_type, auth_type,
	           e->addr.octet[0], e->addr.octet[1], e->addr.octet[2],
	           e->addr.octet[3], e->addr.octet[4], e->addr.octet[5],
	           e->ifidx, e->ifname));

	switch (event_type) {
		case WLC_E_EXTRA_EVENT:
...
static void
wl_cfg80211_extra_event_ap_role(struct bcm_cfg80211 *cfg,
	struct net_device *ndev, const wl_event_msg_t *e, void* data)
{
	uint32 event_type = ntoh32(e->event_type);
	uint16 flags =  ntoh16(e->flags);
	...
	event_param.ndev = ndev;
	event_param.ifidx = ifidx;
	event_param.event_subtype = reason;
	event_param.status = status;
	event_param.chanspec = auth_type;
	event_param.flags = flags;
	event_param.data = data;
	UNUSED_PARAMETER(cfg);
	UNUSED_PARAMETER(datalen);
	UNUSED_PARAMETER(ret);
	UNUSED_PARAMETER(event_param);

	WL_INFORM(("%s: event_type=%d(%s),flags=%d,datalen=%d,status=0x%X(%d),"
	           " reason/subtype=0x%X(%d), auth_type=0x%X(%d), "
	           "scb=%02X:%02X:%02X:%02X:%02X:%02X, ifidx=%d, ifname='%s'\n",
	           __FUNCTION__, event_type, bcmevent_get_name(event_type),
	           flags, datalen, status, status,
	           event_subtype, event_subtype, auth_type, auth_type,
	           e->addr.octet[0], e->addr.octet[1], e->addr.octet[2],
	           e->addr.octet[3], e->addr.octet[4], e->addr.octet[5],
	           e->ifidx, e->ifname));
	...
	switch (event_type) {
		case WLC_E_EXTRA_EVENT:
			switch (event_subtype) {
				case WLC_E_EXTEVENT_AP_EXTSTA_AUTHORIZED:
					// e->addr => STA MAC
					if (DOT11_SC_SUCCESS==status) {
						WL_ERR(("* STA=%02X:%02X:%02X:%02X:%02X:%02X"
						        " is *ready* for data, !\n",
						        e->addr.octet[0], e->addr.octet[1],
						        e->addr.octet[2], e->addr.octet[3],
						        e->addr.octet[4], e->addr.octet[5]));
						//dhd_syna_external_sta_connect(TRUE, e->ifidx, &(e->addr));
					}
					break;
				case WLC_E_EXTEVENT_AP_EXTSTA_DEAUTHORIZED:
					// e->addr => STA MAC
					if (DOT11_SC_SUCCESS==status) {
						WL_ERR(("* STA=%02X:%02X:%02X:%02X:%02X:%02X "
						        "is *left* for data!\n",
						        ...
						        e->addr.octet[2], e->addr.octet[3],
						        e->addr.octet[4], e->addr.octet[5]));
						//dhd_syna_external_sta_connect(FALSE, e->ifidx, &(e->addr));
					}
					break;
				case WLC_E_EXTEVENT_AP_RXFRAG_POOL_EMPTY:
					WL_ERR(("* FW reports RXFRAG POOL empty!\n"));
					break;

				case WLC_E_EXTEVENT_AP_DFS_RADAR:
					// status    => uint8 radar_type
					// auth_type => chanspec_t  chanspec
					// data      => radar_detected_event_info_t  radar_info
					WL_ERR(("* RADAR detected, radar_type=%d!\n", status));
#ifdef DHD_DFS_MASTER
					ret = wl_cfg80211_dfs_master_event(cfg, &event_param, TRUE);
					if ((BCME_OK != ret) && (BCME_NOTREADY != ret)) {
						WL_ERR(("dfs master event process failed\n"));
					}
#endif /* DHD_DFS_MASTER */
					break;
				case WLC_E_EXTEVENT_AP_DFS_AP_START:
					// status    => chanspec_t  next_chanspec
					// auth_type => chanspec_t  current_chanspec
					WL_ERR(("* DFS AP resume to work on chanspec=0x%X!\n", auth_type));
					break;
				case WLC_E_EXTEVENT_AP_DFS_AP_STOP:
					// status => uint silence_time_ms
					// auth_type => chanspec_t  current_chanspec
					WL_ERR(("* DFS AP stop and chanspec=0x%X(%d) "
					        "will keep silence for %d ms!\n",
					        event_param.chanspec, 0xff&event_param.chanspec,
					        auth_type));
					break;
				case WLC_E_EXTEVENT_AP_DFS_CHANSPEC_OPEN:
					// status => chanspec_t  next_chanspec
					// auth_type => chanspec_t  current_chanspec
					WL_ERR(("* DFS chanspec open, chanspec=0x%X!\n", auth_type));
					break;
				case WLC_E_EXTEVENT_AP_DFS_CHANSPEC_CLOSE:
					// status => int32  block_time
					// auth_type => chanspec_t  chanspec
					WL_ERR(("* DFS chanspec close, chanspec=0x%X!\n", auth_type));
					break;
				case WLC_E_EXTEVENT_AP_DFS_STATE_CHANGE:
					// status => DFS state
					// auth_type => chanspec_t  chanspec
					WL_ERR(("* DFS state changed, state=0x%X, CH=0x%X(%d)!\n",
					        status,
					        event_param.chanspec, 0xff&event_param.chanspec));
#ifdef DHD_DFS_MASTER
					ret = wl_cfg80211_dfs_master_event(cfg, &event_param, TRUE);
		...
		case WLC_E_LINK:
			if (!flags) {
				/* BSS LINK DOWN */
#ifdef DHD_DFS_MASTER
				wl_cfg80211_dfs_master_reset(cfg, FALSE);
#endif /* DHD_DFS_MASTER */
			}
			break;
	}
...
static int
wl_cfg80211_extra_event(struct bcm_cfg80211 *cfg, bcm_struct_cfgdev *cfgdev,
	const wl_event_msg_t *e, void *data)
{
	struct net_device *ndev = cfgdev_to_wlc_ndev(cfgdev, cfg);
	uint32 event_type = ntoh32(e->event_type);
	...
	WL_TRACE(("%s: event_type=%d(%s), sub_type=%d\n",
	          __FUNCTION__,
	          event_type, bcmevent_get_name(event_type), reason));

	switch (event_type) {
		case WLC_E_LINK:
			/* for capture LINK_DOWN to reset event queue */
			wl_cfg80211_extra_event_ap_role(cfg, ndev, e, data);
			break;
		case WLC_E_EXTRA_EVENT:
			if ((WLC_E_EXTEVENT_AP_ID_START < reason) &&
				(WLC_E_EXTEVENT_AP_ID_END   > reason)) {
				wl_cfg80211_extra_event_ap_role(cfg, ndev, e, data);
			} else if ((WLC_E_EXTEVENT_STA_ID_START < reason) &&
			           (WLC_E_EXTEVENT_STA_ID_END   > reason)) {
				wl_cfg80211_extra_event_sta_role(cfg, ndev, e, data);
			} else {
				// unsupported
				WL_ERR(("%s: unsupported sub-event: %d\n",
				           __FUNCTION__, reason));
				return -1;
			}
			break;
...
static void wl_init_event_handler(struct bcm_cfg80211 *cfg)
{
	...
	cfg->evt_handler[WLC_E_ASSOC_REQ_IE] = wl_notify_connect_status;
...
#endif /* WL_CLIENT_SAE */
#ifdef WL_PHY_CAL_EVENT
	cfg->evt_handler[WLC_E_PHY_CAL] = wl_phy_cal_event;
#endif
#ifdef DHD_EXTRA_EVENT
	cfg->evt_handler[WLC_E_EXTRA_EVENT] = wl_cfg80211_extra_event;
#endif /* DHD_EXTRA_EVENT */
}
...
static void wl_deinit_priv_mem(struct bcm_cfg80211 *cfg)
{
	...
	bzero(cfg->spmk_info_list, sizeof(*cfg->spmk_info_list));
...
static void wl_del_roam_timeout(struct bcm_cfg80211 *cfg)
{
	...
	dhdp->during_roaming = 0;
}

static void wl_roam_timeout(void *data)
{
	struct bcm_cfg80211 *cfg = (struct bcm_cfg80211 *)data;
	dhd_pub_t *dhdp = (dhd_pub_t *)(cfg->pub);
	...
	enum wl_status state, bool set)
{
	...
	if (set) {
		...
		if (dhd->frameburst == FRAME_BURST_ENABLED && DHD_OPMODE_STA_SOFTAP_CONCURR(dhd) &&
...
#ifdef DISABLE_WL_FRAMEBURST_2G20_SOFTAP_P2P
		if (dhd->frameburst == FRAME_BURST_ENABLED &&
			!DHD_OPMODE_SUPPORTED(cfg->pub, DHD_FLAG_HOSTAP_MODE) &&
			wl_cfg80211_is_concurrent_mode(primary_dev)) {
			/* Enable Frameburst for STA/P2P concurrent mode */
			wl_cfg80211_set_frameburst(cfg, TRUE);
		}
#endif /* DISABLE_WL_FRAMEBURST_2G20_SOFTAP_P2P */
#ifdef DISABLE_WL_FRAMEBURST
		wl_cfg80211_set_frameburst(cfg, FALSE);
#endif /* DISABLE_WL_FRAMEBURST */
#endif /* BCMDONGLEHOST */
	} else { /* clear */
		chspec = INVCHANSPEC;
		...
		if (!dhd->disable_2g20_fb) {
			if (!DHD_OPMODE_SUPPORTED(cfg->pub, DHD_FLAG_HOSTAP_MODE)) {
				/* Restore Frameburst for standalone P2P mode */
				wl_cfg80211_set_frameburst(cfg, dhd->frameburst);
			}
		}
#endif /* DISABLE_FRAMEBURST_VSDB */
#ifdef DISABLE_WL_FRAMEBURST_2G20_SOFTAP_P2P
		if (dhd->disable_2g20_fb && DHD_OPMODE_STA_SOFTAP_CONCURR(dhd) &&
			CHSPEC_IS2G(cfg->ap_oper_channel) && CHSPEC_IS20(cfg->ap_oper_channel)) {
...
#ifdef DISABLE_WL_FRAMEBURST_2G20_SOFTAP_P2P
		if (dhd->disable_2g20_fb && DHD_OPMODE_SUPPORTED(cfg->pub, DHD_FLAG_P2P_MODE) &&
			CHSPEC_IS2G(cfg->ap_oper_channel) && CHSPEC_IS20(cfg->ap_oper_channel)) {
			/* Disable Frameburst for stand-alone 2G20 P2P */
			wl_cfg80211_set_frameburst(cfg, FALSE);
		}
#endif /* DISABLE_WL_FRAMEBURST_2G20_SOFTAP_P2P */
#endif /* BCMDONGLEHOST */
...
#if !defined(DISABLE_AUTO_CONTROL_PM)
	set_bit(WL_STATUS_CONNECTED, &cfg->interrested_state);
#endif // DISABLE_AUTO_CONTROL_PM
...
#ifdef DHD_DFS_MASTER
	wl_cfg80211_dfs_master_reset(cfg, TRUE);
#endif /* DHD_DFS_MASTER */
	return err;
}
		...
		if (cfg->wdev) {
			...
			if (ret > 0) {
				...
				if (cfg->p2p_net) {
					...
					DEV_ADDR_SET(cfg->p2p_net, ndev->dev_addr);
...
s32 wl_cfg80211_attach(struct net_device *ndev, void *context)
{
	...
	wdev = (struct wireless_dev *)MALLOCZ(dhd->osh, sizeof(struct wireless_dev));
	if (unlikely(!wdev)) {
		WL_ERR(("Could not allocate wireless device\n"));
		return -ENOMEM;
	}
	/* early record since FILS checking need handler */
	wdev->netdev = ndev;
...
wl_cfg80211_event(struct net_device *ndev, const wl_event_msg_t * e, void *data)
{
	...
	if (cfg->event_workq == NULL) {
		WL_ERR(("skip since event handler is not created\n"));
		return;
	}
#endif /* OEM_ANDROID */
...
static int wl_eq_empty(struct bcm_cfg80211 *cfg)
{
	/* DO NOT use 'wl_lock_eq/wl_unlock_eq' here to avoid dead lock */
	return list_empty_careful(&cfg->eq_list);
}
	...
	s32 bw_cap_5g, s32 bw_cap_6g)
{
	...
	void *list = NULL;
	int   list_len = 1024 * 4;
	chanvec_t  customer_list = { {0} };
	u32 i, j, index, channel, array_size = 0;
	chanspec_t chspec = 0;
	s32 err = BCME_OK;
	bool ht40_allowed = 0, ht80_allowed = 0, ht160_allowed = 0;
	...
	err = wldev_iovar_getbuf_bsscfg(dev, "chan_info_list", NULL,
		0, list, list_len, 0, &cfg->ioctl_buf_sync);
	if (err == BCME_UNSUPPORTED) {
		WL_INFORM(("get chan_info_list, UNSUPPORTED(ignore)\n"));
	...
	err = wldev_iovar_getbuf_bsscfg(dev, "customer_chanlist", NULL,
		0, &customer_list, sizeof(customer_list), 0, &cfg->ioctl_buf_sync);
	if (err != BCME_OK) {
		WL_ERR(("get customer channel list err(%d), ignore\n", err));
		memset(&customer_list, 0xff, sizeof(customer_list));
	} else {
		if (SYNA_CUSTOMER_CHANLIST_INVALID(customer_list.vec)) {
			WL_ERR(("customer channel ist disabled\n"));
			memset(&customer_list, 0xff, sizeof(customer_list));
		}
	}
	/* show bytes only if needs */
	/* prhex("customer_list", (uchar *)&customer_list, sizeof(customer_list)); */

	WL_BAND_INIT(__wl_band_2ghz, __wl_2ghz_channels);
	WL_BAND_INIT(__wl_band_5ghz_a, __wl_5ghz_a_channels);
#ifdef CFG80211_6G_SUPPORT
	WL_BAND_INIT(__wl_band_6ghz, __wl_6ghz_channels);
#endif /* CFG80211_6G_SUPPORT */
	...
	for (i = 0; i < dtoh32(list_count); i++) {
		...
		if (index <  array_size) {
...
#else /* LINUX_VERSION_CODE == KERNEL_VERSION(2, 6, 38) && !defined(WL_COMPAT_WIRELESS) */
			band_chan_arr[index].center_freq =
			...
			if (CHSPEC_IS20(chspec)) {
				...
				if (!ht40_allowed) {
					if (WL_CHANSPEC_CTL_SB_L == CHSPEC_CTL_SB(chspec)) {
						...
						WL_DBG(("    SB_L, add the NO_HT40-, CH[%d]=%d, flags=0x%X\n",
						        index, channel, flags));
					} else {
						...
						WL_DBG(("    SB_U, add the NO_HT40+, CH[%d]=%d, flags=0x%X\n",
						        index, channel, flags));
					}
				} else {
					if (WL_CHANSPEC_CTL_SB_L == CHSPEC_CTL_SB(chspec)) {
						...
						mask  = ~(IEEE80211_CHAN_DISABLED |
						          ...
						          IEEE80211_CHAN_NO_HT40PLUS);
						value = 0x0;
						WL_DBG(("    SB_L, clean the NO_HT40-, CH[%d]=%d, flags=0x%X\n",
						        index, channel, flags));
					} else {
						min_ch = channel - CH_20MHZ_APART - 1;
						max_ch = channel - 1;
						mask  = ~(IEEE80211_CHAN_DISABLED |
						          ...
						          IEEE80211_CHAN_NO_HT40PLUS);
						value = 0x0;
						WL_DBG(("    SB_U, clean the NO_HT40+, CH[%d]=%d, flags=0x%X\n",
						        index, channel, flags));
					}
				}
			} else {
				...
				if (CHSPEC_IS6G(chspec)) {
					min_ch += CH_BASE_6G_CHANNEL;
					max_ch += CH_BASE_6G_CHANNEL;
				}
			...
			if (0 != min_ch) {
				FOREACH_20_SB(chspec, j) {
					if (!isset(customer_list.vec, j)) {
						WL_DBG(("    skip, sub-ch=%d is invalid for min=%d, "
						        "max=%d, chspec=0x%X\n",
						        j, min_ch, max_ch, chspec));
						min_ch = 0;
						break;
					}
				}
			...
			if (min_ch && max_ch) {
				 ...
				 *
				 * CH[ 8]=50, mask=0x0, value=0x0, flags=0x9B1 DISABLED NO_HT40PLUS NO_HT40MINUS NO_
				 * CH[ 9]=52, mask=0x0, value=0x0, flags=0x10A PASSIVE RADAR NO_160MHz
				 * CH[10]=54, mask=0x0, value=0x0, flags=0x900 NO_160MHz NO_20MHz
				 * CH[11]=56, mask=0x0, value=0x0, flags=0x10A PASSIVE RADAR NO_160MHz
				 */
				if (channel > CH_BASE_6G_CHANNEL) {
					iov_value = CH6G20MHZ_CHSPEC(channel - CH_BASE_6G_CHANNEL);
				} else {
					iov_value = CH20MHZ_CHSPEC(channel);
				}
				ret = wldev_iovar_getint(dev, "per_chan_info", &iov_value);
				iov_value_next = 0;
				if ((channel + CH_10MHZ_APART) <= max_ch) {
					if (channel > CH_BASE_6G_CHANNEL) {
						iov_value_next = CH6G20MHZ_CHSPEC(channel -
						CH_BASE_6G_CHANNEL + CH_10MHZ_APART);
					} else {
						iov_value_next = CH20MHZ_CHSPEC(channel + CH_10MHZ_APART);
					}
					ret *= wldev_iovar_getint(dev, "per_chan_info",
						&iov_value_next);
				}
				if (ret != BCME_OK) {
					WL_ERR(("ret=%d, Force to mark RADAR since get "
						"per_chan_info error for CH=%d!\n", ret, channel));
			...
			if (min_ch) {
				int center_channel = CHSPEC_CHANNEL(chspec);
				if (CHSPEC_IS6G(chspec)) {
					center_channel += CH_BASE_6G_CHANNEL;
				}
				...
				for (j = 0; j < array_size; j++) {
					if ((min_ch <= band_chan_arr[j].hw_value) &&
						...
						if (CHSPEC_BAND(chspec) !=
							wf_mhz2chanspec_band(band_chan_arr[j].center_freq)) {
							continue;
						}

						/* Passive flags is only in-herited up */
						if (band_chan_arr[j].hw_value != center_channel)  {
							flags &= ~CONST_SYNA_REG_FLAG_FILTER_MASK;
						}

						band_chan_arr[j].flags &= mask;
						band_chan_arr[j].flags |= flags;
...
static s32 __wl_update_wiphybands(struct bcm_cfg80211 *cfg, bool notify)
{
	...
	if (nmode ||
		...
		value = WLC_BAND_2G;
		err = wldev_iovar_getint(dev, "bw_cap", (s32 *)&value);
		if (unlikely(err)) {
	...
	for (i = 1; i <= nband && i < ARRAYSIZE(bandlist); i++) {
		...
		if (bandlist[i] == WLC_BAND_2G) {
			if (__wl_band_2ghz.n_channels > 0) {
				index = IEEE80211_BAND_2GHZ;
				bands[index] = &__wl_band_2ghz;
				bw_cap = bw_cap_2g;
			} else {
				WL_ERR(("2GHz channels not listed\n"));
				continue;
			}
		...
		if (nmode) {
			...
			if (WL_BW_CAP_40MHZ(bw_cap)) {
				...
				bands[index]->ht_cap.mcs.rx_highest = cpu_to_le16(150 * rxchain); // Mbps
			} else {
				bands[index]->ht_cap.mcs.rx_highest = cpu_to_le16(72 * rxchain); // Mbps
			}
		...
		if (vhtmode) {
			...
			if (txbf_bfe_cap || txbf_bfr_cap) {
				...
				bands[index]->vht_cap.cap |= IEEE80211_VHT_CAP_VHT_LINK_ADAPTATION_VHT_MRQ_MFB;
			}
			...
			if (WL_BW_CAP_160MHZ(bw_cap)) {
				bands[index]->vht_cap.vht_mcs.tx_highest = cpu_to_le16(866 * txstreams); // Mbps
				bands[index]->vht_cap.vht_mcs.rx_highest = cpu_to_le16(867 * txstreams); // Mbps
			} else if (WL_BW_CAP_80MHZ(bw_cap)) {
				bands[index]->vht_cap.vht_mcs.tx_highest = cpu_to_le16(433 * txstreams); // Mbps
				bands[index]->vht_cap.vht_mcs.rx_highest = cpu_to_le16(433 * txstreams); // Mbps
			} else if (WL_BW_CAP_40MHZ(bw_cap)) {
				bands[index]->vht_cap.vht_mcs.tx_highest = cpu_to_le16(200 * txstreams); // Mbps
				bands[index]->vht_cap.vht_mcs.rx_highest = cpu_to_le16(200 * txstreams); // Mbps
			} else {
				bands[index]->vht_cap.vht_mcs.tx_highest = cpu_to_le16(86 * txstreams); // Mbps
				bands[index]->vht_cap.vht_mcs.rx_highest = cpu_to_le16(86 * txstreams); // Mbps
			}
		...
		if (bands[index]->iftype_data) {
			...
			pItf_data->types_mask = BIT(NL80211_IFTYPE_STATION) | BIT(NL80211_IFTYPE_AP);
			pHE_cap->has_he = TRUE;
			...
			if (WL_BW_CAP_40MHZ(bw_cap)) {
				if (IEEE80211_BAND_2GHZ == index) {
					pHE_elem->phy_cap_info[0] |= IEEE80211_HE_PHY_CAP0_CHANNEL_WIDTH_SET_40MHZ_IN_2G;
				} else if (IEEE80211_BAND_5GHZ == index) {
					if (WL_BW_CAP_80MHZ(bw_cap)) {
						pHE_elem->phy_cap_info[0] |= IEEE80211_HE_PHY_CAP0_CHANNEL_WIDTH_SET_40MHZ_80MHZ_IN_5G;
					}
					if (WL_BW_CAP_160MHZ(bw_cap)) {
						pHE_elem->phy_cap_info[0] |= IEEE80211_HE_PHY_CAP0_CHANNEL_WIDTH_SET_160MHZ_IN_5G;
					}
				}
			}
			...
			if (WL_BW_CAP_80MHZ(bw_cap)) {
				pHE_elem->phy_cap_info[4] |= IEEE80211_HE_PHY_CAP4_BEAMFORMEE_MAX_STS_UNDER_80MHZ_8 |
				                             IEEE80211_HE_PHY_CAP4_BEAMFORMEE_MAX_STS_ABOVE_80MHZ_8;
				pHE_elem->phy_cap_info[5] |= IEEE80211_HE_PHY_CAP5_BEAMFORMEE_NUM_SND_DIM_UNDER_80MHZ_2 |
			...
			pHE_elem->phy_cap_info[8] |= IEEE80211_HE_PHY_CAP8_HE_ER_SU_PPDU_4XLTF_AND_08_US_GI;
			if (WL_BW_CAP_40MHZ(bw_cap)) {
				if (IEEE80211_BAND_2GHZ == index) {
					pHE_elem->phy_cap_info[8] |= IEEE80211_HE_PHY_CAP8_20MHZ_IN_40MHZ_HE_PPDU_IN_2G;
				}
			}
			if (WL_BW_CAP_160MHZ(bw_cap)) {
				pHE_elem->phy_cap_info[8] |= IEEE80211_HE_PHY_CAP8_20MHZ_IN_160MHZ_HE_PPDU |
				                             IEEE80211_HE_PHY_CAP8_80MHZ_IN_160MHZ_HE_PPDU;
			}
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 20, 0))
			pHE_elem->phy_cap_info[9] |= IEEE80211_HE_PHY_CAP9_TX_1024_QAM_LESS_THAN_242_TONE_RU |
			...
			WL_DBG(("%s: band[%d] he_cap=%d "
			        "he_80_tx_mcs=0x%04x he_80_rx_mcs_map=0x%04x\n",
			        __FUNCTION__, index, hemode,
...
static s32 __wl_cfg80211_up(struct bcm_cfg80211 *cfg)
{
	...
	*/
	...
	if (!dhd->fw_preinit) {
		...
		if (unlikely((0 > err) && (err != -EINPROGRESS))) {
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 0, 0)) && defined(WL_SELF_MANAGED_REGDOM)
	if (!global_regd_self) {
		global_regd_self = KVZALLOC(CONST_SYNA_REGD_SELF_SIZE, GFP_ATOMIC);
		if (global_regd_self) {
...
static s32 __wl_cfg80211_down(struct bcm_cfg80211 *cfg)
{
	...
	extern acs_delay_work_t delay_work_acs;
...
#ifdef PROP_TXSTATUS_VSDB
	cancel_delayed_work_sync(&cfg->wlfc_work);
#endif /* PROP_TXSTATUS_VSDB */
...
#if defined(BCMDONGLEHOST) && defined(__linux__)
	if (!dhd_download_fw_on_driverload)
#endif /* defined(BCMDONGLEHOST) && defined(__linux__) */
	{
		mutex_unlock(&cfg->usr_sync);
		wl_destroy_event_handler(cfg);
		mutex_lock(&cfg->usr_sync);
	...
	wl_link_down(cfg);
	if (cfg->p2p_supported) {
		// early mark the p2p invalid to avoid extra p2p action
		cfg->p2p_supported = 0;
		// avoid crash if timer not starting
		if ((cfg->p2p) && timer_pending(&cfg->p2p->listen_timer)) {
			del_timer_sync(&cfg->p2p->listen_timer);
		}
		wl_cfgp2p_down(cfg);
...
s32 wl_cfg80211_up(struct net_device *net)
{
	...
	if(dhd->during_roaming && cfg) {
		wl_roam_timeout((void *)cfg);
	}
	...
	enum wl_management_type type)
{
	...
	if (wl_get_drv_status(cfg, AP_CREATING, ndev)) {
		 ...
		 */
		WL_DBG(("%s: Skipping set IE since AP is not up \n", __func__));
		goto exit;
	} else  if (ndev == bcmcfg_to_prmry_ndev(cfg)) {
		...
		if (wl_get_drv_status(cfg, AP_CREATED, ndev)) {
			...
			WL_DBG(("%s: Apply IEs for Primary AP Interface \n", __func__));
			bssidx = 0;
		} else {
			if (!cfg->p2p) {
				/* If p2p not initialized, return failure */
				WL_ERR(("%s: P2P not initialized \n", __func__));
				goto exit;
			}
			...
			if (!cfg->p2p->on) {
				...
				if (unlikely(ret)) {
					WL_ERR(("%s: Enable discovery failed \n", __func__));
					goto exit;
				}
			}
			ndev = wl_to_p2p_bss_ndev(cfg, P2PAPI_BSSCFG_PRIMARY);
			if (!cfg->p2p_wdev) {
				WL_ERR(("%s: p2p_wdev not present\n", __func__));
				goto exit;
			}
			wdev = cfg->p2p_wdev;
			WL_DBG(("%s: Apply IEs for P2P Discovery Iface wdev:%p\n", __func__, wdev));
			bssidx = wl_to_p2p_bss_bssidx(cfg, P2PAPI_BSSCFG_DEVICE);
		}
	} else {
		/* Virtual AP/ P2P Group Interface */
		WL_DBG(("%s: Apply IEs for iface:%s\n", __func__, ndev->name));
	...
	struct parsed_vndr_ies *vndr_ies, const bcm_tlv_t **ppExt_cap_ies)
{
	...
	while (ie) {
		...
		} else if (DOT11_MNG_EXT_CAP_ID == ie->id) {
			if (ppExt_cap_ies) {
				*ppExt_cap_ies = ie;
			}
		}
	...
	char *vndr_oui, u32 vndr_oui_len)
{
	...
	if ((wl_cfg80211_parse_vndr_ies((u8 *)conn_info->resp_ie,
		conn_info->resp_ie_len, &vndr_ies, NULL)) == BCME_OK) {
	...
	s32 bssidx, s32 pktflag, const u8 *vndr_ie, u32 vndr_ie_len)
{
	...
	const bcm_tlv_t *pExt_cap_ie = NULL;
	s32 i;
	u8 *ptr;
	...
	if (vndr_ie_len > mgmt_ie_buf_len) {
		...
		if (vndr_ie && vndr_ie_len && curr_ie_buf) {
			...
			if ((ret = wl_cfg80211_parse_vndr_ies((const u8 *)vndr_ie,
				vndr_ie_len, &new_vndr_ies, &pExt_cap_ie)) < 0) {
		...
		/* processing ext cap IE */
		if (pExt_cap_ie) {
			ret  = wldev_iovar_setbuf_bsscfg(ndev, "ext_cap_ie",
				pExt_cap_ie, BCM_TLV_HDR_SIZE + pExt_cap_ie->len,
				cfg->ioctl_buf, WLC_IOCTL_MAXLEN,
				bssidx, &cfg->ioctl_buf_sync);
			if (BCME_OK != ret) {
				WL_ERR(("set ext_cap_ie fail, ret=%d,"
				        " but ignore if FW is not supported.\n",
				        ret));
				/* make it pass the check for scan scenario */
				ret = BCME_OK;
			}
		}
...
static void wl_cfg80211_wbtext_update_rcc(struct bcm_cfg80211 *cfg, struct net_device *dev)
{
	...
	if ((cap_ie = bcm_parse_tlvs(conn_info->resp_ie, conn_info->resp_ie_len,
		...
		// check local 'wl rrm' value
		uint32  rrm = 0;
		int ret = 0;

		ret = wldev_iovar_getint(dev, "rrm", &rrm);
		if (unlikely(0 > ret)) {
			WL_ERR(("skip since fail to get rrm for rcc update, ret=%d\n", ret));
		} else if (!isset(&rrm, DOT11_RRM_CAP_NEIGHBOR_REPORT)) {
			WL_ERR(("skip NBR since rrm=0x%X for rcc update\n", rrm));
		} else if (isset(cap_ie->data, DOT11_RRM_CAP_NEIGHBOR_REPORT)) {
	...
	if (!req_sent) {
		if ((cap_ie = bcm_parse_tlvs(conn_info->resp_ie, conn_info->resp_ie_len,
			...
			// check local 'wl wnm' value
			uint32  wnm = 0;
			int ret = 0;

			ret = wldev_iovar_getint(dev, "wnm", &wnm);
			if (unlikely(0 > ret)) {
				WL_ERR(("skip since fail to get wnm for "
				        "rcc update, ret=%d\n", ret));
			} else if (!(WL_WNM_BSSTRANS & wnm)) {
				WL_ERR(("skip BTM-Q since wnm=0x%X for rcc update\n", wnm));
			} else if (cap_ie->len >= DOT11_EXTCAP_LEN_BSSTRANS &&
...
wl_flush_fw_log_buffer(struct net_device *dev, uint32 logset_mask)
{
	...
	if (atomic_read(&exit_in_progress)) {
		WL_ERR(("module exit in progress\n"));
		return;
	}
...
/* Function to check if chanspec is 2G20 */
bool wl_cfg80211_check_is2G20(struct net_device *ndev)
{
	u32 chanspec = 0;
	bool ret = false;

	/* Get the chanspec and convert to host byte order */
	if (wldev_iovar_getint(ndev, "chanspec", (s32 *)&chanspec) == BCME_OK) {
		chanspec = wl_chspec_driver_to_host(chanspec);
	}
	/* Check if valid 2G20 chanspec and return status */
	if (chanspec != INVCHANSPEC && CHSPEC_IS2G(chanspec) && CHSPEC_IS20(chanspec)) {
		ret = true;
	} else {
		ret = false;
	}
...
static void wl_wps_reauth_timeout(void *data)
{
	...
	if ((!ndev) || (!cfg)) {
		WL_ERR(("[%s][WPS] Timeout function skip"
		        " invalid session!\n", __FUNCTION__));
		return;
	}
...
#endif
		cfg->wps_session[inst].state = WPS_STATE_DONE;
		WL_INFORM_MEM(("[%s][WPS] Treat the WPS session done for"
		               " client:" MACDBG "\n",
		               ndev->name, MAC2STRDBG(peer_mac)));
		wps_done = true;
	...
	const wl_event_msg_t *event, void *data)
{
	...
	if ((status == WLC_SUP_KEYED || status == WLC_SUP_KEYXCHANGE_WAIT_G1) &&
		...
		/* NL80211_CMD_PORT_AUTHORIZED supported above >= 6.2 */
		...
		const wl_event_msg_t *e, void *data)
{
	...
	struct net_device *ndev = bcmcfg_to_prmry_ndev(cfg);
	struct wiphy *wiphy = bcmcfg_to_wiphy(cfg);
	uint len = CU_ATTR_HDR_LEN + sizeof(u8);
...
wl_mkeep_alive_update(struct bcm_cfg80211 *cfg)
{
	...
	if ((pbuf = MALLOCZ(cfg->osh, KA_TEMP_BUF_SIZE + 16)) == NULL) {
...
update_exit:
	if (pbuf) {
		MFREE(cfg->osh, pbuf, KA_TEMP_BUF_SIZE + 16);
	}
	return res;
	...
	const wl_event_msg_t *e, void *data)
{
	...
	if (!err) {
		...
		cfg80211_rx_mgmt(cfgdev, freq, mgmt_frame, len, GFP_ATOMIC);
	...
	u32 event_type, const wl_event_msg_t *e, void *data)
{
	...
	if (dhdp->roam_event_rx) {
		...
		if (!skb) {
			WL_ERR(("Roaming event skb alloc failed\n"));
			err = BCME_NOMEM;
			goto done;
		}
		...
		WL_INFORM(("Roaming %d status %d reason %d "
		           "freq %d chanspec %x "
		           "cur_bssid="MACDBG", new_bssid="MACDBG"\n",
		           event_type, status, reason,
		           freq, chanspec?(*chanspec):0x00,
		           MAC2STRDBG(curbssid), MAC2STRDBG(&(e->addr))));
		nla_put_u16(skb, BRCM_ROAMING_STATUS_ATTR_TYPE, (u16)event_type);
		nla_put_u16(skb, BRCM_ROAMING_STATUS_ATTR_REASON, (u16)reason);
		nla_put_u16(skb, BRCM_ROAMING_STATUS_ATTR_STATUS, (u16)status);
		if (freq && notify_channel) {
			nla_put(skb, BRCM_ROAMING_STATUS_ATTR_CHANNEL,
			        sizeof(struct ieee80211_channel), notify_channel);
		}
		if (curbssid)
			nla_put(skb, BRCM_ROAMING_STATUS_ATTR_BSSID, ETHER_ADDR_LEN, curbssid);
...
static int
wl_check_exist_freq_in_list(usable_channel_t *channels, int cur_idx, u32 freq)
{
	int i;
	for (i = 0; i < cur_idx; i++) {
...
static void
wl_usable_channels_filter(struct bcm_cfg80211 *cfg, uint32 cur_chspec, uint32 *mask,
...
#ifdef WL_PHY_CAL_EVENT
static s32
wl_phy_cal_event(struct bcm_cfg80211 *cfg,
	bcm_struct_cfgdev *cfgdev, const wl_event_msg_t *e, void *data)
{
	int err = BCME_OK;
	u32 reason = be32_to_cpu(e->reason);
	...
	if (reason == 1) {
		event_type = WIFI_EVENT_FW_PHY_CAL_START;
	} else {
		event_type = WIFI_EVENT_FW_PHY_CAL_STOP;
	}
	wl_cfgvendor_send_async_event(wiphy, ndev, SYNA_VENDOR_EVENT_PHYCAL,
		&event_type, sizeof(int));
#endif /* (LINUX_VERSION_CODE > KERNEL_VERSION(3, 13, 0)) || (WL_VENDOR_EXT_SUPPORT) */
