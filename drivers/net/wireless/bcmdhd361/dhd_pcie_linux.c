...
#else /* defined(DHD_PCIE_RUNTIMEPM) || defined(DHD_PCIE_NATIVE_RUNTIMEPM) */
...
int dhdpcie_get_oob_irq_level(struct dhd_bus *bus)
{
	...
	if (!bus) {
		...
		DHD_ERROR(("%s: bus->dev is NULL\n", __FUNCTION__));
		return BCME_BADARG;
	} else if (!(pch = pci_get_drvdata(bus->dev))) {
		DHD_ERROR(("%s: pch is NULL\n", __FUNCTION__));
		return BCME_BADARG;
	} else if (!(dhdpcie_osinfo = (dhdpcie_os_info_t *)pch->os_cxt)) {
		DHD_ERROR(("%s: dhdpcie_osinfo is NULL\n", __FUNCTION__));
		return BCME_BADARG;
	} else if (!(adapter = dhdpcie_osinfo->adapter)) {
...
#else /* DHD_USE_PCIE_OOB_THREADED_IRQ */
	dhdpcie_oob_intr_set(bus, FALSE);
	DHD_TRACE(("%s: IRQ ISR\n", __FUNCTION__));
	bus->last_oob_irq_isr_time = OSL_LOCALTIME_NS();
#endif /* DHD_USE_PCIE_OOB_THREADED_IRQ */
