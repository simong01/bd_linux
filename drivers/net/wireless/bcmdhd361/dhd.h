...
#define MAX_RESCHED_CNT 600
	...
	LINUX_VERSION_CODE < KERNEL_VERSION(3, 18, 0) || defined(CONFIG_BCMDHD_VENDOR_EXT))
...
#ifndef DHD_MONITOR_TYPE_MGMT
#define DHD_MONITOR_TYPE_MGMT 4
#endif
 ...
 */
#ifdef DHD_ATE_BUILD
#define MAX_NVRAMBUF_SIZE       (96 * 1024) /* max nvram buf size */
#else
...
#else // OEM_ANDROID
#define CONFIG_BCMDHD_FW_PATH  "rtecdc.bin"
#endif // OEM_ANDROID
...
#else // OEM_ANDROID
#define CONFIG_BCMDHD_NVRAM_PATH  "nvram.txt"
#endif // OEM_ANDROID
...
#define CONFIG_BCMDHD_CLM_PATH "bcmdhd_clm.blob"
#endif /* OEM_ANDROID */
...
#ifndef CONFIG_BCMDHD_CONFIG_SAR_PATH
#ifdef OEM_ANDROID
#define CONFIG_BCMDHD_CONFIG_SAR_PATH  "/vendor/etc/wifi/sar_config.ini"
#else /* OEM_ANDROID */
...
#define CONFIG_BCMDHD_CONFIG_PATH  "/var/run/wlan_config.ini"
	...
	#ifdef OEM_ANDROID
		#define CONFIG_BCMDHD_MAP_PATH  "/vendor/etc/wifi/rtecdc.map"
	#else // OEM_ANDROID
...
enum dhd_hang_reason {
	...
	HANG_REASON_DS_SKIP_TIMEOUT			= 0x880A,
...
#define DHD_COMMON_DUMP_PATH	"/data/"
#else /* Default */
...
#define FREEPKT_MAX_RING 2048
typedef struct {
	uint32 w;
	uint32 r;
	uint32 len;
	void *ring[FREEPKT_MAX_RING];
} freepkt_msg_ring_t;
#endif
...
/* XXX Needed by customer's request */
	...
	uint8 prio_8021x;
	uint8 during_assoc_roaming;
#endif // DHD_LOSSLESS_ROAMING
	uint8 during_roaming;  // indicate roaming scan is on-going
	/* timesync link */
	struct dhd_ts *ts;
...
#ifdef FREE_PKT_IN_RXF
/* msg ring for free pkt in rxf thread */
	freepkt_msg_ring_t freepkt_ring;
#endif
#if defined(DISABLE_WL_FRAMEBURST_2G20_TDLS) || \
	defined(DISABLE_WL_FRAMEBURST_2G20_SOFTAP_P2P) || defined(DISABLE_FRAMEBURST_VSDB)
	/* Store the default value of FB and restore during TDLS teardown */
	uint32 frameburst;
#endif
#ifdef PKT_FILTER_SUPPORT
	uint8 pfaoe_enab;
...
extern bool dhd_prec_enq(dhd_pub_t *dhdp, struct pktq *q, void *pkt, int prec, bool enqhead);
...
#if defined(SHOW_LOGTRACE) || defined(DHD_COREDUMP)
typedef struct {
	uint  num_fmts;
	char **fmts;
	char *raw_fmts;
...
extern int dhd_event_iflink(struct dhd_info *dhdinfo, wl_event_msg_t *event,
...
extern bool dhd_fw_assoc_state(dhd_pub_t *dhd, struct net_device * ndev);
...
#ifndef DHD_IDLETIME_TICKS
#ifdef DHD_USE_IDLECOUNT
#define DHD_IDLETIME_TICKS 5
#else
#define DHD_IDLETIME_TICKS 1
#endif /* DHD_USE_IDLECOUNT */
#endif /* DHD_IDLETIME_TICKS */
...
#define DEFAULT_DHD_WATCHDOG_INTERVAL_MS	5 /* msec */
...
#if (defined(BCMSDIO) && !defined(CUSTOMER_HW4)) || defined(DISABLE_FRAMEBURST)
...
void dhd_bus_update_clm_path(struct dhd_bus *bus, char *pclm_path);
int dhd_bus_clm_load_from_fw(struct dhd_bus *bus);
#ifdef FW_SIGNATURE
void dhd_bus_update_fw_sig_path(struct dhd_bus *bus, char *psig_path);
#endif /* FW_SIGNATURE */
...
#define METADATA_VER_LEN	1
#define METADATA_JUMP_VALUE_LEN	3
...
enum metadata_type
{
	...
	CLM_BLOB_LOCATION,
	SIGNATURE_FILE,
	METADATA_TYPE_LAST = METADATA_TYPE_MAX // Add contents before last
};
...
#endif // defined(DHD_LOG_DUMP) || defined(DHD_FW_COREDUMP)
...
extern void dhd_pktfilter_offload_enable(dhd_pub_t * dhd, char *arg,
	int enable, int change_filter_mode);
...
#if defined(__linux__)
#if defined(DHD_SUPPORT_VFS_CALL) || defined(DHD_DUMP_SUPPORT)
...
static INLINE long dhd_i_size_read(const struct inode *inode)
{
	return (long)i_size_read(inode);
}
...
static INLINE long dhd_i_size_read(const struct inode *inode)    { return 0; }
static INLINE int dhd_kernel_read_compat(struct file *fp, loff_t pos,
	void *buf, size_t count)    { return 0; }
static INLINE int dhd_vfs_read(struct file *filep, char *buf,
	size_t size, loff_t *pos)    { return 0; }
static INLINE int dhd_vfs_write(struct file *filep, char *buf,
	size_t size, loff_t *pos)    { return 0; }
...
static INLINE int dhd_kern_path(char *name, int flags, struct path *file_path)    { return 0; }
#endif /* DHD_SUPPORT_VFS_CALL */
#endif /* __linux__ */
...
#ifdef FREE_PKT_IN_RXF
int wlc_freepkt_init_ring(dhd_pub_t *dhdp);
int wlc_freepkt_ring_unused(dhd_pub_t *dhdp);
void *wlc_freepkt_ring_enq(dhd_pub_t *dhdp, void *sdu);
void *wlc_freepkt_ring_deq(dhd_pub_t *dhdp);

void dhd_sched_rxf_fromq(dhd_pub_t *dhdp);
#endif
...
#endif
extern bool is_mode_different(dhd_pub_t *dhdp, int ifidx, int nmode, int vhtmode,
	int hemode, chanspec_t chanspec, bool force_all_phy_mode_cmp);

/** Store current retry cnt to load DHD */
extern void dhd_load_retry_cnt_set(int retry);
/** get current retry cnt to load DHD */
extern int dhd_load_retry_cnt_get(void);
