...
{
#ifdef DEBUGABILITY
	dhdp->dbg_verbose_ring = dhd_dbg_get_ring_from_ring_id(dhdp, FW_VERBOSE_RING_ID);
	if (dhdp->dbg_verbose_ring) {
		if (!proc_create_data("dhd_trace", S_IRUSR, NULL, &dhd_ring_proc_ops,
			dhdp->dbg_verbose_ring)) {
...
{
#ifdef DEBUGABILITY
	if (NULL != dhdp->dbg_verbose_ring) {
		remove_proc_entry("dhd_trace", NULL);
	}
#endif /* DEBUGABILITY */

#ifdef EWP_ECNTRS_LOGGING
	if (NULL != dhdp->ecntr_dbg_ring) {
		remove_proc_entry("dhd_ecounters", NULL);
	}
#endif /* EWP_ECNTRS_LOGGING */

#ifdef EWP_RTT_LOGGING
	if (NULL != dhdp->rtt_dbg_ring) {
...
static ssize_t
show_logdump_done(struct dhd_info *dhd, char *buf)
{
	ssize_t ret = 0;
	...
	if (dump_status == DUMP_NOT_READY) {
		/* Write kernel until HAL is ready */
		if (dump_in_progress) {
			dump_status = DUMP_IN_PROGRESS;
		} else {
			dump_status = DUMP_READY;
		}
	}
...
static ssize_t
do_logdump_dump(struct dhd_info *dhd, const char *buf, size_t count)
{
	uint32 input;
	dhd_dongledump_status_t dump_status;
...
#if defined(DHD_DUMP_FILE_WRITE_FROM_KERNEL)
	if (dump_status == DUMP_NOT_READY) {
		/* Write from kernel while HAL is not ready yet */
		DHD_ERROR(("%s: Trigger dump: %d\n", __func__, dump_status));
		dhd_log_dump_trigger(&dhd->pub, CMD_DEFAULT);
	} else
#endif /* DHD_DUMP_FILE_WRITE_FROM_KERNEL */
...
static uint64
dhd_get_last_suspend_time(dhd_pub_t *dhdp)
{
#if defined(BCMPCIE)
	...
	if (!dhd_os_check_if_up(&dhd->pub)) {
		if (dhd->pub.busstate == DHD_BUS_SUSPEND) {
			...
			if (last_suspend_end_time <
					dhd_get_last_suspend_time(&dhd->pub)) {
...
static struct dhd_attr dhd_attr_logdump_ecntr =
	__ATTR(logdump_ecntr_enable, 0660, show_logdump_ecntr,
		logdump_ecntr_onoff);
#if defined(DHD_FILE_DUMP_EVENT) && defined(DHD_FW_COREDUMP)
static struct dhd_attr dhd_attr_logdump_dump =
	__ATTR(dump_in_progress, 0660, show_logdump_done,
		do_logdump_dump);
#endif /* DHD_FILE_DUMP_EVENT && DHD_FW_COREDUMP */
#endif /* DHD_LOG_DUMP */
...
#else // defined(DISABLE_HE_ENAB) || defined(CUSTOM_CONTROL_HE_ENAB)
	1;
#endif // defined(DISABLE_HE_ENAB) || defined(CUSTOM_CONTROL_HE_ENAB)
...
#ifdef BT_OVER_SDIO
	len += scnprintf((ptr+len), (buf_size-len), "bus_user_lock: %d\n",
...
static ssize_t
show_ccode(struct dhd_info *dev, char *buf)
{
	dhd_info_t *dhd = (dhd_info_t *)dev;
	dhd_pub_t *dhdp;
	int len;
	...
	if (len == 0) {
		...
		DHD_ERROR(("country code mkiovar failed error %d\n", err));
		return 0;
	} else {
		err = dhd_wl_ioctl_cmd(dhdp, WLC_GET_VAR, iovbuf, sizeof(iovbuf), FALSE, 0);
		if (err) {
			DHD_ERROR(("country code get failed error %d\n", err));
			return 0;
		}
		cspec = (wl_country_t *)iovbuf;
	}

	return snprintf(buf, WLC_CNTRY_BUF_SZ, "%s\n", cspec->ccode);
}

static struct dhd_attr dhd_attr_ccode =
__ATTR(ccode, 0660, show_ccode, NULL);
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 18, 0)
static const struct attribute_group debug_lb_group = {
	.attrs = debug_lb_attrs
};
static const struct attribute_group *debug_lb_groups[] = {
	&debug_lb_group,
	NULL
};
#endif /* LINUX_VER >= 5.18 */
...
int dhd_sysfs_init(dhd_info_t *dhd)
{
	...
	ret  = kobject_init_and_add(&dhd->dhd_lb_kobj,
			&dhd_lb_ktype, &dhd->dhd_kobj, CONST_SYNA_DHD_LB_OBJ_NAME);
	if (ret) {
