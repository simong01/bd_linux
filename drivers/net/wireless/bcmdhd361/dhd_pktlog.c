...
dhd_pktlog_filter_init(int size)
{
	...
	filter = (dhd_pktlog_filter_t *)KVZALLOC(alloc_len, kflags);
	if (unlikely(!filter)) {
		DHD_ERROR(("%s(): could not allocate memory for - "
					"dhd_pktlog_filter_t\n", __FUNCTION__));
		goto fail;
	...
	filter_info = (dhd_pktlog_filter_info_t *)KVZALLOC(alloc_len, kflags);
...
dhd_pktlog_filter_matched(dhd_pktlog_filter_t *filter, char *data, uint32 pktlog_case)
{
	...
	for (i = 0; i < filter->list_cnt; i++) {
		if (filter->info && filter->info[i].id && filter->info[i].enable) {
...
#endif /* DHD_PKT_LOGGING_DBGRING */
{
	...
	for (item_p = dll_head_p(&pktlog_ring->ring_info_head);
		...
		if (report_ptr->info.pkt == NULL) {
			/*
			 * info.pkt is NULL means PKTDUP fail in
			 * dhd_pktlog_ring_add_pkts function there is no malloc
			 * happened, report_ptr->info.pkt don't need to PKTFREE
			 * and this pktlog_ring entery will be reused/release
			 * when pktlog_ring->ring_info_free is empty
			 */
