...
#if (defined(WL_STA_ASSOC_RAND) && defined(WL_STA_INIT_RAND)) || \
	defined(DHD_FILE_DUMP_EVENT)
	dhd_pub_t *dhd = (dhd_pub_t *)(cfg->pub);
#endif /* WL_STA_ASSOC_RAND && WL_STA_INIT_RAND || DHD_FILE_DUMP_EVENT */
	int ret = BCME_OK;
...
wl_cfgvendor_rtt_evt(void *ctx, void *rtt_data)
{
	...
	if (list_empty(rtt_cache_list)) {
		skb = CFG80211_VENDOR_EVENT_ALLOC(wiphy, wdev, 100,
	...
	list_for_each_entry(rtt_header, rtt_cache_list, list) {
		/* Alloc the SKB for vendor_event */
		skb = CFG80211_VENDOR_EVENT_ALLOC(wiphy, wdev, rtt_header->result_tot_len + 100,
			GOOGLE_RTT_COMPLETE_EVENT, kflags);
		if (!skb) {
			WL_ERR(("skb alloc failed"));
			return;
	...
	wl_nan_ev_rng_rpt_ind_t *range_res, uint32 status)
{
	...
	msg = CFG80211_VENDOR_EVENT_ALLOC(wiphy, ndev_to_wdev(dev), 100,
			GOOGLE_RTT_COMPLETE_EVENT, kflags);
	if (!msg) {
	...
	struct wireless_dev *wdev, const void  *data, int len)
{
	...
	if (cfg->cca_ver < 0) {
		...
		if (err == BCME_UNSUPPORTED) {
			cfg->cca_ver = WL_CCA_EXT_REQ_VER;
		} else {
			cfg->cca_ver = dtoh16(((cca_congest_ext_channel_req_v3_t *)iovar_buf)->ver);
		}
	}

	/* Check the version for cca_get_stats_ext iovar */
	if ((err == BCME_OK) &&
			(cfg->cca_ver == WL_CCA_EXT_REQ_VER_V3)) {
		err = wldev_iovar_getbuf(bcmcfg_to_prmry_ndev(cfg), "cca_get_stats_ext", &cca_v3_req,
			sizeof(cca_v3_req), iovar_buf, WLC_IOCTL_MAXLEN, NULL);
		...
		WL_TRACE(("wifi chan statics ver.3 - on_time : %u, cca_busy_time : %u\n",
			...
			(cfg->cca_ver == WL_CCA_EXT_REQ_VER_V2)) {
	...
	struct wireless_dev *wdev, const void *data, int len)
{
	...
	WL_INFORM_MEM(("IOVAR set_pmk ret:%d\n", ret));
exit:
	return ret;
}
...
static int wl_cfgvendor_acs_parse_parameter_save(wl_uint32_list_t *pCH_supported,
...
static int wl_cfgvendor_acs_parse_parameter(wl_uint32_list_t *pCH_supported,
		int *pLen, uint32 *pList, unsigned int chanspec, drv_acs_params_t *pParameter)
{
	...
	do {
		...
		if (((pParameter->ht_enabled) || (pParameter->ht40_enabled) ||
			...
			wl_cfgvendor_acs_parse_parameter_save(pCH_supported, &qty, pList, chanspec);
		}
...
static void wl_cfgvendor_acs_result_event(struct work_struct *work)
{
	...
	do {
		...
		while (retry--) {
			...
			if (chosen) {
				WL_ERR(("%s: * good, selected chosen=0x%X(%d)\n",
				        __FUNCTION__, chosen, CHSPEC_CHANNEL(chosen)));
		...
		if ((!chosen) || (!wf_chspec_valid(chosen))) {
			...
			WL_ERR(("%s: use default channel = 0x%X(%d)\n",
                                    ...
                                    int len, unsigned char *pBuffer, int qty, uint32 *pList)
{
	 ...
	 */
	chanspec = wl_cfg80211_get_sta_channel(cfg);
	...
	if ((WLC_BAND_AUTO == band) || (WLC_BAND_ALL == band)) {
		switch (sta_band) {
			...
			case WLC_BAND_AUTO:
			default:
				WL_ERR(("%s: ***Error, invalid STA band, STA "
				        "chanspec=0x%X, ret=%d\n",
				        __FUNCTION__, chanspec, ret));
				ret = BCME_BADARG;
				goto done;
		}
	}
	if (sta_band == band) {
		chosen = chanspec;
		WL_ERR(("%s: *Warning, STA associated, adopt channel=%d, ret=%d\n",
		        __FUNCTION__, chanspec, ret));
		goto done;
	}
	...
	if (0 <= ret) {
		...
		delay_work_acs.wait_timeout = qty * DHD_SCAN_PASSIVE_TIME;
	...
	struct wireless_dev *wdev, const void *data, int len)
{
	...
	do {
		...
		pCH_supported = MALLOCZ(cfg->osh, chanspec_total);
		if (!pCH_supported) {
		...
		for (i = 0; i < chan_list_len; i++) {
			...
			WL_TRACE(("%s: channel[%d]=%d => ctl_ch=0x%X(%d) (already=%d)\n",
			          __FUNCTION__, i, pElem_chan[i],
			          chanspec, CHSPEC_CHANNEL(chanspec), req_ext_len));
			wl_cfgvendor_acs_parse_parameter(pCH_supported, &req_ext_len, pList, chanspec, parameter);
		}
	...
	struct wireless_dev *wdev, const void  *data, int len)
{
	...
	if (sar_tx_power_val) {
		...
		sar_tx_power_val = 1; /* as already set sar_params for sar_enable 1 */
	}
		...
		bcm_struct_cfgdev *cfgdev, const wl_event_msg_t *e, void *data)
{
	...
	switch (twt_event->event_type) {
		...
		case WL_TWT_EVENT_NOTIFY:
			...
			break;
		default:
			WL_ERR(("Invalid TWT sub event type %d\n", twt_event->event_type));
...
static const struct  nl80211_vendor_cmd_info wl_vendor_events [] = {
		...
		{OUI_BRCM, SYNA_VENDOR_EVENT_PHYCAL} //102
};
...
#else /* (LINUX_VERSION_CODE > KERNEL_VERSION(3, 13, 0)) || defined(WL_VENDOR_EXT_SUPPORT) */
