...
MODULE_PARM_DESC(dhd_oob_gpio_num, "For Synaptics internal use only - deprecated (def: -1)");
 ...
 *
 */
int dhd_customer_oob_irq_mapdhd_customer_oob_irq_map(void *adapter, unsigned long *irq_flags_ptr)
{
	int  host_oob_irq = 0;
	...
	if (dhd_oob_gpio_num < 0) {
		DHD_ERROR(("%s: ERROR customer specific Host GPIO is NOT defined \n",
		__FUNCTION__));
		return (dhd_oob_gpio_num);
	}

	DHD_ERROR(("%s: customer specific Host GPIO number is (%d)\n",
