...
#if !defined(CUSTOMER_HW4) && !defined(DHD_EFI) && !defined(BOARD_HIKEY) && !defined(CONFIG_ARCH_ASTRA)
	| DHD_IOVAR_MEM_VAL
...
/* make IOCTL wait more time especially DEBUG mode to avoid trap */
#ifdef DHD_DEBUG
#define BUS_TIMEOUT_DEFAULT     5000
#define CMD_TIMEOUT_DEFAULT     8000
...
dhd_wl_ioctl(dhd_pub_t *dhd_pub, int ifidx, wl_ioctl_t *ioc, void *buf, int len)
{
	...
	if (dhd_os_proto_block(dhd_pub)) {
		 ...
		 */
		if ((WLC_SET_VAR == ioc->cmd) && (NULL != buf)) {
			action = 0;
			eparams = NULL;
			if (!strcmp("escan", buf)) {
				eparams = (wl_escan_params_t *) (buf_ptr + strlen("escan") + 1);
				action = dtoh16(eparams->action);
			} else if (!strcmp("escanresults", buf)) {
				eparams = (wl_escan_params_t *)
					(buf_ptr + strlen("escanresults") + 1);
				action = dtoh16(eparams->action);
			}
			if (action == WL_SCAN_ACTION_START) {
				/* add to avoid sync_id mismatch */
				#ifdef DUAL_ESCAN_RESULT_BUFFER
				{
					dhd_pub->esync_id = htod16(eparams->sync_id);
				}
				#else /* DUAL_ESCAN_RESULT_BUFFER */
				{
					++dhd_pub->esync_id;
...
#ifdef REPORT_FATAL_TIMEOUTS
		if ((BCME_OK == ret) && (WLC_SET_VAR == ioc->cmd) && (buf) &&
			((!strcmp("escan", buf)) || (!strcmp("escanresults", buf)))) {
			if (action == WL_SCAN_ACTION_START)
...
#ifdef CSI_SUPPORT
static void
f2i_from_bits(uint32 raw_bits, int *int_part, uint *fl_part)
{
	/* IEEE 754 decoding */
	u32 sign;
	u32 exponent;
	...
	*fl_part = 0;
	...
	if (exponent == 0xFF) {
		/* NaN or Inf */
		DHD_ERROR(("f2i: unsupported value (NaN or Inf)\n"));
		return;
	}
	...
	if (exponent != 0)
		full_mantissa |= (1UL << 23);	/* normalized floats */

	/* alignment to integer position */
	shift = e - 23;

	if (shift >= 0) {
		value = full_mantissa << shift;
		*int_part = (int)value;
		*fl_part = 0;
	} else {
		value = full_mantissa >> -shift;
		*int_part = (int)value;
		...
		/* accurate fractional part: extract remainder bits and scale */
		rem = full_mantissa & ((1UL << -shift) - 1);
		scaled_frac = (rem * 1000000ULL) >> -shift;
		*fl_part = (u32)scaled_frac;
	}
	...
	void *raw_event_ptr, char *eventmask)
{
	...
	case WLC_E_AWDL_AW:
		if (datalen >= sizeof(awdl_aws_event_data_t)) {
			...
			BCM_REFERENCE(aw);
			DHD_EVENT(("MACEVENT: %s(%d), MAC %s aw_cnt %u "
			           "ext_cnt %u flags %u aw_ch %u\n",
			           event_name, event_type, eabuf, aw->aw_counter,
			           aw->aw_ext_count, aw->flags,
			           CHSPEC_CHANNEL(aw->aw_chan)));
			host_data = TRUE;
	...
	case WLC_E_AWDL_ROLE:
		DHD_EVENT(("MACEVENT: %s(%d), MAC %s ROLE %d\n",
		           event_name, event_type, eabuf, (int)status));
		break;
	...
	case WLC_E_CSI:
		...
		break;
	case WLC_E_CCD: {
		wl_event_ccd_t *ccd_event = (wl_event_ccd_t*) event_data;
		int int_part = 0;
		uint fl_part = 0;
		uint32 value_bits = 0;

		memcpy_s(&value_bits, sizeof(uint32), &ccd_event->value, sizeof(float));
		f2i_from_bits(value_bits, &int_part, &fl_part);

		DHD_EVENT(("MACEVENT: %s(%d) - Change in Channel Condition (%d.%06u)\n",
			event_name, event_type, int_part, fl_part));
		break;
	}
#endif /* CSI_SUPPORT */
	case WLC_E_OWE_INFO:
...
#if defined(__linux__)
				/* Early set interface "del_in_progress" to prevent
				 * from packets flooding to interface which is going
				 * to be removed. That would cause proptx transit_count
				 * not consistent and has entry suppressed issue.
				 */
...
dhd_pktfilter_mode_change(dhd_pub_t * dhd, int enable)
{
	...
	rc = dhd_wl_ioctl_set_intiovar(dhd, "pkt_filter_mode",
			operation_mode, WLC_SET_VAR, TRUE, 0);
...
dhd_apply_default_clm(dhd_pub_t *dhd, char *clm_path)
{
	...
	*/
	...
	imgbuf = dhd_os_open_image(clm_path);
	if (imgbuf == NULL) {
		goto exit;
	}
...
{
	pub->timeout_info = MALLOCZ(pub->osh, sizeof(timeout_info_t));
...
deinit_dhd_timeouts(dhd_pub_t *pub)
{
	 ...
	 */
	if (!pub->timeout_info) {
		DHD_ERROR(("%s *skip since timeout_info pointer is NULL\n", __FUNCTION__));
		WARN_ON(1);
		return;
	}
	...
	if (trailer->log_magic == LOGSTRS_MAGIC) {
		...
		*/
		if (trailer->version == 1) {
...
#ifdef BCMSDIO
			/* clear the fwpkg struct before using as it will have garbage value */
			fwpkg_deinit(&fwpkg);
			err = fwpkg_init(&fwpkg, fw->size, fw->data);
			if (err == BCME_ERROR) {
				goto error;
			}
			err = fwpkg_open_firmware_img(&fwpkg, &buf_offset);
			if (err == BCME_ERROR) {
				goto error;
			}
			...
			err = fwpkg_init(&fwpkg, st_str_file_path);
			if (err == BCME_ERROR) {
				goto error;
			}
			err = fwpkg_open_firmware_img(&fwpkg, st_str_file_path, &file);
			if (err == BCME_ERROR) {
			...
			if (err == BCME_UNSUPPORTED) {
				file_len = fwpkg.file_size;
				DHD_ERROR(("%s Using SINGLE image (size %d)\n",
					__FUNCTION__, file_len));
				err = BCME_OK;
			} else {
				file_len = fwpkg_get_firmware_img_size(&fwpkg);
				DHD_ERROR(("%s Using COMBINED image (size %d)\n",
					__FUNCTION__, file_len));
			}
#else
...
int dhd_80211_mode_apply(dhd_pub_t *dhdp, uint ifidx, int need_down, int need_up)
{
	...
	if (ifp) {
		...
		if (!ifp->hemode) {
			...
			ifp->hemode = 0;
			if ((OMC_VHT == ifp->onlymode) || (OMC_HE  == ifp->onlymode)) {
				ifp->onlymode = 0;
			}
		} else if (!ifp->nmode) {
			ifp->vhtmode = 0;
