...
#ifdef BCMQT_HW
#define DHD_QT_IOCTL_POLL_INTERVAL 30000
#endif /* BCMQT_HW */
...
MODULE_PARM_DESC(enable_mq, "For Synaptics internal use only (def : 1)");
int mq_select_disable = FALSE;
#endif /* DHD_MQ */

uint gdb_debug = 0;
module_param(gdb_debug, int, 0644);
		...
		struct sk_buff *skb, void *pktdata, uint pktlen)
{
	...
	UNUSED_PARAMETER(pktbuf);
	UNUSED_PARAMETER(skb);
	UNUSED_PARAMETER(pktdata);
...
int dhd_2g_vht = 0;
module_param(dhd_2g_vht, int, 0660);
MODULE_PARM_DESC(dhd_2g_vht, "Enable support of VHT rate on 2G band (def: 0)");
#endif /* SUPPORT_2G_VHT */

#ifdef WL_STA_INIT_RAND_CTRL
int dhd_sta_init_rand = 0;
module_param(dhd_sta_init_rand, int, 0660);
MODULE_PARM_DESC(dhd_sta_init_rand, "Initialize the mac address randomly (def: 0)");
#endif /* WL_STA_INIT_RAND_CTRL */
#ifdef DISABLE_MODE_CHANGE
/* It may used for 2 interface,
 * one interface mode change may effect another
 * interface. So need to set it for this case.
 */
int dhd_force_max_mode = 0;
module_param(dhd_force_max_mode, int, 0660);
#endif /* DISABLE_MODE_CHANGE */

#if defined(WL_HOSTAPD_CFG) || defined(SYNA_80211_MODE)
uint dhd_force_bw = 0;
module_param(dhd_force_bw, uint, 0660);
MODULE_PARM_DESC(dhd_force_bw, "For Synaptics internal use only (def: 0)");
...
MODULE_PARM_DESC(dhd_queue_budget, "For Synaptics internal use only");
...
MODULE_PARM_DESC(dhd_sta_threshold, "For Synaptics internal use only (def: 2048)");
...
MODULE_PARM_DESC(dhd_if_threshold, "For Synaptics internal use only (def: 33536)");
#endif /* BCM_ROUTER_DHD */

#if defined(DHD_DEBUG) || defined(DHD_FW_LOG_SUPPORT)
uint32 dngl_console_addr = 0;
module_param(dngl_console_addr, uint, 0644);
MODULE_PARM_DESC(dngl_console_addr, "For Synaptics internal use only (def: 0)");
...
static int _dhd_apf_add_filter(struct net_device *ndev, uint32 filter_id, u8* program,
	uint32 program_len);
static int _dhd_apf_config_filter(struct net_device *ndev, uint32 filter_id, uint32 enable);
...
#ifdef WL_STATIC_IF
static bool dhd_is_static_ndev(dhd_pub_t *dhdp, struct net_device *ndev);
#endif /* WL_STATIC_IF */
...
MODULE_PARM_DESC(firmware_path,
	"Path to the dongle firmware file. When CONFIG_BCMDHD_STD_FW_API \
	is used, root is from /lib/firmware/");
module_param_string(nvram_path, nvram_path, MOD_PARAM_PATHLEN, 0660);
MODULE_PARM_DESC(nvram_path,
	"Path to the dongle nvram file. When CONFIG_BCMDHD_STD_FW_API \
	is used, root is from /lib/firmware/");
module_param_string(clm_path, clm_path, MOD_PARAM_PATHLEN, 0660);
MODULE_PARM_DESC(clm_path,
	"Path to the dongle CLM file. When CONFIG_BCMDHD_STD_FW_API \
	is used, root is from /lib/firmware/");
module_param_string(signature_path, signature_path, MOD_PARAM_PATHLEN, 0660);
MODULE_PARM_DESC(signature_path,
	"Path to the optional firmware signature file. \
	When CONFIG_BCMDHD_STD_FW_API \
	is used, root is from /lib/firmware/");
#ifdef BL_RAM_BOOT
char boot_path[MOD_PARAM_PATHLEN];
module_param_string(boot_path, boot_path, MOD_PARAM_PATHLEN, 0660);
#endif /* BL_RAM_BOOT */
#ifdef BL_SIM_OTP
char otp_addr[MOD_PARAM_PATHLEN];
module_param_string(otp_addr, otp_addr, MOD_PARAM_PATHLEN, 0660);
#endif /* BL_SIM_OTP */
#ifdef DHD_UCODE_DOWNLOAD
char ucode_path[MOD_PARAM_PATHLEN];
module_param_string(ucode_path, ucode_path, MOD_PARAM_PATHLEN, 0660);
MODULE_PARM_DESC(ucode_path,
	"Path to the dongle ucode file when separate file. \
	When CONFIG_BCMDHD_STD_FW_API \
	is used, root is from /lib/firmware/");
...
MODULE_PARM_DESC(config_path,
	"Path to DHD config.txt file. When CONFIG_BCMDHD_STD_FW_API \
	is used, root is from /lib/firmware/");
#endif /* READ_CONFIG_FROM_FILE */
...
MODULE_PARM_DESC(config_sar_path,
	"Path to the dongle SAR file. When CONFIG_BCMDHD_STD_FW_API \
	is used, root is from /lib/firmware/");
...
MODULE_PARM_DESC(info_string, "information string to keep firmware, \
	chio, cheip version info visiable from log (def: empty)");
int op_mode = 0;
int disable_proptx = 0;
module_param(op_mode, int, 0644);
MODULE_PARM_DESC(op_mode, "Fw operation modes - do not change (def: 0)");
#ifdef WL_STATIC_IF
int no_vif = 0;
module_param(no_vif, int, 0644);
#endif /* WL_STATIC_IF */
#if defined(OEM_ANDROID)
...
MODULE_PARM_DESC(logdump_max_filesize, "For Synaptics internal use only (def: 8MB)");
int logdump_max_bufsize = LOG_DUMP_GENERAL_MAX_BUFSIZE;
module_param(logdump_max_bufsize, int, 0644);
MODULE_PARM_DESC(logdump_max_bufsize, "For Synaptics internal use only (def: 256kB)");
int logdump_periodic_flush = FALSE;
module_param(logdump_periodic_flush, int, 0644);
MODULE_PARM_DESC(logdump_periodic_flush, "For Synaptics internal use only (def: 0)");
...
MODULE_PARM_DESC(logdump_ecntr_enable, "For Synaptics internal use only (def: 0)");
int dump_in_progress = FALSE;
...
MODULE_PARM_DESC(host_edl_support, "For Synaptics internal use only (def: 1)");
#endif

#if defined(DHD_AVOID_PW_OFF) || defined(DHD_NOTIFY_MAC_CHANGED)
static bool dhd_allow_stop = TRUE;
module_param(dhd_allow_stop, bool, 0660);
#endif /* DHD_AVOID_PW_OFF || DHD_NOTIFY_MAC_CHANGED */
...
MODULE_PARM_DESC(dhd_msg_level, "DHD message level (def: DHD_ERROR_VAL | DHD_EVENT_VAL)");
...
MODULE_PARM_DESC(dhd_arp_mode, "ARP offload configuration - do not change (def:0x1F)");
#endif /* ARP_OFFLOAD_SUPPORT */

/* Disable Prop tx */
module_param(disable_proptx, int, 0660);
MODULE_PARM_DESC(disable_proptx, "For Synaptics internal use only (def: 0)");
...
MODULE_PARM_DESC(wl_event_enable, "For Synaptics internal use only (def:0)");
...
MODULE_PARM_DESC(logtrace_pkt_sendup, "For Synaptics internal use only (def:0)");
...
// modify to allow external control
#ifdef DHD_FW_LOG_SUPPORT
module_param(dhd_watchdog_ms, uint, 0644);
MODULE_PARM_DESC(dhd_watchdog_ms, "Watchdog period in ms - do not change (def: 5)");
#else // DHD_FW_LOG_SUPPORT
module_param(dhd_watchdog_ms, uint, 0);
MODULE_PARM_DESC(dhd_watchdog_ms, "For Synaptics internal use only (def: 0)");
#endif // DHD_FW_LOG_SUPPORT
...
#ifdef DHD_FW_LOG_SUPPORT
uint dhd_console_ms = 5;
MODULE_PARM_DESC(dhd_console_ms, "Period of fw console buffer polling - do not change (def: 5)");
#else // DHD_FW_LOG_SUPPORT
#if defined(OEM_ANDROID)
#ifdef CUSTOM_DHD_CONSOLE_MS
uint dhd_console_ms = CUSTOM_DHD_CONSOLE_MS;
MODULE_PARM_DESC(dhd_console_ms, "Period of fw console buffer polling - do not change (def: " XSTR(CUSTOM_DHD_CONSOLE_MS) ")");
#else
uint dhd_console_ms = 250;
MODULE_PARM_DESC(dhd_console_ms, "Period of fw console buffer polling - do not change (def: 250)");
#endif /* CUSTOM_DHD_CONSOLE_MS */
#else
uint dhd_console_ms = 250;
MODULE_PARM_DESC(dhd_console_ms, "Period of fw console buffer polling - do not change (def: 250)");
#endif /* OEM_ANDROID */
...
module_param(dhd_console_ms, uint, 0644);
MODULE_PARM_DESC(dhd_console_ms, "Period of fw console buffer polling - do not change (def: 10)");
#else // DHD_FW_LOG_SUPPORT
...
MODULE_PARM_DESC(dhd_slpauto, "For Synaptics internal use only (def: 1)");

#ifdef PKT_FILTER_SUPPORT
/* Global Pkt filter enable control */
uint dhd_pkt_filter_enable = TRUE;
module_param(dhd_pkt_filter_enable, uint, 0);
MODULE_PARM_DESC(dhd_pkt_filter_enable, "Enable FW support for packet filtering, when FW supports \
		Do not change (def: 1)");
#endif
...
MODULE_PARM_DESC(dhd_pkt_filter_init, "For Synaptics internal use only (def: 0)");

/* Pkt filter mode control */
#ifdef GAN_LITE_NAT_KEEPALIVE_FILTER
uint dhd_master_mode = 0;
MODULE_PARM_DESC(dhd_master_mode, "For Synaptics internal use only (def: 0)");
#else
uint dhd_master_mode = PKT_FILTER_MODE_FORWARD_ON_MATCH;
MODULE_PARM_DESC(dhd_master_mode, "For Synaptics internal use only (def: " XSTR(PKT_FILTER_MODE_FORWARD_ON_MATCH) ")");
#endif /* GAN_LITE_NAT_KEEPALIVE_FILTER */
module_param(dhd_master_mode, uint, 0);

int dhd_watchdog_prio = 0;
module_param(dhd_watchdog_prio, int, 0);
MODULE_PARM_DESC(dhd_watchdog_prio, "For Synaptics internal use only (def: 0)");

/* DPC thread priority */
int dhd_dpc_prio = CUSTOM_DPC_PRIO_SETTING;
module_param(dhd_dpc_prio, int, 0);
MODULE_PARM_DESC(dhd_dpc_prio, "For Synaptics internal use only (def: " XSTR(CUSTOM_DPC_PRIO_SETTING) ")");

/* RX frame thread priority */
int dhd_rxf_prio = CUSTOM_RXF_PRIO_SETTING;
module_param(dhd_rxf_prio, int, 0);
MODULE_PARM_DESC(dhd_rxf_prio, "For Synaptics internal use only (def: " XSTR(CUSTOM_RXF_PRIO_SETTING) ")");

#ifdef BCMDBG
uint dhd_dongle_rambase;
module_param(dhd_dongle_rambase, uint, 0);
MODULE_PARM_DESC(dhd_dongle_rambase, "For Synaptics internal use only (def: 0)");
#endif /* BCMDBG */

#if !defined(BCMDHDUSB)
int dhd_dongle_ramsize;
module_param(dhd_dongle_ramsize, int, 0);
MODULE_PARM_DESC(dhd_dongle_ramsize, "For Synaptics internal use only (def: 0)");
#endif /* BCMDHDUSB */

#ifdef WL_CFG80211
int passive_channel_skip = 0;
module_param(passive_channel_skip, int, (S_IRUSR|S_IWUSR));
MODULE_PARM_DESC(passive_channel_skip, "For Synaptics internal use only (def: 0)");
#endif /* WL_CFG80211 */
static dhd_if_t * dhd_get_ifp_by_ndev(dhd_pub_t *dhdp, struct net_device *ndev);

#ifdef DHD_MSI_SUPPORT
// only turn on when kernel built-in this MSI feature
#ifdef CONFIG_PCI_MSI
uint enable_msi = TRUE;
MODULE_PARM_DESC(enable_msi, "For Synaptics internal use only (def: 1)");
#else // CONFIG_PCI_MSI
uint enable_msi = FALSE;
MODULE_PARM_DESC(enable_msi, "For Synaptics internal use only (def: 0)");
...
MODULE_PARM_DESC(sssr_enab, "For Synaptics internal use only (def: 1)");
module_param(fis_enab, uint, 0);
MODULE_PARM_DESC(fis_enab, "For Synaptics internal use only (def: 0)");
#endif /* DHD_SSSR_DUMP */
...
MODULE_PARM_DESC(instance_base, "For Synaptics internal use only (def: 0)");
...
MODULE_PARM_DESC(dhd_napi_weight, "For Synaptics internal use only (def: NAPI_POLL_WEIGHT)");
#endif /* DHD_LB_RXP */

#ifdef PCIE_FULL_DONGLE
extern int h2d_max_txpost;
module_param(h2d_max_txpost, int, 0644);
MODULE_PARM_DESC(h2d_max_txpost, "For Synaptics internal use only (def: 512)");

#if defined(DHD_HTPUT_TUNABLES)
extern int h2d_htput_max_txpost;
module_param(h2d_htput_max_txpost, int, 0644);
MODULE_PARM_DESC(h2d_htput_max_txpost, "For Synaptics internal use only (def: 2048)");
#endif /* DHD_HTPUT_TUNABLES */

#ifdef AGG_H2D_DB
extern bool agg_h2d_db_enab;
module_param(agg_h2d_db_enab, bool, 0644);
MODULE_PARM_DESC(agg_h2d_db_enab, "For Synaptics internal use only (def: 1)");
extern uint agg_h2d_db_timeout;
module_param(agg_h2d_db_timeout, uint, 0644);
MODULE_PARM_DESC(agg_h2d_db_timeout, "For Synaptics internal use only (def: 1000us)");
extern uint agg_h2d_db_inflight_thresh;
module_param(agg_h2d_db_inflight_thresh, uint, 0644);
MODULE_PARM_DESC(agg_h2d_db_inflight_thresh, "For Synaptics internal use only (def: 48)");
#endif /* AGG_H2D_DB */

extern uint dma_ring_indices;
module_param(dma_ring_indices, uint, 0644);
MODULE_PARM_DESC(dma_ring_indices, "For Synaptics internal use only (def: 0)");

extern bool h2d_phase;
module_param(h2d_phase, bool, 0644);
MODULE_PARM_DESC(h2d_phase, "For Synaptics internal use only (def: 0)");
extern bool force_trap_bad_h2d_phase;
module_param(force_trap_bad_h2d_phase, bool, 0644);
MODULE_PARM_DESC(force_trap_bad_h2d_phase, "For Synaptics internal use only (def: 0)");
#endif /* PCIE_FULL_DONGLE */
...
MODULE_PARM_DESC(tpoweron_scale, "For Synaptics internal use only (def: 0x29)");
...
static char *logstrs_path = WIFI_PATH"logstrs.bin";
char *st_str_file_path = WIFI_PATH"rtecdc.bin";
static char *map_file_path = WIFI_PATH"rtecdc.map";
static char *rom_st_str_file_path = WIFI_PATH"roml.bin";
static char *rom_map_file_path = WIFI_PATH"roml.map";
#else
...
MODULE_PARM_DESC(logstrs_path, "Path to the logstrs.bin file used to decode \
CONSOLE_E event message from the dongle. When CONFIG_BCMDHD_STD_FW_API \
is used, root is from /lib/firmware/");
module_param(st_str_file_path, charp, S_IRUGO);
MODULE_PARM_DESC(st_str_file_path,
"Path to the rtecdc.bin file used to decode \
CONSOLE_E event message from the dongle. When CONFIG_BCMDHD_STD_FW_API \
is used, root is from /lib/firmware/");
module_param(map_file_path, charp, S_IRUGO);
MODULE_PARM_DESC(map_file_path,
"Path to the rtecdc.map file used to decode \
CONSOLE_E event message from the dongle. When CONFIG_BCMDHD_STD_FW_API \
is used, root is from /lib/firmware/");
module_param(rom_st_str_file_path, charp, S_IRUGO);
MODULE_PARM_DESC(rom_st_str_file_path,
"Path to the roml.bin file used to decode \
CONSOLE_E event message from the dongle. When CONFIG_BCMDHD_STD_FW_API \
is used, root is from /lib/firmware/");
module_param(rom_map_file_path, charp, S_IRUGO);
MODULE_PARM_DESC(rom_map_file_path,
"Path to the roml.map file used to decode \
CONSOLE_E event message from the dongle. When CONFIG_BCMDHD_STD_FW_API \
is used, root is from /lib/firmware/");
...
MODULE_PARM_DESC(pcie_txs_metadata_enable, "For Synaptics internal use only (def: 0)");

/* Control radio state */
uint dhd_radio_up = 1;

/* Network inteface name */
char iface_name[IFNAMSIZ + 1] = {'\0'};
module_param_string(iface_name, iface_name, IFNAMSIZ, 0);
MODULE_PARM_DESC(iface_name, "Prefix of the name of the wifi interface (def: empty, use OS provided name)");
...
MODULE_PARM_DESC(dhd_idletime, "Number of idle watchdog periods after which \
	switch the sdio bus off - do not change (def: 5)");

/* Use polling */
uint dhd_poll = FALSE;
module_param(dhd_poll, uint, 0);
MODULE_PARM_DESC(dhd_poll, "For Synaptics internal use only (def: FALSE)");

/* Use interrupts */
uint dhd_intr = TRUE;
module_param(dhd_intr, uint, 0);
MODULE_PARM_DESC(dhd_intr, "For Synaptics internal use only (def: TRUE)");

/* SDIO Drive Strength (in milliamps) */
uint dhd_sdiod_drive_strength = 6;
module_param(dhd_sdiod_drive_strength, uint, 0);
MODULE_PARM_DESC(dhd_sdiod_drive_strength, "For Synaptics internal use only - deprecated (def: 6)");
...
MODULE_PARM_DESC(dhd_txbound, "Max frame to transmit in a bus access (def: DHD_TXBOUND)");
module_param(dhd_rxbound, uint, 0);
MODULE_PARM_DESC(dhd_rxbound, "Max frame to receive in a bus access (def: DHD_RXBOUND)");

/* Deferred transmits */
extern uint dhd_deferred_tx;
module_param(dhd_deferred_tx, uint, 0);
MODULE_PARM_DESC(dhd_deferred_tx, "For Synaptics internal use only (def: 1)");
...
MODULE_PARM_DESC(htclkratio, "For Synaptics internal use only (def: 1)");

int dngl_xtalfreq = 0;
module_param(dngl_xtalfreq, int, 0);
MODULE_PARM_DESC(dngl_xtalfreq, "For Synaptics internal use only (def: 0)");
#endif /* BCMSLTGT */
...
MODULE_PARM_DESC(dhd_pktgen, "For Synaptics internal use only (def: 0)");

/* Echo packet len (0 => sawtooth, max 2040) */
uint dhd_pktgen_len = 0;
module_param(dhd_pktgen_len, uint, 0);
MODULE_PARM_DESC(dhd_pktgen_len, "For Synaptics internal use only (def: 0)");
#endif /* SDTEST */

#ifdef CUSTOM_DSCP_TO_PRIO_MAPPING
uint dhd_dscpmap_enable = 1;
module_param(dhd_dscpmap_enable, uint, 0644);
MODULE_PARM_DESC(dhd_dscpmap_enable, "Enable custom remapping of DSCP to priority (def: 1)");
...
MODULE_PARM_DESC(dhd_use_idsup, "Enable idsup by default (if supported in fw) - do not change (def: 1)");
#else
uint dhd_use_idsup = 0;
MODULE_PARM_DESC(dhd_use_idsup, "Enable idsup by default (if supported in fw) - do not change (def: 0)");
...
MODULE_PARM_DESC(allow_delay_fwdl, "For Synaptics internal use only (def: 0)");
#endif /* !BCMDBUS */
...
MODULE_PARM_DESC(nodeadman, "For Synaptics internal use only (def: 0)");
#endif /* GDB_PROXY */
...
MODULE_PARM_DESC(enable_ecounter, "For Synaptics internal use only (def: 1)");

#if defined(BCMQT_HW) || defined(BL_RAM_BOOT)
int qt_flr_reset = FALSE;
module_param(qt_flr_reset, int, 0);
MODULE_PARM_DESC(qt_flr_reset, "For Synaptics internal use only (def: 0)");

int qt_dngl_timeout = 0; // dongle attach timeout in ms
module_param(qt_dngl_timeout, int, 0);
MODULE_PARM_DESC(qt_dngl_timeout, "For Synaptics internal use only (def: 0)");
#endif /* BCMQT_HW */

/* TCM verification flag */
uint dhd_tcm_test_enable = FALSE;
module_param(dhd_tcm_test_enable, uint, 0644);
MODULE_PARM_DESC(dhd_tcm_test_enable, "For Synaptics internal use only (def: 0)");
...
int dhd_change_mtu(dhd_pub_t *dhd, int new_mtu, int ifidx);
...
/* checking inteface status */
bool dhd_fw_assoc_state(dhd_pub_t *dhd, struct net_device * ndev)
{
	bool fw_assoc_state = FALSE;

	fw_assoc_state = dhd_is_associated(dhd, DHD_DEV_IFIDX(ndev), NULL);

	return fw_assoc_state;
}

#ifdef CUSTOMER_HW4_DEBUG
static void dhd_log_dump_print_tail(dhd_pub_t *dhdp,
	struct dhd_log_dump_buf *dld_buf, uint tail_len);
...
int dhd_bssidx2idx(dhd_pub_t *dhdp, uint32 bssidx)
{
	...
	/* For the case of wlan0/wlan1/wlan2...
	 * or wifi0/wifi1/wifi2...
	 * and DHD tool will pass N+DHD_MAX_IFS down
	 */
	if (bssidx >= DHD_MAX_IFS) {
		if (bssidx < (2*DHD_MAX_IFS)) {
			bssidx -= DHD_MAX_IFS;
			if (dhd->iflist[bssidx]) {
				return bssidx;
			} else {
				return DHD_MAX_IFS;
			}
...
dhd_enable_packet_filter(int value, dhd_pub_t *dhd)
{
	...
	DHD_ERROR(("%s: enter, value = %d, mode:%x\n", __FUNCTION__, value, dhd->op_mode));
#ifndef ALLOW_PKTFILTER_ON_CONCURRENT_MODE
	if (!dhd->pfaoe_enab && (dhd->op_mode &
	...
	if (dhd_pkt_filter_enable && (!value ||
		...
		/* early set mode for disable case */
		if (!value) {
		...
		for (i = 0; i < dhd->pktfilter_count; i++) {
			...
			dhd_pktfilter_offload_enable(dhd, dhd->pktfilter[i], value, FALSE);
		}
...
#ifdef WL_MON_OWN_PKT
#define MONMGMT_BCN_RX_ACTIVE    0x1000
#define MONMGMT_BCN_RX_DEACTIVE  0x2000
	uint32 rx_bcn;
#endif /* WL_MON_OWN_PKT */
...
#endif /* OEM_ANDROID && BCMPCIE */
#ifdef WL_MON_OWN_PKT
		rx_bcn = MONMGMT_BCN_RX_DEACTIVE;
		ret = dhd_iovar(dhd, 0, "monitor_bcn_pkt", (char *)&rx_bcn,
				sizeof(rx_bcn), NULL, 0, TRUE);
		if (ret < 0) {
			DHD_ERROR(("failed to deactive monitor_bcn_pkt (%d)\n", ret));
		}
#endif /* WL_MON_OWN_PKT */
...
#endif /* PASS_ALL_MCAST_PKTS */
#ifdef WL_MON_OWN_PKT
		rx_bcn = MONMGMT_BCN_RX_ACTIVE;
		ret = dhd_iovar(dhd, 0, "monitor_bcn_pkt", (char *)&rx_bcn,
				sizeof(rx_bcn), NULL, 0, TRUE);
		if (ret < 0) {
			DHD_ERROR(("failed to set monitor_bcn_pkt (%d)\n", ret));
		}
#endif /* WL_MON_OWN_PKT */
...
dhd_timeout_start(dhd_timeout_t *tmo, uint usec)
{
	...
	tmo->elapsed = 0;
#ifdef BL_CONS_LOG
	tmo->tick = 1 * USEC_PER_MSEC; /* 1 msec */
#else /* BL_RAM_BOOT */
...
_dhd_set_mac_address(dhd_info_t *dhd, int ifidx, uint8 *addr)
{
	...
	int tmp_allow_stop = dhd_allow_stop;
...
dhd_ifadd_event_handler(void *handle, void *event_info, u8 event)
{
	...
	if (if_event->event.ifidx > 0) {
		...
		if (iface_name[0]) {
			int len;
			char ch;
			char * if_name = (char *)if_event->name;
...
#if (defined(DHD_L2_FILTER) || defined(PROP_TXSTATUS) || (defined(BCM_ROUTER_DHD) && \
	defined(QOS_MAP_SET)))
...
#ifndef DHD_NO_ORPHAN_FOR_ALL_AC
			if ((PKTPRIO(pktbuf) != PRIO_8021D_BE &&
#ifndef DHD_NO_ORPHAN_FOR_AC_VI
				PKTPRIO(pktbuf) != PRIO_8021D_VI &&
#endif
				PKTPRIO(pktbuf) != PRIO_8021D_BK) &&
				(dhdp->wlfc_stop_orphan == OFF)) {
				skb_orphan(pktbuf);
			}
#endif
#else
		...
		if (ether_type == ETHER_TYPE_ARP) {
			...
			if (ipv6->nexthdr == ICMPV6_HEADER_TYPE) {
				struct icmp6_hdr *icmpv6_hdr =
					(struct icmp6_hdr *)(pkt_data + ETHER_HDR_LEN
								+ sizeof(*ipv6));
...
#endif /* ARP_IPV6NSRS_PKTPRIO_OVERRIDE */
#if defined(BCM_ROUTER_DHD)
	traffic_mgmt_pkt_set_prio(dhdp, pktbuf);

#endif /* BCM_ROUTER_DHD */

	/* Workaround for pktprio may exceed the range of 0~7 */
	{
		uint32 pktprio = PKTPRIO(pktbuf);
		if (pktprio > MAXPRIO) {
			DHD_ERROR(("%s: SYNA: the pktprio(%d) is out of range\n",
				__func__, pktprio));
			PKTSETPRIO(pktbuf, PKTPRIO(pktbuf) & 0x7);
			DHD_ERROR(("%s: SYNA: using pktprio(%d) instead\n", __func__,
			       PKTPRIO(pktbuf) & 0x7));
		}
	}
...
#define DHD_DEFAULT_TCP_PACING_SHIFT 7
...
#ifndef DHD_NO_ORPHAN_FOR_ALL_AC
	if ((PKTPRIO(skb) != PRIO_8021D_BE &&
		PKTPRIO(skb) != PRIO_8021D_BK) &&
		(dhd->pub.wlfc_stop_orphan == OFF)) {
		skb_orphan(skb);
	} else
#endif
	if (skb->sk) {
	...
	if (dhd->pub.tcpack_sup_mode == TCPACK_SUP_HOLD) {
		...
		if (dhd_tcpack_hold(&dhd->pub, pktbuf, ifidx)) {
			/* pktbuf has been freed, set to NULL
			 * for dhd_txpkt_log_and_dump() to
			 * operate
			 */
			pktbuf = NULL;
			ret = 0;
	...
	if (ret) {
...
#endif /* PROP_TXSTATUS */
		{
			...
			if (pktbuf) {
				dhd_txpkt_log_and_dump(&dhd->pub, pktbuf, NULL);
			}
#endif /* PROP_TXSTATUS */
		}
...
#else /* BCMDMA64OSL */
		DHD_ERROR(("!!Invalid nr_frags: %u pa: 0x%x"
...
dhd_rx_frame(dhd_pub_t *dhdp, int ifidx, void *pktbuf, int numpkt, uint8 chan)
{
	...
	for (i = 0; pktbuf && i < numpkt; i++, pktbuf = pnext) {
		...
		if (ifidx == 0xff) {
			ifidx = DHD_PKTTAG_IFID((dhd_pkttag_fr_t *)PKTTAG(pktbuf));
		}
#endif
		...
		if (pkt_wake) {
#if defined(CUSTOMER_HW4) || defined(CUSTOMER_HW_AMLOGIC)
			DHD_TRACE(("##### dhdpcie_host_wake caused by packets\n"));
#else
...
#if defined(CUSTOMER_HW4) || defined(CUSTOMER_HW_AMLOGIC)
			DHD_TRACE(("config check in_suspend: %d\n", dhdp->in_suspend));
#ifdef ARP_OFFLOAD_SUPPORT
			DHD_TRACE(("arp hmac_update:%d \n", dhdp->hmac_updated));
#endif /* ARP_OFFLOAD_SUPPORT */
#else
		...
		if (ntoh16(skb->protocol) == ETHER_TYPE_BRCM) {
			...
			if (ret_event != BCME_OK) {
				ret_event = dhd_get_monitor_data(dhdp, pktbuf, skb, pkt_data, len);
				if (ret_event != BCME_OK) {
					DHD_ERROR(("%s: wl_host_event_get_data err = %d\n",
			...
			if (unlikely(pkt_wake)) {
				...
				if (event.event_type < WLC_E_LAST) {
...
#if defined(CUSTOMER_HW4) || defined(CUSTOMER_HW_AMLOGIC)
					DHD_TRACE(("[pkt_wake] event id %u = %s\n",
						event.event_type,
					...
					DHD_ERROR(("[pkt_wake] event id %u = %s\n",
						event.event_type,
						bcmevent_get_name(event.event_type)));
#endif /* CUSTOMER_HW4 || CUSTOMER_HW_AMLOGIC */
...
static void dhd_watchdog(void *data)
{
...
#ifdef DHD_LOSSLESS_ROAMING
	if (dhd->pub.during_assoc_roaming) {
		if (dhd->wd_timer_valid)
			mod_timer(&dhd->timer, jiffies + msecs_to_jiffies(100));
		return;
	}
#endif // DHD_LOSSLESS_ROAMING
...
static void dhd_runtimepm(void *data)
{
	dhd_info_t *dhd = (dhd_info_t *)data;
...
static int
dhd_check_rtp(dhd_pub_t *dhdp, void * skb)
{
	int  ret = BCME_OK;

#if defined(SYNA_RTP_DEBUG) || defined(SYNA_RTP_DUMP)
	/* dump RTP packet information */
	if (skb) {
		uint8 *pptr = PKTDATA(dhdp->osh, skb);
		/* here are IP packet at this point 'dhd_rxf_thread', so skip parts:
		uint8 *pMAC = pptr;
		uint8 *pIP = pMAC + (6 + 6 + 2);
		uint16 proto = (pMAC[12] << 8) | pMAC[13];
		*/
		uint8 *pIP = pptr;
		uint16 proto = ntoh16(((struct sk_buff*)skb)->protocol);
		uint8 *pUDP = pIP  + 20;
		uint8 *pRTP = pUDP + 8;
		uint16 *ppdata;
		uint32 tag, rtp_seq, rtp_time, rtp_ssrc;
		uint8  is_rtp = 0;
		static uint32 previous_rtp_seq = 0;

		ppdata = (uint16 *)pRTP;
		tag = ntoh16(ppdata[0]);
		if (IP_VER(pIP) == IP_VER_4) {
			if (IPV4_PROT(pIP) == IP_PROT_UDP) {
				if (((0xff00 & tag) == (uint16)0x8000) ||
					((0x00ff & tag) == (uint16)0x0080)) {
					is_rtp = 1;
					rtp_seq = ntoh16(ppdata[1]);
					rtp_time =   ((ntoh16(ppdata[2])<<16)&0x0ffff0000)
						   | ((ntoh16(ppdata[3])<< 0)&0x00000ffff);
					rtp_ssrc =   ((ntoh16(ppdata[4])<<16)&0x0ffff0000)
						   | ((ntoh16(ppdata[5])<< 0)&0x00000ffff);
				}
			}
		}
		...
		if (is_rtp) {
#ifdef SYNA_RTP_DUMP
			DHD_ERROR(("%s-%d: RX ptr=0x%px,protocol=0x%04X  RTP:tag=0x%X,"
			          "SSRC=0x%X,seq=0x%X(%d),time=0x%x(%d)\n",
			          __func__, __LINE__, skb, proto,
			          tag, rtp_ssrc, rtp_seq, rtp_seq, rtp_time, rtp_time));
#endif /* SYNA_RTP_DUMP */
			if ((previous_rtp_seq + 1) != rtp_seq) {
				DHD_ERROR(("%s-%d: ***Warning, previous_RTP_seq=0x%X(%d),"
				           " current_seq=0x%X(%d)\n",
				           __func__, __LINE__, previous_rtp_seq,
				           previous_rtp_seq, rtp_seq, rtp_seq));
			}
			previous_rtp_seq = rtp_seq;
		}
#ifdef SYNA_RTP_DUMP
		else {
			DHD_INFO(("%s-%d: RX ptr=0x%px,protocol=0x%04X,tag=0x%X\n",
			           __func__, __LINE__, skb, proto, tag));
		}
#endif /* SYNA_RTP_DUMP */
	}
#endif /* defined(SYNA_RTP_DEBUG) || defined(SYNA_RTP_DUMP) */
...
dhd_rxf_thread(void *data)
{
	tsk_ctl_t *tsk = (tsk_ctl_t *)data;
	dhd_info_t *dhd = (dhd_info_t *)tsk->parent;
#if defined(WAIT_DEQUEUE) && !defined(CUSTOM_SET_CPUCORE)
...
#ifdef FREE_PKT_IN_RXF
			if (true) {
				void *freepkt = NULL;
				while ((freepkt = wlc_freepkt_ring_deq(pub)) != NULL) {
					PKTSETNEXT(pub->osh, freepkt, NULL);
					PKTFREE(pub->osh, freepkt, TRUE);
				}
			}
#endif
...
#ifdef FREE_PKT_IN_RXF
void
dhd_sched_rxf_fromq(dhd_pub_t *dhdp)
{
	dhd_info_t *dhd = (dhd_info_t *)dhdp->info;
...
{
	return ((dhd->info->monitor_type > 0) &&
			(dhd->info->monitor_type != DHD_MONITOR_TYPE_MGMT));
}
#endif /* WL_MON_OWN_PKT */
...
dhd_rx_mon_pkt_sdio(dhd_pub_t *dhdp, void *pkt, int ifidx)
{
	...
	if (dhd->monitor_type && dhd->monitor_dev)
		...
		DHD_ERROR(("ERR:Mon: Drop %p since no monitor enabled %d and %p \n",
				pkt, dhd->monitor_type, dhd->monitor_dev));
		dhd->monitor_skb = NULL;
		return BCME_NOTREADY;
	}

	mon = DHD_MON_DEV_PRIV(dhd->monitor_dev);
	if (mon) {
		mon->stats.rx_bytes += dhd->monitor_skb->len;
		mon->stats.rx_packets++;
	}
	dhd->monitor_skb->protocol =
		eth_type_trans(dhd->monitor_skb, dhd->monitor_skb->dev);
...
static int
dhd_get_monitor_data(dhd_pub_t *dhdp, void *pktbuf, struct sk_buff *skb, void *pktdata, uint pktlen)
{
	int err = BCME_OK;
	uint8 short_subtype = 0;

	/* only care about 16-bit subtype / length versions */
	if (pktlen >= 4) {
		short_subtype = *(uint8 *)(pktdata+ETH_HLEN);
		DHD_TRACE(("MON Rx %p K %p B %p C %p len %d type %x\n",
		           skb, skb->data, pktbuf, pktdata, pktlen, short_subtype));

#ifdef DHD_DEBUG
		if (DHD_BYTES_ON() && DHD_DATA_ON()) {
			prhex("Rx Monitor ", pktdata, pktlen);
		} else if (DHD_HDRS_ON()) {
			prhex("RxMonitorHdr", pktdata, MIN(pktlen, 32));
		}
#endif /* DHD_DEBUG */
		if (!(short_subtype & 0x08)) {
			DHD_ERROR(("Mon err %0x is not supported", short_subtype));
			err =  BCME_VERSION;
			goto done;
		}
		/* Pull monitor type header */
		skb_pull(skb, 2);
		err = dhd_rx_mon_pkt_sdio(dhdp, skb, 0);
	} else {
...
#define CONST_SYNA_MONITOR_IFNAME_DEFAULT    "radiotap"
static void
dhd_add_monitor_if(dhd_info_t *dhd)
{
	...
	devname = CONST_SYNA_MONITOR_IFNAME_DEFAULT;
...
#ifndef WL_CFG80211_MONITOR
	if (FW_SUPPORTED((&dhd->pub), monitor) && (dhd->pub.up)) {
#ifdef DHD_PCIE_RUNTIMEPM
		/* Enable RuntimePM */
		DHD_ENABLE_RUNTIME_PM(&dhd->pub);
		DHD_ERROR(("%s : enabled runtime PM\n", __FUNCTION__));
...
dhd_bus_get(wlan_bt_handle_t handle, bus_owner_t owner)
{
	...
	int f2_sync_start = 0, f2_sync_end = 0;
	...
	if (dhd->bus_user_count == 1) {
		...
		/* update clm path to sdio bus */
		dhd_bus_update_clm_path(dhd->pub.bus, dhd->clm_path);
#ifdef FW_SIGNATURE
		/* update firmware signatre path to sdio bus */
		dhd_bus_update_fw_sig_path(dhd->pub.bus, dhd->sig_path);
#endif
#ifdef BL_RAM_BOOT
		dhd_bus_update_boot_path(dhd->pub.bus, dhd->boot_path);
#endif /* BL_RAM_BOOT */
#ifdef BL_SIM_OTP
		dhd_bus_update_otp_addr(dhd->pub.bus, dhd->otp_addr);
#endif /* BL_SIM_OTP */
		...
		if (!ret) {
			...
			DHD_ERROR(("Time taken for F2 ready is: %d msec\n",
...
int dhd_ioctl_process(dhd_pub_t *pub, int ifidx, dhd_ioctl_t *ioc, void *data_buf)
{
	...
	if (WLC_SET_GMODE == ioc->cmd) {
		int32  int_val = 0;
		bcopy(data_buf, &int_val, sizeof(int_val));
		dhd_80211_mode_update(pub, ifidx, int_val, -1, -1, -1, -1);
	} else if ((WLC_SET_VAR == ioc->cmd) && (NULL != data_buf)) {
		int32  int_val = 0;
		if (!strncmp("gmode", data_buf, 6)) {
			bcopy(((uint8 *)data_buf)+6, &int_val, sizeof(int_val));
			dhd_80211_mode_update(pub, ifidx, int_val, -1, -1, -1, -1);
		} else if (!strncmp("nmode", data_buf, 6)) {
			bcopy(((uint8 *)data_buf)+6, &int_val, sizeof(int_val));
			dhd_80211_mode_update(pub, ifidx, -1, int_val, -1, -1, -1);
		} else if (!strncmp("vhtmode", data_buf, 8)) {
			bcopy(((uint8 *)data_buf)+8, &int_val, sizeof(int_val));
			dhd_80211_mode_update(pub, ifidx, -1, -1, int_val, -1, -1);
		} else if (!strncmp("he", data_buf, 3)) {
			// TLV: 'h' 'e' '\0'    id0 id1    len0 len1    val(uint8)
			bcopy(((uint8 *)data_buf)+3, &int_val, sizeof(int16));
			if (WL_HE_CMD_ENAB == int_val) {
				int_val = 0;
				bcopy(((uint8 *)data_buf)+7, &int_val, sizeof(uint8));
				dhd_80211_mode_update(pub, ifidx, -1, -1, -1, int_val, -1);
			}
		} else if (!strncmp("nreqd", data_buf, 6)) {
			bcopy(((uint8 *)data_buf)+6, &int_val, sizeof(int_val));
			dhd_80211_mode_update(pub, ifidx, -1, -1, -1, -1, OMC_HT);
		} else if ((!strncmp("mode_reqd", data_buf, 10)) &&
#if defined(WL_CFG80211)
			(IS_AP_IFACE(net->ieee80211_ptr)) &&
#endif // defined(WL_CFG80211)
			TRUE) {
			bcopy(((uint8 *)data_buf)+10, &int_val, sizeof(int_val));
			dhd_80211_mode_update(pub, ifidx, -1, -1, -1, -1, int_val);
		}
	}
#if defined(DISABLE_WL_FRAMEBURST_2G20_TDLS) || \
	defined(DISABLE_WL_FRAMEBURST_2G20_SOFTAP_P2P)
	else if (ioc->cmd == WLC_SET_FAKEFRAG) {
		/* Cache the frameburst value during user intervention through wl command to
		* restore the value back
		*/
		pub->frameburst = *((int *)ioc->buf);
	}
#endif /* DISABLE_WL_FRAMEBURST_2G20_TDLS || DISABLE_WL_FRAMEBURST_2G20_SOFTAP_P2P */
	...
	if (bcmerror == BCME_OK) {
		...
		if (ioc->cmd == WLC_SET_SSID) {
			...
			if (BCME_OK <= ret) {
				if (BSSCFG_GENERIC_STA != info->subtype) {
					ret = BCME_NOTSTA;
				}
			}
			/* not apply for AP */
...
dhd_stop(struct net_device *net)
{
	...
	if (dhd->pub.up == 0) {
#ifdef WL_CFG80211
		// still need to check if need cfg80211 down
		if (ifidx == 0) {
	...
	if (!atomic_read(&exit_in_progress) && (IS_CFG80211_STATIC_IF_ACTIVE(cfg) ||
#if defined(DHD_NOTIFY_MAC_CHANGED) || defined(DHD_AVOID_PW_OFF)
		!dhd_allow_stop ||
#endif /* DHD_NOTIFY_MAC_CHANGED || DHD_AVOID_PW_OFF */
		0)) {
		int ret = 0;
#if defined(DHD_AVOID_PW_OFF)
		uint wl_down = 1;
		ret = dhd_wl_ioctl_cmd(&dhd->pub, WLC_DOWN,
			(char *)&wl_down, sizeof(wl_down), TRUE, 0);
#endif /* DHD_AVOID_PW_OFF */
		DHD_ERROR(("static if operational. set FW down ret=%d, skip chip reset.\n", ret));
		skip_reset = true;
	...
	if (ifidx == 0) {
		...
		// early mark 'down' to avoid false hang event
		...
		// match pairs with __wl_cfg80211_up()
...
exit:
	...
	(defined(USE_INITIAL_2G_5G_NODFS_SCAN) || defined(USE_INITIAL_SHORT_DWELL_TIME))
extern bool g_first_broadcast_scan;
#endif /* OEM_ANDROID && WL_CFG80211 && \
	  (USE_INITIAL_2G_5G_NODFS_SCAN || USE_INITIAL_SHORT_DWELL_TIME) */
...
static bool
dhd_check_filesystem_is_up(dhd_info_t *dhd)
{
	struct file *fp;
	const char *clm = dhd->clm_path;
...
dhd_wifi_accel_on_work_cb(struct work_struct *work)
{
	...
	if (!dhd_check_filesystem_is_up(dhd)) {
...
dhd_open(struct net_device *net)
{
	...
	if (!dhd_allow_stop) {
		uint wl_up = 1;
		ret = dhd_wl_ioctl_cmd(&dhd->pub, WLC_UP,
			(char *)&wl_up, sizeof(wl_up), TRUE, 0);
		DHD_ERROR(("static if operational. set FW up ret=%d\n", ret));
	}
#endif /* DHD_AVOID_PW_OFF  */
	if (dhd->pub.up == 1) {
		/* already up */
	...
	if (ifidx == 0) {
		...
		if (!dhd_download_fw_on_driverload) {
...
#if defined(USE_INITIAL_2G_5G_NODFS_SCAN) || defined(USE_INITIAL_SHORT_DWELL_TIME)
			g_first_broadcast_scan = TRUE;
#endif /* USE_INITIAL_2G_5G_NODFS_SCAN || USE_INITIAL_SHORT_DWELL_TIME */
#ifdef SHOW_LOGTRACE
			 ...
			 * not be available
			 */
			if ((ret = dhd_reinit_logtrace_process(dhd)) != BCME_OK) {
				goto exit;
			}
#endif /* SHOW_LOGTRACE */
#if defined(WLAN_ACCEL_BOOT)
...
#endif /* DHD_PM_OVERRIDE */
#if defined(WL_CFG80211)
		if (unlikely(0 > wl_cfg80211_up(net))) {
	...
	dhd_irq_set_affinity(&dhd->pub, cpumask_of(CUSTOM_PCIE_IRQ_CPUCORE));
#endif /* DHD_CONTROL_PCIE_CPUCORE_WIFI_TURNON */
#if defined(BCMPCIE) && defined(DHDTCPACK_SUPPRESS)
#ifdef DHDTCPACK_SUPPRESS_DEF_MODE
...
exit:
...
#if defined(DHD_DEBUG) || defined(DHD_FW_LOG_SUPPORT)
		if (dngl_console_addr && dhd->pub.bus) {
			if (dhd_bus_console_dump(dhd->pub.bus)) {
				DHD_ERROR(("Failed to dump console log over bus\n"));
			}
		}
#endif /* defined(DHD_DEBUG) || defined(DHD_FW_LOG_SUPPORT) */
		if (ret != BCME_NOMEM) {
	...
	else if (!dhd_download_fw_on_driverload) {
...
#ifdef DHD_AVOID_PW_OFF
	if (ret == BCME_OK) {
		if (!(dhd->pub.op_mode & DHD_FLAG_MFG_MODE)) {
			dhd_allow_stop = FALSE;
		}
	}
#endif /* DHD_AVOID_PW_OFF */
...
#ifdef CUSTOMER_HW4
	dhd_read_macaddr_from_file(&dhd->pub, (struct ether_addr *)temp_addr);
	temp_addr[0] |= 0x02;
...
#ifdef DHD_UPDATE_INTF_MAC
int
dhd_event_iflink(struct dhd_info *dhdinfo, wl_event_msg_t *event,
		wl_event_data_if_t *ifevent, char *name, uint8 *mac)
{
	dhd_if_event_t *if_event;
	...
	uint8 *mac, uint8 bssidx, bool need_rtnl_lock, const char *dngl_name)
{
	...
	// initialization 802.11 mode for each interface
	if (!ifidx) {
#ifdef DISABLE_11N
		uint32 nmode = 0;
#else // DISABLE_11N
		...
		uint32 hemode = 0;
#else // defined(DISABLE_HE_ENAB)
		uint32 hemode = 1;
#endif // defined(DISABLE_HE_ENAB)
		...
		dhd_if_t  *ifp_default = dhdinfo->iflist[0];
		// copy from default interface
		ifp->gmode    = ifp_default->gmode;
		ifp->nmode    = ifp_default->nmode;
		ifp->vhtmode  = ifp_default->vhtmode;
		ifp->hemode   = ifp_default->hemode;
		ifp->onlymode = ifp_default->onlymode;
	}
		...
		if (ifp->net != NULL) {
			...
			if (DHD_IF_IS_TX_ACTIVE(ifp)) {
				...
				if (!gdb_debug) {
					...
					wait_event_interruptible(dhdpub->tx_completion_wait,
						((ifp->tx_paths_active & DHD_TX_CONTEXT_MASK) == 0));
					timeout = 100;
				}
...
#ifdef DHDTCPACK_SUPPRESS_DEF_MODE
				dhd_tcpack_suppress_set(dhdpub, DHDTCPACK_SUPPRESS_DEF_MODE);
#else
				dhd_tcpack_suppress_set(dhdpub, TCPACK_SUP_OFF);
#endif /* DHDTCPACK_SUPPRESS_DEF_MODE */
...
int dhd_op_if_update(dhd_pub_t *dhdpub, int ifidx)
{
	...
	ret = dhd_wl_ioctl_cmd(&dhdinfo->pub, WLC_GET_VAR,
	                       buf, sizeof(buf), FALSE, ifp->idx);
	if (ret < 0) {
		DHD_ERROR(("%s: Failed to upudate the MAC address"
		           " for itf=%s, ret=%d\n",
		           __FUNCTION__, ifp->name, ret));
		/* avoid collision */
		dhdinfo->iflist[ifp->idx]->mac_addr[5] += 1;
		/* force locally administrate address */
		ETHER_SET_LOCALADDR(dhdinfo->iflist[ifp->idx]->mac_addr);
	} else {
		DHD_EVENT(("%s: Got mac for itf %s, idx %d,"
		           " MAC=%02X:%02X:%02X:%02X:%02X:%02X\n",
		           __FUNCTION__, ifp->name, ifp->idx,
...
dhd_init_logstrs_array(osl_t *osh, dhd_event_log_t *temp)
{
	...
	if (control_logtrace != LOGTRACE_PARSED_FMT) {
		DHD_ERROR_NO_HW4(("%s: *turned off logstr parsing, mode=%d\n",
...
dhd_init_logstrs_array(osl_t *osh, dhd_event_log_t *temp)
{
	...
	if (dhd_parse_logstrs_file(osh, raw_fmts, logstrs_size, temp)
			== BCME_OK) {
		error = BCME_OK;
		// go on directly for last action
	}
...
response_bt_quiesce_ptr register_request_bt_quiesce(void (*fnc)(bool));
void unregister_request_bt_quiesce(void);
...
MODULE_PARM_DESC(enable_erpom, "For Synaptics internal use only (def: 0)");
...
dhd_attach(osl_t *osh, struct dhd_bus *bus, uint bus_hdrlen)
{
	...
	// intialize the thread pid to avoid wrong stop action
	...
	dhd->pub.wlfc_stop_orphan = OFF;
	dhd->pub.vsdb_mode = FALSE;
	dhd->pub.rsdb_mode = FALSE;

#ifdef PROP_TXSTATUS_VSDB
	dhd->pub.proptx_force = -1;
#endif /* PROP_TXSTATUS_VSDB */
	...
	if (unlikely(0 > wl_cfg80211_attach(net, &dhd->pub))) {
...
#ifdef WL_MON_OWN_PKT
	dhd_add_monitor_if(dhd);
	dhd->monitor_type = DHD_MONITOR_TYPE_MGMT;
	dhd->pub.roam_event_rx = TRUE;
#endif
...
#if defined(DHD_LB_RXP) && !defined(DHD_LB_RXP_DEACTIVATED)
	/* Trun ON the feature by default */
	atomic_set(&dhd->lb_rxp_active, 1);
#endif /* DHD_LB_RXP && !DHD_LB_RXP_DEACTIVATED */
...
#ifdef DHD_LINUX_STD_FW_API
static const char * dhd_get_filename(const char * pFilename)
{
	const char * pName = NULL;

	// kernel 'fw_get_filesystem_firmware' accepts the name with
	// sub-folder, so we only process those absolute path case.
	if ((pFilename) && ('/' == *pFilename)) {
		...
		if (!pName) {
			...
			if (pName[1]) {
				pName++;
				DHD_ERROR(("%s: *Warning, drop PATH from '%s' to '%s' since"
				           " standard FW API only support filename\n",
				           __FUNCTION__, pFilename, pName));
			} else {
				pName = NULL;
			}
		}
...
/* Not only firmware and nvram, but also ucode, clm, ... */
bool dhd_update_fw_nv_path(dhd_info_t *dhdinfo)
{
...
#ifdef BL_RAM_BOOT
	int boot_path_len = sizeof(dhdinfo->boot_path);
#endif /* BL_RAM_BOOT */

	/* Update firmware and nvram path. The path may be from adapter info or module parameter
	 * The path from adapter info is used for initialization only (as it won't change).
	 */
	if ('\0' != firmware_path[0]) {
		...
		fw = firmware_path;
	} else if ('\0' != dhdinfo->fw_path[0]) {
		/* The firmware_path/nvram_path module parameter may be changed
		 * by the system at run time. When it changes we need to copy it
		 * to dhdinfo->fw_path. Also Android private command may change
		 * dhdinfo->fw_path. As such we need to clear the path info in
		 * module parameter after it is copied. We won't update the path
		 * until the module parameter is changed again (first character
		 * is not '\0')
		 */
		fw = NULL;
	} else if (adapter && adapter->fw_path && ('\0' != adapter->fw_path[0])) {
		fw = adapter->fw_path;
	} else {
#ifdef DHD_LINUX_STD_FW_API
		fw = DHD_FW_NAME;
	...
	if ('\0' != nvram_path[0]) {
		...
		nv = nvram_path;
	} else if ('\0' != dhdinfo->nv_path[0]) {
		// keep the original value
		nv = NULL;
	} else if (adapter && adapter->nv_path && ('\0' != adapter->nv_path[0])) {
		nv = adapter->nv_path;
	} else {
#ifdef DHD_LINUX_STD_FW_API
		nv = DHD_NVRAM_NAME;
	...
	if (clm_path[0] != '\0') {
		...
		clm = clm_path;
	} else if ('\0' != dhdinfo->clm_path[0]) {
		// keep the original value
		clm = NULL;
	} else if (adapter && adapter->clm_path && ('\0' != adapter->clm_path[0])) {
		clm = adapter->clm_path;
	} else {
#ifdef DHD_LINUX_STD_FW_API
		clm = DHD_CLM_NAME;
#else // DHD_LINUX_STD_FW_API
		clm = CONFIG_BCMDHD_CLM_PATH;
#endif /* DHD_LINUX_STD_FW_API */
	}
	if ('\0' != signature_path[0]) {
		if (signature_path[strlen(signature_path)-1] == '\n') {
			signature_path[strlen(signature_path)-1] = '\0';
		}
		sig = signature_path;
	} else if ('\0' != dhdinfo->sig_path[0]) {
		/* keep the original value */
		sig = NULL;
	} else if (adapter && adapter->sig_path && ('\0' != adapter->sig_path[0])) {
		sig = adapter->sig_path;
	}
#ifdef BL_RAM_BOOT
	if ('\0' != boot_path[0]) {
		if (boot_path[strlen(boot_path)-1] == '\n') {
			boot_path[strlen(boot_path)-1] = '\0';
		}
		boot = boot_path;
	} else if ('\0' != dhdinfo->boot_path[0]) {
		boot = NULL;
	} else if (adapter && adapter->boot_path && ('\0' != adapter->boot_path[0])) {
		boot = adapter->boot_path;
	}
#endif /* BL_RAM_BOOT */
#ifdef BL_SIM_OTP
	if ('\0' != otp_addr[0]) {
		if (otp_addr[strlen(otp_addr)-1] == '\n') {
			otp_addr[strlen(otp_addr)-1] = '\0';
		}
		otp = otp_addr;
	} else if (0 != dhdinfo->otp_addr) {
		otp = NULL;
	}
#endif /* BL_SIM_OTP */
#ifdef DHD_UCODE_DOWNLOAD
	...
	if (!sig) {
		...
		snprintf(var, sizeof(var), "signature_path%d", dhdinfo->unit);
		nv = nvram_get(var);
	}
	DHD_ERROR(("dhd:%d: fw path:%s nv path:%s sig path:%s\n", dhdinfo->unit, fw, nv, sig));
#endif /* BCM_ROUTER_DHD */

#ifdef DHD_LINUX_STD_FW_API
	// preprocess the filename to only left 'name'
	fw  = (fw) ?dhd_get_filename(fw) :NULL;
	nv  = (nv) ?dhd_get_filename(nv) :NULL;
	clm = (clm)?dhd_get_filename(clm):NULL;
	sig = (sig)?dhd_get_filename(sig):NULL;
#endif // DHD_LINUX_STD_FW_API
	...
	if (fw && fw[0] != '\0') {
		...
		snprintf(dhdinfo->fw_path, fw_path_len, "%s%s", VENDOR_PATH, fw);
#ifdef SHOW_LOGTRACE
	...
	if (nv && nv[0] != '\0') {
		...
		snprintf(dhdinfo->nv_path, nv_path_len, "%s%s", VENDOR_PATH, nv);
	...
	if (clm && clm[0] != '\0') {
		...
		snprintf(dhdinfo->clm_path, clm_path_len, "%s%s", VENDOR_PATH, clm);
	}
	/* Signature file */
	if (sig && sig[0] != '\0') {
		sig_len = strlen(sig);
		if (sig_len >= sig_path_len) {
			DHD_ERROR(("sig path len exceeds max len of dhdinfo->sig_path\n"));
			return FALSE;
		}
		snprintf(dhdinfo->sig_path, sig_path_len, "%s%s", VENDOR_PATH, sig);
	}
#ifdef BL_RAM_BOOT
	if (boot && boot[0] != '\0') {
		boot_len = strlen(boot);
		if (boot_len >= boot_path_len) {
			DHD_ERROR(("boot path len exceeds max len of dhdinfo->boot_path\n"));
			return FALSE;
		}
		snprintf(dhdinfo->boot_path, boot_path_len, "%s%s", VENDOR_PATH, boot);
	}
#endif /* BL_RAM_BOOT */
#ifdef BL_SIM_OTP
	if (otp && otp[0] != '\0') {
		dhdinfo->otp_addr = bcm_strtoul(otp, NULL, 0);
	}
#endif /* BL_SIM_OTP */
#ifdef DHD_UCODE_DOWNLOAD
	if (uc && uc[0] != '\0') {
	...
	if (dhdinfo->sig_path[0] == '\0') {
		DHD_ERROR(("signature path not found\n"));
	}
#ifdef BL_RAM_BOOT
	if (dhdinfo->boot_path[0] == '\0') {
		DHD_ERROR(("otp path not found\n"));
	}
#endif /* BL_RAM_BOOT */
#endif /* BCMEMBEDIMAGE */
...
dhd_update_btfw_path(dhd_info_t *dhdinfo, char* btfw_path)
{
	...
	if (fw && fw[0] != '\0') {
		...
		snprintf(dhdinfo->btfw_path, sizeof(dhdinfo->btfw_path), "%s%s", VENDOR_PATH, fw);
	}
...
#endif /* DHD_DEBUG && BCMSDIO */
#ifdef BL_RAM_BOOT
		DHD_ERROR(("%s download bootrom %s\n",
			__FUNCTION__, dhd->boot_path));
#endif /* BL_RAM_BOOT */
#ifdef BL_SIM_OTP
		DHD_ERROR(("%s download otp 0x%04x\n",
			__FUNCTION__, dhd->otp_addr));
#endif /* BL_SIM_OTP */
		ret = dhd_bus_download_firmware(dhd->pub.bus, dhd->pub.osh,
		                                dhd->fw_path, dhd->nv_path,
		                                dhd->sig_path, dhd->boot_path,
		                                dhd->otp_addr, dhd->clm_path);
#if defined(DHD_DEBUG) && defined(BCMSDIO)
		fw_download_end = OSL_SYSUPTIME();
		DHD_ERROR(("Time taken for FW download is: %d msec\n",
...
#if defined(PCIE_FULL_DONGLE) && defined(PCIE_DS_ENABLE)
	uint32 deep_sleep_disable = 0;
#endif /* PCIE_FULL_DONGLE && PCIE_DS_ENABLE */
...
#ifdef DHD_BLOB_EXISTENCE_CHECK
...
#if defined(USE_WL_FRAMEBURST) || defined(DISABLE_WL_FRAMEBURST_2G20_SOFTAP_P2P)
...
#endif /* USE_WL_FRAMEBURST || DISABLE_WL_FRAMEBURST_2G20_SOFTAP_P2P */
...
#endif // efined(BCMSDIO) && defined(DHD_LOSSLESS_ROAMING)
...
#if defined(PCIE_FULL_DONGLE) && defined(PCIE_DS_ENABLE)
	ret = dhd_iovar(dhd, 0, "bus:deepsleep_disable", (char *)&deep_sleep_disable,
		sizeof(deep_sleep_disable), NULL, 0, TRUE);
	if (ret < 0) {
		DHD_ERROR(("%s: busdeepsleep disable IOVAR not present, proceed\n", __FUNCTION__));
	}
#endif /* PCIE_FULL_DONGLE && PCIE_DS_ENABLE */
...
#ifdef DHD_PROPTX_FORCEON_BY_CHIP
bool dhd_get_proptx_forceon(dhd_pub_t *dhd)
{
	if (dhd_bus_chip_id(dhd) == BCM43012_CHIP_ID) {
		return TRUE;
	} else {
		return FALSE;
	}
}
#endif /* DHD_PROPTX_FORCEON_BY_CHIP */
static int
dhd_legacy_preinit_ioctls(dhd_pub_t *dhd)
{
	...
	*/
	...
	uint32 ampdu_ba_wsize_fw_def = 0;
...
#endif
/* xxx andrey tmp fix for dk8000 build error  */
#if (defined(AP) && !defined(WLP2P)) || !defined(AP)
	struct ether_addr p2p_ea;
#endif
#ifdef BCMCCX
	uint32 ccx = 1;
	...
	uint32 rrm_bcn_req_max_off_chan_time = RRM_BCNREQ_MAX_CHAN_TIME;
#endif /* WBTEXT && RRM_BCNREQ_MAX_CHAN_TIME */
#ifdef DHD_PRUNE_SCAN_RSSI_LOWER
	uint32 scanresults_minrssi = DHD_PRUNE_SCAN_RSSI_LOWER;
#endif /* DHD_PRUNE_SCAN_RSSI_LOWER */
	...
	if ((!dhd_bus_clm_load_from_fw(dhd->bus)) &&
		(ret = dhd_apply_default_clm(dhd, dhd->info->clm_path)) < 0) {
		DHD_ERROR(("%s: CLM set failed. Ignore and continue initialization.\n",
			__FUNCTION__));
		// goto done;
	}
...
#ifdef DISABLE_TXACK_ALIVE
	ret = dhd_iovar(dhd, 0, "txack_alive", (char *)&txack_alive,
			sizeof(txack_alive), NULL, 0, TRUE);
	if (ret < 0) {
...
#ifdef DHD_PRUNE_SCAN_RSSI_LOWER
	dhd_iovar(dhd, 0, "scanresults_minrssi", (char *)&scanresults_minrssi,
		sizeof(scanresults_minrssi), NULL, 0, TRUE);
#endif /* DHD_PRUNE_SCAN_RSSI_LOWER */
...
#ifdef ASSOC_PREF_5G
	(void) dhd_wl_ioctl_cmd(dhd, WLC_SET_ASSOC_PREFER, (char *)&assoc_pref,
			sizeof(assoc_pref), TRUE, 0);
#endif
#ifdef ASSOC_PREFER_RSSI_THRESH
...
#if defined(DISABLE_WL_FRAMEBURST_2G20_SOFTAP_P2P) || \
	defined(DISABLE_WL_FRAMEBURST_2G20_TDLS) || defined(DISABLE_FRAMEBURST_VSDB)
	/* Cache the frameburst during the driver load time to restore back */
	dhd->frameburst = frameburst;
#endif /* DISABLE_WL_FRAMEBURST_2G20_SOFTAP_P2P || DISABLE_WL_FRAMEBURST_2G20_TDLS ||
	* DISABLE_FRAMEBURST_VSDB
	*/
	...
	if (ampdu_ba_wsize != 0) {
		if (dhd_wl_ioctl_get_intiovar(dhd, "ampdu_ba_wsize", &ampdu_ba_wsize_fw_def,
				WLC_GET_VAR, FALSE, 0) != BCME_OK) {
			DHD_ERROR(("failed to get fw default ampdu_ba_wsize\n"));
		}

		ampdu_ba_wsize = MIN(ampdu_ba_wsize, ampdu_ba_wsize_fw_def);
		...
		if (ret < 0) {
			...
			DHD_ERROR(("%s Set ampdu_ba_wsize to %d done\n",
...
#endif /* WL_TWT_HAL_IF */
#ifdef WL_PHY_CAL_EVENT
	setbit(mask, WLC_E_PHY_CAL);
#endif /* WL_PHY_CAL_EVENT */
#ifdef DHD_EXTRA_EVENT
	setbit(mask, WLC_E_EXTRA_EVENT);
#endif /* DHD_EXTRA_EVENT */
#ifdef CSI_SUPPORT
	setbit(mask, WLC_E_CSI);
	setbit(mask, WLC_E_CCD);
#endif /* CSI_SUPPORT */
	...
	if (dhd_wl_ioctl_set_intiovar(dhd, "arpoe",
		1, WLC_SET_VAR, TRUE, 0) != BCME_OK) {
		DHD_ERROR(("failed to enable ARPOE\n"));
	}
	if (dhd_wl_ioctl_set_intiovar(dhd, "arp_per_garp",
		DHD_GARP_KA_PERIOD, WLC_SET_VAR, TRUE, 0) != BCME_OK) {
		DHD_ERROR(("failed to set periodic GARP to %d\n", DHD_GARP_KA_PERIOD));
	}
...
#ifdef PROP_TXSTATUS
	if (disable_proptx ||
		// make the WLFC turn on for non-MFG mode by default
		(DHD_FLAG_MFG_MODE == dhd->op_mode) ||
#ifdef PROP_TXSTATUS_VSDB
	...
	/* XXX The TLV overwrite problem has been fixed */
	if (wlfc_enable)
		dhd_wlfc_init(dhd);
#ifndef DISABLE_11N
	else if (hostreorder)
	...
	*/
	...
	if (op_mode == DHD_FLAG_HOSTAP_MODE) {
		uint32 value = 1;
		ret = dhd_iovar(dhd, 0, "apsta", (char *)&value, sizeof(value), NULL, 0, TRUE);
		if (ret < 0)
			DHD_ERROR(("%s APSTA for HOSTAP mode failed ret= %d\n", __FUNCTION__, ret));
	}
#endif /* defined(WL_HOSTAPD_CFG) || defined(SYNA_80211_MODE) */
...
#ifdef CUSTOM_SET_CPUCORE
	DHD_ERROR(("Change MTU to 1500\n"));
	dhd_change_mtu(dhd, 1500, 0);
#endif /* CUSTOM_SET_CPUCORE */
...
#ifdef READ_CONFIG_FROM_FILE
	dhd_preinit_config(dhd, 0, config_path);
#endif /* READ_CONFIG_FROM_FILE */

#if defined(SYNA_80211_MODE)
	/* Set default 802.11 mode
	   Note that here uses need_up=FALSE to leave 'ifconfig' to call
	   the WLC_UP to avoid some no-CLM-blob FW encounter exception.
	    Also, another way is using dhd_check_current_clm_data(dhd).
	*/
	dhd_80211_mode_apply(dhd, 0, TRUE, FALSE);
#endif /* defined(SYNA_80211_MODE) */
...
#if defined(DISABLE_WL_FRAMEBURST_2G20_SOFTAP_P2P) || \
	defined(DISABLE_WL_FRAMEBURST_2G20_TDLS) || defined(DISABLE_FRAMEBURST_VSDB)
static void dhd_set_2g20_fb(dhd_pub_t *dhdp)
{
	uint chipid = dhd_bus_chip_id(dhdp);
	if ((chipid == BCM43711_CHIP_ID) ||
		(chipid == BCM43710_CHIP_ID)) {
		dhdp->disable_2g20_fb = TRUE;
	} else {
		dhdp->disable_2g20_fb = FALSE;
	}
...
int dhd_inet6addr_notifier_call(struct notifier_block *this, unsigned long event, void *ptr)
{
	...
	ndo_info = (struct ipv6_work_info_t *)KVZALLOC(sizeof(struct ipv6_work_info_t), GFP_ATOMIC);
	if (!ndo_info) {
...
void dhd_detach(dhd_pub_t *dhdp)
{
	...
	if (dhd->dhd_dpc_dispatcher_work.work.func) {
		cancel_delayed_work_sync(&dhd->dhd_dpc_dispatcher_work);
	}
#ifdef DHD_LB
...
#if defined(DNGL_AXI_ERROR_LOGGING) && defined(DHD_USE_WQ_FOR_DNGL_AXI_ERROR)
	if (dhd->axi_error_dispatcher_work.func) {
		cancel_work_sync(&dhd->axi_error_dispatcher_work);
	}
#endif /* DNGL_AXI_ERROR_LOGGING && DHD_USE_WQ_FOR_DNGL_AXI_ERROR */

#if defined(DHD_FILE_DUMP_EVENT) && defined(DHD_FW_COREDUMP)
	if (dhd->dhd_dump_proc_work.func) {
...
#ifdef WL_CFG80211
	// always do since 'DHD_ATTACH_STATE_CFG80211' have
	// not marked due to 'wl_cfg80211_attach()' failed
	...
	if (dhd->dhd_hang_process_work.func) {
...
#ifdef BTLOG
	/* Wait till bt_log_dispatcher_work finishes */
	if (dhd->bt_log_dispatcher_work.func) {
		cancel_work_sync(&dhd->bt_log_dispatcher_work);
	}
#endif /* BTLOG */

#ifdef EWP_EDL
	if (dhd->edl_dispatcher_work.work.func) {
		cancel_delayed_work_sync(&dhd->edl_dispatcher_work);
	}
#endif
...
#endif /* defined(DHD_TX_PROFILE) */
#ifdef WL_CFGVENDOR_SEND_ALERT_EVENT
	if (dhd->dhd_alert_process_work.func) {
...
int g_dhd_load_retry_cnt = 0;
module_param(g_dhd_load_retry_cnt, int, 0644);
MODULE_PARM_DESC(g_dhd_load_retry_cnt, "For Synaptics internal use only (def : 1)");

void
dhd_load_retry_cnt_set(int retry)
{
	g_dhd_load_retry_cnt = retry;
}

int
dhd_load_retry_cnt_get(void)
{
	return g_dhd_load_retry_cnt;
}
...
_dhd_module_init(void)
{
	...
	do {
		/* Number of retries attempted is max minus current attempt number */
		dhd_load_retry_cnt_set(POWERUP_MAX_RETRY - retry);
...
dhd_os_ioctl_resp_wait(dhd_pub_t *pub, uint *condition)
{
	...
	/* Convert timeout in millsecond to jiffies */
	time_left = msecs_to_jiffies(dhd_ioctl_timeout_msec);

	while (time_left > 0) {
		duration = (time_left >= DHD_QT_IOCTL_POLL_INTERVAL)
			? DHD_QT_IOCTL_POLL_INTERVAL : time_left;
		DHD_ERROR(("Waiting for IOCTL response\n"));
		if (!gdb_debug) {
			timeout = wait_event_timeout(dhd->ioctl_resp_wait, (*condition), duration);
		} else {
			DHD_ERROR(("GDB connected\n"));
			wait_event_interruptible(dhd->ioctl_resp_wait, (*condition));
			timeout = 100;
		}
		/* Positive case when condition is satisfied before timeout */
		if (timeout > 0) {
			DHD_ERROR(("Returning timeout value(Success case) = %d ms\n", timeout));
			return timeout;
		}
		time_left -= duration;
	}
...
dhd_os_busbusy_wait_negation(dhd_pub_t *pub, uint *condition)
{
	...
	if (!gdb_debug)
		...
		wait_event_interruptible(dhd->dhd_bus_busy_state_wait, !(*condition));
		...
		uint bitmask, uint condition)
{
	...
	if (!gdb_debug)
		...
		wait_event_interruptible(dhd->dhd_bus_busy_state_wait,
				((*var & bitmask) == condition));
...
dhd_os_dmaxfer_wait(dhd_pub_t *pub, uint *condition)
{
	...
	if (!gdb_debug)
		...
		wait_event_interruptible(dhd->dmaxfer_wait, (*condition));
...
#if defined(BCMPCIE)
	ret = request_firmware_direct(fw, file_path, dhd_bus_to_dev(g_dhd_pub->bus));
#else
	ret = request_firmware_direct(fw, file_path, osl_get_device(g_dhd_pub->osh));
#endif

	if (ret < 0) {
		DHD_ERROR(("%s: request_firmware_direct err: %d, file_path='%s'\n",
		           __FUNCTION__, ret, file_path));
		if (-ENOENT == ret) {
			DHD_ERROR(("%s: * pleaes check your only provide filename and "
			           "the file is in the kernel specified standard path "
			           "like '/lib/firmware'\n", __FUNCTION__));
		}
...
void dhd_wait_for_event(dhd_pub_t *dhd, bool *lockvar)
{
	...
	if (!gdb_debug)
		...
		wait_event_interruptible(dhdinfo->ctrl_wait, (*lockvar == FALSE));
...
static int
_dhd_apf_config_filter(struct net_device *ndev, uint32 filter_id, uint32 enable)
{
	dhd_info_t *dhd = DHD_DEV_INFO(ndev);
	dhd_pub_t *dhdp = &dhd->pub;
	...
	/* make sure the default pkt filter are sync up */
	if ((enable) && (!dhdp->pktfilter_count)) {
		DHD_ERROR(("%s: *Warning, no default pkt filter!\n", __FUNCTION__));
	}
#ifdef PKT_FILTER_SUPPORT
	dhd_enable_packet_filter(enable, dhdp);
#endif /* PKT_FILTER_SUPPORT */
exit:
	if (buf) {
		MFREE(dhdp->osh, buf, buf_len);
	...
	uint32 program_len)
{
	...
	if (dhdp->in_suspend && dhdp->apf_set &&
		 ...
		 */
		ret = _dhd_apf_config_filter(ndev, PKT_FILTER_APF_ID, TRUE);
		if (ret) {
			goto exit;
		}
	}
...
dhd_dev_apf_disable_filter(struct net_device *ndev)
{
	...
	if (dhdp->apf_set) {
		ret = _dhd_apf_config_filter(ndev, PKT_FILTER_APF_ID, FALSE);
	}
...
exit:
	...
	if (apf_disabled && _dhd_apf_config_filter(ndev, PKT_FILTER_APF_ID, TRUE)) {
...
#if defined(BCM_ROUTER_DHD) || defined(DHD_DEBUG) || defined(DHD_COREDUMP) || \
	defined(DHD_LOG_DUMP)
static int
...
#endif /* BCM_ROUTER_DHD || DHD_DEBUG || DHD_COREDUMP || DHD_LOG_DUMP */

#if defined(DHD_DEBUG) || defined(DHD_COREDUMP) || defined(DHD_LOG_DUMP)
static void
...
#endif /* DHD_DEBUG || DHD_COREDUMP || DHD_LOG_DUMP */
...
#define STORE_WKLOCK_RECORD(wklock_type) \
	do { \
		...
		if (wklock_info) { \
			...
			wklock_info = KVZALLOC(sizeof(*wklock_info), GFP_ATOMIC); \
			if (!wklock_info) {\
...
#ifdef WL_NAN
	cfg = wl_get_cfg(net);
	if (cfg && (net == cfg->nmi_ndev)) {
		WL_ERR(("abort access by nmi iface\n"));
		return -EACCES;
	}
#endif /* WL_NAN */
...
#ifdef PKT_FILTER_SUPPORT
		/* disable pkt filter */
		dhd_enable_packet_filter(FALSE, dhdp);
#endif /* PKT_FILTER_SUPPORT */
			/* Disable MPC */
			powervar = 0;
...
void dhd_set_cpucore(dhd_pub_t *dhd, int set)
{
	...
	if (dhd->current_dpc && DPC_CPUCORE && (DPC_CPUCORE < num_cpus)) {
		DHD_ERROR(("%s: set=%d, DPC_CPUCORE=%d, num_cpus=%d, max_retry=%d\n",
		           __FUNCTION__, set, DPC_CPUCORE, num_cpus,
	...
	if (dhd->current_rxf && RXF_CPUCORE && (RXF_CPUCORE < num_cpus)) {
		DHD_ERROR(("%s: set=%d, RXF_CPUCORE=%d, num_cpus=%d, max_retry=%d\n",
		           __FUNCTION__, set, RXF_CPUCORE, num_cpus,
		           MAX_RETRY_SET_CPUCORE));
		retry_set = 0;
		do {
			if (set == TRUE) {
...
#if defined(CUSTOMER_HW4) || defined(DHD_USE_WARN_RATHER_THAN_BUG_ON)
		WARN_ON(1);
#else // DHD_USE_WARN_RATHER_THAN_BUG_ON
		BUG_ON(1);
#endif /* CUSTOMER_HW4 ||  DHD_USE_WARN_RATHER_THAN_BUG_ON */
	}
...
#else
char map_path[PATH_MAX] = CONFIG_BCMDHD_MAP_PATH;
...
dhd_mem_dump(void *handle, void *event_info, u8 event)
{
...
#if (defined(WL_CFG80211) && defined(DHD_FILE_DUMP_EVENT)) || \
	(defined(DHD_SSSR_DUMP) && defined(DHD_COREDUMP))
	int ret = 0;
	...
	if ((-EOPNOTSUPP) != wifi_platform_set_coredump(dhd->adapter,
	                         dump->buf, dump->bufsize, dhdp->memdump_str)) {
		DHD_ERROR(("%s-%d: writing SoC_RAM dump failed\n", __FUNCTION__, __LINE__));
#ifdef DHD_DEBUG_UART
...
exit:
	...
	/* Work is done, reflect in /sys/wifi */
	dump_in_progress = 0;
...
dhd_create_to_notifier_skt(void)
{
	...
	if (!nl_to_event_sk)
	{
		printf("%s: Error creating NL socket.\n", __FUNCTION__);
		return -1;
	}
	DHD_INFO(("nl_to socket created successfully...\n"));
	return 0;
...
dhd_set_blob_support(dhd_pub_t *dhdp, char *fw_path)
{
	...
	dhd_autosel_blob_name(dhdp, &filepath);
...
#ifdef BCMSDIO
                dhd_bus_update_clm_path(dhdp->bus, filepath);
#endif /* BCMSDIO */
		dhdp->is_blob = TRUE;
...
#endif /* DHD_FW_COREDUMP */
	DHD_OS_WAKE_UNLOCK(dhdp);
#if defined(CUSTOMER_HW4) && defined(DHD_LOG_DUMP)
	if (dhdp->memdump_type != DUMP_MEMFILE_BUGON) {
		WARN_ON(1);
		return;
	}
#endif /* CUSTOMER_HW4 && DHD_LOG_DUMP */
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(4, 14, 0)
					ops->ndo_add_slave(br_dev, dev, NULL);
#else
