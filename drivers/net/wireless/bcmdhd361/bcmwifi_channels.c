...
#ifndef WF_ERROR
#define WF_ERROR(param)    printf param
#endif /* WF_ERROR */
#ifndef WF_TRACE
#define WF_TRACE(param)
#endif /* WF_TRACE */
 ...
 * have the same primary 20MHz channel. This function will get a certain
 * rule to get a predefined sidt band(l for <7) to generate BW40 chanspec.
 *
...
wf_create_chspec_from_primary(uint primary_channel, chanspec_bw_t bw, chanspec_band_t band)
{
	...
	if (bw == WL_CHANSPEC_BW_20) {
		...
		/* guess the sideband when no extra information for BW40 */
		if (13 < primary_channel) { /* exception like JP 14 */
			WF_ERROR(("%s: *Error, 2G CH=%d doesn't support "
			          "BW40, and fallback to BW20!\n",
			          __FUNCTION__, primary_channel));
			chspec = wf_chspec_primary20_chan(chspec);
		} else {
			/* l:    1l/2l/3l/4l           */
			/* l+u: 5l/6l/7l    5u/6u/7u   */
			/* u:    8u/9u/10u/11u/12u/13u */
			/* Note:
			 *   Since we don't have enough information to decide the
			 * side band for channels 5/6/7, so here we may apply a
			 * temporary rule to adopt 5l/6l/7u by default, and this
			 * can be tuned accordingly if needs
			 */
			if (primary_channel < 7) { /* temporary strategy */
				chspec = wf_create_40MHz_chspec_primary_sb(
				                primary_channel,
				                WL_CHANSPEC_CTL_SB_L,
				                WL_CHANSPEC_BAND_2G);
				WF_ERROR(("%s-%d: 2G primary_ch=%d, WL_CHANSPEC_CTL_SB_L,"
				          " new chspec=0x%X\n",
				          __FUNCTION__, __LINE__, primary_channel, chspec));
			} else {
				chspec = wf_create_40MHz_chspec_primary_sb(
				                primary_channel,
				                WL_CHANSPEC_CTL_SB_U,
				                WL_CHANSPEC_BAND_2G);
				WF_ERROR(("%s-%d: 2G primary_ch=%d, WL_CHANSPEC_CTL_SB_U,"
				          " new chspec=0x%X\n",
				          __FUNCTION__, __LINE__, primary_channel, chspec));
			}
		}
...
#endif /* WL_6G_BAND */
	default:
		WF_ERROR(("%s: Invalid Frequency Band\n", __FUNCTION__));
	}
