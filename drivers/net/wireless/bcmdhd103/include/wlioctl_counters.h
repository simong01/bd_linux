 ...
 * Portions generated by GENAI:Cursor Claude Sonnet 4,3.5
 */
...
typedef struct {
	...
	uint32	rxscf_mic_err;		/**< Rx SCF MIC error count */
	uint32	rxscf_pn_err;		/**< Rx SCF PN error count */
	uint32	mba_rxduo_cnt;		/**< MBA Rx DUO count */
	uint32	mba_rxnonduo_cnt;	/**< MBA Rx NON DUO count */
	uint32	ifcs_fail_cnt;		/**< ifcs fail count */
	uint32	ifcs_pass_cnt;		/**< ifcs pass count */
...
typedef struct {
	...
	uint32 goodfcs;			/**< number of rx'd goodfcs cnts */
	uint32 rxdtmcast;		/**< # of rx'd Data mcast frames */
	uint32	dps_ss_lc_exit;		/**< DPS scan Slice LowCap exit count */
	uint32	dps_ss_lc_enter;	/**< DPS scan Slice LowCap entry count */
	uint32	rxscf_mic_err;		/**< Rx SCF MIC error count */
	uint32	rxscf_pn_err;		/**< Rx SCF PN error count */
	uint32	ifcs_fail_cnt;		/**< ifcs fail count */
	uint32	ifcs_pass_cnt;		/**< ifcs pass count */
...
/* Note uint64 struct members below need 8 bytes alignment on ARM platforms */

/* PHY-type specific definitions for HE (802.11ax), EHT (802.11be), and UHR standards */
#define WL_IFSTATS_HE_MCS_NUM			12u
#define WL_IFSTATS_EHT_MCS_NUM			16u
#define WL_IFSTATS_UHR_MCS_NUM			24u

#define WL_IFSTATS_HE_RU_NUM			7u
#define WL_IFSTATS_EHT_RU_NUM			16u
#define WL_IFSTATS_UHR_RU_NUM			16u

#define WL_IFSTATS_HE_BW_NUM			4u /* 20, 40, 80, 160 */
#define WL_IFSTATS_EHT_BW_NUM			5u /* 20, 40, 80, 160, 320 */
#define WL_IFSTATS_UHR_BW_NUM			5u /* 20, 40, 80, 160, 320 */

/* Common across air interfaces >= HE */
#define WL_IFSTATS_NSS_NUM			2u
#define WL_IFSTATS_AMPDU_DENS_NUM		64u
#define WL_IFSTATS_CS_REQD_NUM			2u
#define WL_IFSTATS_AP_TX_PWR_NUM		16u

/* HE (802.11ax) Statistics Structures */
#define WL_IFSTATS_HE_TXSU_VER_1		1u

typedef struct wl_ifstats_he_txsu_v1 {
	uint16	version;
	uint16	length;			/* length of entire structure */
	uint32	txsu_mpdu_data;		/* Acked data MPDUs */
	uint64	txsu_bytes;		/* Byte count */
	uint32	nss[WL_IFSTATS_NSS_NUM];		/* NSS histogram */
	uint32	txsu_mcs_attempt[WL_IFSTATS_HE_MCS_NUM];	/* MCS attempt histogram */
	uint32	txsu_mcs_success[WL_IFSTATS_HE_MCS_NUM];	/* MCS success histogram */
} wl_ifstats_he_txsu_v1_t;

#define WL_IFSTATS_HE_RXSU_VER_1		1u

typedef struct wl_ifstats_he_rxsu_v1 {
	uint16	version;
	uint16	length;			/* length of entire structure */
	uint32	rxsu_ppdu;		/* Count of how many Rx SU PPDU acked */
	uint64	rxsu_bytes;		/* Count of Rx SU data bytes */
	uint32	rxsu_mpdu_data;		/* Count of how many Rx SU MPDU Data frames acked */
	uint32	nss[WL_IFSTATS_NSS_NUM];		/* NSS histogram */
	uint32	rxsu_ampdu_dens[WL_IFSTATS_AMPDU_DENS_NUM];	/* MPDUs within AMPDU */
	uint32	rxsu_mcs_success[WL_IFSTATS_HE_MCS_NUM];	/* acked MPDU at MCS */
	uint32	rxsu_bw[WL_IFSTATS_HE_BW_NUM];	/* PPDU BW 20, 40, 80, 160 */
} wl_ifstats_he_rxsu_v1_t;

/* HE ULOFDMA, DLOFDMA and DLMUMIMO structures are present for future use, if required.
 * Data for these structures is not currently reported as part of ifstats.
 * On EHT/UHR capable devices, HE UL/DL OFDMA and DLMUMIMO stats are reported as part of
 * the respective EHT/UHR structures.
 */
#define WL_IFSTATS_HE_DLMUMIMO_VER_1		1u

typedef struct wl_ifstats_he_dlmumimo_v1 {
	uint16	version;
	uint16	length;			/* length of entire structure */
	uint32	dlmumimo_ppdu;		/* Count of how many Rx MU-MIMO PPDU acked */
	uint64	dlmumimo_bytes;		/* Count of Rx MU-MIMO data bytes */
	uint32	dlmumimo_mpdu_data;	/* Count of how many Rx MU-MIMO MPDU Data frames acked */
	uint32	nss[WL_IFSTATS_NSS_NUM];		/* NSS histogram */
	uint32	dlmumimo_ampdu_dens[WL_IFSTATS_AMPDU_DENS_NUM];	/* MPDUs in AMPDU */
	uint32	dlmumimo_mcs_success[WL_IFSTATS_HE_MCS_NUM];	/* acked MPDUs at MCS */
} wl_ifstats_he_dlmumimo_v1_t;

#define WL_IFSTATS_HE_DLOFDMA_VER_1		1u

typedef struct wl_ifstats_he_dlofdma_v1 {
	uint16	version;
	uint16	length;			/* length of entire structure */
	uint32	dlofdma_ppdu;		/* Count of how many Rx PPDU received */
	uint64	dlofdma_bytes;		/* Count of Rx OFDMA data bytes */
	uint32	dlofdma_mpdu_data;	/* Count of how many Rx MPDU Data */
	uint32	nss[WL_IFSTATS_NSS_NUM];		/* NSS histogram */
	uint32	dlofdma_ampdu_dens[WL_IFSTATS_AMPDU_DENS_NUM];	/* Count of MPDU within AMPDU */
	uint32	dlofdma_mcs_success[WL_IFSTATS_HE_MCS_NUM];	/* Successfully acked MPDU at MCS */
	/* Allocation of RU on BW basis. Counts PPDUs */
	uint32	dlofdma_bwru[WL_IFSTATS_HE_BW_NUM][WL_IFSTATS_HE_RU_NUM];
} wl_ifstats_he_dlofdma_v1_t;

#define WL_IFSTATS_HE_ULOFDMA_VER_1		1u

typedef struct wl_ifstats_he_ulofdma_v1 {
	uint16	version;
	uint16	length;			/* length of entire structure */
	uint32	ulofdma_ppdu;		/* Tx TB PPDUs */
	uint64	ulofdma_tb_bytes;	/* Tx TB UL data bytes */
	uint32	ulofdma_mpdu_data;	/* Tx MPDU Data acked */
	uint32	nss[WL_IFSTATS_NSS_NUM];		/* Number of spatial streams used */
	uint32	cs_reqd[WL_IFSTATS_CS_REQD_NUM];	/* CS reqd: Yes/No */
	uint32	ulofdma_ampdu_dens[WL_IFSTATS_AMPDU_DENS_NUM];	/* Count of MPDU within AMPDU */
	uint32	ulofdma_request_power[WL_IFSTATS_AP_TX_PWR_NUM];	/* Requested AP Tx Power */
	uint32	ulofdma_padding_bytes;	/* Number of octets used in Padding during OFDMA */
	uint32	ulofdma_mcs_assigned_hist[WL_IFSTATS_HE_MCS_NUM];	/* Assigned MCS */
	uint32	ulofdma_mcs_success[WL_IFSTATS_HE_MCS_NUM];	/* Successfully acked MPDU at MCS */
	uint32	ulofdma_bwru[WL_IFSTATS_HE_BW_NUM][WL_IFSTATS_HE_RU_NUM];
} wl_ifstats_he_ulofdma_v1_t;

/* EHT (802.11be) Statistics Structures */

#define WL_IFSTATS_EHT_TXSU_VER_1		1u

typedef struct wl_ifstats_eht_txsu_v1 {
	uint16	version;
	uint16	length;			/* length of entire structure */
	uint32	txsu_mpdu_data;		/* Acked data MPDUs */
	uint64	txsu_bytes;		/* Byte count */
	uint32	nss[WL_IFSTATS_NSS_NUM];		/* NSS histogram */
	uint32	txsu_mcs_attempt[WL_IFSTATS_EHT_MCS_NUM];	/* MCS attempt histogram */
	uint32	txsu_mcs_success[WL_IFSTATS_EHT_MCS_NUM];	/* MCS success histogram */
} wl_ifstats_eht_txsu_v1_t;

#define WL_IFSTATS_EHT_RXSU_VER_1		1u

typedef struct wl_ifstats_eht_rxsu_v1 {
	uint16	version;
	uint16	length;			/* length of entire structure */
	uint32	rxsu_ppdu;		/* Count of how many Rx PPDU received */
	uint64	rxsu_bytes;		/* Count of RX SU data bytes */
	uint32	rxsu_mpdu_data;		/* Count of how many Rx SU MPDU Data frames acked */
	uint32	nss[WL_IFSTATS_NSS_NUM];		/* NSS histogram */
	uint32	rxsu_ampdu_dens[WL_IFSTATS_AMPDU_DENS_NUM]; /* Count of MPDU within AMPDU */
	uint32	rxsu_mcs_success[WL_IFSTATS_EHT_MCS_NUM]; /* acked MPDUs at MCS */
	uint32	rxsu_bw[WL_IFSTATS_EHT_BW_NUM];	/* PPDU BW 20, 40, 80, 160, 320 */
} wl_ifstats_eht_rxsu_v1_t;

#define WL_IFSTATS_EHT_DLMUMIMO_VER_1		1u

typedef struct wl_ifstats_eht_dlmumimo_v1 {
	uint16	version;
	uint16	length;			/* length of entire structure */
	uint32	dlmumimo_ppdu;		/* Count of how many Rx MU-MIMO PPDU acked */
	uint64	dlmumimo_bytes;		/* Count of Rx MU-MIMO data bytes */
	...
	uint32	nss[WL_IFSTATS_NSS_NUM];		/* NSS histogram */
	uint32	dlmumimo_ampdu_dens[WL_IFSTATS_AMPDU_DENS_NUM];	/* MPDUs in AMPDU */
	uint32	dlmumimo_mcs_success[WL_IFSTATS_EHT_MCS_NUM];	/* acked MPDUs at MCS */
} wl_ifstats_eht_dlmumimo_v1_t;

#define WL_IFSTATS_EHT_DLOFDMA_VER_1		1u

typedef struct wl_ifstats_eht_dlofdma_v1 {
	uint16	version;
	uint16	length;			/* length of entire structure */
	uint32	dlofdma_ppdu;		/* Count of how many Rx PPDU received */
	uint64	dlofdma_bytes;		/* Count of Rx OFDMA data bytes */
	...
	uint32	nss[WL_IFSTATS_NSS_NUM];		/* NSS histogram */
	uint32	dlofdma_ampdu_dens[WL_IFSTATS_AMPDU_DENS_NUM];	/* Count of MPDU within AMPDU */
	uint32	dlofdma_mcs_success[WL_IFSTATS_EHT_MCS_NUM];	/* Successfully acked MPDU at MCS */
	/* Allocation of RU on BW basis. Counts PPDUs */
	uint32	dlofdma_bwru[WL_IFSTATS_EHT_BW_NUM][WL_IFSTATS_EHT_RU_NUM];
} wl_ifstats_eht_dlofdma_v1_t;

#define WL_IFSTATS_EHT_ULOFDMA_VER_1		1u

typedef struct wl_ifstats_eht_ulofdma_v1 {
	uint16	version;
	uint16	length;			/* length of entire structure */
	uint32	ulofdma_ppdu;		/* Tx TB PPDUs */
	uint64	ulofdma_tb_bytes;	/* Tx TB UL data bytes */
	...
	uint32	ulofdma_request_power[WL_IFSTATS_AP_TX_PWR_NUM];	/* Requested AP Tx Power */
	uint32	ulofdma_padding_bytes;	/* Number of octets used in Padding during OFDMA */
	uint32	ulofdma_mcs_assigned_hist[WL_IFSTATS_EHT_MCS_NUM];	/* Assigned MCS */
	uint32	ulofdma_mcs_success[WL_IFSTATS_EHT_MCS_NUM];	/* Successfully acked MPDU at MCS */
	uint32	ulofdma_bwru[WL_IFSTATS_EHT_BW_NUM][WL_IFSTATS_EHT_RU_NUM];
} wl_ifstats_eht_ulofdma_v1_t;

/* UHR Statistics Structures */
#define WL_IFSTATS_UHR_TXSU_VER_1		1u

typedef struct wl_ifstats_uhr_txsu_v1 {
	uint16	version;
	uint16	length;			/* length of entire structure */
	uint32	txsu_mpdu_data;		/* Acked data MPDUs */
	uint64	txsu_bytes;		/* Byte count */
	uint32	nss[WL_IFSTATS_NSS_NUM];		/* NSS histogram */
	uint32	txsu_mcs_attempt[WL_IFSTATS_UHR_MCS_NUM];	/* MCS attempt histogram */
	uint32	txsu_mcs_success[WL_IFSTATS_UHR_MCS_NUM];	/* MCS success histogram */
} wl_ifstats_uhr_txsu_v1_t;

#define WL_IFSTATS_UHR_RXSU_VER_1		1u

typedef struct wl_ifstats_uhr_rxsu_v1 {
	uint16	version;
	uint16	length;			/* length of entire structure */
	uint32	rxsu_ppdu;		/* Count of how many Rx SU PPDU acked */
	uint64	rxsu_bytes;		/* Count of Rx SU data bytes */
	...
	uint32	nss[WL_IFSTATS_NSS_NUM];		/* NSS histogram */
	uint32	rxsu_ampdu_dens[WL_IFSTATS_AMPDU_DENS_NUM];	/* MPDUs within AMPDU */
	uint32	rxsu_mcs_success[WL_IFSTATS_UHR_MCS_NUM];	/* acked MPDU at MCS */
	uint32	rxsu_bw[WL_IFSTATS_UHR_BW_NUM];	/* PPDU BW 20, 40, 80, 160, 320 */
} wl_ifstats_uhr_rxsu_v1_t;

#define WL_IFSTATS_UHR_DLMUMIMO_VER_1		1u

typedef struct wl_ifstats_uhr_dlmumimo_v1 {
	uint16	version;
	uint16	length;			/* length of entire structure */
	uint32	dlmumimo_ppdu;		/* Count of how many Rx MU-MIMO PPDU acked */
	uint64	dlmumimo_bytes;		/* Count of Rx MU-MIMO data bytes */
	...
	uint32	nss[WL_IFSTATS_NSS_NUM];		/* NSS histogram */
	uint32	dlmumimo_ampdu_dens[WL_IFSTATS_AMPDU_DENS_NUM];	/* MPDUs in AMPDU */
	uint32	dlmumimo_mcs_success[WL_IFSTATS_UHR_MCS_NUM];	/* acked MPDUs at MCS */
} wl_ifstats_uhr_dlmumimo_v1_t;

#define WL_IFSTATS_UHR_DLOFDMA_VER_1		1u

typedef struct wl_ifstats_uhr_dlofdma_v1 {
	uint16	version;
	uint16	length;			/* length of entire structure */
	uint32	dlofdma_ppdu;		/* Count of how many Rx PPDU received */
	uint64	dlofdma_bytes;		/* Count of Rx OFDMA data bytes */
	...
	uint32	nss[WL_IFSTATS_NSS_NUM];		/* NSS histogram */
	uint32	dlofdma_ampdu_dens[WL_IFSTATS_AMPDU_DENS_NUM];	/* Count of MPDU within AMPDU */
	uint32	dlofdma_mcs_success[WL_IFSTATS_UHR_MCS_NUM];	/* Successfully acked MPDU at MCS */
	/* Allocation of RU on BW basis. Counts PPDUs */
	uint32	dlofdma_bwru[WL_IFSTATS_UHR_BW_NUM][WL_IFSTATS_UHR_RU_NUM];
} wl_ifstats_uhr_dlofdma_v1_t;

#define WL_IFSTATS_UHR_ULOFDMA_VER_1		1u

typedef struct wl_ifstats_uhr_ulofdma_v1 {
	uint16	version;
	uint16	length;			/* length of entire structure */
	uint32	ulofdma_ppdu;		/* Tx TB PPDUs */
	uint64	ulofdma_tb_bytes;	/* Tx TB UL data bytes */
	...
	uint32	ulofdma_request_power[WL_IFSTATS_AP_TX_PWR_NUM];	/* Requested AP Tx Power */
	uint32	ulofdma_padding_bytes;	/* Number of octets used in Padding during OFDMA */
	uint32	ulofdma_mcs_assigned_hist[WL_IFSTATS_UHR_MCS_NUM];	/* Assigned MCS */
	uint32	ulofdma_mcs_success[WL_IFSTATS_UHR_MCS_NUM];	/* Successfully acked MPDU at MCS */
	uint32	ulofdma_bwru[WL_IFSTATS_UHR_BW_NUM][WL_IFSTATS_UHR_RU_NUM];
} wl_ifstats_uhr_ulofdma_v1_t;

/* RX Trigger Buffer (RXTB) stats version 1 */
#define WL_IFSTATS_RXTB_VER_1			1u

typedef struct {
	uint16	version;
	uint16	length;			/* length of entire structure */
	/* The following ones are in sync with FW-ucode interface */
	uint32	myaid;			/* Trigger frames with my aid */
	uint32	noaid;			/* Trigger frames seen for aid =0 */
	uint32	basic;			/* Basic Trigger frames */
	uint32	bfrp;			/* BFRP Trigger frames */
	uint32	mu_bar;			/* MU-BAR Trigger frames */
	uint32	mu_rts;			/* MU-RTS Trigger frames */
	uint32	bsrp;			/* BSRP Trigger frames */
	uint32	gcr_mu_bar;		/* GCR MU-BAR Trigger frames */
	uint32	bqrp;			/* BQRP Trigger frames */
	uint32	ndp_feedback_poll_nfrp;	/* NDP Feedback Poll Trigger frames */
} wl_ifstats_rxtb_v1_t;

/* Action frame stats version 1 */
#define WL_IFSTATS_ACTION_VER_1			1u

typedef struct {
	uint16	version;
	uint16	length;				/* length of entire structure */
	/* RX Action frame counters */
	uint32	rx_addba_req;			/* RX Action frame with ADDBA request */
	uint32	rx_addba_resp;			/* RX Action frame with ADDBA response */
	uint32	rx_delba;			/* RX Action frame with DELBA request */
	/* TX Action frame counters */
	uint32	tx_addba_req;			/* TX Action frame with ADDBA request */
	uint32	tx_addba_resp;			/* TX Action frame with ADDBA response */
	uint32	tx_delba;			/* TX Action frame with DELBA request */
} wl_ifstats_action_v1_t;
