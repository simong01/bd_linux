...
{
	uint32 val, i, lsc;
	const uint16 cfg_offset[] = {PCIECFGREG_STATUS_CMD, PCIECFGREG_PM_CSR,
	...
	W_REG(osh, PCIE_REG_ADDR(pcieregs, ConfigIndAddr), PCIECFGREG_LINK_STATUS_CTRL);
	lsc = R_REG(osh, PCIE_REG_ADDR(pcieregs, ConfigIndData));
	val = lsc & (~PCIE_ASPM_ENAB);
	W_REG(osh, PCIE_REG_ADDR(pcieregs, ConfigIndData), val);
	...
	if (CCREV(sih->ccrev) >= 65) {
		...
		si_corereg_writeonly(sih, SI_CC_IDX, CC_REG_OFF(WatchdogCounter), ~0, 4);
	...
	W_REG(osh, PCIE_REG_ADDR(pcieregs, ConfigIndAddr), PCIECFGREG_LINK_STATUS_CTRL);
	W_REG(osh, PCIE_REG_ADDR(pcieregs, ConfigIndData), lsc);
	...
	if (sih->buscorerev <= 13) {
		...
		for (i = 0; i < ARRAYSIZE(cfg_offset); i++) {
			W_REG(osh, PCIE_REG_ADDR(pcieregs, ConfigIndAddr), cfg_offset[i]);
			val = R_REG(osh, PCIE_REG_ADDR(pcieregs, ConfigIndData));
			W_REG(osh, PCIE_REG_ADDR(pcieregs, ConfigIndData), val);
		}
	}
...
{
	const sbpcieregs_t *pcie = NULL;
	uint crwlpciegen2_117_disable = 0;
	uint32 origidx = si_coreidx(sih);

	crwlpciegen2_117_disable = PCIE_PipeIddqDisable0 | PCIE_PipeIddqDisable1;
	/* Switch to PCIE2 core */
	pcie = (const sbpcieregs_t *)si_setcore(sih, PCIE2_CORE_ID, 0);
	BCM_REFERENCE(pcie);
	ASSERT(pcie != NULL);

	OR_REG(osh, PCIE_REG_ADDR(sbpcieregs, pciecontrol), crwlpciegen2_117_disable);

	si_setcoreidx(sih, origidx);
}

#define PCIE_PMCR_REFUP_MASK 0x3f0001e0u
#define PCIE_PMCR_REFEXT_MASK 0x400000u
#define PCIE_PMCR_REFUP_100US 0x38000080u
#define PCIE_PMCR_REFEXT_100US 0x400000u

/* Set PCIE TRefUp time to 100us */
void pcie_set_trefup_time_100us(si_t *sih)
{
	si_corereg(sih, sih->buscoreidx,
		PCIE_REG_OFF(ConfigIndAddr), ~0, PCI_PMCR_REFUP);
	si_corereg(sih, sih->buscoreidx,
		PCIE_REG_OFF(ConfigIndData), PCIE_PMCR_REFUP_MASK, PCIE_PMCR_REFUP_100US);

	si_corereg(sih, sih->buscoreidx,
		PCIE_REG_OFF(ConfigIndAddr), ~0, PCI_PMCR_REFUP_EXT);
	si_corereg(sih, sih->buscoreidx,
		PCIE_REG_OFF(ConfigIndData), PCIE_PMCR_REFEXT_MASK, PCIE_PMCR_REFEXT_100US);
}
