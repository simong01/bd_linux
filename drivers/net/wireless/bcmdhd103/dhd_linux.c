...
extern uint dhd_use_tcp_window_size_adjust;
...
#define RX_HC_RTS_CTS_NOUCAST_INTERVAL 5u /* sec */
uint dhd_rx_hc_rts_cts_noucast_interval = RX_HC_RTS_CTS_NOUCAST_INTERVAL;
module_param(dhd_rx_hc_rts_cts_noucast_interval, uint, 0644);
...
#define EVENTMSGS_EXT_STRUCT_SIZE_FLEX0        ((uint)(sizeof(eventmsgs_ext_t)))
...
int dhd_config_twt_event_mask_in_suspend(dhd_pub_t *dhdp, bool suspend)
{
	...
	int msglen = WL_EVENTING_MASK_EXT_LEN + EVENTMSGS_EXT_STRUCT_SIZE_FLEX0;
...
bool dhd_check_if_role(dhd_pub_t *dhdp, int ifidx, uint8 type)
{
	...
	if (!dhdp->info->iflist[ifidx])
		return FALSE;

	return (dhdp->info->iflist[ifidx]->role == type);
...
void dhd_set_if_role(dhd_pub_t *dhdp, int ifidx, uint8 type)
{
	...
	dhdp->info->iflist[ifidx]->role = type;
}
...
#ifdef SYNA_FW_PKT_FWD_DISABLED
	dhd_set_if_role(&dhd->pub, ifidx, if_event->event.role);
#endif /* SYNA_FW_PKT_FWD_DISABLED */
...
#if defined(DHD_HWTSTAMP) || defined(DHD_HWTSTAMP_PCIE)
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6, 11, 0))
static int
dhd_ethtool_get_ts_info(struct net_device *net, struct kernel_ethtool_ts_info *info)
{
...
#endif /* DHD_HWTSTAMP || DHD_HWTSTAMP_PCIE */
...
dhd_dbg_monitor_pkt(dhd_pub_t *dhdp, host_rxbuf_cmpl_t *msg, void *pkt, int ifidx)
{
	...
	wpa_sup = !!(hdr.flags & WL_AML_F_MGMT);
	...
	if (hdr.version == WL_AML_HEADER_VERSION) {
		if (hdr.flags & WL_AML_F_MGMT) {
			...
			if (hdr.flags & WL_AML_F_DIRECTION) {
				...
				DHD_DBG_PKT_MON_TX(dhdp, pkt, 0,
					FRAME_TYPE_80211_MGMT, (uint8)ack, TRUE);
			} else {
				DHD_DBG_PKT_MON_RX(dhdp, (struct sk_buff *)pkt,
					FRAME_TYPE_80211_MGMT, TRUE);
			}
			return TRUE;
		}
	}
	...
	struct net_device *dev)
{
	...
	if (rate_found) {
		...
		if (rate_flags & IEEE80211_TX_RC_MCS) {
			...
			DHD_TRACE(("%s-%d, MCS case. rate_flags: 0x%x, rate: %d\n",
				__func__, __LINE__, rate_flags, rate));
		} else if (rate_flags & IEEE80211_TX_RC_VHT_MCS) {
			/* reset antennas if not enough */
			if (vht_nss > hweight8(info->control.antennas))
				info->control.antennas = 0;
			DHD_TRACE(("%s, VHT case. rate_flags: 0x%x, vht_mcs: %d, vht_nss: %d\n",
...
BCMFASTPATH(dhd_monitor_start_xmit)(struct sk_buff *skb, struct net_device *ndev)
{
	...
	/* skip radiotap header and add Ehternet header */
	{
...
#else /* DHD_WFB */
static netdev_tx_t
dhd_monitor_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
...
dhd_del_monitor_if(dhd_info_t *dhd)
{
	...
	dev_priv->dhd = (dhd_info_t *)NULL;
	memset(&dev_priv->stats, 0, sizeof(dev_priv->stats));
	memset(dhd->monitor_type, 0, DHD_MAX_IFS*sizeof(dhd->monitor_type[0]));
#endif /* WL_CFG80211_MONITOR */
...
dhd_pri_stop(struct net_device *net)
{
	...
	/*
	 * The timing of these DAL API calls might be adjusted in the future
	 * to support dynamic switching between offload mode and bypass mode.
	 */
	platform_bus_stop(dhd->pub.bus);
	platform_bus_exit(dhd->pub.bus);
...
#ifdef PCIE_FULL_DONGLE
	dhd_flow_reset_op_flags(dhdpub, ifidx);
#endif /* PCIE_FULL_DONGLE */
...
dhd_remove_static_if(dhd_pub_t *dhdpub, struct net_device *ndev, bool need_rtnl_lock)
{
	...
	if (static_ifp != NULL) {
		...
		if (ifp) {
			...
			DHD_PRINT(("%s: ifp reference present for %s. clean up\n",
...
#ifdef CONFIG_BCMDHD_DAL
extern struct platform_bus_ops brcm_bus_ops;
#endif /* CONFIG_BCMDHD_DAL */
...
dhd_bus_start(dhd_pub_t *dhdp)
{
	 ...
	 * to support dynamic switching between offload mode and bypass mode.
	 */
	platform_bus_init(dhd_bus_to_pdev(dhdp->bus), dhdp->bus, &brcm_bus_ops);
	platform_bus_request_irq(dhdp->bus);
	platform_bus_start(dhdp->bus);
#endif /* CONFIG_BCMDHD_DAL */
...
dhd_optimised_preinit_ioctls(dhd_pub_t *dhd)
{
	...
	char iovbuf[WL_EVENTING_MASK_EXT_LEN + EVENTMSGS_EXT_STRUCT_SIZE_FLEX0 + 16];
...
// mark flag for later blob checking
	...
	*/
	...
	if (FW_SUPPORTED(dhd, igmpoe)) {
		...
		DHD_PRINT(("%s: IGMP Offload is enabled in FW cap\n", __FUNCTION__));
	}
#endif /* IGMP_OFFLOAD_SUPPORT */
...
#if defined(DISABLE_SCAN_FILTER_6G_SPVLP)
	val = 0;
	ret2 = dhd_iovar(dhd, 0, "scan_filter_6g_spvlp", (char *)&val,
		sizeof(val), NULL, 0, TRUE);
	if (ret2 < 0) {
		DHD_ERROR(("%s: failed to disable scan_filter_6g_spvlp ret=%d\n",
			__FUNCTION__, ret2));
	}
#endif /* DISABLE_SCAN_FILTER_6G_SPVLP */
...
/* andrey tmp fix for dk8000 build error  */
	...
	msglen = WL_EVENTING_MASK_EXT_LEN + EVENTMSGS_EXT_STRUCT_SIZE_FLEX0;
...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
	...
	if (wlfc_enable)
		...
		ret_simutx = dhd_iovar(dhd, 0, "bus:simutx_limit", NULL, 0,
			(char *)&simutx_limit, sizeof(simutx_limit), FALSE);

		if ((ret_simutx == BCME_OK) && (simutx_limit != 0)) {
...
#ifdef APF
	dhd_dev_apf_delete_filter(ifp->net);
#endif /* APF */
#ifdef CUSTOM_EVENT_PM_WAKE
	/* Clear EXCESS_PM_PERIOD explicitly when Wi-Fi turn off */
	dhd_set_excess_pm_awake(&dhd->pub, FALSE);
...
#ifdef APF_DBG
void
dhd_apf_buffer_print(u8 *pc, u32 plen)
{
	u32 i = 0;
	int offset = 0;
	char *buf = (char *)get_zeroed_page(GFP_KERNEL);
	if (buf) {
		offset += snprintf(buf + offset, PAGE_SIZE - offset, "APF program: ");
		do {
			for (; pc && (i < plen) && (offset <= PAGE_SIZE - 3); pc++, i++) {
				// Append each byte value to the buffer
				offset += snprintf(buf + offset, PAGE_SIZE - offset, "%02x", *pc);
			}
			DHD_APF_PRINT(("%s\n", buf));
			buf[0] = '\0';
			offset = 0;
...
#ifdef APF_DBG
	/* dump apf info */
	DHD_APF_PRINT(("%s enter\n", __FUNCTION__));
	dhd_apf_buffer_print(program, program_len);
#endif /* APF_DBG */
...
#if defined(BCMPCIE)
	UNUSED_PARAMETER(trap_code);
	UNUSED_PARAMETER(trap_subcode);
#endif /* BCMPCIE */
	UNUSED_PARAMETER(uc_status);
#endif /* DHD_COREDUMP */
...
#ifdef DHD_PKTTS
/* Getting build error for ipv6_find_hdr in HW2 sdk, temporarily disable it */
#ifndef CUSTOMER_HW2
/**
...
dhd_send_msg_to_ts(struct sk_buff *skb, void *data, int size)
{
	...
	ret = nlmsg_unicast(nl_to_ts, skb_out, sender_pid_ts);
	if (ret < 0) {
...
dhd_set_pktts_flow(dhd_pub_t *dhdp, void *params, int plen)
{
	...
	if (config->pkt_offset == PKTTS_OFFSET_INVALID) {
		...
		bzero(&dhd->config[idx], sizeof(dhd->config[idx]));
	} else {
