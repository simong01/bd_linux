...
#else
#define BLOCKABLE()	(!in_interrupt()) /* Doesn't handle CONFIG_PREEMPT? */
#endif
...
#ifdef BCMSPI_ANDROID /* BCMSPI_ANDROID */
...
spi_sendrecv(sdioh_info_t *sd, uint8 *msg_out, uint8 *msg_in, int msglen)
{
	...
	bzero(t, 2 * sizeof(struct spi_transfer));

	if (sd->wordlen == 2)
#if !(defined(SPI_PIO_RW_BIGENDIAN) && defined(SPI_PIO_32BIT_RW))
		write = msg_out[2] & 0x80;	/* bit 7: read:0, write :1 */
#else
		write = msg_out[1] & 0x80;	/* bit 7: read:0, write :1 */
#endif /* !(defined(SPI_PIO_RW_BIGENDIAN) && defined(SPI_PIO_32BIT_RW)) */
	if (sd->wordlen == 4)
#if !(defined(SPI_PIO_RW_BIGENDIAN) && defined(SPI_PIO_32BIT_RW))
		write = msg_out[0] & 0x80;	/* bit 7: read:0, write :1 */
#else
		write = msg_out[3] & 0x80;	/* bit 7: read:0, write :1 */
#endif /* !(defined(SPI_PIO_RW_BIGENDIAN) && defined(SPI_PIO_32BIT_RW)) */
