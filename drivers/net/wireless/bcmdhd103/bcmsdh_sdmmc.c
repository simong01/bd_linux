...
#endif /* DYNAMIC_F2_BLKSIZE_FOR_NONLEGACY */
...
sdioh_attach(osl_t *osh, struct sdio_func *func)
{
	...
	/* 430132E only support clk rate up to SDR50/80Mhz */
		...
		if (sdmmc_get_clock_rate(sd) > 80000000) {
			/* Current clk is larger than 80Mhz, reduce it to 80Mhz. */
			sdmmc_set_clock_rate(sd, 80000000);
		}

		/* Save the device ID for Wi-Fi reset next time */
		sd->func[0]->device = func->device;
	/* 43436s/43436p/43436pp only support clk rate up to SDR25/50Mhz */
	} else if (func->device == BCM43430_CHIP_ID) {
		sdioh_set_driver_strength(sd->func[0], 0x10);
		if (sdmmc_get_timing(sd) > MMC_TIMING_UHS_SDR25) {
			/* Current timing is higher than SDR25, reduce timing to SDR25. */
			sdmmc_set_timing(sd, MMC_TIMING_UHS_SDR25);
		}
                           ...
                           void *params, int plen, void *arg, uint len, bool set)
{
	...
	/* Copied from dhd, copied from wl; certainly overkill here? */
	...
	case IOV_SVAL(IOV_BLOCKSIZE):
	{
		...
		/* These hardcoded sizes are a hack, remove after proper CIS parsing. */
		switch (func) {
	...
	{
		/* TODO: Should copy for alignment reasons */
...
exit:
	...
	/* Remove protective lock after clients all clean... */
	return bcmerror;
}

#if defined(OOB_INTR_ONLY) && defined(HW_OOB)
/*
 * dhd -i eth0 sd_devreg 0 0xf2 0x3
 */
...
sdioh_cis_read(sdioh_info_t *sd, uint func, uint8 *cisd, uint32 length)
{
	...
	sd_print(("%s: func_cis_ptr[%d]=0x%04x\n", __FUNCTION__, func, sd->func_cis_ptr[func]));
...
sdioh_request_byte(sdioh_info_t *sd, uint rw, uint func, uint regaddr, uint8 *byte)
{
	...
	if (err_ret) {
		if ((regaddr == 0x1001F) && ((err_ret == -ETIMEDOUT) || (err_ret == -EILSEQ))) {
			/*	Read/Write to SBSDIO_FUNC1_SLEEPCSR could return -110(timeout)
...
sdioh_abort(sdioh_info_t *sd, uint func)
{
	...
	/* Standard Linux SDIO Stack cannot perform an abort. */
#if defined(MMC_SDIO_ABORT)
...
static void IRQHandler(struct sdio_func *func)
{
	...
	if (sd->use_client_ints) {
		...
		/* Do not remove these sd_err messages.  Need to figure
			...
			*/
			if ((ret = sdio_sw_reset(sd, sd->func[0]->card))) {
				...
				/* 430132E only support clk rate up to 80Mhz */
					...
					if (sdmmc_get_clock_rate(sd) > 80000000) {
						/* Current clk is larger than 80Mhz,
						 * reduce it to 80Mhz.
						 */
						sdmmc_set_clock_rate(sd, 80000000);
					}
				/* 43436s/43436p/43436pp only support clk rate up to 50Mhz */
				} else if (sd->func[0]->device == BCM43430_CHIP_ID) {
					sdioh_set_driver_strength(sd->func[0], 0x10);
					if (sdmmc_get_timing(sd) > MMC_TIMING_UHS_SDR25) {
						/* Current timing is higher than SDR25,
						 * reduce timing to SDR25.
						 */
						sdmmc_set_timing(sd, MMC_TIMING_UHS_SDR25);
					}
...
sdmmc_set_clock_rate(sdioh_info_t *sd, uint hz)
{
	...
	if (sdio_func->device != BCM4381_CHIP_ID) {
		if (hz > host->f_max) {
