...
/** function name could be more descriptive, eg use 'tx' and 'flow ring' in name */
{
...

	{
		...
		while ((txp = dhd_flow_queue_dequeue(bus->dhd, queue)) != NULL) {
			...
			ret = platform_bus_tx((void *)bus->dhd, txp, ifidx);
#else
...
#endif /* DHD_HP2P */
#ifdef CONFIG_BCMDHD_DAL
	more |= platform_bus_tx_cpl((void *)bus->dhd, &txcpl_items);
#else
...
#endif /* DHD_MESH */
#ifdef CONFIG_BCMDHD_DAL
	more |= platform_bus_rx(bus->dhd, &rxcpl_items);
#else
	more |= dhd_prot_process_msgbuf_rxcpl(bus->dhd, DHD_REGULAR_RING, &rxcpl_items);
#endif /* CONFIG_BCMDHD_DAL */
#define SECOND_FROM_NS	1000000000 /* 1 second */
	bus->last_process_rxcpl_time = OSL_LOCALTIME_NS();
	if ((bus->last_process_rxcpl_time - bus->last_rx_measure_time) >
		SECOND_FROM_NS) {
		bus->dhd->curr_rxcpl = bus->tot_rxcpl - bus->old_rxcpl;
		bus->old_rxcpl = bus->tot_rxcpl;
		bus->last_rx_measure_time = bus->last_process_rxcpl_time;
	}
