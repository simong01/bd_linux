	...
	while (cnt--) {
		if (bcmp(mgmt->list[cnt].oui, WFA_OUI, WFA_OUI_LEN) == 0) {
			...
			case RSN_AKM_SAE_PSK:
				...
				break;
			case RSN_AKM_SUITEB_SHA256_1X:
				wpa_auth |= WPA3_AUTH_1X_SUITE_B_SHA256;
				break;
...
#elif LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 0) && LINUX_VERSION_CODE > KERNEL_VERSION(5, 15, 140)
