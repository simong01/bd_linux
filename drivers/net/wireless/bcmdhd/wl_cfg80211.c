...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 0) && LINUX_VERSION_CODE > KERNEL_VERSION(5, 15, 140)
static int wl_cfg80211_config_default_key(struct wiphy *wiphy,
	struct net_device *dev,
	int link_id, u8 key_idx, bool unicast, bool multicast);
static int wl_cfg80211_add_key(struct wiphy *wiphy, struct net_device *dev,
	int link_id, u8 key_idx, bool pairwise, const u8 *mac_addr,
	struct key_params *params);
static int wl_cfg80211_del_key(struct wiphy *wiphy, struct net_device *dev,
	int link_id, u8 key_idx, bool pairwise, const u8 *mac_addr);
static int wl_cfg80211_get_key(struct wiphy *wiphy, struct net_device *dev,
	int link_id, u8 key_idx, bool pairwise, const u8 *mac_addr,
...
extern void dhd_poweroff_wowl(struct net_device *net);
	...
	u16 reason_code)
{
	...
	if (dhd_force_no_off) {
		WL_ERR(("%s:WLAN won't power-off, when platform power-off!\n", __func__));
		/* Since no "device stop" or "suspend" process are invoked during power-off,
			we have to add power-off wowl setting process here.
		*/
		dhd_poweroff_wowl(dev);
		return BCME_OK;
	}
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 0) && LINUX_VERSION_CODE > KERNEL_VERSION(5, 15, 140) || defined(CFG80211_BKPORT_MLO)
	roam_info.links[0].channel = notify_channel;
	roam_info.links[0].bssid = curbssid;
#else
	roam_info.channel = notify_channel;
	roam_info.bssid = curbssid;
#endif /* LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 0) || defined(CFG80211_BKPORT_MLO) */
...
wl_update_twt_setup_evt_info(struct sk_buff *skb, void *event_data)
{
	...
	err = nla_put_u8(skb, WIFI_TWT_ATTR_SETUP_CMD, sdesc->setup_cmd);
	if (unlikely(err)) {
		WL_ERR(("nla_put_u8 WIFI_TWT_ATTR_SETUP_CMD failed\n"));
		goto fail;
	}
	err = nla_put_u8(skb, WIFI_TWT_ATTR_FLOW_FLAGS, sdesc->flow_flags);
	if (unlikely(err)) {
		WL_ERR(("nla_put_u8 WIFI_TWT_ATTR_FLOW_FLAGS failed\n"));
		goto fail;
	}
	err = nla_put_u8(skb, WIFI_TWT_ATTR_FLOW_ID, sdesc->flow_id);
	if (unlikely(err)) {
		WL_ERR(("nla_put_u8 WIFI_TWT_ATTR_FLOW_ID failed\n"));
		goto fail;
	}
	err = nla_put_u8(skb, WIFI_TWT_ATTR_CHANNEL, sdesc->channel);
	if (unlikely(err)) {
		WL_ERR(("nla_put_u8 WIFI_TWT_ATTR_CHANNEL failed\n"));
		goto fail;
	}
...
wl_update_twt_teardown_evt_info(struct sk_buff *skb, void *event_data)
{
	...
	err = nla_put_u8(skb, WIFI_TWT_ATTR_FLOW_ID, teardesc->flow_id);
	if (unlikely(err)) {
		WL_ERR(("nla_put_u8 WIFI_TWT_ATTR_FLOW_ID failed\n"));
		goto fail;
	}
	err = nla_put_u8(skb, WIFI_TWT_ATTR_NEGOTIATION_TYPE, teardesc->negotiation_type);
	if (unlikely(err)) {
		WL_ERR(("nla_put_u8 WIFI_TWT_ATTR_NEGOTIATION_TYPE failed\n"));
		goto fail;
	}
	err = nla_put_u8(skb, WIFI_TWT_ATTR_BID, teardesc->bid);
	if (unlikely(err)) {
		WL_ERR(("nla_put_u8 WIFI_TWT_ATTR_BID failed\n"));
		goto fail;
	}
	err = nla_put_s32(skb, WIFI_TWT_ATTR_ALLTWT, teardesc->alltwt);
	if (unlikely(err)) {
		WL_ERR(("nla_put_s32 WIFI_TWT_ATTR_ALLTWT failed\n"));
		goto fail;
	}
...
wl_update_twt_info_frm_evt_info(struct sk_buff *skb, void *event_data)
{
	...
	err = nla_put_u8(skb, WIFI_TWT_ATTR_FLOW_FLAGS, infodesc->flow_flags);
	if (unlikely(err)) {
		WL_ERR(("nla_put_u8 WIFI_TWT_ATTR_FLOW_FLAGS failed\n"));
		goto fail;
	}
	err = nla_put_u8(skb, WIFI_TWT_ATTR_FLOW_ID, infodesc->flow_id);
	if (unlikely(err)) {
		WL_ERR(("nla_put_u8 WIFI_TWT_ATTR_FLOW_ID failed\n"));
		goto fail;
	}
