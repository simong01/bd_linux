...
static int disable_cores(dhd_bus_t *bus, int id);
...
dhdsdio_sr_init(dhd_bus_t *bus)
{
	...
	if (CHIPID(bus->sih->chip) == BCM43430_CHIP_ID ||
		...
		CHIPID(bus->sih->chip) == BCM4382_CHIP_ID ||
...
dhdsdio_clk_kso_enab(dhd_bus_t *bus, bool on)
{
	...
	if ((BCM4612_CHIP(bus->sih->chip) || BCM5312_CHIP(bus->sih->chip)) && !on) {
...
static int disable_cores(dhd_bus_t *bus, int id)
{
	...
	if (!(si_setcore(bus->sih, id, 0))) {
		DHD_ERROR(("%s: Failed to find core, %d\n", __FUNCTION__, __LINE__));
		return BCME_ERROR;
	}
	...
	if (enter) {
		...
		if (!foundcr4) {
			...
			*/
			...
			disable_cores(bus, CC_CORE_ID);
			disable_cores(bus, D11_CORE_ID);
			disable_cores(bus, PCIE2_CORE_ID);
		...
		if (!si_setcore(bus->sih, ARMCR4_CORE_ID, 0)) {
			...
			if (bus->fwsig_filename[0] != 0) {
				DHD_ERROR(("%s: Signature File %s present "
...
#ifdef DHD_LINUX_STD_FW_API
	fw = (struct firmware *)filep;
	offset = fwpkg_get_unit_offset(&bus->fwpkg, FWPKG_TAG_SIG);
	bcmerror = memcpy_s(buf, SIG_HEADER_LEN, &(fw->data[offset]), SIG_HEADER_LEN);
	if (bcmerror != BCME_OK) {
...
dhdsdio_download_sig_file(dhd_bus_t *bus, char *path, uint32 type)
{
	...
	fw = (struct firmware *)filep;
	offset = fwpkg_get_unit_offset(&bus->fwpkg, FWPKG_TAG_SIG);
	bcmerror = memcpy_s(srcbuf, srcsize, &(fw->data[offset]), srcsize);
	if (bcmerror != BCME_OK) {
		DHD_ERROR(("%s: memcpy_s failed, err : %d\n",
				__FUNCTION__, bcmerror));
		goto exit;
                     ...
                     uint16 devid)
{
	...
	if (dhd_gci_chipctrl_reg4 && (bcmsdh_reg_read(bus->sdh, si_enum_base(devid), 2) == 0x4612)) {
		DHD_ERROR(("4612WAR: dhd_gci_chipctrl_reg4=0x%x\n", dhd_gci_chipctrl_reg4));
	...
	if (CHIPID(bus->sih->chip) == BCM4381_CHIP_GRPID ||
		...
		DHD_ERROR(("0x18012618=0x%4x, 0x1801260c=0x%4x, "
			"0x180331e0=0x%4x, 0x180001e8=0x%x\n",
			bcmsdh_reg_read(bus->sdh, 0x18012618, 4),
			bcmsdh_reg_read(bus->sdh, 0x1801260c, 4),
			bcmsdh_reg_read(bus->sdh, 0x180331e0, 4),
			bcmsdh_reg_read(bus->sdh, 0x180001e8, 4)));

		bcmsdh_reg_write(bus->sdh, 0x18010040, 2, 0x7);
		DHD_ERROR(("0x180127ec=0x%x, 0x18010040=0x%4x, 0x18010204=0x%4x\n",
			bcmsdh_reg_read(bus->sdh, 0x180127ec, 4),
			bcmsdh_reg_read(bus->sdh, 0x18010040, 4),
			bcmsdh_reg_read(bus->sdh, 0x18010204, 4)));

		DHD_ERROR(("0x18131800=0x%x, 0x18130800=0x%x, "
			"0x18100800=0x%x, 0x18000000=0x%4x\n",
			bcmsdh_reg_read(bus->sdh, 0x18131800, 4),
			bcmsdh_reg_read(bus->sdh, 0x18130800, 4),
			bcmsdh_reg_read(bus->sdh, 0x18100800, 4),
			bcmsdh_reg_read(bus->sdh, 0x18000000, 4)));
	}
...
#ifdef DHD_METADATA_DOWNLOAD
static int
dhd_get_file_array(void *file, int offset, int size, uint8 *buf)
{
#ifdef DHD_LINUX_STD_FW_API
	const struct firmware *fw = (struct firmware *)file;
#else
	int rdlen, seek_len = 0;
#endif /* DHD_LINUX_STD_FW_API */
	int ret = BCME_OK;

	memset(buf, 0, size);
#ifdef DHD_LINUX_STD_FW_API
	memcpy(buf, fw->data + offset, size);
#else
	/* offset the pointer for next read */
	if (dhd_os_seek_file(file, offset) != BCME_OK) {
		ret = BCME_ERROR;
		goto exit;
	}
	seek_len = offset;

	rdlen = dhd_os_get_image_block((char*)buf, size, file);
	if (rdlen < 0) {
		ret = BCME_ERROR;
		goto exit;
	}
...
exit:
	...
	if (dhd_os_seek_file(file, -seek_len) != BCME_OK) {
		ret = BCME_ERROR;
		goto exit;
	}
#endif /* !DHD_LINUX_STD_FW_API */
...
dhd_is_metadata_present(dhd_bus_t *bus, char *fwpath)
{
	...
	offset = fwpkg_get_unit_offset(fwpkg, FWPKG_TAG_FW);
	offset += size - METADATA_HDR_LEN;
#else
	ret = fwpkg_init(&bus->fwpkg, fwpath);
	if (ret == BCME_ERROR) {
		goto exit;
	...
	ret = dhd_get_file_array(file, offset, METADATA_HDR_LEN, metahdr);
	if (ret) {
	...
	ret = memcmp(&metahdr[offset], metadata_str, sizeof(metadata_str));

	DHD_ERROR(("Metadata %s\n", (ret == BCME_OK) ? "Present" : "Absent"));

exit:
	
#ifdef DHD_LINUX_STD_FW_API
	if (file) {
		dhd_os_close_img_fwreq(file);
	}
#else
	if (file) {
...
dhd_get_metadata_ver(dhd_bus_t *bus, char *fwpath)
{
	...
	fw = (struct firmware *)file;
	offset = fwpkg_get_unit_offset(fwpkg, FWPKG_TAG_FW);
	offset += size - METADATA_LEN - METADATA_VER_LEN;
	memset(version, 0, METADATA_VER_LEN);
	ret = memcpy_s(version, METADATA_VER_LEN,
		       &(fw->data[offset]),
		       METADATA_VER_LEN);
	if (ret < 0) {
		goto exit;
...
dhd_get_metadata_size(dhd_bus_t *bus, char *fwpath)
{
	...
	int i = 0;
	int ret = BCME_OK;
	char buf[METADATA_JUMP_VALUE_LEN];
	fwpkg_info_t *fwpkg;
	...
	fw = (const struct firmware *)file;
	offset = fwpkg_get_unit_offset(fwpkg, FWPKG_TAG_FW);

	offset += size - METADATA_LEN - METADATA_VER_LEN - METADATA_JUMP_VALUE_LEN;

	memset(buf, 0, METADATA_JUMP_VALUE_LEN);
	ret = memcpy_s(buf, METADATA_JUMP_VALUE_LEN,
		       &(fw->data[offset]),
		       METADATA_JUMP_VALUE_LEN);
	if (ret < 0) {
		goto exit;
	}
#else
...
dhd_fetch_metadata_array(dhd_bus_t *bus, char *fwpath, uint8 *metadata_block)
{
	...
	offset += size - METADATA_LEN - METADATA_VER_LEN
			- METADATA_JUMP_VALUE_LEN - metadata.jump_value;
	/* Fetch Complete Metadata contents */
	memset(metadata_block, 0, metadata.jump_value);

	ret = memcpy_s((char*)metadata_block, metadata.jump_value,
		       &(fw->data[offset]),
		       metadata.jump_value);
	if (ret < 0) {
		goto exit;
...
dhd_update_tag_info(dhd_bus_t *bus, char *fwpath)
{
	...
	fw = (struct firmware *)file;
	size = size - METADATA_LEN - METADATA_VER_LEN - METADATA_JUMP_VALUE_LEN;
	size = size - metadata.jump_value - MAX_TAG_INFO_LEN;
	offset = fwpkg_get_unit_offset(fwpkg, FWPKG_TAG_FW) + size;
	...
	ret = memcpy_s((char *)buffer, MAX_TAG_INFO_LEN,
		       &(fw->data[offset]),
		       MAX_TAG_INFO_LEN);
	if (ret < 0) {
		goto exit;
...
dhdsdio_download_code_file(struct dhd_bus *bus, char *pfw_path)
{
	 ...
	 * entry or in module param.
	 */
	bcmerror = fwpkg_open_firmware_img(fwpkg, pfw_path, (void **)&fw);
	if (bcmerror == BCME_ERROR) {
		goto err;
	}
	...
	residual_len = file_size;
	buf_offset = fwpkg_get_unit_offset(fwpkg, FWPKG_TAG_FW);
...
dhdsdio_sectionwise_fw_download(struct dhd_bus *bus, char *pfw_path)
{
	...
	for (idx = 0; idx < metadata.fw_dl_loc.fw_sec_num; idx++) {
		...
		/* Download image */
		while (offset < dl_end) {
			len = (offset + memblock_size < dl_end) ? memblock_size : rem_size;
...
dhdsdio_download_clm_file(dhd_bus_t *bus, char *path)
{
	...
	DHD_ERROR(("download clmblob file: %s,0x%x.\n",
		path, bus->clm_addr));
	...
	while (residual_len) {
		...
		buf_offset += len;
		offset += memblock_size;
		if ((offset - bus->clm_addr) >= fw->size)
			break;
	}
...
#if defined(DHD_AUTOSEL_BINARY_FILENAME)
		dhd_autosel_blob_name(bus->dhd, &clm_blob_path);
		/* update clm path to sdio bus */
		dhd_bus_update_clm_path(bus, clm_blob_path);
#endif /* DHD_AUTOSEL_BINARY_FILENAME */
