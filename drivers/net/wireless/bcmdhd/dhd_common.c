...
dhd_apply_default_clm(dhd_pub_t *dhd, char *clm_path)
{
	...
	if (!clm_blob_path) {
		printk("No need download clm_blob, chip %08x, %08x\n",
			dhd_bus_chip_id(dhd), dhd_bus_chiprev_id(dhd));
		return 0;
	}
...
/* default FW/NV/CLM is for pcie bus, use keywords '_sd_' to distinguish sdio bus */
#if defined(DHD_AUTOSEL_BINARY_FILENAME)
