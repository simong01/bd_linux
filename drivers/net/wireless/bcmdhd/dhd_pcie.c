...
#ifdef LINUX
#define MAX_WKLK_IDLE_CHECK	2000 /* times dhd_wake_lock checked before deciding not to suspend */
#endif /* LINUX */
...
dhd_bus_register(void)
{
    ...
    do{
        error = dhdpcie_bus_register();
        if(error ==0){
            DHD_TRACE(("%s: dhdpcie_bus_register ok.\n", __FUNCTION__));
            break;
        }
        DHD_ERROR(("%s: dhdpcie_bus_register has errors, retry.\n", __FUNCTION__));
        msleep(50);
        count ++;
    }while(error && count < 3);
...
dhdpcie_bus_isr(dhd_bus_t *bus)
{
	...
	do {
		...
		if (!bus->intr_enabled) {
			DHD_ERROR(("%s, not ready to receive interrupts\n", __FUNCTION__));
			break;
		}
...
skip_intstatus_read:
		 ...
		 * interrupt from the host side, so that host will not receive
...
static int
dhd_check_multiple_nvram(dhd_bus_t *bus)
{
	...
	if (nvram_file_exists) {
		for (i = 0; i < MAX_FILE_COUNT; i++) {
			...
			if (bcmerror == BCME_OK) {
				strlcpy(bus->nv_path, pnv_path[i], MAX_FILE_LEN);
				break;
			}
		}
	}
...
#if defined(SUPPORT_MULTIPLE_NVRAM)
	if (dhd_check_multiple_nvram(bus) != BCME_OK) {
		DHD_ERROR(("%s: Defined NVRAMs don't exist.\n", __FUNCTION__));
	}
#endif /* SUPPORT_MULTIPLE_NVRAM */
...
dhdpcie_bus_write_vars(dhd_bus_t *bus)
{
	...
	DHD_ERROR(("New varsize is %d, length token(nvram_csm)=0x%08x\n", varsize, varsizew));
...
dhd_bus_dump_txcpl_info(dhd_pub_t *dhdp, struct bcmstrbuf *strbuf)
{
	...
	bcm_bprintf(strbuf, "Host(us)\t\tPTM_high(ns)\t\tPTM_low(ns)\t\tTID\t\tFlowID\n");
	for (i = 0; i < MAX_TXCPL_HISTORY; i ++) {
		if (txcpl_info->tx_history[i].ptm_low) {
			bcm_bprintf(strbuf, "0x%x\t\t0x%x\t\t0x%x\t\t%u\t\t%u\n",
...
dhdpcie_bus_process_mailbox_intr(dhd_bus_t *bus, uint32 intstatus)
{
	...
	if (DHD_CHK_BUS_LPS_D3_ACKED(bus)) {
		DHD_ERROR(("%s: D3 Ack Received, skip dpc\n", __FUNCTION__));
		return resched;
	}
