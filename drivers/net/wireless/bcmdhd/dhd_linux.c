...
/* Flag for platform power OFF but WiFi power ON */
uint dhd_force_no_off = FALSE;
module_param(dhd_force_no_off, uint, 0644);

/* Flag for platform faststandby mode */
#if defined(DHD_PCIE_RUNTIMEPM) && defined(DHD_PCIE_RUNTIMEPM_ONLY_IN_FASTSTANDBY)
	uint dhd_platform_faststandby = FALSE;
	module_param(dhd_platform_faststandby, uint, 0644);
...
#if defined(DHD_PCIE_RUNTIMEPM) && defined(DHD_PCIE_RUNTIMEPM_ONLY_IN_FASTSTANDBY)
				if(TRUE == dhd_platform_faststandby) {
					/* Only Enable RPM in faststandy mode */
					if (FALSE == dhd_get_rpm_state(&dhd->pub)) {
						DHD_ERROR(("%s : Enable PCIe RuntimePM in faststandy mode\n", __FUNCTION__));
						DHD_ENABLE_RUNTIME_PM(&dhd->pub);
					}
				} else {
					/* Disable RPM in none-faststandy mode */
					if (TRUE == dhd_get_rpm_state(&dhd->pub)) {
						DHD_ERROR(("%s : Disable PCIe RuntimePM in none-faststandy mode\n", __FUNCTION__));
 ...
 */
static int
dhd_ioctl_entry(struct net_device *net, struct ifreq *ifr, void *data, int cmd)
{
	dhd_info_t *dhd = DHD_DEV_INFO(net);
	dhd_ioctl_t ioc;
	int bcmerror = 0;
...
void
dhd_poweroff_wowl(struct net_device *net)
{
	dhd_info_t *dhd = DHD_DEV_INFO(net);

	DHD_ERROR(("%s:WLAN won't power-off, when platform power-off!\n", __func__));
	dhd_suspend_resume_helper(dhd, 1, 1);

#ifdef PCIE_FULL_DONGLE
#ifdef DHD_PCIE_NATIVE_RUNTIMEPM
	dhdpcie_bus_suspend(dhd->pub.bus, TRUE, TRUE);
#else
	dhdpcie_bus_suspend(dhd->pub.bus, TRUE);
#endif /* DHD_PCIE_NATIVE_RUNTIMEPM */
...
bool dhd_update_fw_nv_path(dhd_info_t *dhdinfo)
{
	...
	if (fw && fw[0] != '\0' && (dhdinfo->fw_path[0] == '\0') ) {
		fw_len = strlen(fw);
	...
	if (nv && nv[0] != '\0' && (dhdinfo->nv_path[0] == '\0')) {
		nv_len = strlen(nv);
	...
	if (dhdinfo->sig_path[0] == '\0') {
		DHD_INFO(("signature path not found\n"));
	}
#ifdef BL_RAM_BOOT
...
#if defined(DHD_PCIE_RUNTIMEPM) && defined(DHD_PCIE_RUNTIMEPM_ONLY_IN_FASTSTANDBY)
	/* Disable RuntimePM in by default */
	DHD_DISABLE_RUNTIME_PM(dhd);
	DHD_ERROR(("%s : Disable RuntimePM by Default\n", __FUNCTION__));
#endif /* DHD_PCIE_RUNTIME_PM */
...
#ifdef DHD_DISABLE_ASPM
	ret = dhd_iovar(dhd, 0, "bus:deepsleep_disable", (char *)&deepsleep_disable,
		sizeof(deepsleep_disable), NULL, 0, TRUE);
	if (ret < 0) {
		DHD_ERROR(("%s: pcie_deepsleep_disable IOVAR not present\n", __FUNCTION__));
	} else {
		if (deepsleep_disable == 0) {
			DHD_ERROR(("%s: pcie_deepsleep_disable unset\n",
				__FUNCTION__));
		} else {
			DHD_ERROR(("%s: pcie_deepsleep_disable set\n", __FUNCTION__));
		}
	}
#endif /* DHD_DISABLE_ASPM */
...
dhd_os_get_img_fwreq(const struct firmware **fw, char *file_path)
{
	...
	if (ret < 0) {
		...
		DHD_ERROR(("%s: request_firmware %s success\n", __FUNCTION__, file_path));
	}
...
dhd_print_kirqstats(dhd_pub_t *dhd, unsigned int irq_num)
{
	...
	for_each_online_cpu(i)
		bcm_bprintf(&strbuf, "%10u ",
			...
			(per_cpu_ptr(desc->kstat_irqs, i))->cnt
#else
			*per_cpu_ptr(desc->kstat_irqs, i)
#endif /* KERNEL >= 6.10 */
			: 0);
