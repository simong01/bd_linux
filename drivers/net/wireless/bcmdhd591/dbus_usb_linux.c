 ...
 */
static urb_req_t *
BCMFASTPATH(dbus_usbos_qdeq)(struct list_head *urbreq_q, spinlock_t *lock)
{
	unsigned long flags;
	urb_req_t *req;
...
static void
BCMFASTPATH(dbus_usbos_qenq)(struct list_head *urbreq_q, urb_req_t *req, spinlock_t *lock)
{
	unsigned long flags;
 ...
 */
static int
BCMFASTPATH(dbus_usbos_recv_urb_submit)(usbos_info_t *usbos_info, dbus_irb_rx_t *rxirb,
	uint32 ep_idx)
{
	urb_req_t *req;
	int ret = DBUS_OK;
...
static void
BCMFASTPATH(dbus_usbos_recv_dpc)(usbos_info_t *usbos_info)
{
	urb_req_t *req = NULL;
...
static int
BCMFASTPATH(dbus_usbos_dpc_thread)(void *data)
{
 ...
 */
static void
BCMFASTPATH(dbus_usbos_recv_complete_handle)(urb_req_t *req, int len, int status)
{
#ifdef DBUS_LINUX_RXDPC
	usbos_info_t *usbos_info = req->usbinfo;
	unsigned long flags;
 ...
 */
static int
BCMFASTPATH(get_qtd_status)(struct ehci_qtd *qtd, int token, int *actual_length)
{
 ...
 */
static void
BCMFASTPATH(ehci_bypass_callback)(int pipeindex, struct ehci_qh *qh, spinlock_t *lock)
{
							/* Loop variables */
...
BCMFASTPATH(get_ep)(usbos_info_t *usbos_info, int ep)
{
#ifdef KERNEL26
...
static int
BCMFASTPATH(optimize_gather)(const dbus_pub_t *pub, void *pkt, void **buf)
{
	int len = 0;
...
/** EHCI fastpath specific function */
int
BCMFASTPATH(optimize_qtd_fill_with_rpc)(const dbus_pub_t *pub, int epn,
...
int
BCMFASTPATH(optimize_qtd_fill_with_data)(const dbus_pub_t *pub, int epn,
 ...
 */
int
BCMFASTPATH(optimize_submit_async)(struct ehci_qtd *qtd, int epn)
{
...
void
BCMFASTPATH(optimize_submit_rx_request)(const dbus_pub_t *pub, int epn, struct ehci_qtd *qtd_in,
                                            void *buf)
{
...
/* Use the op_mode parameter to check which fw to load */
extern int op_mode;
static void *
dbus_get_fwfile(int devid, int chiprev, uint8 **fw, int *fwlen, uint16 boardtype, uint16 boardrev)
{
	...
	/* Load firmware */
	if (op_mode == DHD_FLAG_MFG_MODE) {
		snprintf(file_name, sizeof(file_name),
			"brcm/bcm%s%s-usb_mfg.bin", device_id, chip_rev);
	} else {
		snprintf(file_name, sizeof(file_name),
			"brcm/bcm%s%s-usb.bin", device_id, chip_rev);
	}
