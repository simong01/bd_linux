...
dbus_irbq_init(dbus_info_t *dbus_info, dbus_irbq_t *q, int nq, int size_irb)
{
	...
	for (i = 0; i < nq; i++) {
		...
		irb = (dbus_irb_t *) MALLOCZ(dbus_info->pub.osh, size_irb);
		if (irb == NULL) {
			ASSERT(irb);
			return DBUS_ERR;
		}
...
dbus_rxirbs_fill(dbus_info_t *dbus_info)
{
	...
	while (atomic_read(&dbus_info->rx_outstanding) < 100) /* TODO: Improve constant */
	{
		...
		void *buf = MALLOCZ(dbus_info->pub.osh, 4000); /* usbos_info->rxbuf_len */
...
#else
		dbus_info->image = MALLOCZ(dbus_info->pub.osh, len);
#endif /* USBAP */
...
dbus_do_download(dbus_info_t *dbus_info)
{
	...
	if (dbus_info->nvfile) {
		int8 *tmp = MALLOCZ(dbus_info->pub.osh, temp_len);
		if (tmp) {
	...
	if (dbus_info->nvram) {
#ifdef USBAP
		VMFREE(dbus_info->pub.osh, dbus_info->image, dbus_info->image_len);
#else
 ...
 */
static void
BCMFASTPATH(dbus_if_send_irb_complete)(void *handle, dbus_irb_tx_t *txirb, int status)
{
	dbus_info_t *dbus_info = (dbus_info_t *) handle;
	int txirb_pending;
	struct exec_parms args;
 ...
 * level (eg dhd_linux.c) has to be notified, and fresh free receive IRBs may have to be given
 * to lower levels.
 */
static void
BCMFASTPATH(dbus_if_recv_irb_complete)(void *handle, dbus_irb_rx_t *rxirb, int status)
{
	dbus_info_t *dbus_info = (dbus_info_t *) handle;
	int rxirb_pending;
	struct exec_parms args;
...
/** Forward request for packet from lower DBUS layer to higher layer (eg dhd_linux.c) */
static void *
...
/** Forward request to free packet from lower DBUS layer to higher layer (eg dhd_linux.c) */
static void
...
/**
 * Called as part of DBUS bus registration. Calls back into higher level (eg dhd_linux.c) probe
 * function.
 */
static void *
	...
	dbus_callbacks_t *cbs, dbus_extdl_t *extdl, struct shared_info *sh)
{
	...
	dbus_info = MALLOCZ(osh, sizeof(dbus_info_t));
	if (dbus_info == NULL)
		return NULL;
	...
	dbus_info->tx_q = MALLOCZ(osh, sizeof(dbus_irbq_t));
	...
	dbus_info->rx_q = MALLOCZ(osh, sizeof(dbus_irbq_t));
	if (dbus_info->rx_q == NULL)
		goto error;
	else {
...
#ifdef BCMDBG
	dbus_info->txpend_q_hist = MALLOCZ(osh, dbus_info->pub.ntxq * sizeof(int));
	if (dbus_info->txpend_q_hist == NULL)
		goto error;

	dbus_info->rxpend_q_hist = MALLOCZ(osh, dbus_info->pub.nrxq * sizeof(int));
	...
	if (extdl && extdl->fw && (extdl->fwlen > 0)) {
		dbus_info->extdl.fw = MALLOCZ(osh, extdl->fwlen);
	...
	if (extdl && extdl->vars && (extdl->varslen > 0)) {
		dbus_info->extdl.vars = MALLOCZ(osh, extdl->varslen);
