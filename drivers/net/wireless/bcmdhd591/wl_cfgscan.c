	...
	struct cfg80211_ssid *this_ssid)
{
	 ...
	 */
	/* Some legacy chips cannot support bsscfg role change/up-gradation.
	 * Just allow the AP to scan to avoid the role changing between AP and STA.
	 */
	if (
#ifdef SYNA_ALLOW_AP_SCAN
		!cfg->legacy_chip &&
#endif /* SYNA_ALLOW_AP_SCAN */
		request && (scan_req_iftype(request) == NL80211_IFTYPE_AP)) {
		/* Directly return UNSUPPORTED even HOSTAPD, and then the Hostapd
		   will try to switch the interface type to STA temporary for the
		   scan, after that HOSTAPD will switch back to AP type.
		 */
