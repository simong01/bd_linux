...
)
{
	...
	switch (dhd_force_bw) {
		...
		case CH_BANDWIDTH_80MHZ:
			...
			bw = WL_CHANSPEC_BW_80;
			break;
		case CH_BANDWIDTH_160MHZ:
			WL_ERR(("Force BW to 160\n"));
			bw = WL_CHANSPEC_BW_160;
			break;
		default:
			bw = 0;
	...
	if (0 < bw) {
		WL_ERR(("dhd_force_bw set band_width=0x%X to be 0x%X\n", band_width, bw));
		band_width = bw;
	}
#endif /* WL_HOSTAPD_CFG */
	...
	struct net_device *dev)
{
	...
	if (bssidx == 0) {
		...
		if (cfg->legacy_chip) {
			s32 up = 1;
			WL_INFORM_MEM(("wl up to allow ap scan for legacy chip\n"));
 ...
 */
	...
	WL_INFORM_MEM(("%s: SYNA: wl_cfg80211_bcn_set_params\n", __func__));
