...
#ifndef DISABLE_CCMP_GCMP256
	WLAN_AKM_SUITE_8021X_SUITE_B_192,
#endif /* DISABLE_CCMP_GCMP256 */
...
#ifdef  DIS_COEX_FOR_P2P_DISC
static void wl_cfg80211_coex_dis_work_handler(struct work_struct *work);
...
#endif /* WL_6G_BAND */
	default:
		WL_ERR(("Invalid Frequency Band:0x%x\n", band));
	}
	return 0; /* not supported */
}
...
#ifdef DHD_PROPTX_FORCEON_4381
	if (dhd_bus_chip_id(dhd) == BCM4381_CHIP_ID) {
		enable = TRUE;
	}
#endif /* DHD_PROPTX_FORCEON_4381 */
	...
	struct net_device *ndev)
{
	...
	if (ret < 0) {
		WL_ERR(("wl_legacy_chip_check: GET revinfo FAILED. ret:%d\n", ret));
		return false;
	}

	chipnum = revinfo.chipnum;
	WL_INFORM_MEM(("GET_REVINFO device 0x%x, vendor 0x%x, chipnum 0x%x\n",
	...
	if (
		...
		((chipnum == BCM43430_CHIP_ID) && cfg->p2p_supported) ||
		(chipnum == BCM4345_CHIP_ID) ||
		(chipnum == BCM43598_CHIP_ID) || (chipnum == BCM43596_CHIP_ID) ||
		false) {
		/* WAR required */
		WL_INFORM_MEM(("legacy chip identified\n"));
		return true;
	}
		...
		if (sme->crypto.n_akm_suites > 1) {
			...
			if (val & (WPA_AUTH_PSK |
				...
				case WL_AKM_SUITE_SHA256_1X:
					if (wl_legacy_chip_check(cfg, dev)) {
						val = WPA2_AUTH_UNSPECIFIED;
					} else {
	...
	struct key_params *params)
{
	...
	/* Backward compatible with old chip 43456/43430  for GTK install */
...
#if defined(DHD_DEFAULT_BCNTIMEOUT_4381) || defined(WL_6G_BAND)
		if ((dhd_bus_chip_id(dhd) == BCM4381_CHIP_ID) ||
			(dhd_bus_chip_id(dhd) == BCM4382_CHIP_ID)) {
			bcn_timeout =  suspend ?
			CUSTOM_BCN_TIMEOUT_IN_SUSPEND : CUSTOM_BCN_TIMEOUT_6G;
		}
#endif /* DHD_DEFAULT_BCNTIMEOUT_4381 || WL_6G_BAND */
...
#if !defined(DISABLE_CFG_GET_CHANNEL) && (LINUX_VERSION_CODE >= KERNEL_VERSION (3, 6, \
	0))
static int
...
#endif /* !DISABLE_CFG_GET_CHANNEL && LINUX_VERSION_CODE >= KERNEL_VERSION (3, 6, 0) */
...
static int wl_cfg80211_regd_update(struct bcm_cfg80211 *cfg, char *ccode, bool user_trigger)
{
	int err = BCME_OK;
...
#else /* EXT_REGD_INFO */
static void wl_notify_regd(struct wiphy *wiphy, char *country_code, bool user_trigger)
{
	struct ieee80211_regdomain *regd_copy = NULL;
	int regd_len;
	struct bcm_cfg80211 *cfg = wiphy_priv(wiphy);
	...
	bool notify, bool user_enforced, int revinfo)
{
	 ...
	 */
#ifdef EXT_REGD_INFO
	ret = wl_cfg80211_regd_update(cfg, country_code, user_enforced);
#else
	if (!IS_REGDOM_SELF_MANAGED(wiphy)) {
		regulatory_hint(wiphy, country_code);
	} else {
		wl_notify_regd(wiphy, country_code, user_enforced);
	}
#endif /* EXT_REGD_INFO */
...
#else
	wl_notify_regd(wiphy, NULL, false);
#endif /* EXT_REGD_INFO */
...
static s32 wl_setup_wiphy(struct wireless_dev *wdev, struct device *sdiofunc_dev, void *context)
{
	...
	*/
	...
	if (IS_REGDOM_SELF_MANAGED(wdev->wiphy)) {
		rtnl_lock();
		wl_notify_regd(wdev->wiphy, NULL, false);
		rtnl_unlock();
	}
#endif /* EXT_REGD_INFO */
...
wl_handle_link_down(struct bcm_cfg80211 *cfg, wl_assoc_status_t *as)
{
	...
	if (memcmp(as->curbssid, as->addr, ETHER_ADDR_LEN) != 0) {
		/* Don't ignore linkdown event if BSSID already was cleared
		 * as 00:00:00:00:00:00
		 */
		if (ETHER_ISNULLADDR(as->addr) ||
#ifdef WLFBT
			(memcmp(&cfg->start_roam_addr, as->addr, ETHER_ADDR_LEN) == 0) ||
#endif /* WLFBT */
		0) {
...
wl_handle_roam_done(struct bcm_cfg80211 *cfg, wl_assoc_status_t *as)
{
	...
	dhd_aoe_hostip_clr(dhdp, 0);
	dhd_aoe_arp_clr(dhdp, 0);
	...
	struct wireless_dev *wdev, const wl_event_msg_t *e, void *data)
{
	...
	if (wl_get_drv_status(cfg, CONNECTING, ndev)) {
...
#ifdef WLFBT
		/* When roaming,After WLC_E_AUTH SUCCESS,fw will clear PENDING_AUTH flag,starting
		 * ASSOC REQ. But During this time,if device received special packets,such as
		 * PS_POLL,BLOCKACK,BLOCKACK_REQ coming from origin router(due to two routers
		 * in the same band),fw will send DEAUTH packets to origin router and sendup
		 * WLC_E_DEAUTH to driver,because the WLC_E_DEAUTH event came earlier than
		 * WLC_E_JOIN/WLC_E_BSSID/WLC_E_ROAM,it will cause uplayer thinking the roaming
		 * was fail. In fw logical,when roaming complete,it must wait for the first beacon
		 * frame of the roamed router, then will sendup WLC_E_JOIN/WLC_E_BSSID/WLE_E_ROAM
		 * event to driver. So do the patch below:
		 * 1, After driver received WLC_E_AUTH SUCCESS event,save the roamed addr.
		 * 2, Between WLC_E_AUTH SUCCESS and WLC_E_JOIN/WLC_E_BSSID/WLC_E_ROAM,if received
		 *    WLC_E_DEAUTH, check the addr whether the same with the saved roamed addr,
		 *    if not, just ingore this event.
		 * 3, After driver received WLE_E_JOIN/WLC_E_BSSID/WLC_E_ROAM,clear the saved
		 *    roamed addr.
		 */
		WL_INFORM_MEM(("Connected, event_type=%d, status=%d, "
			"start_roam_addr=%pM, e->addr=%pM\n",
		...
		} else if (event_type == WLC_E_JOIN || event_type == WLC_E_BSSID ||
			event_type == WLC_E_ROAM) {
			bzero((u8*)&cfg->start_roam_addr, ETHER_ADDR_LEN);
		} else if (event_type == WLC_E_DEAUTH &&
			!ETHER_ISNULLADDR(&cfg->start_roam_addr.octet) &&
	...
	const wl_event_msg_t *event, void *data)
{
	...
	wl_cfg80211_regd_update(cfg, country_str, false);
#else
	if (!IS_REGDOM_SELF_MANAGED(wiphy)) {
		...
		wl_notify_regd(wiphy, country_str, false);
	}
#endif /* EXT_REGD_INFO */
...
#ifdef DIS_COEX_FOR_P2P_DISC
	/* init timers */
	INIT_DELAYED_WORK(&cfg->coex_dis_work, wl_cfg80211_coex_dis_work_handler);
#endif /* DIS_COEX_FOR_P2P_DISC */
...
#ifdef DIS_COEX_FOR_P2P_DISC
	cancel_delayed_work(&cfg->coex_dis_work);
#endif /* DIS_COEX_FOR_P2P_DISC */
...
static s32 __wl_update_wiphybands(struct bcm_cfg80211 *cfg, bool notify)
{
	...
	if (unlikely(err)) {
		...
		/* Override error  status to 0, for chip not
		 * supporting VHT
		 */
		vhtmode = 0;
	}
...
#endif /* LINUX_VERSION_CODE >= KERNEL_VERSION(4, 0, 0) */
#else
			wl_notify_regd(wiphy, NULL, notify);
#endif /* EXT_REGD_INFO */
		}
...
static s32 __wl_cfg80211_up(struct bcm_cfg80211 *cfg)
{
	...
	*/
	...
	cfg->legacy_chip = wl_legacy_chip_check(cfg, ndev);
	WL_INFORM_MEM(("%s: cfg->legacy_chip=%d\n", __func__, cfg->legacy_chip));
...
#endif /* >= KERNEL_VERSION(4, 0, 0) && WL_SELF_MANAGED_REGDOM && EXT_REGD_INFO */
		...
		struct cfg80211_gtk_rekey_data *data)
{
	...
	memcpy(&bcol_keyinfo.replay[0], data->replay_ctr, RSN_REPLAY_LEN);
	if ((err = wldev_iovar_setbuf(dev, "bcol_gtk_rekey_ptk", &bcol_keyinfo,
		sizeof(bcol_keyinfo), cfg->ioctl_buf, WLC_IOCTL_SMLEN, &cfg->ioctl_buf_sync)) < 0) {
		WL_ERR(("seting bcol_gtk_rekey_ptk failed code=%d\n", err));
	}
			...
			if (mkeep_alive_pktp_v1->version == 2) {
				...
				if (dtoh32(mkeep_alive_pktp_v2->period_msec != 0)) {
					...
					res = wldev_ioctl_set(primary_ndev, WLC_SET_VAR, pbuf,
						KA_TEMP_BUF_SIZE);
					if (res < 0) {
						WL_ERR(("%s: Set mkeep_alive failed (error=%d)"
							" id=%d\n",
...
#define BT_COEX_DEFAULT_MODE (1)
/* Duration of coex disabled for P2P, in ms */
uint btc_dis_dur = 500;
module_param(btc_dis_dur, uint, 0660);

int32
wl_cfg80211_btc_mode(struct bcm_cfg80211 *cfg, bool on)
{
	struct net_device *ndev = NULL;
	bool btc_mode = BT_COEX_DEFAULT_MODE;
	int err = BCME_OK;

	if ((ndev = bcmcfg_to_prmry_ndev(cfg)) == NULL) {
		WL_ERR(("%s: cfg=0x%p, invalid ndev!\n", __func__, cfg));
		return BCME_NODEVICE;
	}

	/* ON: Stop timer. Turn on */
	if (on) {
		if (delayed_work_pending(&cfg->coex_dis_work)) {
			cancel_delayed_work(&cfg->coex_dis_work);
		}
		err = wldev_iovar_setint_bsscfg(ndev, "btc_mode", btc_mode, 0);
		if (err < 0) {
			WL_ERR(("Failed to set btc_mode %d  err:%d\n", btc_mode, err));
		} else {
			WL_ERR(("(%d) Set btc_mode:%d\n", on, btc_mode));
		}
	} else {
		/* OFF: If timer running, do nothing, else
		 * turn coex off and start timer to turn  on again
		 */
		if (!delayed_work_pending(&cfg->coex_dis_work)) {
			btc_mode = 0;
			err = wldev_iovar_setint_bsscfg(ndev, "btc_mode", btc_mode, 0);
			if (unlikely(err)) {
				WL_ERR(("%s get error (%d)\n", __func__, err));
			}
			schedule_delayed_work(&cfg->coex_dis_work,
				msecs_to_jiffies(btc_dis_dur));
			WL_ERR(("%s: Arm the coex dis  timer. for %dms\n",
					__FUNCTION__, btc_dis_dur));
		}
	}
	return err;
}
...
static void
wl_cfg80211_coex_dis_work_handler(struct work_struct *work)
{
	struct bcm_cfg80211 *cfg = NULL;
	struct net_device *ndev = NULL;
	bool btc_mode = BT_COEX_DEFAULT_MODE;
	int err = BCME_OK;
	BCM_SET_CONTAINER_OF(cfg, work, struct bcm_cfg80211, coex_dis_work.work);
	...
	/* Restore default coex setting */
	WL_ERR(("Restore btc_mode %d\n", btc_mode));

	err = wldev_iovar_setint_bsscfg(ndev, "btc_mode", btc_mode, 0);
	if (err < 0) {
		WL_ERR(("Failed to set btc_mode %d  err:%d\n", btc_mode, err));
	} else {
		WL_ERR(("Set btc_mode:%d\n", btc_mode));
	}
}
#endif /* DIS_COEX_FOR_P2P_DISC */
...
#if defined(WL_NEWCFG_PRIVCMD_SUPPORT)
bool
wl_cfg80211_is_p2p_net(struct net_device *dev)
{
	struct bcm_cfg80211 *cfg = wl_get_cfg(dev);
