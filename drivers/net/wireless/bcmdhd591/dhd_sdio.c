...
#define PKTGEN_RCV_ONGOING  (1)
	...
	bool		force_poll;			/* Force to enable polling */
...
dhdsdio_clk_kso_enab(dhd_bus_t *bus, bool on)
{
	...
	uint deadline = jiffies + msecs_to_jiffies(KSO_ENAB_TIMEOUT);
#endif /* CUSTOMER_HW4 */
...
#if defined(CUSTOMER_HW4)
		timedout = (jiffies > deadline);
#endif /* CUSTOMER_HW4 */

		bcmsdh_cfg_write(bus->sdh, SDIO_FUNC_1, SBSDIO_FUNC1_SLEEPCSR, wr_val, &err);
	} while ((try_cnt++ < CUSTOM_MAX_KSO_ATTEMPTS) && !timedout);

	if (try_cnt > 2)
		KSO_DBG(("%s> op:%s, try_cnt:%d, rd_val:%x, ERR:%x \n",
			__FUNCTION__, (on ? "KSO_SET" : "KSO_CLR"), try_cnt, rd_val, err));

	if ((try_cnt > CUSTOM_MAX_KSO_ATTEMPTS) || timedout)  {
		DHD_ERROR(("%s> op:%s, ERROR: try_cnt:%d, tout:%d rd_val:%x, ERR:%x \n",
			__FUNCTION__, (on ? "KSO_SET" : "KSO_CLR"), try_cnt,
			timedout, rd_val, err));
	}
#endif /* !defined(NDIS) */
...
dhd_bus_stop(struct dhd_bus *bus, bool enforce_mutex)
{
	...
	if ((bus->dhd->busstate == DHD_BUS_DOWN) || bus->dhd->hang_was_sent) {
		...
		DHD_ERROR(("%s: disable SDIO interrupts\n", __FUNCTION__));
#if !defined(NDIS)
...
clkwait:
	...
	if ((bus->dhd->busstate == DHD_BUS_DOWN) || bcmsdh_regfail(sdh)) {
		if ((bus->sih && bus->sih->buscorerev >= 12) && !(dhdsdio_sleepcsr_get(bus) &
			...
			bus->kso = FALSE;
#if defined(CUSTOMER_HW4)
			bus->sleeping = TRUE; /* Let BUS_WAKE trigger wake attempt */
#endif /* CUSTOMER_HW4 */
		} else {
...
#if defined(CUSTOMER_HW4)
		if (resched) {
			DHD_ERROR(("dhdsdio_dpc: don't reschedule till clocked\n"));
		}
		resched = FALSE;
#endif /* CUSTOMER_HW4 */
...
dhd_bus_watchdog(dhd_pub_t *dhdp)
{
	...
	if ((!SLPAUTO_ENAB(bus) && (bus->poll && (++bus->polltick >= bus->pollrate))) ||
		(bus->force_poll && (bus->poll && (++bus->polltick >= bus->pollrate)))) {
		uint32 intstatus = 0;
...
#if defined(CUSTOMER_HW4) && defined(WL_STATIC_IF)
...
#else
		dhd_set_macaddr_from_file(bus->dhd);
...
#endif /* CUSTOMER_HW4 && WL_STATIC_IF */
	}
#endif /* LINUX || linux */
#if defined(BT_OVER_SDIO)
